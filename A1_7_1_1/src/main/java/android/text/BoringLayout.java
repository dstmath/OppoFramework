...
public class BoringLayout extends Layout implements EllipsizeCallback {
    ...
    public static Metrics isBoring(CharSequence text, TextPaint paint, TextDirectionHeuristic textDir, Metrics metrics) {
        ...
        for (int start = 0; start < textLength; start += 500) {
            ...
            for (int i = 0; i < len; i++) {
                char c = buffer[i];
                if (c == 10 || c == 9 || ((c >= 1424 && c <= 2303) || c == 8207 || ((c >= 8234 && c <= 8238) || ((c >= 8294 && c <= 8297) || ((c >= 55296 && c <= 57343) || ((c >= 64285 && c <= 65023) || (c >= 65136 && c <= 65278))))))) {
                    boring = false;
                    break loop0;
                }
            }
    ...
    public void draw(Canvas c, Path highlight, Paint highlightpaint, int cursorOffset) {
        ...
        if (TextUtils.DEBUG_LOG) {
            TextUtils.printDebugLog(TAG, "[draw] " + this.mDirect);
        }
