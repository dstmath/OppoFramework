...
class TextLine {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: android.text.TextLine.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: android.text.TextLine.<clinit>():void, dex: 
        */
    ...
    private float handleText(TextPaint wp, int start, int end, int contextStart, int contextEnd, boolean runIsRtl, Canvas c, float x, int top, int y, int bottom, FontMetricsInt fmi, boolean needWidth, int offset) {
        if (fmi != null) {
            expandMetricsFromPaint(fmi, wp, this.mText);
        }
        if (end - start == 0) {
            return 0.0f;
        }
    ...
    private float handleRun(int start, int measureLimit, int limit, boolean runIsRtl, Canvas c, float x, int top, int y, int bottom, FontMetricsInt fmi, boolean needWidth) {
        ...
        if (start == measureLimit) {
            ...
            wp.set(this.mPaint);
            mlimit = measureLimit;
            boolean z = needWidth || measureLimit < measureLimit;
            ...
            while (i < measureLimit) {
                ...
                if (replacement != null) {
                    boolean z2 = needWidth || mlimit < measureLimit;
                    ...
                    while (j < mlimit) {
                        ...
                        boolean z3 = needWidth || jnext < measureLimit;
    ...
    private void drawTextRun(Canvas c, TextPaint wp, int start, int end, int contextStart, int contextEnd, boolean runIsRtl, float x, int y) {
        if (TextUtils.DEBUG_LOG) {
            Trace.traceBegin(8, "Text-drawTextRun");
        }
        if (this.mCharsValid) {
            Canvas canvas = c;
            int i = start;
