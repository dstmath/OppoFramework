...
public abstract class BroadcastReceiver {
    ...
    public static class PendingResult {
        ...
        String mTraceStrForFinish = null;
        final int mType;
        ...
        public void sendFinished(IActivityManager am) {
            synchronized (this) {
                if (this.mFinished) {
                    if (this.mTraceStrForFinish != null) {
                        Log.w("PendingResult", "multi finish the same receiver. First finish:");
                        Log.w("PendingResult", "from:" + this.mTraceStrForFinish);
                    }
                    if (ActivityThread.isSystem()) {
                        Log.w("PendingResult", "multi finish from systemServer!", new RuntimeException().fillInStackTrace());
                        return;
                    }
                    throw new IllegalStateException("Broadcast already finished");
                }
                this.mFinished = true;
                if (ActivityThread.DEBUG_BROADCAST && this.mType == 1) {
                    this.mTraceStrForFinish = Debug.getCallers(6);
                }
                try {
                    if (this.mResultExtras != null) {
                    ...
                    if (this.mOrderedHint) {
                        if (ActivityThread.DEBUG_BROADCAST && this.mType == 1) {
                            Slog.d(ActivityThread.TAG, "BR BDC-Calling finishReceiver: IIntentReceiver=" + Integer.toHexString(System.identityHashCode(this.mToken)));
                        }
