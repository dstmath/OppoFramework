    ...
    public void clear() {
        if (this.mThemedEntries != null) {
            for (int i = this.mThemedEntries.size() - 1; i >= 0; i--) {
                if (clear((LongSparseArray) this.mThemedEntries.valueAt(i))) {
                    this.mThemedEntries.removeAt(i);
                }
            }
        }
        clear(this.mNullThemedEntries);
        clear(this.mUnthemedEntries);
        this.mNullThemedEntries = null;
        this.mUnthemedEntries = null;
        this.mThemedEntries = null;
    }

    private boolean clear(LongSparseArray<WeakReference<T>> entries) {
        boolean z = true;
        if (entries == null) {
            return true;
        }
