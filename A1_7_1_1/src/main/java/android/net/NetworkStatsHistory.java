...
public class NetworkStatsHistory implements Parcelable {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: android.net.NetworkStatsHistory.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: android.net.NetworkStatsHistory.<clinit>():void, dex: 
        */
    ...
    public void recordData(long start, long end, android.net.NetworkStats.Entry entry) {
        ...
        if (entry.isNegative()) {
            ...
            for (int i = getIndexAfter(end); i >= 0; i--) {
                ...
                long curEnd = this.bucketStart[i] + this.bucketDuration;
                if (curEnd < start) {
                    break;
                }
                if (curStart <= end) {
                    ...
                    if (overlap <= 0) {
                        ...
                        Log.d(TAG, "recordData error i=" + i + " duration=" + duration + " start=" + start + " end=" + end + " overlap=" + overlap + " curEnd=" + curEnd + " curStart=" + curStart + " bucketDuration=" + this.bucketDuration);
                        Log.d(TAG, "bucket bucketCount=" + this.bucketCount + " startIndex=" + getIndexAfter(end));
                        for (int k = getIndexAfter(end); k >= 0; k--) {
                            Log.d(TAG, "bucket bucketStart[" + k + "]=" + this.bucketStart[k]);
                        }
                    } else {
    ...
    public void logoutData(String prefix) {
        ...
        for (int i = 0; i < this.bucketCount; i++) {
            ...
            builder.append("buck[").append(i).append("] = start:");
            builder.append(this.bucketStart[i] / 1000);
            builder.append(',');
            if (this.rxBytes != null) {
                builder.append("rxB:").append(this.rxBytes[i]);
            } else {
                builder.append("rxB:").append(NetworkCapabilities.MATCH_ALL_REQUESTS_NETWORK_SPECIFIER);
            }
            builder.append(',');
            if (this.rxPackets != null) {
                builder.append("rxP:").append(this.rxPackets[i]);
            } else {
                builder.append("rxP:").append(NetworkCapabilities.MATCH_ALL_REQUESTS_NETWORK_SPECIFIER);
            }
            builder.append(',');
            if (this.txBytes != null) {
                builder.append("txB:").append(this.txBytes[i]);
            } else {
                builder.append("txB:").append(NetworkCapabilities.MATCH_ALL_REQUESTS_NETWORK_SPECIFIER);
            }
            builder.append(',');
            if (this.txPackets != null) {
                builder.append("txP:").append(this.txPackets[i]);
            } else {
                builder.append("txP:").append(NetworkCapabilities.MATCH_ALL_REQUESTS_NETWORK_SPECIFIER);
            }
            builder.append(',');
            if (this.operations != null) {
                builder.append("op:").append(this.operations[i]);
            } else {
                builder.append("op:").append(NetworkCapabilities.MATCH_ALL_REQUESTS_NETWORK_SPECIFIER);
            }
