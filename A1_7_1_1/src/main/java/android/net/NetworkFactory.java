...
public class NetworkFactory extends Handler {
    ...
    protected void handleRemoveRequest(NetworkRequest request) {
        NetworkRequestInfo n = (NetworkRequestInfo) this.mNetworkRequests.get(request.requestId);
        log("handleRemoveRequest" + request);
        for (int i = 0; i < this.mNetworkRequests.size(); i++) {
            log("req:" + ((NetworkRequestInfo) this.mNetworkRequests.valueAt(i)).requested + ":" + ((NetworkRequestInfo) this.mNetworkRequests.valueAt(i)).request);
        }
        if (n != null) {
            log("find n");
    ...
    private void evalRequest(NetworkRequestInfo n) {
        log("evalRequest request = " + n.request + " with requested = " + n.requested);
