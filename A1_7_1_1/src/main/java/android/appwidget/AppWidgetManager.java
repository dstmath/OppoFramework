...
public class AppWidgetManager {
    ...
    public List<ComponentName> getAppWidgetOfHost(String pkg, int uid) {
        if (this.mService == null || !isRunningBoosterSupport()) {
            return Collections.emptyList();
        }
        try {
            return this.mService.getAppWidgetOfHost(pkg, uid);
        } catch (RemoteException e) {
    ...
    private boolean isRunningBoosterSupport() {
        return SystemProperties.get("persist.runningbooster.support").equals(WifiEnterpriseConfig.ENGINE_ENABLE);
    }
}
