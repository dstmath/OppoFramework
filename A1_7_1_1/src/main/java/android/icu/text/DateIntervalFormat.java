...
public class DateIntervalFormat extends UFormat {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: android.icu.text.DateIntervalFormat.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: android.icu.text.DateIntervalFormat.<clinit>():void, dex: 
        */
    ...
    private final StringBuffer fallbackFormat(Calendar fromCalendar, Calendar toCalendar, boolean fromToOnSameDay, StringBuffer appendTo, FieldPosition pos) {
        ...
        objArr[0] = earlierDate.toString();
        objArr[1] = laterDate.toString();
        String fallbackRange = MessageFormat.format(fallbackPattern, objArr);
        if (formatDatePlusTimeRange) {
            this.fDateFormat.applyPattern(this.fDatePattern);
            StringBuffer stringBuffer = new StringBuffer(64);
            otherPos.setBeginIndex(0);
            otherPos.setEndIndex(0);
            StringBuffer datePortion = this.fDateFormat.format(fromCalendar, stringBuffer, otherPos);
            adjustPosition(this.fDateTimeFormat, fallbackRange, pos, datePortion.toString(), otherPos, pos);
            String str = this.fDateTimeFormat;
            Object[] objArr2 = new Object[2];
            objArr2[0] = fallbackRange;
            objArr2[1] = datePortion.toString();
            fallbackRange = MessageFormat.format(str, objArr2);
        }
    ...
    private Map<String, PatternInfo> initializeIntervalPattern(String fullPattern, ULocale locale) {
        ...
        if (genSeparateDateTimePtn(normalizedDateSkeleton, normalizedTimeSkeleton, intervalPatterns, dtpng)) {
            if (stringBuilder.length() != 0) {
                if (date.length() == 0) {
                    patternInfo = new PatternInfo(null, dtpng.getBestPattern(DateFormat.YEAR_NUM_MONTH_DAY + timeSkeleton), this.fInfo.getDefaultOrder());
    ...
    private void concatSingleDate2TimeInterval(String dtfmt, String datePattern, int field, Map<String, PatternInfo> intervalPatterns) {
        ...
        if (timeItvPtnInfo != null) {
            ...
            Object[] objArr = new Object[2];
            objArr[0] = timeIntervalPattern;
            objArr[1] = datePattern;
            intervalPatterns.put(DateIntervalInfo.CALENDAR_FIELD_TO_PATTERN_LETTER[field], DateIntervalInfo.genPatternInfo(MessageFormat.format(dtfmt, objArr), timeItvPtnInfo.firstDateInPtnIsLaterDate()));
        }
    }
