...
public class DataRequestDispatcher {
    ...
    public class DispatcherServer implements Runnable {
        ...
        public void run() {
            ...
            while (DataRequestDispatcher.this.active && requests.size() > 0) {
                try {
                    synchronized (this) {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: android_maps_conflict_avoidance.com.google.googlenav.datarequest.DataRequestDispatcher.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: android_maps_conflict_avoidance.com.google.googlenav.datarequest.DataRequestDispatcher.<clinit>():void, dex: 
        */
    ...
    protected void serviceRequests(Vector requests, DispatcherServer dispatcherServer) throws IOException, SecurityException {
        ...
        StringBuffer drdDebug = new StringBuffer("DRD");
        StringBuffer append = drdDebug.append("(");
        int i2 = requestId;
        requestId = i2 + 1;
        append.append(i2).append("): ");
        for (i = 0; i < requests.size(); i++) {
            drdDebug.append(((DataRequest) requests.elementAt(i)).getRequestType());
        ...
        try {
            ...
            drdDebug.append(", ");
            if (firstByteTime < 1000) {
                drdDebug.append("<1s");
            } else {
                drdDebug.append(firstByteTime / 1000).append("s");
            }
            if (responseCode == 501) {
                ...
                Log.logToScreen("Bad Response Code " + responseCode + " " + drdDebug.toString());
                if (responseCode == 500) {
                ...
                if (responseLength >= 8192 && ((long) elapsedTime) <= 60000) {
                ...
                if (responseLength < 1000) {
                    ...
                    drdDebug.append(responseLength / 1000).append("kb");
                }
                ...
                Log.logToScreen("Bad HTTP content type: " + contentType + " " + drdDebug.toString());
