...
class MapFlashService implements MapTileStorage {
    ...
    int doPersist(Hashtable mapCache) throws IOException {
        ...
        synchronized (this) {
            ...
            if (bytesAvail < 72000 || getNumBlocks() >= this.maxRecordBlocks) {
                int worstIndex = -1;
                ...
                for (int index = 0; index < numEntries; index++) {
                    ...
                    if (secondWorstIndex == -1 || score > secondWorstScore) {
                        if (worstIndex == -1 || score > worstScore) {
        ...
        if (newRecordSize >= 6000) {
            ...
            synchronized (this) {
                ...
                int i2 = 0;
                while (i2 < 2) {
                    if (recordsToFree[i2] != null && recordsToFree[i2].isSaved() && recordsToFree[i2].getScore(startTime) > newRecord.getScore(startTime)) {
                        zArr[i2] = true;
                        displaceBytes += recordsToFree[i2].getDataSize();
                    }
                ...
                if (newRecordSize > bytesAvail + displaceBytes) {
                    status = 2;
                } else if (!zArr[0] || (newRecordSize <= bytesAvail && numBlocks < this.maxRecordBlocks)) {
