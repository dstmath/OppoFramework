...
public final class CodedInputByteBufferNano {
    ...
    public int readRawVarint32() throws IOException {
        ...
        int result = tmp & SupportedRates.MASK;
        tmp = readRawByte();
        if (tmp >= (byte) 0) {
            result |= tmp << 7;
        } else {
            result |= (tmp & SupportedRates.MASK) << 7;
            tmp = readRawByte();
            if (tmp >= (byte) 0) {
                result |= tmp << 14;
            } else {
                result |= (tmp & SupportedRates.MASK) << 14;
                tmp = readRawByte();
                if (tmp >= (byte) 0) {
                    result |= tmp << 21;
                } else {
                    result |= (tmp & SupportedRates.MASK) << 21;
                    tmp = readRawByte();
                    result |= tmp << 28;
                    if (tmp < (byte) 0) {
                        for (int i = 0; i < 5; i++) {
    ...
    public long readRawVarint64() throws IOException {
        ...
        for (int shift = 0; shift < DEFAULT_RECURSION_LIMIT; shift += 7) {
            byte b = readRawByte();
            result |= ((long) (b & SupportedRates.MASK)) << shift;
            if ((b & 128) == 0) {
                return result;
            }
        }
