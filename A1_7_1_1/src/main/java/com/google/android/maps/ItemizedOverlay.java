...
public abstract class ItemizedOverlay<Item extends OverlayItem> extends Overlay implements Snappable {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.google.android.maps.ItemizedOverlay.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.google.android.maps.ItemizedOverlay.<clinit>():void, dex: 
        */
    ...
    public void setFocus(Item item) {
        ...
        int index = 0;
        Iterator i$ = this.mItemsOrderedByRank.iterator();
        while (i$.hasNext()) {
            Item candidate = (OverlayItem) i$.next();
            if (candidate == item) {
    ...
    private boolean handleMotionEvent(boolean trackball, int gestureMask, MotionEvent event, MapView mapView, int x, int y) {
        ...
        if (isDown) {
            if (hitSomething) {
                ...
                this.mInGestureMask &= gestureMask ^ -1;
            }
        }
    ...
    private int maskHelper(int oldRank, int newRank, int mask) {
        if (oldRank != newRank) {
            ...
            if (oldRank != -1) {
                ...
                iArr[oldRank] = iArr[oldRank] & (mask ^ -1);
            }
            if (newRank != -1) {
                iArr = this.mItemState;
    ...
    private int getItemAtLocation(int hitX, int hitY, MapView mapView) {
        ...
        Iterator i$ = getItemsAtLocation(hitX, hitY, mapView).iterator();
        while (i$.hasNext()) {
            int rank = ((Integer) i$.next()).intValue();
