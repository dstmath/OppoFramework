    ...
    private void doFullBackup(int socketFd) {
        ...
        if (doEverything || saveShared || packages.size() != 0) {
            ...
            try {
                ...
                this.mBackupManager.fullBackup(fd, saveApks, saveObbs, saveShared, doWidgets, doEverything, allIncludesSystem, doCompress, (String[]) packages.toArray(new String[packages.size()]));
                if (fd != null) {
                    try {
                        fd.close();
                    } catch (IOException e) {
