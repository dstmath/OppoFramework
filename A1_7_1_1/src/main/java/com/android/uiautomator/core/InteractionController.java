...
class InteractionController {
    ...
    private static final long TOUCH_MOVE_EVENT_INTERVAL = 10;
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 00e9 in method: com.android.uiautomator.core.InteractionController.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 00e9 in method: com.android.uiautomator.core.InteractionController.<clinit>():void, dex: 
        */
    ...
    public boolean clickAndSync(int x, int y, long timeout) {
        ...
        Log.d(LOG_TAG, String.format("clickAndSync(%d, %d)", objArr));
    ...
    public boolean clickAndWaitForNewWindow(int x, int y, long timeout) {
        ...
        objArr[0] = Integer.valueOf(x);
        objArr[1] = Integer.valueOf(y);
        Log.d(LOG_TAG, String.format("clickAndWaitForNewWindow(%d, %d)", objArr));
    ...
    public boolean swipe(int downX, int downY, int upX, int upY, int steps, boolean drag) {
        ...
        for (int i = 1; i < swipeSteps; i++) {
            ...
            long eventInjectExpiredTime = SystemClock.uptimeMillis() - SystemClock.uptimeMillis();
            if (DEBUG) {
            ...
            if (sleep_time > 0) {
                ...
                Log.i(LOG_TAG, "inject event spent time : " + eventInjectExpiredTime + " > 10 ms , no sleep");
            }
        }
        if (drag) {
            SystemClock.sleep(REGULAR_CLICK_LENGTH);
    ...
    private boolean injectEventAsync(InputEvent event) {
        return this.mUiAutomatorBridge.injectInputEvent(event, DEBUG);
    }
