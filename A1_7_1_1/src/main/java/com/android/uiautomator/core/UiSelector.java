...
public class UiSelector {
    ...
    private boolean matchOrUpdateInstance() {
        ...
        if (currentSelectorInstance > currentSelectorCounter) {
            this.mSelectorAttributes.put(SELECTOR_COUNT, Integer.valueOf(currentSelectorCounter + SELECTOR_TEXT));
        }
        return false;
    }
    ...
    String dumpToString(boolean all) {
        ...
        for (int i = SELECTOR_NIL; i < criterionCount; i += SELECTOR_TEXT) {
            if (i > 0) {
                builder.append(", ");
            }
