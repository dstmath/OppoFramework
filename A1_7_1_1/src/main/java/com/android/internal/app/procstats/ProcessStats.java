...
public final class ProcessStats implements Parcelable {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.internal.app.procstats.ProcessStats.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.internal.app.procstats.ProcessStats.<clinit>():void, dex: 
        */
    ...
    public void updateFragmentation() {
        ...
        try {
            ...
            try {
                Matcher matcher = sPageTypeRegex.matcher(PhoneConstants.MVNO_TYPE_NONE);
    ...
    private void writeCompactedLongArray(Parcel out, long[] array, int num) {
        for (int i = 0; i < num; i++) {
            ...
            if (val <= 2147483647L) {
                ...
                int bottom = (int) (ExpandableListView.PACKED_POSITION_VALUE_NULL & val);
                                        ...
                                        while (NUID > 0) {
                                            ...
                                            while (NVERS > 0) {
                                                ...
                                                while (NPROCS > 0) {
                                                    ...
                                                    } else if (!procName.equals(mCtmName)) {
    ...
    private void dumpFragmentationLocked(PrintWriter pw) {
        ...
        for (int i = 0; i < NPAGETYPES; i++) {
            ...
            objArr[0] = this.mPageTypeZones.get(i);
            objArr[1] = this.mPageTypeLabels.get(i);
            pw.format("Zone %3d  %14s ", objArr);
            int[] sizes = (int[]) this.mPageTypeSizes.get(i);
            int N = sizes == null ? 0 : sizes.length;
            for (int j = 0; j < N; j++) {
                objArr = new Object[1];
                objArr[0] = Integer.valueOf(sizes[j]);
                pw.format("%6d", objArr);
            }
            pw.println();
        }
