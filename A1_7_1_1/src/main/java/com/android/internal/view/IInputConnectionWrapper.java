...
public abstract class IInputConnectionWrapper extends Stub {
    ...
    private static final int FAILED_COUNT_TO_TRIGGER_RECORD = 6;
    static final String TAG = "IInputConnectionWrapper";
    private static final long TIME_PERIOD_TO_CHECK = 20000;
    private long mCommitFailedCheckStartTime = 0;
    private int mCountForCommitFailed = 0;
    @GuardedBy("mLock")
    ...
    private String mPkgName = PhoneConstants.MVNO_TYPE_NONE;
    ...
    public IInputConnectionWrapper(Looper mainLooper, InputConnection inputConnection) {
        ...
        try {
            this.mPkgName = IPackageManager.Stub.asInterface(ServiceManager.getService("package")).getNameForUid(Process.myUid());
        } catch (RemoteException e) {
            Log.w(TAG, "getNameForUid failed.", e);
        }
    }
    ...
    void executeMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case 50:
                ic = getInputConnection();
                if (ic == null || !isActive()) {
                    Log.w(TAG, "commitText on inactive InputConnection. isActive:" + isActive() + ", ic:" + ic);
                    this.mCountForCommitFailed++;
                    if (1 == this.mCountForCommitFailed) {
                        this.mCommitFailedCheckStartTime = System.currentTimeMillis();
                    } else if (this.mCountForCommitFailed >= 6) {
                        if (System.currentTimeMillis() - this.mCommitFailedCheckStartTime <= TIME_PERIOD_TO_CHECK) {
                            String commitFailedMsg = "Text commit failed for pkg:" + this.mPkgName;
                        } else {
                            this.mCountForCommitFailed = 0;
                        }
                    }
                    return;
                }
            ...
            case 160:
                int flags = msg.arg1;
                boolean grantUriPermission = (flags & 1) != 0;
                args = (SomeArgs) msg.obj;
                try {
                    ic = getInputConnection();
                    if (ic == null || !isActive()) {
                    ...
                    if (grantUriPermission) {
                        try {
                            ...
                            Log.e(TAG, "InputConnectionInfo.requestPermission() failed", e3);
                            args.callback.setCommitContentResult(false, args.seq);
                            return;
                        }
                    }
                    boolean result = ic.commitContent(inputContentInfo, flags, (Bundle) args.arg2);
    ...
    Message obtainMessageIOOSC(int what, int arg1, Object objArg1, Object objArg2, int seq, IInputContextCallback callback) {
        SomeArgs args = new SomeArgs();
        args.arg1 = objArg1;
        args.arg2 = objArg2;
        args.callback = callback;
