...
public class TelephonyNetworkFactory extends NetworkFactory {
    ...
    private void applyRequests(HashMap<NetworkRequest, LocalLog> requestMap, boolean action, String logStr) {
        for (NetworkRequest networkRequest : requestMap.keySet()) {
            if (!ignoreImsPdn(networkRequest.networkCapabilities)) {
    ...
    private void onNeedNetworkFor(Message msg) {
        ...
        String s;
        if ((this.mIsActive && isApplicable) || ignoreImsPdn(networkRequest.networkCapabilities)) {
            s = "onNeedNetworkFor";
            localLog.log(s);
    ...
    private boolean ignoreImsPdn(NetworkCapabilities capabilities) {
