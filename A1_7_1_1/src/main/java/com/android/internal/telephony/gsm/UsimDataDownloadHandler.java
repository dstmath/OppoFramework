...
public class UsimDataDownloadHandler extends Handler {
    ...
    private void handleDataDownload(SmsMessage smsMessage) {
        ...
        byte[] envelope = new byte[((bodyLength + 1) + (bodyLength > CallFailCause.INTERWORKING_UNSPECIFIED ? 2 : 1))];
        int i = 1;
        envelope[0] = (byte) -47;
    ...
    private static int getEnvelopeBodyLength(int scAddressLength, int tpduLength) {
        int length = (tpduLength + 5) + (tpduLength > CallFailCause.INTERWORKING_UNSPECIFIED ? 2 : 1);
        if (scAddressLength != 0) {
    ...
    private static boolean is7bitDcs(int dcs) {
        return (dcs & 140) == 0 || (dcs & 244) == CallFailCause.CALL_BARRED;
    }
