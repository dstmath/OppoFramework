...
public class NativeLibraryHelper {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 00e9 in method: com.android.internal.content.NativeLibraryHelper.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 00e9 in method: com.android.internal.content.NativeLibraryHelper.<clinit>():void, dex: 
        */
    ...
    private static native int nativeFindSupportedAbi(long j, boolean z, String[] strArr);
    ...
    public static int findSupportedAbi(Handle handle, String[] supportedAbis) {
        return findSupportedAbi(handle, supportedAbis, false);
    }

    public static int findSupportedAbi(Handle handle, String[] supportedAbis, boolean fromCopy) {
        int finalRes = -114;
        boolean hasSecondAbi = false;
        for (long apkHandle : handle.apkHandles) {
            int res = nativeFindSupportedAbi(apkHandle, fromCopy, supportedAbis);
            if (res != -114) {
                if (res == -113) {
                    if (finalRes < 0) {
                        finalRes = -113;
        ...
        if (hasSecondAbi && finalRes >= 0 && finalRes < supportedAbis.length && !supportedAbis[finalRes].equals("arm64-v8a")) {
            finalRes = 110;
        }
        return finalRes;
    }
    ...
    private static long sumNativeBinariesForSupportedAbi(Handle handle, String[] abiList) {
        int abi = findSupportedAbi(handle, abiList, true);
        if (abi == 110) {
            int[] abis = getabiIndexes(abiList);
            return sumNativeBinaries(handle, abiList[abis[0]]) + sumNativeBinaries(handle, abiList[abis[1]]);
        } else if (abi >= 0) {
    ...
    private static int[] getabiIndexes(String[] abiList) {
        int[] results = new int[]{0, 0};
        for (int i = 0; i < abiList.length; i++) {
            if (abiList[i].equals("armeabi-v7a")) {
                results[0] = i;
            } else if (abiList[i].equals("armeabi")) {
                results[1] = i;
            }
        }
        return results;
    ...
    public static int copyNativeBinariesForSupportedAbi(Handle handle, File libraryRoot, String[] abiList, boolean useIsaSubdir) throws IOException {
        ...
        if (abi >= 0) {
            ...
            File isaSubdir;
            File subDir;
            int copyNativeBinaries;
            if (secondAbi > 0) {
                instructionSet = VMRuntime.getInstructionSet(abiList[secondAbi]);
                if (useIsaSubdir) {
                ...
                copyNativeBinaries = copyNativeBinaries(handle, subDir, abiList[secondAbi]);
            }
            instructionSet = VMRuntime.getInstructionSet(abiList[abi]);
            if (useIsaSubdir) {
                isaSubdir = new File(libraryRoot, instructionSet);
                createNativeLibrarySubdir(isaSubdir);
                subDir = isaSubdir;
            ...
            copyNativeBinaries = copyNativeBinaries(handle, subDir, abiList[abi]);
