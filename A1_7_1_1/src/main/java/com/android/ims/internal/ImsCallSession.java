...
public class ImsCallSession {
    ...
    private class IImsCallSessionListenerProxy extends Stub {
        /* synthetic */ IImsCallSessionListenerProxy(ImsCallSession this$0, IImsCallSessionListenerProxy iImsCallSessionListenerProxy) {
            this();
        }
            ...
            if (result != null) {
                try {
                    if (result.getTarget() != null) {
                        this.miSession.sendDtmfbyTarget(c, result, new Messenger(result.getTarget()));
                    }
                } catch (RemoteException e) {
                }
            }
    ...
    public boolean isIncomingCallMultiparty() {
        ...
        try {
            return this.miSession.isIncomingCallMultiparty();
        } catch (RemoteException e) {
            return false;
        }
    }
    ...
    public void explicitCallTransfer() {
        if (!this.mClosed) {
            try {
                this.miSession.explicitCallTransfer();
            } catch (RemoteException e) {
    ...
    public void unattendedCallTransfer(String number, int type) {
        if (!this.mClosed) {
            try {
                this.miSession.unattendedCallTransfer(number, type);
            } catch (RemoteException e) {
                Log.e(TAG, "explicitCallTransfer: RemoteException!");
            }
        }
