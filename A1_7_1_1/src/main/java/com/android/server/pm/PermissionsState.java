...
public final class PermissionsState {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.pm.PermissionsState.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.pm.PermissionsState.<clinit>():void, dex: 
        */
    ...
    public int grantRuntimePermission(BasePermission permission, int userId) {
        ...
        int result = grantPermission(permission, userId);
        if (userId == 0) {
            grantPermission(permission, OppoMultiAppManager.USER_ID);
        } else if (userId == OppoMultiAppManager.USER_ID) {
            grantPermission(permission, 0);
        }
        return result;
    }
    ...
    public int revokeRuntimePermission(BasePermission permission, int userId) {
        ...
        int result = revokePermission(permission, userId);
        if (userId == 0) {
            revokePermission(permission, OppoMultiAppManager.USER_ID);
        } else if (userId == OppoMultiAppManager.USER_ID) {
            revokePermission(permission, 0);
        }
        return result;
    }
    ...
    public boolean updatePermissionFlags(BasePermission permission, int userId, int flagMask, int flagValues) {
        ...
        if (userId == 0) {
            permissionData.updateFlags(OppoMultiAppManager.USER_ID, flagMask, flagValues);
        } else if (userId == OppoMultiAppManager.USER_ID) {
            permissionData.updateFlags(0, flagMask, flagValues);
        }
        return updated;
    }
    ...
    public boolean updatePermissionFlagsForAllPermissions(int userId, int flagMask, int flagValues) {
        ...
        boolean multiApp_changed = false;
        if (userId == 0) {
            for (i = 0; i < permissionCount; i++) {
                multiApp_changed |= ((PermissionData) this.mPermissions.valueAt(i)).updateFlags(OppoMultiAppManager.USER_ID, flagMask, flagValues);
            }
        } else if (userId == OppoMultiAppManager.USER_ID) {
            for (i = 0; i < permissionCount; i++) {
                multiApp_changed |= ((PermissionData) this.mPermissions.valueAt(i)).updateFlags(0, flagMask, flagValues);
            }
        }
        return changed;
    }
