...
public class WakeUp extends UnlockState {
    ...
    public synchronized void updateState() {
        boolean z = false;
        synchronized (this) {
            if ((this.mOrderedEvent == ORDERED_EVENTS.TOUCHDOWN || this.mOrderedEvent == ORDERED_EVENTS.HOMEKEY_TOUCHDOWN) && this.mIsScreenOff && this.mAuthenticated == -1) {
                z = true;
            }
            this.mState = z;
            ...
            boolean exitWakeUp = (isWakeUpEnabled() || this.mIsExitWakeUpMode) ? false : this.mOrderedEvent == ORDERED_EVENTS.TOUCHDOWN_TOUCHUP || this.mOrderedEvent == ORDERED_EVENTS.TOUCHUP;
            if (exitWakeUp) {
                exitWakeUpMode();
    ...
    public void finishAction(AuthenticatedInfo authenticatedInfo) {
        ...
        if (this.mIsExitWakeUpMode) {
            ...
            getFPMS().wakeUpByReason("android.service.fingerprint:WAKEUP");
