...
class OppoBroadcastManager {
    ...
    private static final String OPPO_SKIP_BROADCAST_CONFIG = "/data/system/config/sys_ams_skipbroadcast.xml";
    private static final String OPPO_SKIP_BROADCAST_PATH = "/data/system/config";
    private static int REASONABLE_COUNT_MAX = 0;
    private static int REASONABLE_COUNT_PERCENT_MAX = 0;
    private static float REASONABLE_INDEX_DISTRIBUTION_REGION_BOUNDARY = 0.0f;
    ...
    private static OppoBroadcastManager mInstance;
    ...
    private List<String> mCallBroadcastLocalList;
    private FileObserverPolicy mConfigFileObserver;
    private Context mContext;
    private String mCurrentDefaultImePkgName;
    private boolean mEnableAdjustOrderedBroadcastRecQue;
    private boolean mEnableAdjustParallelBroadcastRecQue;
    private boolean mEnableJumpQueue;
    private boolean mFilterPowerBroadcastForSomeApps;
    private boolean mFinishBootComplete;
    private List<String> mGovList;
    private Object mGovernmentLocker;
    private int mImeListInitCount;
    private ImeSettingsObserver mImeSettingsObserver;
    private boolean mIsImeListInited;
    private List<String> mLocalBroadcastName;
    private List<InputMethodInfo> mLocalEnableImeInfoList;
    private Handler mLocalHandler;
    private Object mLocalImeListLocker;
    BroadcastQueue mOppoBgBroadcastQueue;
    OppoActivityControlerScheduler mOppoBroadcastBlockMonitor;
    ServiceThread mOppoBroadcastThread;
    BroadcastQueue mOppoFgBroadcastQueue;
    private PowerManagerInternal mPowerManagerInternal;
    private boolean mSkipFeature;
    private final Runnable mUpdateEnableImeCallBack;
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.am.OppoBroadcastManager.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.am.OppoBroadcastManager.<clinit>():void, dex: 
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.am.OppoBroadcastManager.<clinit>():void");
    }
    ...
    private OppoBroadcastManager(ActivityManagerService service) {
        ...
        this.mAppWidgetMgr = null;
        this.mAudioManager = null;
        this.mPowerManagerInternal = null;
        this.mLocalEnableImeInfoList = null;
        this.mCurrentDefaultImePkgName = null;
        this.mIsImeListInited = false;
        this.mImeListInitCount = 0;
        this.mLocalImeListLocker = new Object();
        this.mLocalHandler = null;
        this.mImeSettingsObserver = null;
        this.mEnableJumpQueue = false;
        this.mFinishBootComplete = false;
        this.mEnableAdjustParallelBroadcastRecQue = false;
        this.mEnableAdjustOrderedBroadcastRecQue = false;
        this.mOppoBroadcastBlockMonitor = null;
        this.mGovernmentLocker = new Object();
        this.mFilterPowerBroadcastForSomeApps = false;
        String[] strArr = new String[5];
        strArr[0] = "android.intent.action.SCREEN_ON";
        strArr[1] = "android.intent.action.SCREEN_OFF";
        strArr[2] = "android.intent.action.USER_PRESENT";
        strArr[3] = "android.intent.action.CLOSE_SYSTEM_DIALOGS";
        strArr[4] = "android.intent.action.TIME_TICK";
        this.mLocalBroadcastName = Arrays.asList(strArr);
        strArr = new String[3];
        strArr[0] = "com.appstar.callrecorder";
        strArr[1] = "com.truecaller";
        strArr[2] = "gogolook.callgogolook2";
        this.mCallBroadcastLocalList = Arrays.asList(strArr);
        this.mUpdateEnableImeCallBack = new Runnable() {
            public void run() {
                ...
                if ((list == null || list.isEmpty()) && OppoBroadcastManager.this.mImeListInitCount <= 3) {
                    OppoBroadcastManager.this.updateLocalEnableImeList(true, 5000);
                    return;
                }
    ...
    /* JADX WARNING: Missing block: B:23:0x0039, code:
            if (isBroadcastInControledList(r11.getAction()) != false) goto L_0x003f;
     */
    ...
    /* JADX WARNING: Missing block: B:32:?, code:
            ...
            r2 = (com.android.server.am.ProcessRecord) r8.mAms.mProcessNames.get(r12, r13);
     */
    ...
    public boolean skipSpecialBroadcast(ProcessRecord app, String pkgName, Intent intent, String processName, int uid, ApplicationInfo info) {
        ...
        if (proc == null) {
            ...
            if (this.mAppWhiteList != null && this.mAppWhiteList.contains(pkgName)) {
                if (DEBUG_BROADCAST_FIREWALL) {
                    Slog.d(TAG, "pkg " + pkgName + " in whitelist" + "==" + intent.getAction());
                }
                return false;
            } else if (this.mAppBlackList == null || !this.mAppBlackList.contains(pkgName)) {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:58:0x00ee A:{Catch:{ IOException -> 0x0161 }} */
    /* Code decompiled incorrectly, please refer to instructions dump. */
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        Slog.w(TAG, "Got execption close permReader.", e32);
                    }
                } catch (Exception e4) {
                    e = e4;
                    fileReader = xmlReader;
                    try {
                        ...
                        try {
                            ...
                            Slog.w(TAG, "Got execption close permReader.", e322);
                        }
                        return;
                    } catch (Throwable th2) {
                        th = th2;
                        try {
                            ...
                            Slog.w(TAG, "Got execption close permReader.", e3222);
                        }
                        throw th;
                    }
                } catch (Throwable th3) {
    ...
    public void adjustQueueIfNecessary(ArrayList<BroadcastRecord> broadcastsQueue, BroadcastRecord r) {
        if (this.mEnableJumpQueue && this.mFinishBootComplete) {
            ...
            if (actionIntent != null) {
                isBroadcastQueuePrior = (actionIntent.getFlags() & 4194304) != 0;
    ...
    private boolean estimateAdjust(int countOfBroadcastR, int sumOfSeq, int totalSize) {
        ...
        if (countOfBroadcastR <= REASONABLE_COUNT_MAX) {
            isCountReasonable = true;
        } else {
            isCountReasonable = false;
        }
        if (isCountReasonable) {
            boolean isCountPerReasonable;
            if (((int) (((float) (countOfBroadcastR * 100)) / (((float) totalSize) * 1.0f))) <= REASONABLE_COUNT_PERCENT_MAX) {
                isCountPerReasonable = true;
            } else {
                isCountPerReasonable = false;
            }
            if (isCountPerReasonable) {
                boolean isIndexDistriReasonable;
                if (sumOfSeq / countOfBroadcastR >= ((int) (((float) totalSize) * REASONABLE_INDEX_DISTRIBUTION_REGION_BOUNDARY))) {
                    isIndexDistriReasonable = true;
                } else {
                    isIndexDistriReasonable = false;
                }
                if (isIndexDistriReasonable) {
                    return true;
                }
                if (DEBUG_JUMP_QUEUE) {
