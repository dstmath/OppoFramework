...
class RecentTasks extends ArrayList<TaskRecord> {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 00e9 in method: com.android.server.am.RecentTasks.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 00e9 in method: com.android.server.am.RecentTasks.<clinit>():void, dex: 
        */
        ...
        while (i < recentsCount) {
            ...
            if (task != tr) {
                if ((task.stack == null || tr.stack == null || task.stack == tr.stack) && task.userId == tr.userId) {
                    ...
                    boolean sameIntentFilter = intent != null ? intent.filterEquals(trIntent) : false;
                    ...
                    boolean bothDocuments = document ? trIsDocument : false;
                    if ((sameAffinity || sameIntentFilter || bothDocuments) && (!sameAffinity || intent == null || trIntent == null || intent.getComponent().flattenToShortString().equals(trIntent.getComponent().flattenToShortString()) || (intent.getFlags() & 8388608) != 8388608)) {
                        if (bothDocuments) {
    ...
    public void clearTasksWhileRebooting() {
        if (this.mTaskPersister != null) {
            TaskPersister taskPersister = this.mTaskPersister;
            TaskPersister.removeAllTasksRecordForReboot(0);
            taskPersister = this.mTaskPersister;
            TaskPersister.removeAllTasksRecordForReboot(OppoMultiAppManager.USER_ID);
        }
    }
}
