...
final class TaskRecord {
    ...
    boolean mLastTaskInFreeformStack;
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 00e9 in method: com.android.server.am.TaskRecord.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 00e9 in method: com.android.server.am.TaskRecord.<clinit>():void, dex: 
        */
    ...
    TaskRecord(ActivityManagerService service, int _taskId, ActivityInfo info, Intent _intent, IVoiceInteractionSession _voiceSession, IVoiceInteractor _voiceInteractor) {
        ...
        this.mSticky = false;
        this.mLastTaskInFreeformStack = false;
    ...
    TaskRecord(ActivityManagerService service, int _taskId, ActivityInfo info, Intent _intent, TaskDescription _taskDescription, TaskThumbnailInfo thumbnailInfo) {
        this.mLockTaskAuth = 1;
        this.mLockTaskUid = -1;
        this.lastTaskDescription = new TaskDescription();
        this.isPersistable = false;
    ...
    void setTaskToReturnTo(int taskToReturnTo) {
        ...
        if (ActivityManagerDebugConfig.DEBUG_TASKS) {
            Slog.i(TAG, "setTaskToReturnTo  this " + this + " mTaskToReturnTo " + this.mTaskToReturnTo + " call by " + Debug.getCallers(8));
        }
    }
    ...
    void removedFromRecentsImmediatly() {
        ...
        if (this.inRecents) {
            this.inRecents = false;
            this.mService.notifyTaskPersisterLocked(this, true);
        }
    }
    ...
    ActivityRecord topRunningActivityLocked() {
        if (this.stack != null) {
            ...
            for (int activityNdx = this.mActivities.size() - 1; activityNdx >= 0; activityNdx--) {
                ...
                if (ActivityManagerDebugConfig.DEBUG_TASKS) {
                    Slog.v(TAG, "TaskRecord topRunningActivityLocked activityNdx: " + activityNdx + " r " + r + " " + r.finishing);
                }
    ...
    void saveToXml(XmlSerializer out) throws IOException, XmlPullParserException {
        ...
        out.attribute(null, ATTR_CALLING_PACKAGE, this.mCallingPackage == null ? IElsaManager.EMPTY_PACKAGE : this.mCallingPackage);
    ...
    /* JADX WARNING: Removed duplicated region for block: B:122:0x033d  */
    ...
    static TaskRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor) throws IOException, XmlPullParserException {
        ...
        int nextTaskId = -1;
        int callingUid = -1;
        String callingPackage = IElsaManager.EMPTY_PACKAGE;
        int resizeMode = 4;
    ...
    private void adjustForMinimalTaskDimensions(Rect bounds) {
        if (bounds != null) {
            ...
            if (!(this.stack == null || this.stack.mStackId == 4)) {
                if (this.mService.mContext.getPackageManager().isFullFunctionMode()) {
                    if (minWidth == -1) {
                    ...
                    if (minWidth == -1) {
                        minWidth = this.mService.mStackSupervisor.mOppoCustomMinSizeOfResizeableTask;
                    }
                    if (minHeight == -1) {
                        minHeight = this.mService.mStackSupervisor.mOppoCustomMinSizeOfResizeableTask;
                    }
                }
            }
    ...
    Configuration updateOverrideConfiguration(Rect bounds, Rect insetBounds) {
        boolean z = true;
        if (Objects.equals(this.mBounds, bounds)) {
            return null;
        }
        ...
        if (this.mFullscreen) {
            if (!(this.mBounds == null || this.stack == null || !StackId.persistTaskBounds(this.stack.mStackId))) {
    ...
    Rect getLaunchBounds() {
        Rect rect = null;
        if (this.mService.mLockScreenShown == 2 && this.mShowForAllUsers) {
            Slog.d(TAG, "we're over lockscreen, forget about stack bounds and use fullscreen in " + this.stack);
            return null;
        } else if (this.stack == null) {
            return null;
        } else {
            ...
            if (stackId == 0 || stackId == 1 || (stackId == 3 && !isResizeable())) {
                ...
                if (MultiWindowManager.isSupported() && stackId == 2 && this.mLastNonFullscreenBounds != null && this.mLastNonFullscreenBounds.left == 0 && this.mLastNonFullscreenBounds.top == 0) {
                    return null;
                }
                if (MultiWindowManager.isSupported() && stackId == 2 && this.mLastNonFullscreenBounds == null && this.mBounds != null && !this.mBounds.isEmpty()) {
                    return this.mBounds;
                }
    ...
    private void setShowForAllUsers(ActivityInfo info) {
        ...
        if (ActivityManagerDebugConfig.DEBUG_TASKS || !ActivityManagerService.IS_USER_BUILD) {
            Slog.d(TAG, "setShowForAllUsers: flags=" + Integer.toHexString(info.flags) + " mShowForAllUsers = " + this.mShowForAllUsers + " callers=" + Debug.getCallers(4));
        }
    }
}
