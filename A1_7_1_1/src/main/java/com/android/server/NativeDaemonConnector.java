...
final class NativeDaemonConnector implements Runnable, Callback, Monitor {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.NativeDaemonConnector.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.NativeDaemonConnector.<clinit>():void, dex: 
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.NativeDaemonConnector.<clinit>():void");
    }
    ...
    public boolean handleMessage(Message msg) {
        ...
        try {
            if (!this.mCallbacks.onEvent(msg.what, event, NativeDaemonEvent.unescapeArgs(event))) {
                ...
                objArr2[0] = event;
                log(String.format("Unhandled event '%s'", objArr2));
            }
            ...
            if (start > sent && ((long) (start - sent)) > 500) {
                ...
                objArr2[0] = event;
                objArr2[1] = Integer.valueOf(start - sent);
                loge(String.format("NDC event {%s} processed too late: %dms", objArr2));
            }
            if (end > start && ((long) (end - start)) > 500) {
                objArr2 = new Object[2];
                objArr2[0] = event;
                objArr2[1] = Integer.valueOf(end - start);
                loge(String.format("NDC event {%s} took too long: %dms", objArr2));
            }
        } catch (Exception e) {
            ...
            if (start > sent && ((long) (start - sent)) > 500) {
                objArr = new Object[2];
                objArr[0] = event;
                objArr[1] = Integer.valueOf(start - sent);
                loge(String.format("NDC event {%s} processed too late: %dms", objArr));
            }
            if (end > start && ((long) (end - start)) > 500) {
                objArr = new Object[2];
                objArr[0] = event;
                objArr[1] = Integer.valueOf(end - start);
