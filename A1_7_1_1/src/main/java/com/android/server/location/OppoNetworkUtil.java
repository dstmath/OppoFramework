...
public class OppoNetworkUtil {
    ...
    public static boolean isRedTeaSoftSimSupport(Context context) {
        ...
        if (context.getPackageManager().resolveActivity(new Intent(ACTION_REDTEAMOBILE_ROAMING_MAIN), DumpState.DUMP_INSTALLS) != null) {
            return true;
        }
        return false;
    }
    ...
    public static boolean checkGoogleNetwork() {
        ...
        try {
            ...
            if (rspCode < DisplayTransformManager.LEVEL_COLOR_MATRIX_GRAYSCALE || rspCode >= DisplayTransformManager.LEVEL_COLOR_MATRIX_COLOR) {
