...
class OppoAlarmWakeupDetection {
    ...
    private static final String ALARM_WAKEUP_CHECK_NOT_RESTRICT_ACTION = "android.intent.action.ALARM_WAKEUP_CHECK_NOT_RESTRICT";
    private static final String ATAG = "AlarmWakeupCheck";
    private static final boolean DBG = false;
    private static final long INTERVAL_WAKEUP_CHECK = 1200000;
    private static final int MSG_CHECK_ALARM_ALL_TYPE = 105;
    private static final int MSG_CHECK_ALARM_WAKEUP = 101;
    private static final int MSG_CHECK_ALARM_WAKEUP_NOT_RESTRICT_APP = 104;
    private static final int MSG_REPORT_FREQUENT_ALARM = 103;
    ...
    private class AlarmDetectionReceiver extends BroadcastReceiver {
        /* synthetic */ AlarmDetectionReceiver(OppoAlarmWakeupDetection this$0, AlarmDetectionReceiver alarmDetectionReceiver) {
            this();
        }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("android.intent.action.SCREEN_ON")) {
                ...
                OppoAlarmWakeupDetection.this.mAlarmMS.removeImpl(OppoAlarmWakeupDetection.this.mPendingIntent);
                ...
                OppoAlarmWakeupDetection.this.mHandlerTimeout.sendEmptyMessage(103);
                ...
                OppoAlarmWakeupDetection.this.scheduleAlarmWakeupCheck();
            } else if (action.equals(OppoAlarmWakeupDetection.ALARM_WAKEUP_CHECK_NOT_RESTRICT_ACTION)) {
                OppoAlarmWakeupDetection.this.mWakeLockCheck.acquire(1000);
                OppoAlarmWakeupDetection.this.mHandlerTimeout.sendEmptyMessage(104);
            }
        }
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 102:
                    ...
                    return;
                case 103:
                    OppoAlarmWakeupDetection.this.reportFrequentAlarmList();
                    OppoAlarmWakeupDetection.this.mReportList.clear();
                    if (OppoAlarmWakeupDetection.this.mWakeLockCheck.isHeld()) {
                        OppoAlarmWakeupDetection.this.mWakeLockCheck.release();
                        return;
                    ...
                    return;
                case 104:
                    OppoAlarmWakeupDetection.this.checkNotRestrictApp();
                    OppoAlarmWakeupDetection.this.scheduleAlarmWakeupCheck();
                    if (OppoAlarmWakeupDetection.this.mWakeLockCheck.isHeld()) {
                        OppoAlarmWakeupDetection.this.mWakeLockCheck.release();
                        return;
    ...
    public OppoAlarmWakeupDetection(Context context, Object lock, AlarmManagerService alarmMS, SparseArray<ArrayMap<String, BroadcastStats>> broadcastStats, Looper loop) {
        ...
        filter.addAction(ALARM_WAKEUP_CHECK_NOT_RESTRICT_ACTION);
        ...
        this.mPendingIntent = PendingIntent.getBroadcastAsUser(this.mContext, 0, new Intent(ALARM_WAKEUP_CHECK_NOT_RESTRICT_ACTION), 0, UserHandle.ALL);
    }
    ...
    public void alarmTriggerFrequentDetection(Alarm alarm, BroadcastStats bs, long nowELAPSED, boolean isPendingIntentCanceled) {
        ...
        if (!this.mScreenOn && this.mScreenOffRecorded && OppoAbnormalAppManager.getInstance().isScreenOffRestrict() && bs.mPackageName != null && !this.mSysState.isNotRestrictPkg(bs.mPackageName)) {
    ...
    public int SyncAlarmHandle(int type, PendingIntent pi) {
        ...
        if (this.isSyncAlarmWakeupFrequent && pi.getCreatorUid() == 1000) {
            String tag = pi.getTag(IElsaManager.EMPTY_PACKAGE);
    ...
    private boolean alarmWakeupDetection(BroadcastStats bs, long nowELAPSED, long lastTimeWakeup) {
        ...
        if (numSecondsPerWakeup <= this.thresholdSeriousPerWakeup) {
            ...
            data.putInt(PackageProcessList.KEY_UID, bs.mUid);
    ...
    private void alarmAllTypeDetection(BroadcastStats bs, long nowELAPSED, long lastTimeTrigger) {
        if (!isSystemPackage(bs.mPackageName)) {
            if (!isOppoPackage(bs.mPackageName)) {
                if (nowELAPSED - lastTimeTrigger >= 1200000) {
                    resetState(bs, 0, nowELAPSED, false, true);
                    return;
                }
    ...
    private void alarmWakeupHandle(Bundle data) {
        if (!this.mScreenOn && data != null) {
            ...
            int uid = data.getInt(PackageProcessList.KEY_UID);
                    ...
                    for (int is = 0; is < bs.filterStats.size(); is++) {
                        ...
                        if (fsNumWakeup > 0) {
                            ...
                            if (!this.isSyncAlarmWakeupFrequent && fsNumWakeup >= 5 && bs.mUid == 1000 && fs.mTag != null && fs.mTag.endsWith("syncmanager.SYNC_ALARM") && totalCheckTime / ((long) fsNumWakeup) <= this.thresholdSeriousPerWakeup) {
                                ...
                                if (this.ADBG) {
                                    Slog.w(ATAG, "alarmWakeupHandle: isSyncAlarmWakeupFrequent set true!");
                                }
                            }
                        }
                    }
    ...
    private void checkNotRestrictApp() {
        synchronized (this.mLock) {
            ...
            for (int iu = 0; iu < this.mBroadcastStats.size(); iu++) {
                ...
                for (int ip = 0; ip < uidStats.size(); ip++) {
                    ...
                    if (bs.mPackageName != null && OppoAbnormalAppManager.getInstance().isNotRestrictApp(bs.mPackageName) && bs.numAllWakeup != 0 && bs.numAllWakeup > bs.numWakeupWhenScreenoff) {
                        long totalCheckTime = (bs.lastTimeWakeup - bs.wakeupCountStartTime) / 1000;
                        if (totalCheckTime >= 1200) {
                            long numSecondsPerWakeup = totalCheckTime / ((long) (bs.numAllWakeup - bs.numWakeupWhenScreenoff));
                            if (numSecondsPerWakeup <= this.thresholdSeriousPerWakeup) {
                                ArrayList<AlarmIntentRecord> alarmIntentRecordList = new ArrayList();
                                for (int is = 0; is < bs.filterStats.size(); is++) {
                                    FilterStats fs = (FilterStats) bs.filterStats.valueAt(is);
                                ...
                                Collections.sort(alarmIntentRecordList, this.comparatorIntent);
                                addReportList(bs.mUid, numSecondsPerWakeup, totalCheckTime, bs.mPackageName, alarmIntentRecordList, true, true, bs.numCanceledWakeup);
                            } else {
                                continue;
                            }
                        } else {
    ...
    private void scheduleAlarmWakeupCheck() {
        this.mAlarmMS.setImpl(3, SystemClock.elapsedRealtime() + 1200000, 0, 0, this.mPendingIntent, null, null, 9, null, null, Process.myUid(), "android");
    }
    ...
    private void reportFrequentAlarmList() {
        int len = this.mReportList.size();
        ArrayList<String> uidListWorst = new ArrayList();
        ArrayList<String> uidListSerious = new ArrayList();
        ArrayList<String> uidListWarnning = new ArrayList();
        for (int i = 0; i < len; i++) {
            AlarmWakeupRecord alarmWakeupRecord = (AlarmWakeupRecord) this.mReportList.get(i);
            String reportString = alarmWakeupRecord.getReportString(true);
            if (alarmWakeupRecord.isWorst) {
                uidListWorst.add(reportString);
                if (this.ADBG) {
                    Slog.w(ATAG, "reportFrequentAlarmList: report Worst Alarm. reportString=" + reportString);
                }
            } else if (alarmWakeupRecord.isSerious) {
                uidListSerious.add(reportString);
                if (this.ADBG) {
                    Slog.w(ATAG, "reportFrequentAlarmList: report Serious Alarm. reportString=" + reportString);
                }
            } else {
                uidListWarnning.add(reportString);
                if (this.ADBG) {
                    Slog.w(ATAG, "reportFrequentAlarmList: report Warnning Alarm. reportString=" + reportString);
                }
            }
        }
    }
    ...
    private void reportFrequentAlarmListForceStop(AlarmWakeupRecord record) {
        ...
        if (record != null && record.isSerious && record.mPkgName != null && !record.mPkgName.equals("android")) {
            record.isForcestopReported = true;
            String reportString = record.getReportString(true);
            uidList.add(reportString);
            if (this.ADBG) {
                Slog.w(ATAG, "FrequentAlarmListF: reportString=" + reportString);
            }
            if (uidList.size() > 0) {
