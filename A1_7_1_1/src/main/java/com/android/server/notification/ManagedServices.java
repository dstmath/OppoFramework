...
public abstract class ManagedServices {
     ...
     */
        ...
        public boolean enabledAndUserMatches(int nid) {
            ...
            if (this.userid == -1 || this.isSystem || nid == -1 || nid == this.userid || nid == OppoMultiAppManager.USER_ID) {
                return true;
            }
            if (supportsProfiles()) {
                z = ManagedServices.this.mUserProfiles.isCurrentProfile(nid);
            }
            return z;
        }
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        /* synthetic */ SettingsObserver(ManagedServices this$0, Handler handler, SettingsObserver settingsObserver) {
            this(handler);
        }
        ...
        private void update(Uri uri) {
            if (uri == null || this.mSecureSettingsUri.equals(uri) || uri.equals(this.mSecondarySettingsUri)) {
                ...
                ManagedServices.this.rebindServices(false, OppoAppStartupManager.REBIND_SETTING_CHANGE);
    ...
    public void dump(PrintWriter pw, DumpFilter filter) {
        ...
        for (ManagedServiceInfo info : this.mServices) {
            if (filter == null || filter.matches(info.component)) {
                pw.println("      " + info.component + " (user " + info.userid + "): " + info.service + (info.isSystem ? " SYSTEM" : IElsaManager.EMPTY_PACKAGE) + (info.isGuest(this) ? " GUEST" : IElsaManager.EMPTY_PACKAGE));
            }
        }
    ...
    private void rebindServices(boolean forceRebind, String rebindType) {
        ...
        for (i = 0; i < nUserIds; i++) {
            for (ComponentName component2 : (Set) toAdd.get(userIds[i])) {
                if (component2 == null || OppoAppStartupManager.getInstance().isAllowStartFromNotificationService(component2, rebindType)) {
