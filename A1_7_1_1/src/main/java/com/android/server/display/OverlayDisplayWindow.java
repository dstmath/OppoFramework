...
final class OverlayDisplayWindow implements Dump {
    ...
    private void resize(int width, int height, int densityDpi, boolean doLayout) {
        ...
        this.mTitle = this.mContext.getResources().getString(17040661, new Object[]{this.mName, Integer.valueOf(this.mWidth), Integer.valueOf(this.mHeight), Integer.valueOf(this.mDensityDpi)});
        if (this.mSecure) {
            this.mTitle += this.mContext.getResources().getString(17040662);
        }
        if (doLayout) {
            relayout();
        }
    ...
    private void createWindow() {
        int i;
        int i2 = 0;
        this.mWindowContent = LayoutInflater.from(this.mContext).inflate(17367197, null);
        ...
        if (this.mSecure) {
            ...
            layoutParams.flags |= DumpState.DUMP_PREFERRED_XML;
        }
        layoutParams = this.mWindowParams;
