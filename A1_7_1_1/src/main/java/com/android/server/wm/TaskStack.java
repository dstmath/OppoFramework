...
public class TaskStack implements DimLayerUser, AnimateBoundsUser {
    ...
    TaskStack(WindowManagerService service, int stackId) {
        ...
        this.mDockedStackMinimizeThickness = service.mContext.getResources().getDimensionPixelSize(17104933);
    ...
    private void snapDockedStackAfterRotation(Rect outBounds) {
        ...
        if (displayWidth < displayHeight && outBounds != null && displayHeight == outBounds.height()) {
            dividerPosition = displayHeight / 2;
        }
    ...
    void positionTask(Task task, int position, boolean showForAllUsers) {
        ...
        if (WindowManagerDebugConfig.DEBUG_TASK_MOVEMENT) {
            Slog.d("WindowManager", "positionTask: task=" + task + " position=" + position);
        }
    ...
    void close() {
        ...
        if (WindowManagerDebugConfig.DEBUG_STACK) {
            Slog.d("WindowManager", "mDisplayContent will be null " + Debug.getCallers(8));
        }
    ...
    void setAdjustedForIme(WindowState imeWin, boolean forceUpdate) {
        ...
        if (!this.mAdjustedForIme || forceUpdate) {
            ...
            updateAdjustForIme(OppoBrightUtils.MIN_LUX_LIMITI, OppoBrightUtils.MIN_LUX_LIMITI, true);
        }
    }
    ...
    boolean isVisibleLocked(boolean ignoreKeyguard) {
        ...
        if (!ignoreKeyguard && keyguardOn && !StackId.isAllowedOverLockscreen(this.mStackId)) {
            return false;
        }
        for (int i = this.mTasks.size() - 1; i >= 0; i--) {
    ...
    boolean isInPendingTransactionStateLocked(boolean ignoreKeyguard) {
        boolean keyguardOn = this.mService.mPolicy.isKeyguardShowingOrOccluded() ? !this.mService.mAnimator.mKeyguardGoingAway : false;
        if (!ignoreKeyguard && keyguardOn && !StackId.isAllowedOverLockscreen(this.mStackId)) {
            return false;
        }
        for (int i = this.mTasks.size() - 1; i >= 0; i--) {
            Task task = (Task) this.mTasks.get(i);
            int j = task.mAppTokens.size() - 1;
            while (j >= 0) {
                AppWindowToken appWindowToken = (AppWindowToken) task.mAppTokens.get(j);
                if (appWindowToken == null || !appWindowToken.waitingToShow) {
                    j--;
                } else {
                    if (WindowManagerDebugConfig.DEBUG_STACK) {
                        Slog.d("WindowManager", "isInPendingTransaction, appWindowToken:" + appWindowToken);
                    }
                    return true;
                }
            }
    ...
    /* JADX WARNING: Missing block: B:9:?, code:
            r9.mService.mActivityManager.resizeStack(r9.mStackId, r10, false, true, false, -1);
     */
    ...
    /* JADX WARNING: Missing block: B:15:?, code:
            r5.mService.mActivityManager.resizePinnedStack(r6, r7);
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    ...
    void overridePlayingAppAnimations(Animation a) {
        for (int i = this.mTasks.size() - 1; i >= 0; i--) {
            ((Task) this.mTasks.get(i)).overridePlayingAppAnimations(a);
        }
    }
}
