...
public final class NetworkStatsAccess {
    ...
    public static boolean isAccessibleToUser(int uid, int callerUid, int accessLevel) {
        ...
        switch (accessLevel) {
            case 1:
                if (!(uid == 1000 || uid == -4 || uid == -5 || uid == -6 || UserHandle.getUserId(uid) == UserHandle.getUserId(callerUid))) {
                    z = false;
                }
                return z;
            case 2:
                if (!(uid == 1000 || uid == -4 || uid == -5 || uid == -1 || uid == -6 || UserHandle.getUserId(uid) == UserHandle.getUserId(callerUid))) {
                    z = false;
                }
                return z;
            case 3:
