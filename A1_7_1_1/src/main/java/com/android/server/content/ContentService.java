...
public final class ContentService extends Stub {
    ...
    private final ObserverNode mRootNode = new ObserverNode(IElsaManager.EMPTY_PACKAGE);
    ...
    public static final class ObserverNode {
        ...
        public void dumpLocked(FileDescriptor fd, PrintWriter pw, String[] args, String name, String prefix, int[] counts, SparseIntArray pidCounts) {
            ...
            if (this.mObservers.size() > 0) {
                if (IElsaManager.EMPTY_PACKAGE.equals(name)) {
    ...
    protected synchronized void dump(FileDescriptor fd, PrintWriter pw_, String[] args) {
        ...
        try {
            ...
            synchronized (this.mRootNode) {
                ...
                this.mRootNode.dumpLocked(fd, pw, args, IElsaManager.EMPTY_PACKAGE, "  ", counts, pidCounts);
                pw.println();
    ...
    public void registerContentObserver(Uri uri, boolean notifyForDescendants, IContentObserver observer, int userHandle) {
        ...
        userHandle = handleIncomingUser(uri, pid, uid, 1, userHandle);
        ...
        if (OppoMultiAppManager.USER_ID == UserHandle.getUserId(uid) && OppoMultiLauncherUtil.getInstance().isMultiApp(pkgName)) {
            userHandle = -1;
        }
        synchronized (this.mRootNode) {
    ...
    public void notifyChange(Uri uri, IContentObserver observer, boolean observerWantsSelfNotifications, int flags, int userHandle) {
        ...
        userHandle = handleIncomingUser(uri, pid, uid, 2, userHandle);
        String msg = ((ActivityManagerInternal) LocalServices.getService(ActivityManagerInternal.class)).checkContentProviderAccess(uri.getAuthority(), userHandle);
        if (msg != null) {
    ...
    private int checkUriPermission(Uri uri, int pid, int uid, int modeFlags, int userHandle) {
        try {
            return ActivityManagerNative.getDefault().checkUriPermission(uri, pid, uid, modeFlags, userHandle, null);
        } catch (RemoteException e) {
            return -1;
        }
    }
