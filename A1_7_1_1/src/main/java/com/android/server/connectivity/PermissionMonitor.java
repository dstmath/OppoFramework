...
public class PermissionMonitor {
    ...
    private boolean hasNetworkPermission(PackageInfo app) {
        return hasPermission(app, OppoPermissionConstants.PERMISSION_GPRS);
    }
    ...
    private void update(Set<Integer> users, Map<Integer, Boolean> apps, boolean add) {
        ...
        for (Entry<Integer, Boolean> app : apps.entrySet()) {
            ...
            for (Integer intValue : users) {
                ...
                if (add || ((Integer) app.getKey()).intValue() != 1000) {
                    list.add(Integer.valueOf(UserHandle.getUid(user, ((Integer) app.getKey()).intValue())));
                } else {
    ...
    private synchronized void onAppAdded(String appName, int appUid) {
        ...
        try {
            ...
            if (isNetwork || hasRestrictedPermission) {
                ...
                if (permission == null || !permission.booleanValue()) {
                    this.mApps.put(Integer.valueOf(appUid), Boolean.valueOf(hasRestrictedPermission));
                    Map<Integer, Boolean> apps = new HashMap();
                    apps.put(Integer.valueOf(appUid), Boolean.valueOf(hasRestrictedPermission));
    ...
    private synchronized void onAppRemoved(int appUid) {
        ...
        Map<Integer, Boolean> apps = new HashMap();
        apps.put(Integer.valueOf(appUid), Boolean.valueOf(false));
