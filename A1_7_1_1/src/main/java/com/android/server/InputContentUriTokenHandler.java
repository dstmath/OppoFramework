...
final class InputContentUriTokenHandler extends Stub {
    ...
    private void doTakeLocked(IBinder permissionOwner) {
        ...
        try {
            ActivityManagerNative.getDefault().grantUriPermissionFromOwner(permissionOwner, this.mSourceUid, this.mTargetPackage, this.mUri, 1, this.mSourceUserId, this.mTargetUserId);
        } catch (RemoteException e) {
            e.rethrowFromSystemServer();
        } catch (Throwable th) {
