...
public class AgingCriticalEvent {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.AgingCriticalEvent.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.AgingCriticalEvent.<clinit>():void, dex: 
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.AgingCriticalEvent.<clinit>():void");
    }
    ...
    private void writeEventInner(String strEvent, String... args) {
        ...
        Writer writer;
        Throwable th;
        String strOutput = ((IElsaManager.EMPTY_PACKAGE + "[" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(System.currentTimeMillis())) + "]") + "[" + Long.toString(SystemClock.elapsedRealtime() / 1000) + "s]") + strEvent;
        for (String arg : args) {
            ...
            try {
                ...
                try {
                    if (EVENT_SYSTEM_BOOTUP == strEvent) {
                        String strPowerLongpressOutput = SystemProperties.get(powerLongPressTempProperty, IElsaManager.EMPTY_PACKAGE);
                        if (!strPowerLongpressOutput.equals(IElsaManager.EMPTY_PACKAGE)) {
                            SystemProperties.set(powerLongPressTempProperty, IElsaManager.EMPTY_PACKAGE);
                            out2.write(strPowerLongpressOutput + "\n");
                        }
                        String systemserverCrashOutput = SystemProperties.get(systemserverCrashTempProperty, IElsaManager.EMPTY_PACKAGE);
                        if (!systemserverCrashOutput.equals(IElsaManager.EMPTY_PACKAGE)) {
