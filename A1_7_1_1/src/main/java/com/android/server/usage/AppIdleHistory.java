...
public class AppIdleHistory {
    ...
    private static class PackageHistory {
        ...
        /* synthetic */ PackageHistory(PackageHistory packageHistory) {
            this();
        }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:21:0x0099 A:{Splitter: B:1:0x0003, ExcHandler: java.io.IOException (e java.io.IOException), PHI: r2 } */
    /* JADX WARNING: Missing block: B:23:?, code:
            android.util.Slog.e(TAG, "Unable to read app idle file for user " + r13);
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
        ...
        try {
            ...
            if (type != 2) {
                ...
                while (true) {
                    ...
                    if (type == 1) {
                        break loop1;
                    } else if (type == 2 && parser.getName().equals(TAG_PACKAGE)) {
    ...
    public void dump(IndentingPrintWriter idpw, int userId) {
        ...
        if (userHistory != null) {
            ...
            for (int p = 0; p < P; p++) {
                ...
                idpw.print(" idle=" + (isIdleLocked(packageName, userId, elapsedRealtime) ? "y" : OppoCrashClearManager.CRASH_COUNT));
                idpw.println();
            }
            idpw.println();
