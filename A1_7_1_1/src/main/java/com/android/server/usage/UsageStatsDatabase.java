...
class UsageStatsDatabase {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.usage.UsageStatsDatabase.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.usage.UsageStatsDatabase.<clinit>():void, dex: 
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.usage.UsageStatsDatabase.<clinit>():void");
    }

    public UsageStatsDatabase(File dir) {
        this.mLock = new Object();
        File[] fileArr = new File[4];
        fileArr[0] = new File(dir, "daily");
        fileArr[1] = new File(dir, "weekly");
        fileArr[2] = new File(dir, "monthly");
        fileArr[3] = new File(dir, "yearly");
        this.mIntervalDirs = fileArr;
        this.mVersionFile = new File(dir, CompatibilityHelper.VERSION_NAME);
    ...
    /* JADX WARNING: Missing block: B:74:0x0211, code:
            return r13;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
        ...
        synchronized (this.mLock) {
            if (intervalType >= 0) {
                if (intervalType < this.mIntervalDirs.length) {
                    ...
                    ActivityManagerService ams = (ActivityManagerService) ActivityManagerNative.getDefault();
                    ComponentName topApp = null;
                    if (ams != null) {
                        topApp = ams.getTopAppName();
                    }
                    if (topApp != null) {
                        isChooserActivity = "com.android.internal.app.ChooserActivity".equals(topApp.getClassName());
                    } else {
                        isChooserActivity = false;
                    }
                    for (i = startIndex; i <= endIndex; i++) {
                        f = (AtomicFile) intervalStats.valueAt(i);
                        if (isChooserActivity) {
                            try {
                                if (f.getBaseFile().length() > 204800) {
                                    Slog.i(TAG, "ChooserActivity not read from disk file(>200KB) " + f.getBaseFile().length());
                                }
                            } catch (IOException e) {
                                Slog.e(TAG, "Failed to read usage stats file", e);
                            }
                        ...
                        if (mHyp != null) {
                            mHyp.hypnusSetAction(19, 10000);
                            Slog.i(TAG, "hypnusSetAction start action");
                        }
                        UsageStatsXml.read(f, stats);
                        if (mHyp != null) {
                            mHyp.hypnusSetAction(19, 0);
                            Slog.i(TAG, "hypnusSetAction cancel action");
                        }
                    ...
                    if (end - start > 15000) {
                        Slog.i(TAG, "endTime(" + endTime + ") and  beginTime(" + beginTime + ") + duration(" + (end - start) + ")");
                        Slog.i(TAG, "startIndex= " + startIndex + ", endIndex= " + endIndex);
                        for (i = startIndex; i <= endIndex; i++) {
                            f = (AtomicFile) intervalStats.valueAt(i);
                            Slog.i(TAG, "Reading stat file " + f.getBaseFile().getAbsolutePath());
                        }
                    }
                }
            }
    ...
    private static IntervalStats deserializeIntervalStats(byte[] data) {
        ...
        try {
            ...
            UsageStatsXml.read(new BufferedInputStream(bais), stats);
            return stats;
        } catch (IOException ioe) {
