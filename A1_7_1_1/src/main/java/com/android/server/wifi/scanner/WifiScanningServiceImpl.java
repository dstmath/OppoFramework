...
public class WifiScanningServiceImpl extends Stub {
    ...
    private final RequestList<Void> mSingleScanListeners;
    ...
    private class ClientHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            boolean verboseLoggingLevel = false;
                ...
                default:
                    try {
                        ...
                        switch (msg.what) {
                            ...
                            case 159773:
                                if (msg.arg1 > 0) {
                                    verboseLoggingLevel = true;
                                }
                                WifiScanningServiceImpl.DBG = verboseLoggingLevel;
                                WifiScanningServiceImpl.this.localLog("DBG " + WifiScanningServiceImpl.DBG + " verboseLoggingLevel: " + verboseLoggingLevel);
                                WifiScanningServiceImpl.this.enableVerboseLogging(msg.arg1);
                                break;
                            default:
    ...
    private class RequestList<T> extends ArrayList<RequestInfo<T>> {
        /* synthetic */ RequestList(WifiScanningServiceImpl this$0, RequestList requestList) {
            this();
        }
    ...
    class WifiBackgroundScanStateMachine extends StateMachine implements ScanEventHandler, HotlistEventHandler {
        ...
        private final RequestList<HotlistSettings> mActiveHotlistSettings = new RequestList(WifiScanningServiceImpl.this, null);
            ...
            public boolean processMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case 159748:
                        ...
                        break;
                    case 159750:
                        if (!WifiBackgroundScanStateMachine.this.addHotlist(ci, msg.arg2, (HotlistSettings) msg.obj)) {
                            WifiScanningServiceImpl.this.replyFailed(msg, -3, "bad request");
                            break;
                        }
                        WifiScanningServiceImpl.this.replySucceeded(msg);
        ...
        public void onHotlistApFound(ScanResult[] results) {
            if (WifiScanningServiceImpl.DBG) {
                WifiScanningServiceImpl.this.localLog("onHotlistApFound event received");
            }
        ...
        public void onHotlistApLost(ScanResult[] results) {
            if (WifiScanningServiceImpl.DBG) {
                WifiScanningServiceImpl.this.localLog("onHotlistApLost event received");
            }
        ...
        private boolean updateSchedule() {
            ...
            for (b = 0; b < schedule.num_buckets; b++) {
                ...
                WifiScanningServiceImpl.this.localLog("bucket " + bucket.bucket + " (" + bucket.period_ms + "ms)" + "[" + bucket.report_events + "]: " + ChannelHelper.toString(bucket) + "(MAX" + bucket.max_period_ms + ") ");
            }
        ...
        private boolean addHotlist(ClientInfo ci, int handler, HotlistSettings settings) {
            if (ci == null) {
                Log.d(WifiScanningServiceImpl.TAG, "Failing hotlist request ClientInfo not found " + handler);
                return false;
            }
        ...
        private void reportHotlistResults(int what, ScanResult[] results) {
            if (WifiScanningServiceImpl.DBG) {
                WifiScanningServiceImpl.this.localLog("reportHotlistResults " + what + " results " + results.length);
            }
    ...
    class WifiChangeStateMachine extends StateMachine implements SignificantWifiChangeEventHandler {
            ...
            public boolean processMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case 159756:
                        break;
                    case 159755:
                        if (!WifiChangeStateMachine.this.addWifiChangeHandler(ci, msg.arg2)) {
                            WifiScanningServiceImpl.this.replyFailed(msg, -3, "bad request");
                            break;
                        }
                        WifiScanningServiceImpl.this.replySucceeded(msg);
                ...
                switch (msg.what) {
                    case 159749:
                        ...
                        if (this.mScanResultsPending) {
                            ...
                            WifiChangeStateMachine.this.reconfigureScan(msg.obj.getResults(), WifiChangeStateMachine.STATIONARY_SCAN_PERIOD_MS);
        ...
        private void reconfigureScan(ScanData[] results, int period) {
            ...
            for (i = 0; i < brightest.length; i++) {
                boolean found = false;
                for (j = i + 1; j < brightest.length; j++) {
        ...
        private boolean addWifiChangeHandler(ClientInfo ci, int handler) {
            if (ci == null) {
                Log.d(WifiScanningServiceImpl.TAG, "Failing wifi change request ClientInfo not found " + handler);
                return false;
            }
    ...
    class WifiSingleScanStateMachine extends StateMachine implements ScanEventHandler {
        ...
        class ScanningState extends State {
            ...
            public void enter() {
                if (WifiScanningServiceImpl.DBG) {
                    Log.d(WifiScanningServiceImpl.TAG, "WifiSingleScanStateMachine enter " + getClass().getSimpleName());
                }
        ...
        boolean activeScanSatisfies(WifiScanner.ScanSettings settings) {
            ...
            if (settings.hiddenNetworkIds != null) {
                if (this.mActiveScanSettings.hiddenNetworkIds == null) {
                    return false;
                }
                Set<Integer> activeHiddenNetworkIds = new HashSet();
                for (int id : this.mActiveScanSettings.hiddenNetworkIds) {
                    activeHiddenNetworkIds.add(Integer.valueOf(id));
                }
                for (int id2 : settings.hiddenNetworkIds) {
                    if (!activeHiddenNetworkIds.contains(Integer.valueOf(id2))) {
                        return false;
                    }
                }
            }
        ...
        void tryToStartNewScan() {
            if (this.mPendingScans.size() != 0) {
                ...
                BucketSettings[] bucketSettingsArr = new BucketSettings[1];
                bucketSettingsArr[0] = bucketSettings;
                settings.buckets = bucketSettingsArr;
                WifiScanningServiceImpl.this.localLog("tryToStartNewScan startSingleScan: base_period_ms" + settings.base_period_ms + " num_buckets= " + settings.num_buckets);
        ...
        void reportScanResults(ScanData results) {
            ...
            ScanData[] allResults = new ScanData[1];
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.wifi.scanner.WifiScanningServiceImpl.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.wifi.scanner.WifiScanningServiceImpl.<clinit>():void, dex: 
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.wifi.scanner.WifiScanningServiceImpl.<clinit>():void");
    }
    ...
    WifiScanningServiceImpl(Context context, Looper looper, WifiScannerImplFactory scannerImplFactory, IBatteryStats batteryStats, WifiInjector wifiInjector) {
        this.mLocalLog = new LocalLog(512);
        this.mSingleScanListeners = new RequestList(this, null);
    ...
    private void logStateAndMessage(Message message, State state) {
        if (DBG) {
            Log.d(TAG, " " + state.getClass().getSimpleName() + " " + message);
        }
    }
}
