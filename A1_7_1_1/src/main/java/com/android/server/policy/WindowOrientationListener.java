...
public abstract class WindowOrientationListener {
    ...
    final class AccelSensorJudge extends OrientationJudge {
        ...
        public AccelSensorJudge(Context context) {
            super();
            int[][] iArr = new int[4][];
            iArr[0] = new int[]{-25, 70};
            iArr[1] = new int[]{-25, 65};
            iArr[2] = new int[]{-25, 60};
            iArr[3] = new int[]{-25, 65};
            this.mTiltToleranceConfig = iArr;
            int[] tiltTolerance = context.getResources().getIntArray(17235999);
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            synchronized (WindowOrientationListener.this.mLock) {
                ...
                if (now < then || now > 1000000000 + then || (x == OppoBrightUtils.MIN_LUX_LIMITI && y == OppoBrightUtils.MIN_LUX_LIMITI && z == OppoBrightUtils.MIN_LUX_LIMITI)) {
                    if (WindowOrientationListener.LOG) {
    ...
    final class OrientationSensorJudge extends OrientationJudge {
        ...
        private Runnable mRotationEvaluator = new Runnable() {
            public void run() {
                ...
                if (newRotation >= 0) {
                    Slog.v(WindowOrientationListener.TAG, "Proposed rotation changed!  newRotation=" + newRotation);
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (newRotation >= 0) {
                Slog.v(WindowOrientationListener.TAG, "Proposed rotation changed!  mDesiredRotation=" + this.mDesiredRotation + ", newRotation=" + newRotation);
                WindowOrientationListener.this.onProposedRotationChanged(newRotation);
            }
        }
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.policy.WindowOrientationListener.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.policy.WindowOrientationListener.<clinit>():void, dex: 
        */
