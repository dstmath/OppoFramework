...
public class PackageManagerBackupAgent extends BackupAgent {
    ...
    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: com.android.server.backup.PackageManagerBackupAgent.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: com.android.server.backup.PackageManagerBackupAgent.<clinit>():void, dex: 
        */
        throw new UnsupportedOperationException("Method not decompiled: com.android.server.backup.PackageManagerBackupAgent.<clinit>():void");
    }
    ...
    public void onBackup(ParcelFileDescriptor oldState, BackupDataOutput data, ParcelFileDescriptor newState) {
        ...
        try {
            boolean needHomeBackup = (homeVersion == this.mStoredHomeVersion && Objects.equals(home, this.mStoredHomeComponent)) ? home != null ? !BackupUtils.signaturesMatch(this.mStoredHomeSigHashes, packageInfo) : false : true;
            if (needHomeBackup) {
                if (DEBUG) {
            ...
            if (DEBUG && this.mExisting.size() > 0) {
                StringBuilder stringBuilder = new StringBuilder(64);
                stringBuilder.append("Preserving metadata for deleted packages:");
    ...
    public void onRestore(BackupDataInput data, int appVersionCode, ParcelFileDescriptor newState) throws IOException {
        ...
        while (data.readNextHeader()) {
            ...
            if (key.equals(GLOBAL_METADATA_KEY)) {
                ...
                if (DEBUG) {
                    Slog.v(TAG, "   storedSystemVersion = " + -1);
                }
                ...
                if (DEBUG) {
                    Slog.i(TAG, "Restore set version " + -1 + " is compatible with OS version " + VERSION.SDK_INT + " (" + this.mStoredIncrementalVersion + " vs " + VERSION.INCREMENTAL + ")");
                }
                ...
                if (DEBUG) {
                    Slog.i(TAG, "   read preferred home app " + this.mRestoredHome + " version=" + this.mRestoredHomeVersion + " installer=" + this.mRestoredHomeInstaller + " sig=" + this.mRestoredHomeSigHashes);
                }
            } else {
