...
public class BigDecimal extends Number implements Comparable<BigDecimal>, Serializable {
    /* renamed from: -java-math-RoundingModeSwitchesValues */
    private static final /* synthetic */ int[] f12-java-math-RoundingModeSwitchesValues = null;
    ...
    /* renamed from: -getjava-math-RoundingModeSwitchesValues */
    private static /* synthetic */ int[] m4-getjava-math-RoundingModeSwitchesValues() {
        if (f12-java-math-RoundingModeSwitchesValues != null) {
            return f12-java-math-RoundingModeSwitchesValues;
        }
        ...
        f12-java-math-RoundingModeSwitchesValues = iArr;
        return iArr;
    }

    /*  JADX ERROR: Method load error
        jadx.core.utils.exceptions.DecodeException: Load method exception: bogus opcode: 0073 in method: java.math.BigDecimal.<clinit>():void, dex: 
    ...
    static {
        /*
        // Can't load method instructions: Load method exception: bogus opcode: 0073 in method: java.math.BigDecimal.<clinit>():void, dex: 
        */
    ...
    private static BigDecimal divideBigIntegers(BigInteger scaledDividend, BigInteger scaledDivisor, int scale, RoundingMode roundingMode) {
        ...
        if (scaledDivisor.bitLength() < 63) {
            compRem = roundingBehavior(quotient.testBit(0) ? 1 : 0, (longCompareTo(Math.abs(remainder.longValue()) * 2, Math.abs(scaledDivisor.longValue())) + 5) * sign, roundingMode);
        } else {
    ...
    private static BigDecimal dividePrimitiveLongs(long scaledDividend, long scaledDivisor, int scale, RoundingMode roundingMode) {
        ...
        if (remainder != 0) {
            quotient += (long) roundingBehavior(((int) quotient) & 1, (longCompareTo(Math.abs(remainder) * 2, Math.abs(scaledDivisor)) + 5) * sign, roundingMode);
        }
    ...
    public BigDecimal divide(BigDecimal divisor, MathContext mc) {
        ...
        BigInteger[] quotAndRem = new BigInteger[1];
        quotAndRem[0] = getUnscaledValue();
    ...
    private void smallRound(MathContext mc, int discardedPrecision) {
        ...
        if (fraction != 0) {
            integer += (long) roundingBehavior(((int) integer) & 1, Long.signum(fraction) * (longCompareTo(Math.abs(fraction) * 2, sizeOfFraction) + 5), mc.getRoundingMode());
    ...
    private static int roundingBehavior(int parityBit, int fraction, RoundingMode roundingMode) {
        switch (m4-getjava-math-RoundingModeSwitchesValues()[roundingMode.ordinal()]) {
            case 1:
