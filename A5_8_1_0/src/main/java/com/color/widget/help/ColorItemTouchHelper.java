...
import com.color.widget.ColorRecyclerView.LayoutManager;
...
import com.color.widget.ColorRecyclerView.State;
...
public class ColorItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener {
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        public boolean onInterceptTouchEvent(ColorRecyclerView recyclerView, MotionEvent event) {
            ColorItemTouchHelper.this.mGestureDetector.onTouchEvent(event);
            int action = event.getActionMasked();
            if (action == 0) {
                ColorItemTouchHelper.this.mActivePointerId = event.getPointerId(0);
                ColorItemTouchHelper.this.mInitialTouchX = event.getX();
                ColorItemTouchHelper.this.mInitialTouchY = event.getY();
                ColorItemTouchHelper.this.obtainVelocityTracker();
                if (ColorItemTouchHelper.this.mSelected == null) {
                    RecoverAnimation animation = ColorItemTouchHelper.this.findAnimation(event);
                    if (animation != null) {
                        ColorItemTouchHelper colorItemTouchHelper = ColorItemTouchHelper.this;
                        colorItemTouchHelper.mInitialTouchX -= animation.mX;
                        colorItemTouchHelper = ColorItemTouchHelper.this;
                        colorItemTouchHelper.mInitialTouchY -= animation.mY;
                        ColorItemTouchHelper.this.endRecoverAnimation(animation.mViewHolder, true);
                        if (ColorItemTouchHelper.this.mPendingCleanup.remove(animation.mViewHolder.itemView)) {
                            ColorItemTouchHelper.this.mCallback.clearView(ColorItemTouchHelper.this.mRecyclerView, animation.mViewHolder);
                        }
                        ColorItemTouchHelper.this.select(animation.mViewHolder, animation.mActionState);
                        ColorItemTouchHelper.this.updateDxDy(event, ColorItemTouchHelper.this.mSelectedFlags, 0);
                    }
                }
            } else if (action == 3 || action == 1) {
                ColorItemTouchHelper.this.mActivePointerId = -1;
                ColorItemTouchHelper.this.select(null, 0);
            } else if (ColorItemTouchHelper.this.mActivePointerId != -1) {
                int index = event.findPointerIndex(ColorItemTouchHelper.this.mActivePointerId);
                if (index >= 0) {
                    ColorItemTouchHelper.this.checkSelectForSwipe(action, event, index);
                }
            }
            if (ColorItemTouchHelper.this.mVelocityTracker != null) {
                ColorItemTouchHelper.this.mVelocityTracker.addMovement(event);
            }
            if (ColorItemTouchHelper.this.mSelected != null) {
                return true;
            }
        ...
        public void onTouchEvent(ColorRecyclerView recyclerView, MotionEvent event) {
            ColorItemTouchHelper.this.mGestureDetector.onTouchEvent(event);
            if (ColorItemTouchHelper.this.mVelocityTracker != null) {
                ColorItemTouchHelper.this.mVelocityTracker.addMovement(event);
            }
            if (ColorItemTouchHelper.this.mActivePointerId != -1) {
                int action = event.getActionMasked();
                int activePointerIndex = event.findPointerIndex(ColorItemTouchHelper.this.mActivePointerId);
                if (activePointerIndex >= 0) {
                    ColorItemTouchHelper.this.checkSelectForSwipe(action, event, activePointerIndex);
                }
                ViewHolder viewHolder = ColorItemTouchHelper.this.mSelected;
                if (viewHolder != null) {
                    switch (action) {
                        case 1:
                            break;
                        case 2:
                            if (activePointerIndex >= 0) {
                                ColorItemTouchHelper.this.updateDxDy(event, ColorItemTouchHelper.this.mSelectedFlags, activePointerIndex);
                                ColorItemTouchHelper.this.moveIfNecessary(viewHolder);
                                ColorItemTouchHelper.this.mRecyclerView.removeCallbacks(ColorItemTouchHelper.this.mScrollRunnable);
                                ColorItemTouchHelper.this.mScrollRunnable.run();
                                ColorItemTouchHelper.this.mRecyclerView.invalidate();
                                break;
                            }
                            break;
                        case 3:
                            if (ColorItemTouchHelper.this.mVelocityTracker != null) {
                                ColorItemTouchHelper.this.mVelocityTracker.clear();
                                break;
                            }
                            break;
                        case 6:
                            int pointerIndex = event.getActionIndex();
                            if (event.getPointerId(pointerIndex) == ColorItemTouchHelper.this.mActivePointerId) {
                                ColorItemTouchHelper.this.mActivePointerId = event.getPointerId(pointerIndex == 0 ? 1 : 0);
                                ColorItemTouchHelper.this.updateDxDy(event, ColorItemTouchHelper.this.mSelectedFlags, pointerIndex);
                                break;
                            }
                            break;
                    }
    ...
    private final Runnable mScrollRunnable = new Runnable() {
        public void run() {
            if (ColorItemTouchHelper.this.mSelected != null && ColorItemTouchHelper.this.scrollIfNecessary()) {
                if (ColorItemTouchHelper.this.mSelected != null) {
                    ColorItemTouchHelper.this.moveIfNecessary(ColorItemTouchHelper.this.mSelected);
                }
                ColorItemTouchHelper.this.mRecyclerView.removeCallbacks(ColorItemTouchHelper.this.mScrollRunnable);
                ColorItemTouchHelper.this.mRecyclerView.postOnAnimation(this);
            }
        }
    };
    ...
    public static abstract class Callback {
        ...
        private static final ColorItemTouchUIUtil sUICallback;
        ...
        final int getAbsoluteMovementFlags(ColorRecyclerView recyclerView, ViewHolder viewHolder) {
            return convertToAbsoluteDirection(getMovementFlags(recyclerView, viewHolder), recyclerView.getLayoutDirection());
        }

        private boolean hasDragFlag(ColorRecyclerView recyclerView, ViewHolder viewHolder) {
            if ((ColorItemTouchHelper.ACTION_MODE_DRAG_MASK & getAbsoluteMovementFlags(recyclerView, viewHolder)) != 0) {
                return true;
            }
            return false;
        }

        private boolean hasSwipeFlag(ColorRecyclerView recyclerView, ViewHolder viewHolder) {
            if ((ColorItemTouchHelper.ACTION_MODE_SWIPE_MASK & getAbsoluteMovementFlags(recyclerView, viewHolder)) != 0) {
                return true;
            }
            return false;
        }
        ...
        private int getMaxDragScroll(ColorRecyclerView recyclerView) {
            if (this.mCachedMaxScrollSpeed == -1) {
                this.mCachedMaxScrollSpeed = recyclerView.getResources().getDimensionPixelSize(201655454);
            }
            return this.mCachedMaxScrollSpeed;
        }

        public void onMoved(ColorRecyclerView recyclerView, ViewHolder viewHolder, int fromPos, ViewHolder target, int toPos, int x, int y) {
        ...
        private void onDraw(Canvas c, ColorRecyclerView parent, ViewHolder selected, List<RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            int count;
        ...
        private void onDrawOver(Canvas c, ColorRecyclerView parent, ViewHolder selected, List<RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            int i;
        ...
        public void clearView(ColorRecyclerView recyclerView, ViewHolder viewHolder) {
            sUICallback.clearView(viewHolder.itemView);
        }

        public void onChildDraw(Canvas c, ColorRecyclerView recyclerView, ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
            sUICallback.onDraw(c, recyclerView, viewHolder.itemView, dX, dY, actionState, isCurrentlyActive);
        }

        public void onChildDrawOver(Canvas c, ColorRecyclerView recyclerView, ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
            sUICallback.onDrawOver(c, recyclerView, viewHolder.itemView, dX, dY, actionState, isCurrentlyActive);
        }

        public long getAnimationDuration(ColorRecyclerView recyclerView, int animationType, float animateDx, float animateDy) {
            ...
            if (itemAnimator == null) {
                ...
                if (animationType == 8) {
                    ...
                    i = DEFAULT_SWIPE_ANIMATION_DURATION;
                }
                return (long) i;
            }
            long moveDuration;
    ...
    private class ItemTouchHelperGestureListener extends SimpleOnGestureListener {
        /* synthetic */ ItemTouchHelperGestureListener(ColorItemTouchHelper this$0, ItemTouchHelperGestureListener -this1) {
            this();
        }
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            View child = ColorItemTouchHelper.this.findChildView(e);
            if (child != null) {
                ViewHolder vh = ColorItemTouchHelper.this.mRecyclerView.getChildViewHolder(child);
                if (vh != null) {
                    ColorItemTouchHelper.this.mRecyclerView.playSoundEffect(0);
                    ColorItemTouchHelper.this.mCallback.onItemClick(vh);
                }
            }
            return false;
        }
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (child != null) {
                ViewHolder vh = ColorItemTouchHelper.this.mRecyclerView.getChildViewHolder(child);
                if (vh != null && ColorItemTouchHelper.this.mCallback.hasDragFlag(ColorItemTouchHelper.this.mRecyclerView, vh) && e.getPointerId(0) == ColorItemTouchHelper.this.mActivePointerId) {
                    int index = e.findPointerIndex(ColorItemTouchHelper.this.mActivePointerId);
                    float x = e.getX(index);
                    float y = e.getY(index);
                    ColorItemTouchHelper.this.mInitialTouchX = x;
                    ColorItemTouchHelper.this.mInitialTouchY = y;
                    ColorItemTouchHelper colorItemTouchHelper = ColorItemTouchHelper.this;
                    ColorItemTouchHelper.this.mDy = 0.0f;
                    colorItemTouchHelper.mDx = 0.0f;
                    if (ColorItemTouchHelper.this.mCallback.isLongPressDragEnabled()) {
                        ColorItemTouchHelper.this.select(vh, 2);
                    }
                }
            }
        }
    ...
    public static abstract class SimpleCallback extends Callback {
        ...
        public int getSwipeDirs(ColorRecyclerView recyclerView, ViewHolder viewHolder) {
            return this.mDefaultSwipeDirs;
        }

        public int getDragDirs(ColorRecyclerView recyclerView, ViewHolder viewHolder) {
            return this.mDefaultDragDirs;
        }

        public int getMovementFlags(ColorRecyclerView recyclerView, ViewHolder viewHolder) {
            return Callback.makeMovementFlags(getDragDirs(recyclerView, viewHolder), getSwipeDirs(recyclerView, viewHolder));
        }
    }

    public ColorItemTouchHelper(Callback callback) {
    ...
    public void attachToRecyclerView(ColorRecyclerView recyclerView) {
        if (this.mRecyclerView != recyclerView) {
            ...
            if (this.mRecyclerView != null) {
                ...
                this.mSwipeEscapeVelocity = resources.getDimension(201655496);
                this.mMaxSwipeVelocity = resources.getDimension(201655497);
                setupCallbacks();
            }
        }
    }
    ...
    private void setupCallbacks() {
        ...
        this.mRecyclerView.addOnItemTouchListenerSy(this.mOnItemTouchListener);
    ...
    private void postDispatchSwipe(final RecoverAnimation anim, final int swipeDir) {
        this.mRecyclerView.post(new Runnable() {
            public void run() {
                if (ColorItemTouchHelper.this.mRecyclerView != null && ColorItemTouchHelper.this.mRecyclerView.isAttachedToWindow() && (anim.mOverridden ^ 1) != 0 && anim.mViewHolder.getAdapterPosition() != -1) {
                    ItemAnimator animator = ColorItemTouchHelper.this.mRecyclerView.getItemAnimator();
                    if ((animator == null || (animator.isRunning(null) ^ 1) != 0) && (ColorItemTouchHelper.this.hasRunningRecoverAnim() ^ 1) != 0) {
                        ColorItemTouchHelper.this.mCallback.onSwiped(anim.mViewHolder, swipeDir);
                    } else {
                        ColorItemTouchHelper.this.mRecyclerView.post(this);
                    }
                }
            }
        });
            ...
            if (this.mChildDrawingOrderCallback == null) {
                this.mChildDrawingOrderCallback = new ChildDrawingOrderCallback() {
                    public int onGetChildDrawingOrder(int childCount, int i) {
                        if (ColorItemTouchHelper.this.mOverdrawChild == null) {
                            return i;
                        }
                        int childPosition = ColorItemTouchHelper.this.mOverdrawChildPosition;
                        if (childPosition == -1) {
                            childPosition = ColorItemTouchHelper.this.mRecyclerView.indexOfChild(ColorItemTouchHelper.this.mOverdrawChild);
                            ColorItemTouchHelper.this.mOverdrawChildPosition = childPosition;
                        }
                        if (i == childCount - 1) {
