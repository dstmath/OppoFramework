...
class TelephonyRegistry extends Stub {
    ...
    TelephonyRegistry(Context context) {
        ...
        for (i = 0; i < numPhones; i++) {
            ...
            this.mVoLteServiceState[i] = new VoLteServiceState(5);
        }
        if (location != null) {
            for (i = 0; i < numPhones; i++) {
        ...
        if (events != 0) {
            ...
            synchronized (this.mRecords) {
                ...
                try {
                    ...
                    boolean disableVolteCallBack = false;
                    SubscriptionManager subscriptionManager = SubscriptionManager.from(this.mContext);
                    if (!(subscriptionManager == null || r == null || (events & 16384) == 0 || (subscriptionManager.isActiveSubId(r.subId) ^ 1) == 0)) {
                        log("listen:  Register callingPackage " + callingPackage + " with out-of-date subid " + r.subId);
                        disableVolteCallBack = true;
                    }
                    ...
                    if (notifyNow && validatePhoneId(phoneId)) {
                        ...
                        if (!((events & 16384) == 0 || (disableVolteCallBack ^ 1) == 0)) {
                            try {
                                r.callback.onVoLteServiceStateChanged(this.mVoLteServiceState[phoneId]);
    ...
    private boolean canReadPhoneState(String callingPackage) {
        ...
        boolean canReadPhoneState = this.mContext.checkCallingOrSelfPermission(OppoPermissionConstants.PERMISSION_READ_PHONE_STATE) == 0;
    ...
    public void notifyDataConnectionForSubscriber(int subId, int state, boolean isDataAllowed, String reason, String apn, String apnType, LinkProperties linkProperties, NetworkCapabilities networkCapabilities, int networkType, boolean roaming) {
        if (checkNotifyPermission("notifyDataConnection()")) {
            ...
            if (state != 0 || canBrocast) {
                String[] defaultApnTypes = new String[]{"default", "mms", "supl", "hipri", "fota", "ims", "cbs"};
                if (apn != null) {
                    isBrocast = true;
                    log("notifyDataConnection apn != null");
                    ...
                    for (String str2 : defaultApnTypes) {
                        log("notifyDataConnection defaultApnTypes = " + str2);
                        broadcastDataConnectionStateChanged(state, isDataAllowed, reason, apn, apnType, linkProperties, networkCapabilities, roaming, subId);
                        broadcastPreciseDataConnectionStateChanged(state, networkType, apnType, apn, reason, linkProperties, "");
                    }
                }
            }
            if (!isBrocast && ("dataDisabled".equals(reason) || "default".equals(apnType) || "mms".equals(apnType))) {
                log("notifyDataConnection apnType == default or mms");
                broadcastDataConnectionStateChanged(state, isDataAllowed, reason, apn, apnType, linkProperties, networkCapabilities, roaming, subId);
                broadcastPreciseDataConnectionStateChanged(state, networkType, apnType, apn, reason, linkProperties, "");
            }
        }
    ...
    public void notifyVoLteServiceStateChanged(int phoneId, int subId, VoLteServiceState lteState) {
        if (checkNotifyPermission("notifyVoLteServiceStateChanged()")) {
            synchronized (this.mRecords) {
                this.mLocalLog.log("notifyVoLteServiceStateChanged: subId=" + subId + " phoneId=" + phoneId + " VoLteServiceState=" + lteState);
                if (validatePhoneId(phoneId)) {
                    this.mVoLteServiceState[phoneId] = lteState;
                    for (Record r : this.mRecords) {
                        if (r.matchPhoneStateListenerEvent(16384) && idMatch(r.subId, subId, phoneId)) {
                            try {
                                r.callback.onVoLteServiceStateChanged(new VoLteServiceState(lteState));
                            } catch (RemoteException e) {
                                this.mRemoveList.add(r.binder);
                            }
                        }
                    ...
                    log("notifyVoLteServiceStateChanged: INVALID phoneId=" + phoneId);
                }
                handleRemoveListLocked();
            }
        }
