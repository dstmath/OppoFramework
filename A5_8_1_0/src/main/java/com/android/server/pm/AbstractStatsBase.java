...
public abstract class AbstractStatsBase<T> {
    ...
    protected boolean maybeWriteAsync(final T data) {
        if ((SystemClock.elapsedRealtime() - this.mLastTimeWritten.get() < 1800000 && (PackageManagerService.DEBUG_DEXOPT ^ 1) != 0) || !this.mBackgroundWriteRunning.compareAndSet(false, true)) {
            return false;
        }
