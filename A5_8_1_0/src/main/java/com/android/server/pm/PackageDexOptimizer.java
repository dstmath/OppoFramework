            ...
            if (pathsWithCode[i]) {
                ...
                if (options.getSplitName() == null || options.getSplitName().equals(new File(path).getName())) {
                    ...
                    while (true) {
                        ...
                        int newResult = dexOptPath(pkg, path, dexCodeInstructionSets[i3], compilerFilter, profileUpdated, classLoaderContexts[i], dexoptFlags, sharedGid, packageStats, options.isDowngrade());
