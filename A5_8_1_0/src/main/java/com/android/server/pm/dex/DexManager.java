...
public class DexManager {
    ...
    private void notifyDexLoadInternal(ApplicationInfo loadingAppInfo, List<String> classLoaderNames, List<String> classPaths, String loaderIsa, int loaderUserId) {
        if (classLoaderNames.size() != classPaths.size()) {
            ...
            while (true) {
                int i2 = i;
                if (i2 < length) {
                    String dexPath = dexPathsToRegister[i2];
    ...
    public RegisterDexModuleResult registerDexModule(ApplicationInfo info, String dexPath, boolean isUsedByOtherApps, int userId) {
        ...
        int i = 0;
        int length = appDexInstructionSets.length;
        while (true) {
            int i2 = i;
            if (i2 >= length) {
                break;
            ...
            String str = dexPath;
            int i3 = userId;
            update |= this.mPackageDexUsage.record(searchResult.mOwningPackageName, str, i3, appDexInstructionSets[i2], isUsedByOtherApps, false, searchResult.mOwningPackageName, "=UnknownClassLoaderContext=");
            i = i2 + 1;
        }
        if (update) {
            this.mPackageDexUsage.maybeWriteAsync();
