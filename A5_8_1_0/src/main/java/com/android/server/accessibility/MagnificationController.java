...
class MagnificationController implements Callback {
    ...
    public static class SettingsBridge {
        ...
        public float getMagnificationScale(int userId) {
            return Secure.getFloatForUser(this.mContentResolver, "accessibility_display_magnification_scale", 2.0f, userId);
        }
    }
    ...
    private static class SpecAnimationBridge implements AnimatorUpdateListener {
        ...
        private SpecAnimationBridge(Context context, Object lock, WindowManagerInternal wm, ValueAnimator animator) {
            ...
            this.mValueAnimator.setFloatValues(new float[]{OppoBrightUtils.MIN_LUX_LIMITI, 1.0f});
    ...
    private boolean updateMagnificationSpecLocked(float scale, float centerX, float centerY) {
        ...
        changed |= updateCurrentSpecWithOffsetsLocked(((((float) this.mMagnificationBounds.width()) / 2.0f) + ((float) this.mMagnificationBounds.left)) - (centerX * normScale), ((((float) this.mMagnificationBounds.height()) / 2.0f) + ((float) this.mMagnificationBounds.top)) - (centerY * normScale));
        if (changed) {
            onMagnificationChangedLocked();
        }
        return changed;
    ...
    private boolean updateCurrentSpecWithOffsetsLocked(float nonNormOffsetX, float nonNormOffsetY) {
        boolean changed = false;
        float offsetX = MathUtils.constrain(nonNormOffsetX, getMinOffsetXLocked(), OppoBrightUtils.MIN_LUX_LIMITI);
        if (Float.compare(this.mCurrentMagnificationSpec.offsetX, offsetX) != 0) {
            this.mCurrentMagnificationSpec.offsetX = offsetX;
            changed = true;
        }
        float offsetY = MathUtils.constrain(nonNormOffsetY, getMinOffsetYLocked(), OppoBrightUtils.MIN_LUX_LIMITI);
