...
public class MediaSessionService extends SystemService implements Monitor {
    ...
    private String EXP_VERSION_NAME = "oppo.version.exp";
    ...
    class SessionManagerImpl extends Stub {
        ...
        class KeyEventWakeLockReceiver extends ResultReceiver implements Runnable, OnFinished {
            ...
            public void aquireWakeLockLocked() {
                ...
                this.mHandler.postDelayed(this, FaceDaemonWrapper.TIMEOUT_FACED_BINDERCALL_CHECK);
            }
        ...
        public void setCallback(ICallback callback) {
            ...
            long token = Binder.clearCallingIdentity();
            try {
                if (UserHandle.isSameApp(uid, FingerprintAcquiredInfo.ACQUIRED_ALREADY_ENROLLED)) {
                    synchronized (MediaSessionService.this.mLock) {
    ...
    private void destroySessionLocked(MediaSessionRecord session) {
        ...
        if (this.mGlobalPrioritySession == session) {
            ...
            if (!(user == null || user.mLastMediaButtonReceiver == null)) {
                componentName = user.mLastMediaButtonReceiver.getIntent().getComponent();
                if (componentName != null && session.getPackageName().equals(componentName.getPackageName())) {
    ...
    private MediaSessionRecord createSessionLocked(int callerPid, int callerUid, int userId, String callerPackageName, ISessionCallback cb, String tag) {
        if (userId == OppoMultiAppManager.USER_ID && OppoMultiAppManagerUtil.getInstance().isMultiAllowedApp(callerPackageName)) {
            userId = 0;
        }
        FullUserRecord user = getFullUserRecordLocked(userId);
        if (user == null) {
