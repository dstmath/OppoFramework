...
public class TrustManagerService extends SystemService {
    ...
    private final IBinder mService = new Stub() {
        /* renamed from: lambda$-com_android_server_trust_TrustManagerService$1_33892 */
        static /* synthetic */ void m202lambda$-com_android_server_trust_TrustManagerService$1_33892() {
        }
                                ...
                                if (enabledAgents != null) {
                                    for (ResolveInfo resolveInfo : resolveAllowedTrustAgents(pm, userInfo.id)) {
                                        ...
                                        if (enabledAgents.contains(name)) {
                                            ...
                                            if (this.mUserManager.isUserUnlockingOrUnlocked(userInfo.id) || (directUnlock ^ 1) == 0) {
    ...
    private void refreshDeviceLockedForUser(int userId) {
        ...
        for (int i = 0; i < userInfos.size(); i++) {
            ...
            if (!(info == null || info.partial || (info.isEnabled() ^ 1) != 0 || info.guestToRemove || (info.supportsSwitchToByUser() ^ 1) != 0)) {
                ...
                if (secure && showingKeyguard && (trusted ^ 1) != 0) {
                    deviceLocked = fingerprintAuthenticated ^ 1;
                } else {
                    deviceLocked = false;
                }
    ...
    private static ComponentName getDefaultFactoryTrustAgent(Context context) {
        String defaultTrustAgent = context.getResources().getString(17039667);
