...
public class AppIdleHistory {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:21:0x0099 A:{PHI: r2 , Splitter: B:1:0x0003, ExcHandler: java.io.IOException (e java.io.IOException)} */
    ...
    public void dump(IndentingPrintWriter idpw, int userId) {
        ...
        if (userHistory != null) {
            ...
            for (int p = 0; p < P; p++) {
                ...
                idpw.print(" idle=" + (isIdle(packageName, userId, elapsedRealtime) ? "y" : OppoCrashClearManager.CRASH_COUNT));
                idpw.println();
            }
            idpw.println();
