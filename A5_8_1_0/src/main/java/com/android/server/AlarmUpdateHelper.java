...
public class AlarmUpdateHelper extends RomUpdateHelper {
    ...
    public int convertType(int type, PendingIntent operation, String packageName) {
        ...
        if (this.mNetstateReceiver == null || !temp.convertType(packageName, this.mNetstateReceiver.getState())) {
            if (this.mNetstateReceiver == null || !this.mNetstateReceiver.needBlockForeignApp() || !temp.isForeignApp(packageName)) {
                return type;
            }
            if (type == 0) {
                return 1;
