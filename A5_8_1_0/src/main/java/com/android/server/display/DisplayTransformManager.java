...
public class DisplayTransformManager {
    ...
    private final SparseArray<float[]> mColorMatrix = new SparseArray(4);
    ...
    private void setNativeMode(boolean enabled) {
        SystemProperties.set(PERSISTENT_PROPERTY_NATIVE_MODE, enabled ? LocationManagerService.OPPO_FAKE_LOCATOIN_SWITCH_ON : "0");
        if (getSurfaceFlinger() != null) {
            int i;
            Parcel data = Parcel.obtain();
            data.writeInterfaceToken("android.ui.ISurfaceComposer");
            ...
            data.writeInt(i);
            try {
                getSurfaceFlinger().transact(1023, data, null, 0);
            } catch (RemoteException ex) {
