...
public class VirtualDisplayAdapter extends DisplayAdapter {
    ...
    private final class VirtualDisplayDevice extends DisplayDevice implements DeathRecipient {
        ...
        public VirtualDisplayDevice(IBinder displayToken, IBinder appToken, int ownerUid, String ownerPackageName, String name, int width, int height, int densityDpi, Surface surface, int flags, Callback callback, String uniqueId, int uniqueIndex, IMediaProjection projection) {
        ...
        public void destroyLocked(boolean binderAlive) {
            ...
            try {
                if (this.mIMediaProjection != null) {
                    this.mIMediaProjection.unregisterCallback(this.mMediaProjectionCallback);
                }
            } catch (RemoteException e) {
                Slog.i(VirtualDisplayAdapter.TAG, "device.mIMediaProjection.unregisterCallback failed, device.mMediaProjectionCallback = " + this.mMediaProjectionCallback);
            }
        }
    ...
    public DisplayDevice createVirtualDisplayLocked(IVirtualDisplayCallback callback, IMediaProjection projection, int ownerUid, String ownerPackageName, String name, int width, int height, int densityDpi, Surface surface, int flags, String uniqueId) {
        ...
        VirtualDisplayDevice device = new VirtualDisplayDevice(displayToken, appToken, ownerUid, ownerPackageName, name, width, height, densityDpi, surface, flags, new Callback(callback, this.mHandler), uniqueId, uniqueIndex, projection);
        this.mVirtualDisplayDevices.put(appToken, device);
        if (projection != null) {
            try {
                device.mMediaProjectionCallback = new MediaProjectionCallback(appToken);
                projection.registerCallback(device.mMediaProjectionCallback);
            } catch (RemoteException e) {
