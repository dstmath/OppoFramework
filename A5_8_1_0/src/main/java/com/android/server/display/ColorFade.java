...
final class ColorFade {
    ...
    private boolean captureScreenshotTextureAndSetViewport() {
        ...
        try {
            ...
            s = new Surface(st);
            if (DEBUG_SCREEN) {
                Slog.d(TAG, "captureScreenshot start");
            }
            SurfaceControl.screenshot(SurfaceControl.getBuiltInDisplay(0), s);
            if (DEBUG_SCREEN) {
                Slog.d(TAG, "captureScreenshot done");
            }
            st.updateTexImage();
    ...
    private boolean createSurface() {
        if (DEBUG_SCREEN) {
            Slog.d(TAG, "createSurface start");
        }
    ...
    public void dump(PrintWriter pw) {
        ...
        DEBUG_SCREEN = DisplayManagerService.DEBUG_POWER;
    }
}
