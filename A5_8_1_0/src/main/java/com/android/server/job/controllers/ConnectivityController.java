...
public final class ConnectivityController extends StateController implements OnNetworkActiveListener {
    ...
    private boolean updateConstraintsSatisfied(JobStatus jobStatus) {
        ...
        NetworkCapabilities capabilities = network != null ? this.mConnManager.getNetworkCapabilities(network) : null;
        ...
        boolean connectionUsable = connected ? validated : false;
        if (!connected || capabilities == null) {
            metered = false;
        } else {
            metered = capabilities.hasCapability(11) ^ 1;
        }
        if (!connected || capabilities == null) {
            unmetered = false;
        } else {
            unmetered = capabilities.hasCapability(11);
        }
        ...
        boolean changed = ((jobStatus.setConnectivityConstraintSatisfied(connectionUsable) | jobStatus.setMeteredConstraintSatisfied(metered)) | jobStatus.setUnmeteredConstraintSatisfied(unmetered)) | jobStatus.setNotRoamingConstraintSatisfied(notRoaming);
        if (jobUid == 1000) {
