...
public class ConnectToNetworkNotificationBuilder {
    ...
    public Notification createConnectToNetworkNotification(ScanResult network) {
        Action connectAction = new Builder(null, this.mResources.getText(17041104), getPrivateBroadcast(ACTION_CONNECT_TO_NETWORK)).build();
        return createNotificationBuilder(this.mContext.getText(17041107), network.SSID).addAction(connectAction).addAction(new Builder(null, this.mResources.getText(17041103), getPrivateBroadcast(ACTION_PICK_WIFI_NETWORK)).build()).build();
    }

    public Notification createNetworkConnectingNotification(ScanResult network) {
        return createNotificationBuilder(this.mContext.getText(17041109), network.SSID).setProgress(0, 0, true).build();
    }

    public Notification createNetworkConnectedNotification(ScanResult network) {
        return createNotificationBuilder(this.mContext.getText(17041108), network.SSID).build();
    }

    public Notification createNetworkFailedNotification() {
        return createNotificationBuilder(this.mContext.getText(17041110), this.mContext.getText(17041105)).setContentIntent(getPrivateBroadcast(ACTION_PICK_WIFI_NETWORK_AFTER_CONNECT_FAILURE)).setAutoCancel(true).build();
    }

    private Notification.Builder createNotificationBuilder(CharSequence title, CharSequence content) {
        return this.mFrameworkFacade.makeNotificationBuilder(this.mContext, SystemNotificationChannels.NETWORK_AVAILABLE).setSmallIcon(17303428).setTicker(title).setContentTitle(title).setContentText(content).setDeleteIntent(getPrivateBroadcast(ACTION_USER_DISMISSED_NOTIFICATION)).setShowWhen(false).setLocalOnly(true).setColor(this.mResources.getColor(17170763, this.mContext.getTheme()));
    }
