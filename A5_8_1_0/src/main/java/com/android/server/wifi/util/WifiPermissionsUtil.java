...
public class WifiPermissionsUtil {
    ...
    private static final String DEFAULT_GET_SCAN_RESULTS_PACKAGE = "com.coloros.assistantscreen";
    ...
    private final ArrayList<String> mNLPPackages = new ArrayList();
    ...
    public WifiPermissionsUtil(WifiPermissionsWrapper wifiPermissionsWrapper, Context context, WifiSettingsStore settingsStore, UserManager userManager, NetworkScoreManager networkScoreManager, WifiInjector wifiInjector) {
        ...
        this.mNLPPackages.addAll(Arrays.asList(context.getResources().getStringArray(17236016)));
    ...
    public boolean canAccessScanResults(String pkgName, int uid, int minVersion) throws SecurityException {
        ...
        int canAppPackageUseLocation2 = !(!canAppPackageUseLocation ? this.mNLPPackages.contains(pkgName) : true) ? isPackageCanGetScanResults(pkgName) : 1;
    ...
    private boolean isPackageCanGetScanResults(String pkg) {
        if (getRomUpdateValue(GET_SCAN_RESULTS_PACKAGE, DEFAULT_GET_SCAN_RESULTS_PACKAGE).contains(pkg)) {
            return true;
        }
        return false;
    }
