...
public class WifiConfigurationUtil {
    ...
    public static boolean isConfigForWapiPskNetwork(WifiConfiguration config) {
        return config.allowedKeyManagement.get(190);
    }

    public static boolean isConfigForWapiCertNetwork(WifiConfiguration config) {
        return config.allowedKeyManagement.get(191);
    }
    ...
    public static boolean isConfigForEapNetwork(WifiConfiguration config) {
        ...
        return config.allowedKeyManagement.get(3);
    }
    ...
    public static boolean isConfigForOpenNetwork(WifiConfiguration config) {
        int i;
        if (isConfigForWepNetwork(config) || isConfigForPskNetwork(config) || isConfigForWapiPskNetwork(config) || isConfigForWapiCertNetwork(config)) {
            i = 1;
        } else {
            i = isConfigForEapNetwork(config);
        }
        return i ^ 1;
    }
    ...
    public static boolean hasEnterpriseConfigChanged(WifiEnterpriseConfig existingEnterpriseConfig, WifiEnterpriseConfig newEnterpriseConfig) {
        ...
        } else if (!(existingEnterpriseConfig.getEapMethod() == newEnterpriseConfig.getEapMethod() && existingEnterpriseConfig.getPhase2Method() == newEnterpriseConfig.getPhase2Method() && getEapSimNum(existingEnterpriseConfig) == getEapSimNum(newEnterpriseConfig) && TextUtils.equals(existingEnterpriseConfig.getIdentity(), newEnterpriseConfig.getIdentity()) && (TextUtils.equals(existingEnterpriseConfig.getAnonymousIdentity(), newEnterpriseConfig.getAnonymousIdentity()) ^ 1) == 0 && Arrays.equals(existingEnterpriseConfig.getCaCertificates(), newEnterpriseConfig.getCaCertificates()))) {
            return true;
        }
        return false;
    }
    ...
    public static int getEapSimNum(WifiEnterpriseConfig enterpriseConfig) {
        if (enterpriseConfig.getSimNum() == null || (enterpriseConfig.getSimNum().isEmpty() ^ 1) == 0) {
            return 0;
        }
        return Integer.parseInt(enterpriseConfig.getSimNum());
    }

    public static boolean hasCredentialChanged(WifiConfiguration existingConfig, WifiConfiguration newConfig) {
        if (!Objects.equals(existingConfig.allowedKeyManagement, newConfig.allowedKeyManagement) || !Objects.equals(existingConfig.allowedProtocols, newConfig.allowedProtocols) || !Objects.equals(existingConfig.allowedAuthAlgorithms, newConfig.allowedAuthAlgorithms) || !Objects.equals(existingConfig.allowedPairwiseCiphers, newConfig.allowedPairwiseCiphers) || !Objects.equals(existingConfig.allowedGroupCiphers, newConfig.allowedGroupCiphers) || !Objects.equals(existingConfig.preSharedKey, newConfig.preSharedKey)) {
            return true;
        }
        if (existingConfig.allowedKeyManagement.get(190) && (existingConfig.wapiPskType != newConfig.wapiPskType || !Objects.equals(existingConfig.wapiPsk, newConfig.wapiPsk))) {
            return true;
        }
        if ((!existingConfig.allowedKeyManagement.get(191) || (existingConfig.wapiCertSelMode == newConfig.wapiCertSelMode && Objects.equals(existingConfig.wapiCertSel, newConfig.wapiCertSel))) && Arrays.equals(existingConfig.wepKeys, newConfig.wepKeys) && existingConfig.wepTxKeyIndex == newConfig.wepTxKeyIndex && existingConfig.hiddenSSID == newConfig.hiddenSSID && !hasEnterpriseConfigChanged(existingConfig.enterpriseConfig, newConfig.enterpriseConfig)) {
            return false;
        }
        return true;
    }
