...
public class WificondScannerImpl extends WifiScannerImpl implements Callback {
    ...
    public static class HwPnoDebouncer {
        ...
        private final OnAlarmListener mAlarmListener = new OnAlarmListener() {
            public void onAlarm() {
                if (WificondScannerImpl.DBG) {
                    Log.d(WificondScannerImpl.TAG, "PNO timer expired, expected state " + HwPnoDebouncer.this.mExpectedPnoState);
                }
        ...
        private boolean startPnoScanInternal() {
            if (this.mCurrentPnoState) {
                if (WificondScannerImpl.DBG) {
                    Log.d(WificondScannerImpl.TAG, "PNO state is already enable");
                }
                return true;
            } else if (this.mPnoSettings == null) {
        ...
        private boolean stopPnoScanInternal() {
            ...
            if (WificondScannerImpl.DBG) {
                Log.d(WificondScannerImpl.TAG, "PNO state is already disable");
            }
            return true;
        }
        ...
        public boolean startPnoScan(PnoSettings pnoSettings, Listener listener) {
            if (WificondScannerImpl.DBG) {
                Log.d(WificondScannerImpl.TAG, "Starting PNO scan");
            }
        ...
        public void stopPnoScan() {
            if (WificondScannerImpl.DBG) {
                Log.d(WificondScannerImpl.TAG, "Stopping PNO scan");
            }
            setPnoState(false);
            this.mListener = null;
        }
        ...
        public void forceStopPnoScan() {
            if (WificondScannerImpl.DBG) {
                Log.d(WificondScannerImpl.TAG, "Force stopping Pno scan");
            }
            if (this.mWaitForTimer) {
    ...
    /* JADX WARNING: Missing block: B:71:0x0225, code:
            return;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
