...
public class WifiAwareDataPathStateManager {
        ...
        protected void needNetworkFor(NetworkRequest networkRequest, int score) {
            ...
            if (nnri == null) {
                ...
                if (nnri.networkSpecifier.role == 0) {
                    ...
                    nnri.state = StatusCode.DENIED_DUE_TO_SPECTRUM_MANAGEMENT;
    ...
    public void onDataPathInitiateSuccess(WifiAwareNetworkSpecifier networkSpecifier, int ndpId) {
        ...
        if (nnri == null) {
        ...
        } else if (nnri.state != StatusCode.DENIED_DUE_TO_SPECTRUM_MANAGEMENT) {
            ...
            nnri.state = StatusCode.MAF_LIMIT_EXCEEDED;
            nnri.ndpId = ndpId;
        }
    }
    ...
    public void onDataPathInitiateFail(WifiAwareNetworkSpecifier networkSpecifier, int reason) {
        ...
        if (nnri.state != StatusCode.DENIED_DUE_TO_SPECTRUM_MANAGEMENT) {
    ...
    public WifiAwareNetworkSpecifier onDataPathRequest(int pubSubId, byte[] mac, int ndpId) {
        ...
        if (awareNetworkRequestInformation == null) {
            ...
            return null;
        } else if (awareNetworkRequestInformation.state != StatusCode.ASSOC_DENIED_NO_VHT) {
    ...
    public void onRespondToDataPathRequest(int ndpId, boolean success, int reasonOnFailure) {
        ...
        if (awareNetworkRequestInformation == null) {
            ...
            this.mMgr.endDataPath(ndpId);
            this.mNetworkRequestsCache.remove(networkSpecifier);
            this.mAwareMetrics.recordNdpStatus(reasonOnFailure, networkSpecifier.isOutOfBand(), awareNetworkRequestInformation.startTimestamp);
            ...
            awareNetworkRequestInformation.state = StatusCode.MAF_LIMIT_EXCEEDED;
        }
    }
    ...
    public WifiAwareNetworkSpecifier onDataPathConfirm(int ndpId, byte[] mac, boolean accept, int reason, byte[] message) {
        ...
        if (nnri.state != StatusCode.MAF_LIMIT_EXCEEDED) {
        ...
        if (accept) {
            nnri.state = StatusCode.MCCA_TRACK_LIMIT_EXCEEDED;
    ...
    public void onDataPathEnd(int ndpId) {
        ...
        if (nnriE != null) {
            ...
            if (((AwareNetworkRequestInformation) nnriE.getValue()).state == StatusCode.MCCA_TRACK_LIMIT_EXCEEDED || ((AwareNetworkRequestInformation) nnriE.getValue()).state == StatusCode.RESTRICTION_FROM_AUTHORIZED_GDB) {
    ...
    private boolean isInterfaceUpAndUsedByAnotherNdp(AwareNetworkRequestInformation nri) {
        for (AwareNetworkRequestInformation lnri : this.mNetworkRequestsCache.values()) {
            if (lnri != nri && nri.interfaceName.equals(lnri.interfaceName)) {
                if (lnri.state == StatusCode.MCCA_TRACK_LIMIT_EXCEEDED || lnri.state == StatusCode.RESTRICTION_FROM_AUTHORIZED_GDB) {
                    return true;
                }
            }
        }
