...
public class UsbHostManager {
    ...
    public UsbHostManager(Context context, UsbAlsaManager alsaManager, UsbSettingsManager settingsManager) {
        ...
        String deviceConnectionHandler = context.getResources().getString(17039644);
    ...
    private boolean beginUsbDeviceAdded(String deviceName, int vendorID, int productID, int deviceClass, int deviceSubclass, int deviceProtocol, String manufacturerName, String productName, int version, String serialNumber) {
        if (isBlackListed(deviceName) || isBlackListed(deviceClass, deviceSubclass)) {
            return false;
        }
        synchronized (this.mLock) {
    ...
    private void endUsbDeviceAdded() {
        ...
        synchronized (this.mLock) {
            if (this.mNewDevice != null) {
                ...
                if (parser.parseDevice(this.mNewDevice.getDeviceName())) {
                    isInputHeadset = parser.isInputHeadset();
                    isOutputHeadset = parser.isOutputHeadset();
                    Slog.i(TAG, "---- isHeadset[in: " + isInputHeadset + " , out: " + isOutputHeadset + "]");
                }
                this.mUsbAlsaManager.usbDeviceAdded(this.mNewDevice, isInputHeadset, isOutputHeadset);
            } else {
