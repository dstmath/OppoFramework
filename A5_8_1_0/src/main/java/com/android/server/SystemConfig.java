...
public class SystemConfig {
    ...
    SystemConfig() {
        ...
        readOppoRegionMarkFeature(Environment.buildPath(Environment.getRootDirectory(), new String[]{"etc", "oppoRegionMarkFeatures"}), -1);
    }
    ...
    void readOppoRegionMarkFeature(File regionMarkDir, int permissionFlag) {
        if (!regionMarkDir.exists() || (regionMarkDir.isDirectory() ^ 1) != 0) {
            if (permissionFlag == -1) {
                Slog.w(TAG, "No directory " + regionMarkDir + ", skipping");
            }
        } else if (regionMarkDir.canRead()) {
            File regionMarkFile = null;
            ...
            for (File f : regionMarkDir.listFiles()) {
            ...
            if (regionMarkFile != null) {
                ...
                readPermissionsFromXml(regionMarkFile, permissionFlag);
            }
        } else {
    ...
    void readPermissions(File libraryDir, int permissionFlag) {
        if (!libraryDir.exists() || (libraryDir.isDirectory() ^ 1) != 0) {
            ...
            for (File f : libraryDir.listFiles()) {
                ...
                } else if (!f.getPath().startsWith("/system/etc/permissions/com.oppo.features_allnet_common_") || (f.getPath().endsWith(SystemProperties.get("ro.separate.soft", "oppo") + ".xml") ^ 1) == 0) {
