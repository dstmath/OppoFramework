    ...
    ArrayList<String> updateOverlaysForUser(int newUserId) {
        ...
        for (i = 0; i < overlayPackagesSize; i++) {
            ...
            if (oi == null || (oi.targetPackageName.equals(overlayPackage.overlayTarget) ^ 1) != 0) {
                ...
                if (oi == null) {
                    if (isPackageStaticOverlay(overlayPackage) || this.mDefaultOverlays.contains(overlayPackage.packageName)) {
                        this.mSettings.setEnabled(overlayPackage.packageName, newUserId, true);
                    }
                    if (RegionalizationEnvironment.isRegionalizationCarrierOverlayPackage(overlayPackage.applicationInfo.getBaseCodePath(), "app")) {
                        this.mSettings.setEnabled(overlayPackage.packageName, newUserId, true);
                    }
                } else {
