...
class RecentTasks extends ArrayList<TaskRecord> {
    ...
    int trimForTaskLocked(TaskRecord task, boolean doTrim) {
        ...
        while (i < recentsCount) {
            ...
            if (task != tr) {
                if ((stack == null || trStack == null || stack == trStack || trStack.mStackId == 2 || stack.mStackId == 2) && task.userId == tr.userId) {
                    if (i > 3) {
                    ...
                    if ((sameAffinity || (sameIntentFilter ^ 1) == 0 || (bothDocuments ^ 1) == 0) && (!sameAffinity || intent == null || trIntent == null || (intent.getComponent().flattenToShortString().equals(trIntent.getComponent().flattenToShortString()) ^ 1) == 0 || (intent.getFlags() & DumpState.DUMP_VOLUMES) != 8388608)) {
                        if (bothDocuments != 0) {
