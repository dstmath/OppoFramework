...
final class StrictModeViolationDialog extends BaseErrorDialog {
    ...
    public StrictModeViolationDialog(Context context, ActivityManagerService service, AppErrorResult result, ProcessRecord app) {
        ...
        if (app.pkgList.size() != 1 || context.getPackageManager().getApplicationLabel(app.info) == null) {
            setMessage(res.getString(17040892, new Object[]{app.processName.toString()}));
        } else {
            setMessage(res.getString(17040891, new Object[]{context.getPackageManager().getApplicationLabel(app.info).toString(), app.info.processName}));
        }
        setCancelable(false);
        setButton(-1, res.getText(17039803), this.mHandler.obtainMessage(0));
        if (app.errorReportReceiver != null) {
            setButton(-2, res.getText(17040766), this.mHandler.obtainMessage(1));
        }
