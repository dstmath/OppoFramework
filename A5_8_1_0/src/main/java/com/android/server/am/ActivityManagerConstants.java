...
final class ActivityManagerConstants extends ContentObserver {
    ...
    static {
        int i;
        if ("true".equals(SystemProperties.get("ro.config.oppo.low_ram", "false"))) {
            i = SystemProperties.getInt("ro.vendor.qti.sys.fw.bg_apps_limit", 16);
        } else {
            i = SystemProperties.getInt("ro.vendor.qti.sys.fw.bg_apps_limit", 32);
        }
        DEFAULT_MAX_CACHED_PROCESSES = i;
