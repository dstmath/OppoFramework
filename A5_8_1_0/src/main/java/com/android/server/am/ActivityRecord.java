...
final class ActivityRecord extends ConfigurationContainer implements AppWindowContainerListener {
    ...
    private static final /* synthetic */ int[] f27-com-android-server-am-ActivityStack$ActivityStateSwitchesValues = null;
    ...
    boolean mFinishFromClose;
    boolean mFromFreeform = false;
    boolean mHasRelaunch;
    ...
    private class PreferredAppsTask extends AsyncTask<Void, Void, Void> {
        /* synthetic */ PreferredAppsTask(ActivityRecord this$0, PreferredAppsTask -this1) {
            this();
        }

        private PreferredAppsTask() {
        }

        protected Void doInBackground(Void... params) {
            if (ActivityRecord.this.mUxPerf != null) {
                String res = ActivityRecord.this.mUxPerf.perfUXEngine_trigger(1);
                if (res == null) {
                    return null;
                }
                ...
                if (p_apps.length != 0) {
                    ...
                    ActivityRecord.this.service.startActivityAsUserEmpty(null, null, ActivityRecord.this.intent, null, null, null, 0, 0, null, bParams, 0);
                }
            }
            return null;
        }
    ...
    private static /* synthetic */ int[] m23x775af271() {
        if (f27-com-android-server-am-ActivityStack$ActivityStateSwitchesValues != null) {
            return f27-com-android-server-am-ActivityStack$ActivityStateSwitchesValues;
        }
        ...
        f27-com-android-server-am-ActivityStack$ActivityStateSwitchesValues = iArr;
        return iArr;
    }
    ...
    ActivityRecord(ActivityManagerService _service, ProcessRecord _caller, int _launchedFromPid, int _launchedFromUid, String _launchedFromPackage, Intent _intent, String _resolvedType, ActivityInfo aInfo, Configuration _configuration, ActivityRecord _resultTo, String _resultWho, int _reqCode, boolean _componentSpecified, boolean _rootVoiceInteraction, ActivityStackSupervisor supervisor, ActivityOptions options, ActivityRecord sourceRecord) {
        ...
        int translucent = ent != null ? !ent.array.getBoolean(5, false) ? !ent.array.hasValue(5) ? ent.array.getBoolean(25, false) : 0 : 1 : 0;
        if (ent == null || (ent.array.getBoolean(4, false) ^ 1) == 0) {
            z = false;
        } else {
            z = translucent ^ 1;
        }
    ...
    void reparent(TaskRecord newTask, int position, String reason) {
        ...
        if (prevTask == newTask) {
            ...
            if (this.mWindowContainerController == null) {
                Slog.v(TAG, "mWindowContainerController is null " + this);
            }
            if (this.mWindowContainerController != null) {
    ...
    boolean checkEnterPictureInPictureState(String caller, boolean beforeStopping) {
        ...
        switch (m23x775af271()[this.state.ordinal()]) {
            case 1:
            case 2:
    ...
    void setVisibility(boolean visible) {
        if (this.mWindowContainerController == null) {
            Slog.v(TAG, "setVisibility,but mWindowContainerController is null,return " + this);
            return;
        }
        this.mWindowContainerController.setVisibility(visible, this.mDeferHidingClient);
        this.mStackSupervisor.mActivityMetricsLogger.notifyVisibilityChanged(this);
    }

    void setVisible(boolean newVisible) {
        if (ActivityManagerDebugConfig.DEBUG_VISIBILITY) {
            Slog.v(TAG, "setVisible " + this + " " + newVisible);
        }
    ...
    void oppoSnapShot() {
        if (ActivityManager.ENABLE_TASK_SNAPSHOTS) {
            this.mUpdateTaskThumbnailWhenHidden = false;
            this.mWindowContainerController.oppoSnapShot();
        }
    }
    ...
    void notifyWindowFreezing(boolean freezing, int stackId) {
        this.mWindowContainerController.notifyWindowFreezing(freezing, stackId);
    }
    ...
    boolean shouldBeVisibleIgnoringKeyguard(boolean behindFullscreenActivity) {
        ...
        boolean isVisible = behindFullscreenActivity ? this.mLaunchTaskBehind : true;
    ...
    void makeVisibleIfNeeded(ActivityRecord starting) {
        ...
        try {
            ...
            if (this.service.mFinishWhenLockedState == 2) {
                stack.stopActivityLocked(this);
            }
        } catch (Exception e) {
    ...
    void completeResumeLocked() {
        ...
        if (isHomeActivity()) {
            ...
            try {
                new PreferredAppsTask(this, null).execute(new Void[0]);
            } catch (Exception e) {
        ...
        if (this.service.mOppoKinectController != null) {
            try {
                if (this.realActivity == null) {
                    this.service.mOppoKinectController.activityResuming(null);
                } else {
                    this.service.mOppoKinectController.activityResuming(this.realActivity.getClassName());
                }
            } catch (Exception e2) {
                this.service.mOppoKinectController = null;
                Slog.v(TAG, "mOppoKinectController exception e = " + e2.toString());
            }
        }
        if (this.app != null) {
        ...
        } else if (this.service.mIgnoreSleepCheckLater) {
            Slog.v(TAG, "mIgnoreSleepCheckLater, do not care.");
        } else {
            stack.checkReadyForSleep();
        }
        if (this.realActivity == null || this.app == null) {
            Slog.i(TAG, "OifaceManager cann't get currentPackage");
        } else {
            OifaceProxyUtils.getInstance().currentPackage(this.realActivity.getPackageName(), this.app.uid, this.app.pid);
        }
    }
        ...
        if (stack != null) {
            ...
            if (thisTime != totalTime) {
                ...
                if (this.mUxPerf != null) {
                    this.mUxPerf.perfUXEngine_events(3, 0, this.packageName, (int) totalTime);
                }
            } else if (this.mUxPerf != null) {
                this.mUxPerf.perfUXEngine_events(3, 0, this.packageName, (int) thisTime);
            }
            Log.i(TAG, sb.toString());
            if (this.mPerf_iop == null) {
                this.mPerf_iop = new BoostFramework();
            }
            if (!(this.mPerf_iop == null || this.app == null)) {
                this.mPerf_iop.perfIOPrefetchStart(this.app.pid, this.packageName, this.appInfo.sourceDir.substring(0, this.appInfo.sourceDir.lastIndexOf(47)));
            }
            ...
            if (mPerfFirstDraw != null) {
                mPerfFirstDraw.perfHint(4162, this.info.packageName, (int) thisTime, 1);
            }
            int i = (totalTime > 0 ? 1 : (totalTime == 0 ? 0 : -1));
    ...
    void onOverrideConfigurationChanged(Configuration newConfig) {
        if (!getOverrideConfiguration().equals(newConfig)) {
    ...
    private void computeBounds(Rect outBounds) {
        ...
        ActivityStack stack = getStack();
        if (this.task != null && stack != null && (this.task.mFullscreen ^ 1) == 0 && maxAspectRatio != OppoBrightUtils.MIN_LUX_LIMITI && !isInVrUiMode(getConfiguration())) {
    ...
    boolean ensureActivityConfigurationLocked(int globalChanges, boolean preserveWindow) {
        ...
        if (stack.mConfigWillChange) {
            ...
            stopFreezingScreenLocked(false);
            return true;
        } else if (this.state == ActivityState.STOPPING || this.state == ActivityState.STOPPED) {
            if (ActivityManagerDebugConfig.DEBUG_SWITCH || ActivityManagerDebugConfig.DEBUG_CONFIGURATION) {
            ...
            if (!getConfiguration().equals(this.mTmpConfig) || (this.forceNewConfig ^ 1) == 0 || (displayChanged ^ 1) == 0 || (this.mShouldRelaunch ^ 1) == 0) {
                boolean needFull = false;
                if (this.mStackSupervisor.getStack(3) != null) {
                    needFull = OppoDockedManagerService.getInstance().needFullscreenActivity(this);
                }
                int changes = getConfigurationChanges(this.mTmpConfig);
                if (this.task != null && ((!this.task.mFullscreen || needFull) && this.info != null)) {
                    ActivityInfo activityInfo = this.info;
                    if (ActivityInfo.isResizeableMode(this.info.resizeMode) && (changes & 256) != 0) {
                        changes -= 256;
                    }
                }
                ...
                if (changes != 0 || (this.forceNewConfig ^ 1) == 0 || (this.mShouldRelaunch ^ 1) == 0) {
                    if (ActivityManagerDebugConfig.DEBUG_SWITCH || ActivityManagerDebugConfig.DEBUG_CONFIGURATION) {
                    ...
                    if (shouldRelaunchLocked(changes, this.mTmpConfig) || this.forceNewConfig || this.mShouldRelaunch) {
    ...
    void relaunchActivityLocked(boolean andResume, boolean preserveWindow) {
        ...
        try {
            ...
            if (this.mShouldRelaunch) {
                this.mHasRelaunch = false;
                this.mShouldRelaunch = false;
                Slog.v(TAG, "oppo freeform relaunch :" + this);
            }
