...
public final class OppoSplitWindowAppReader {
        ...
        try {
            ...
            for (int eventType = parser.getEventType(); eventType != 1; eventType = parser.next()) {
                ...
                if (eventType == 2 && "p".equals(parser.getName())) {
                    ...
                    if (FULL_NAME.equals(parser.getName())) {
                        pName = parser.getAttributeValue(null, PKG_ATTR);
                        this.mFullPackageName.add(pName);
                        mHasUpdate = true;
                        if (DEBUG) {
                            Log.i(TAG, "readFullAppList: " + pName);
                        }
                    }
                }
            }
    ...
    public boolean needFullscreenName(String fullName) {
        if (!(fullName == null || this.mFullPackageName == null)) {
            for (String name : this.mFullPackageName) {
