...
public class ActivityStackSupervisor extends ConfigurationContainer implements DisplayListener {
    ...
    public ActivityStackSupervisor(ActivityManagerService service, Looper looper) {
        ...
        this.mActivityMetricsLogger = new ActivityMetricsLogger(this, this.mService.mContext, looper);
    ...
    boolean isFrontStackOnDisplay(ActivityStack stack) {
        if (stack.getDisplay() != null) {
            return isFrontOfStackList(stack, stack.getDisplay().mStacks);
        }
        Slog.e(TAG, "stack.getDisplay() is null, isFrontStackOnDisplay return false");
        return false;
    }
        ...
        if (allPausedActivitiesComplete()) {
            ...
            try {
                ...
                try {
                    ...
                    Slog.w(TAG, "updateServiceConnectionActivitiesLocked catch NullPointerException", e2);
                }
                return true;
            } catch (Throwable e3) {
                if (r.launchFailed) {
    ...
    private void moveTasksToFullscreenStackInSurfaceTransaction(int fromStackId, boolean onTop) {
        ...
        if (stack != null) {
            int i;
            OppoDockedManagerService.getInstance().reset();
            this.mWindowManager.deferSurfaceLayout();
            if (fromStackId == 3) {
                i = 0;
                while (i <= 6) {
    ...
    void resizeDockedStackLocked(Rect dockedBounds, Rect tempDockedTaskBounds, Rect tempDockedTaskInsetBounds, Rect tempOtherTaskBounds, Rect tempOtherTaskInsetBounds, boolean preserveWindows, boolean deferResume) {
        if (this.mAllowDockedStackResize) {
            ...
            boolean needFullscreen = OppoDockedManagerService.getInstance().getHasFullscreen();
            int fullscreenState = OppoDockedManagerService.getInstance().getFullscreenState();
            stack.resize(dockedBounds, tempDockedTaskBounds, tempDockedTaskInsetBounds);
            if ((stack.mFullscreen || (dockedBounds == null && (stack.isAttached() ^ 1) != 0)) && (needFullscreen ^ 1) != 0) {
                    ...
                    while (i <= 6) {
                        ...
                        if (current != null && StackId.isResizeableByDockedStack(i)) {
                            ...
                            if (current.mStackId == 1 && i == 1 && current.mFullscreen && current.mResumedActivity != null && current.mResumedActivity.shortComponentName != null && current.mResumedActivity.shortComponentName.contains(OPPO_SAFECENTER_PASSWORD)) {
                                resizeStackLocked(i, null, null, null, true, true, true);
                            } else if (fullscreenState == 1 && current.mFullscreen) {
                                resizeStackLocked(i, null, null, null, true, true, true);
                            } else {
                                Rect rect;
    ...
    void lambda$-com_android_server_am_ActivityStackSupervisor_136575(int stackId) {
        ActivityStack stack = getStack(stackId);
        if (stack != null) {
            ArrayList<TaskRecord> tasks = stack.getAllTasks();
    ...
    void acquireAppLaunchPerfLock(String packageName) {
        ...
        if (this.mPerfIop != null) {
            this.mPerfIop.perfIOPrefetchStart(-1, packageName, "");
        }
    }
    ...
    void acquireAppLaunch() {
        ...
        if (mHyp != null) {
            mHyp.hypnusSetAction(13, OppoArpPeer.ARP_FIRST_RESPONSE_TIMEOUT);
        }
    }
    ...
    void startActivityBoost(String className) {
        if (className != null) {
            if (className.equals("com.tencent.mm.plugin.voip.ui.VideoActivity") || className.equals("com.tencent.av.ui.AVLoadingDialogActivity") || className.equals("com.tencent.mm.plugin.exdevice.ui.ExdeviceRankInfoUI") || className.equals("com.tencent.mm.plugin.webview.ui.tools.WebViewUI") || className.equals("com.jingdong.app.mall.pay.CashierDeskActivity") || className.equals("com.tmall.wireless.pay.TMPayActivity") || className.equals("com.alipay.android.app.pay.MiniLaucherActivity") || className.equals("com.alipay.android.app.ui.quickpay.window.MiniPayActivity") || className.equals("com.taobao.tao.alipay.cashdesk.CashDeskActivity")) {
                ...
                if (mHyp != null) {
                    mHyp.hypnusSetAction(19, OppoBrightUtils.HIGH_BRIGHTNESS_LUX_STEP);
                }
            }
        }
    }
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; stackNdx--) {
                ...
                if (checkActivityBelongsInStack(r, stack)) {
                    ...
                    if (this.mTmpFindTaskResult.r == null) {
                        ...
                        if (this.mTmpFindTaskResult.r.state == ActivityState.STOPPED) {
        ...
        for (int displayNdx = 0; displayNdx < this.mActivityDisplays.size(); displayNdx++) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; stackNdx--) {
                ...
                if (OppoFreeFormManagerService.getInstance().mDynamicDebug) {
                    Slog.d(TAG, "dumpActivitiesLocked stack = " + stack);
                }
    ...
    private void handleNonResizableTaskIfNeeded(TaskRecord task, int preferredStackId, int preferredDisplayId, int actualStackId, boolean forceNonResizable) {
        ...
        if ((isStackDockedInEffect(actualStackId) || preferredStackId == 3 || (isSecondaryDisplayPreferred ^ 1) == 0) && !task.isHomeTask()) {
            ...
            boolean needFullscreen = OppoDockedManagerService.getInstance().needFullscreenActivity(topActivity);
            ...
            if (task.supportsSplitScreen() && actualStackId == 1 && task != null && (OppoDockedManagerService.getInstance().getHasFullscreen() ^ 1) != 0) {
            ...
            if (launchOnSecondaryDisplayFailed || (task.supportsSplitScreen() ^ 1) != 0 || forceNonResizable) {
                ...
                moveTasksToFullscreenStackLocked(3, z);
            } else if (!(topActivity == null || !topActivity.isNonResizableOrForcedResizable() || (needFullscreen ^ 1) == 0 || (topActivity.noDisplay ^ 1) == 0 || task.affinity == null || !task.affinity.equals(topActivity.taskAffinity))) {
                int reason;
    ...
    final int startActivityFromRecentsInner(int taskId, Bundle bOptions) {
        ...
        targetActivity = task.getTopActivity();
    ...
    final int startActivityForFreeform(Intent intent, Bundle bOptions, int userId, int callUid, String callPkg) {
        ActivityOptions activityOptions;
        if (bOptions != null) {
            ActivityOptions activityOptions2 = new ActivityOptions(bOptions);
        } else {
        ...
        int launchStackId = activityOptions != null ? activityOptions.getLaunchStackId() : -1;
        int callingUid = callUid;
        String callingPackage = callPkg;
        if (StackId.isHomeOrRecentsStack(launchStackId) || intent == null) {
            throw new IllegalArgumentException("startActivityFromRecentsInner: Task " + intent + " can't be launch in the home/recents stack.");
        }
        this.mWindowManager.deferSurfaceLayout();
        try {
            TaskRecord task = findTaskForFreeformLocked(intent, userId);
            if (ActivityManagerDebugConfig.DEBUG_STACK) {
                Slog.v(TAG, "oppo freeform findTaskForFreeformLocked: task0 " + task);
            }
            if (task == null) {
                ActivityRecord intentActivity = findActivityForFreeformLocked(intent, userId, false);
                if (ActivityManagerDebugConfig.DEBUG_STACK) {
                    Slog.v(TAG, "oppo freeform startActivityForFreeform: intentActivity " + intentActivity);
                }
                int taskId = -1;
                if (!(intentActivity == null || intentActivity.task == null)) {
                    taskId = intentActivity.task.taskId;
                    if (ActivityManagerDebugConfig.DEBUG_STACK) {
                        Slog.v(TAG, "oppo freeform startActivityForFreeform: task " + intentActivity.task);
                    }
                }
                task = anyTaskForIdLocked(taskId, 2, launchStackId);
                if (ActivityManagerDebugConfig.DEBUG_STACK) {
                    Slog.v(TAG, "oppo freeform startActivityForFreeform: task2 " + task);
                }
                if (!(task == null || task.intent == null)) {
                    String taskAction = task.intent.getAction();
                    if (!(taskAction == null || ("android.intent.action.VIEW".equals(taskAction) ^ 1) == 0)) {
                        if (ActivityManagerDebugConfig.DEBUG_STACK) {
                            Slog.v(TAG, "oppo freeform startActivityForFreeform: use task " + task);
                        }
                        callingUid = 1000;
                        callingPackage = OppoFreeFormManagerService.FREEFORM_CALLER_PKG;
                        intent = task.intent;
                    }
                }
            }
            OppoFreeFormManagerService.getInstance().handleActivityFromFreeformFullscreen(task);
            this.mWindowManager.setFreeformState(true);
            OppoFreeFormManagerService.getInstance().setParentInfo(this.mFocusedStack);
            Slog.v(TAG, "oppo freeform startActivityForFreeform: " + task);
            intent.setLaunchStackId(1);
            int result = this.mService.startActivityInPackage(callingUid, callingPackage, intent, null, null, null, 0, 0, bOptions, userId, task, "startActivityForFreeform");
            return result;
        } finally {
            this.mWindowManager.continueSurfaceLayout();
        }
    ...
    void dataCollectionInfoExp(ProcessRecord callerApp, String callingPackage, Intent intent) {
        ...
        String pkgInstallerActExp = "com.android.packageinstaller.InstallStart";
        if (intent != null && intent.getComponent() != null) {
            ComponentName realActivityComp = intent.getComponent();
            ComponentName googleInstallActivityComp = new ComponentName(pkgInstallerExp, pkgInstallerActExp);
            if (realActivityComp != null && googleInstallActivityComp.equals(realActivityComp) && callerApp != null && callerApp.info.processName != null) {
