...
public class OppoShellPermissionUtils {
    ...
    static {
        SHELL_REVOKE_PERMISSIONS.add(OppoPermissionConstants.PERMISSION_SEND_SMS);
        SHELL_REVOKE_PERMISSIONS.add(OppoPermissionConstants.PERMISSION_CALL_PHONE);
    ...
    public static boolean revokeShellPermission(Context cotnext, String permName, int uid) {
        ...
        if (OppoArpPeer.ARP_FIRST_RESPONSE_TIMEOUT == uid && SHELL_REVOKE_PERMISSIONS.contains(permName) && SystemProperties.getBoolean("persist.sys.permission.enable", true)) {
            result = true;
