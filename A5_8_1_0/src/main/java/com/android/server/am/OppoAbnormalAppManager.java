...
public class OppoAbnormalAppManager {
    ...
    public static int MAX_LIST_COUNT = 100;
    private static final int SERVICE_MSG = 102;
    public static String TAG = "OppoAbnormalAppManager";
    ...
    private static OppoAbnormalAppManager mOppoAbnormalAppManager = null;
    final String ACTION_OPPO_GUARD_ELF_COUNT_RESTRICT_LIST = "android.intent.action.OPPO_GUARD_ELF_COUNT_RESTRICT_LIST";
    final String ACTION_OPPO_GUARD_ELF_MONITOR = "android.intent.action.OPPO_GUARD_ELF_MONITOR";
    final String ACTION_OPPO_GUARD_ELF_SET_RESTRICT = "android.intent.action.OPPO_GUARD_ELF_SET_RESTRICT";
    final String ACTION_OPPO_GUARD_TIME_INFO = "android.intent.action.OPPO_GUARD_TIME_INFO";
    boolean DEBUG = true;
    boolean DEBUG_SWITCH = (DEBUG_DETAIL | this.DynamicDebug);
    boolean DynamicDebug = false;
    final String GUARD_ELF_FEATURE_NAME = "oppo.guard.elf.support";
    final int RESTRICT_ABNORMAL = 2;
    final int RESTRICT_NO = 0;
    final int RESTRICT_REPEAT = 1;
    final BroadcastReceiver dateChangedReceiver = new BroadcastReceiver() {
    ...
    private Comparator<OppoAnrAppInfo> mBroadcastComparator = new Comparator<OppoAnrAppInfo>() {
    ...
    ArrayList<String> mBroadcastReceiverInfo = new ArrayList();
    List<OppoAnrAppInfo> mBroadcastReceiverList = new ArrayList();
    ArrayList<String> mBroadcastTimeInfo = new ArrayList();
    List<OppoAnrAppInfo> mBroadcastTimeList = new ArrayList();
    ArrayList<String> mCountRestrictedList = new ArrayList();
    private Handler mHandler = null;
    protected boolean mHandlerReady = false;
    long mLastCheckBroadcastTime = 0;
    long mLastCheckServiceTime = 0;
    long mLastCheckTime = 0;
    List<String> mNotRestrictAppList = new ArrayList();
    List<String> mPersistRestrictAppList = new ArrayList();
    List<String> mScreenOffRestrictAppList = new ArrayList();
    ArrayList<String> mServiceTimeInfo = new ArrayList();
    List<OppoAnrAppInfo> mServiceTimeList = new ArrayList();
    List<OppoAppStartInfo> mStartAppList = new ArrayList();
    List<String> mStartInfoWhiteList = new ArrayList();
    boolean mSwitch = false;
    ArrayList<String> mTopBroadcastInfo = new ArrayList();
    List<OppoAnrAppInfo> mTopBroadcastList = new ArrayList();
    List<OppoAnrAppInfo> mTopThirdBroadcastList = new ArrayList();
    ArrayList<String> mUploadInfoList = new ArrayList();

    private class AbnormalAppManagerHandler extends Handler {
        /* synthetic */ AbnormalAppManagerHandler(OppoAbnormalAppManager this$0, AbnormalAppManagerHandler -this1) {
            this();
        }
    ...
    private boolean validNewProc(String packageName, String type) {
        ...
        if ("activity".equals(type) || !isPackageRestricted(packageName)) {
    ...
    public boolean handleAbnormalApp(OppoAppStartInfo appinfo) {
        ...
        if (appinfo.getIsThird()) {
            ...
            if (r != null && pkgName.equals(r.packageName)) {
                ...
                return false;
            } else if (this.mAms == null || !this.mAms.mContext.getPackageManager().isClosedSuperFirewall()) {
    ...
    public void handleBroadcastTimeInfo(ProcessRecord proc, long receiverTime, String broadcastName, boolean isTimeout, boolean ordered, boolean sticky, String callerPackage, BroadcastQueue queue) {
        if (this.mHandlerReady && OppoGuardElfConfigUtil.getInstance().getCloseFlag()) {
            ...
            if (!"android.intent.action.OPPO_GUARD_TIME_INFO".equals(broadcastName) && proc != null && broadcastName != null && queue != null) {
                if (queue == null || (!("background".equals(queue.mQueueName) || "oppobackground".equals(queue.mQueueName)) || (isTimeout ^ 1) == 0 || (broadcastName.equals("android.intent.action.BOOT_COMPLETED") ^ 1) == 0)) {
    ...
    public void handleServicesTimeInfo(ServiceRecord r, boolean isTimeout) {
        if (this.mHandlerReady && OppoGuardElfConfigUtil.getInstance().getCloseFlag()) {
            ...
            if (r != null && r.app != null) {
                if (r.app.execServicesFg || (isTimeout ^ 1) == 0) {
                    if (r != null) {
                        OppoAnrAppInfo info = new OppoAnrAppInfo();
    ...
    public void registerLogModule() {
        try {
            ...
            Log.i(TAG, "invoke " + m);
            m.invoke(cls.newInstance(), new Object[]{OppoAbnormalAppManager.class.getName()});
    ...
    public boolean validStartActivity(ActivityInfo aInfo) {
        ...
        if (aInfo.applicationInfo != null) {
            ...
            if (OppoAppStartupManagerUtils.getInstance().isInAamActivityWhitePkgList(aInfo.packageName)) {
                ...
                return false;
            } else if (validNewProc(aInfo.packageName, "activity") && (isInLruProcessesLocked(aInfo.applicationInfo.uid) ^ 1) != 0) {
                result = true;
            }
        }
        return result;
