    ...
    int sendInner(int code, Intent intent, String resolvedType, IBinder whitelistToken, IIntentReceiver finishedReceiver, String requiredPermission, IBinder resultTo, String resultWho, int requestCode, int flagsMask, int flagsValues, Bundle options) {
        ...
        synchronized (this.owner) {
            try {
                ...
                switch (this.key.type) {
                    ...
                    case 2:
                        ...
                        if (this.key.allIntents == null || this.key.allIntents.length <= 1) {
                            this.owner.startActivityInPackage(this.uid, this.key.packageName, finalIntent, resolvedType, resultTo, resultWho, requestCode, 0, options, userId, null, "PendingIntentRecord");
                        } else {
                    ...
                    case 5:
                        try {
                            ...
                            Slog.w(TAG, "Unable to send startService intent", e2);
                ...
                Slog.w(TAG, "Unable to send startActivity intent", e22);
            } catch (Throwable th) {
                ActivityManagerService.resetPriorityAfterLockedSection();
            }
        }
