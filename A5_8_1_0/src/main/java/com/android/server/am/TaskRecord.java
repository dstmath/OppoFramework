...
final class TaskRecord extends ConfigurationContainer implements TaskWindowContainerListener {
    ...
    boolean resize(Rect bounds, int resizeMode, boolean preserveWindow, boolean deferResume) {
        if (isResizeable()) {
            ...
            if (this.mWindowContainerController == null) {
                ...
                return true;
            } else if (canResizeToBounds(bounds) || (this.mService.mStartFreeform ^ 1) == 0) {
    ...
    boolean reparent(int preferredStackId, boolean toTop, int moveStackMode, boolean animate, boolean deferResume, String reason) {
        return reparent(preferredStackId, toTop ? Integer.MAX_VALUE : 0, moveStackMode, animate, deferResume, true, reason);
    }
    ...
    boolean reparent(int preferredStackId, boolean toTop, int moveStackMode, boolean animate, boolean deferResume, boolean schedulePictureInPictureModeChange, String reason) {
        return reparent(preferredStackId, toTop ? Integer.MAX_VALUE : 0, moveStackMode, animate, deferResume, schedulePictureInPictureModeChange, reason);
    }
    ...
    boolean reparent(int preferredStackId, int position, int moveStackMode, boolean animate, boolean deferResume, boolean schedulePictureInPictureModeChange, String reason) {
        ...
        if (sourceStackId == 3 && stackId == 1 && this.mBounds != null && (this.mBounds.isEmpty() ^ 1) != 0) {
            supervisor.resizeStackLocked(stackId, null, null, null, true, true, true);
        }
        ...
        int kept = 1;
        ActivityRecord tmpActivity = null;
        try {
            ActivityRecord r = topRunningActivityLocked();
            if ((sourceStackId == 2 && stackId == 1) || (sourceStackId == 1 && stackId == 2)) {
                tmpActivity = r;
                if (r != null) {
                    r.mFromFreeform = false;
                    r.mHasRelaunch = false;
                }
            }
            ...
            if (stackId == 1 && (Objects.equals(this.mBounds, toStack.mBounds) ^ 1) != 0) {
                ...
                Rect bounds = this.mBounds == null ? null : getLaunchBounds();
                Slog.v(TAG, "oppo freeform: " + this.mBounds + " task: " + this);
                if (bounds == null) {
                ...
                if (stackId == 3 && moveStackMode == 1) {
                    if (!(reason != null ? reason.contains(SWAP_DOCKED_STACK) : false)) {
            ...
            if (!deferResume) {
                ...
                if (!(tmpActivity == null || tmpActivity.task == null)) {
                    for (i = tmpActivity.task.mActivities.size() - 1; i >= 0; i--) {
                        tmp = (ActivityRecord) tmpActivity.task.mActivities.get(i);
                        if (!(tmp.app == null || tmp.app.thread == null || (tmp.mHasRelaunch ^ 1) == 0)) {
                            if (tmp == tmpActivity) {
                                tmp.relaunchActivityLocked(false, true);
                                tmp.mHasRelaunch = false;
                            } else {
                                tmp.mShouldRelaunch = true;
                            }
                        }
                    }
                }
                supervisor.resumeFocusedStackTopActivityLocked();
            } else if (!(tmpActivity == null || tmpActivity.task == null)) {
                for (i = tmpActivity.task.mActivities.size() - 1; i >= 0; i--) {
                    tmp = (ActivityRecord) tmpActivity.task.mActivities.get(i);
    ...
    ActivityRecord topRunningActivityLocked() {
        if (this.mStack != null) {
            boolean z = ActivityManagerDebugConfig.DEBUG_TASKS;
    ...
    final void performClearTaskAtIndexLocked(int activityNdx, boolean pauseImmediately) {
        ...
        if (this.mStack != null) {
            this.mStack.mIsClearTask = false;
        }
    }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:148:0x03d5  */
    ...
    boolean updateOverrideConfiguration(Rect bounds, Rect insetBounds) {
        ...
        if (this.mFullscreen) {
            if (this.mBounds != null && StackId.persistTaskBounds(this.mStack.mStackId)) {
            ...
            computeOverrideConfiguration(newConfig, rect, insetBounds, z2, z);
        }
