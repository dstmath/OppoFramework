...
public class TaskStack extends WindowContainer<Task> implements DimLayerUser, BoundsAnimationTarget {
    ...
    void applyAdjustForImeIfNeeded(Task task) {
        if (this.mMinimizeAmount == OppoBrightUtils.MIN_LUX_LIMITI && (this.mAdjustedForIme ^ 1) == 0 && !this.mAdjustedBounds.isEmpty()) {
    ...
    int taskIdFromPoint(int x, int y, boolean skipFullscreen) {
        ...
        for (int taskNdx = this.mChildren.size() - 1; taskNdx >= 0; taskNdx--) {
            ...
            if (task.getTopVisibleAppMainWindow() != null) {
                if (!skipFullscreen || !task.isFullscreen() || !task.isHomeTask()) {
                    task.getDimBounds(this.mTmpRect);
                    if (this.mTmpRect.contains(x, y)) {
                        return task.mTaskId;
                    }
                } else if (WindowManagerDebugConfig.DEBUG_TASK_MOVEMENT) {
    ...
    void findTaskForResizePoint(int x, int y, int delta, TaskForResizePointSearchResult results) {
        if (StackId.isTaskResizeAllowed(this.mStackId)) {
            for (int i = this.mChildren.size() - 1; i >= 0; i--) {
                ...
                if (this.mTmpRect.contains(x, y)) {
                    this.mTmpRect.inset((int) (((float) delta) * 1.3f), (int) (((float) delta) * 1.3f));
                    results.searchDone = true;
                    if (!this.mTmpRect.contains(x, y)) {
        ...
        synchronized (this.mService.mWindowMap) {
            try {
                ...
                if (schedulePipModeChangedCallback) {
                    forAllWindows((Consumer) -$Lambda$-ShbHzWzMvKATSUwSngPXEFkvyU.$INST$6, false);
                }
            } finally {
                WindowManagerService.resetPriorityAfterLockedSection();
            }
        ...
        for (int i = 0; i < this.mTmpAppTokens.size(); i++) {
            ...
            if (appAnimator.stepAnimationLocked(currentTime)) {
                ...
                if (WindowManagerDebugConfig.DEBUG_ANIM) {
                    Slog.v("WindowManager", "updateWindowsApps...: done animating exiting " + appAnimator.mAppToken);
                }
            }
        }
        this.mTmpAppTokens.clear();
    ...
    void getTaskBounds(Rect outBounds) {
        for (int i = this.mChildren.size() - 1; i >= 0; i--) {
            Task task = (Task) this.mChildren.get(i);
            if (task != null) {
