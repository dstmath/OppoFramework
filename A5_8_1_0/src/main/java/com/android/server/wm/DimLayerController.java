...
class DimLayerController {
    ...
    private static final int DEFAULT_DIM_DURATION = 180;
    ...
    private boolean animateDimLayers(DimLayerUser dimLayerUser) {
        ...
        if (targetAlpha != dimAmount) {
            ...
            if (dimLayer >= state.dimLayer.getLayer() || state.animator.mWin == null || state.animator.mWin.mAppToken == null) {
                state.dimLayer.setLayer(dimLayer);
            } else {
                try {
                    if (state.animator.mAnimating || (state.animator.mWin.isVisibleLw() ^ 1) == 0 || !state.animator.mWin.mAppToken.mFinishing) {
                        state.dimLayer.setLayer(dimLayer);
                    } else {
                        Slog.v(TAG, "animateDimLayers,don't setlayer,mAnimating=" + state.animator.mAnimating + ",isVisibleLw=" + state.animator.mWin.isVisibleLw() + ",mFinishing=" + state.animator.mWin.mAppToken.mFinishing);
                    }
                } catch (Exception e) {
                    Slog.e(TAG, "animateDimLayers  ", e);
                    state.dimLayer.setLayer(dimLayer);
                }
            }
        }
