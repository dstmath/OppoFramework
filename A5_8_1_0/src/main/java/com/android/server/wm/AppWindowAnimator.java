...
public class AppWindowAnimator {
    ...
    public void setAnimation(Animation anim, int width, int height, int parentWidth, int parentHeight, boolean skipFirstFrame, int stackClip, int transit, int transitFlags) {
        if (WindowManagerService.localLOGV) {
            Slog.v(TAG, "Setting animation in " + this.mAppToken + ": " + anim + " wxh=" + width + "x" + height + " hasContentToDisplay=" + this.mAppToken.hasContentToDisplay());
        }
        this.animation = anim;
    ...
    public void setDummyAnimation() {
        int i = 1;
        if (WindowManagerService.localLOGV) {
            Slog.v(TAG, "Setting dummy animation in " + this.mAppToken + " hasContentToDisplay=" + this.mAppToken.hasContentToDisplay());
        }
    ...
    boolean stepAnimationLocked(long currentTime) {
        ...
        if (WindowManagerDebugConfig.DEBUG_ANIM) {
            Slog.v(TAG, "Animation done in " + this.mAppToken + ": reportedVisible=" + this.mAppToken.reportedVisible + " okToDisplay=" + this.mAppToken.okToDisplay() + " okToAnimate=" + this.mAppToken.okToAnimate() + " startingDisplayed=" + this.mAppToken.startingDisplayed);
        }
        this.transformation.clear();
