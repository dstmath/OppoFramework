...
class TaskSnapshotSurface implements StartingSurface {
    ...
    /* JADX WARNING: Missing block: B:39:0x0172, code:
            com.android.server.wm.WindowManagerService.resetPriorityAfterLockedSection();
     */
    /* JADX WARNING: Missing block: B:41:?, code:
            r43 = r3.addToDisplay(r4, r4.mSeq, r6, 0, r49.getDisplayContent().getDisplayId(), r9, r9, r9, null);
     */
    /* JADX WARNING: Missing block: B:42:0x0187, code:
            if (r43 >= 0) goto L_0x01ad;
     */
    /* JADX WARNING: Missing block: B:43:0x0189, code:
            android.util.Slog.w(TAG, "Failed to add snapshot starting window res=" + r43);
     */
    /* JADX WARNING: Missing block: B:45:0x01a5, code:
            return null;
     */
    ...
    /* JADX WARNING: Missing block: B:29:0x00b0, code:
            if (com.android.server.wm.WindowManagerDebugConfig.DEBUG_STARTING_WINDOW == false) goto L_0x00ba;
     */
    /* JADX WARNING: Missing block: B:30:0x00b2, code:
            android.util.Slog.v(TAG, "Removing snapshot surface");
     */
    /* JADX WARNING: Missing block: B:31:0x00ba, code:
            r10.mSession.remove(r10.mWindow);
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    /* renamed from: remove */
    public void -com_android_server_wm_TaskSnapshotSurface-mthref-1() {
        synchronized (this.mService.mWindowMap) {
            try {
                WindowManagerService.boostPriorityForLockedSection();
                long now = SystemClock.uptimeMillis();
                if (mName != null && mName.contains("com.android.browser") && this.mSizeMismatch) {
                    this.mSizeMismatch = false;
                    this.mHandler.postAtTime(new -$Lambda$aEpJ2RCAIjecjyIIYTv6ricEwh4((byte) 9, this), this.mShownTime + 500);
                    if (WindowManagerDebugConfig.DEBUG_STARTING_WINDOW) {
                        Slog.v(TAG, "Defer removing snapshot surface (com.android.browser) in " + (now - this.mShownTime) + "ms");
                    }
                } else if (!this.mSizeMismatch || now - this.mShownTime >= SIZE_MISMATCH_MINIMUM_TIME_MS) {
                } else {
                    this.mHandler.postAtTime(new -$Lambda$aEpJ2RCAIjecjyIIYTv6ricEwh4((byte) 10, this), this.mShownTime + SIZE_MISMATCH_MINIMUM_TIME_MS);
                    if (WindowManagerDebugConfig.DEBUG_STARTING_WINDOW) {
    ...
    private void drawSnapshot() {
        ...
        if (mName != null && mName.contains("com.android.browser")) {
            this.mSizeMismatch = true;
            if (WindowManagerDebugConfig.DEBUG_STARTING_WINDOW) {
                Slog.v(TAG, "setFrames mSizeMismatch = " + this.mSizeMismatch);
            }
        }
        synchronized (this.mService.mWindowMap) {
            try {
