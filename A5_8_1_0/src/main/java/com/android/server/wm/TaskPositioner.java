...
class TaskPositioner implements DimLayerUser {
    ...
    private final class WindowPositionerEventReceiver extends BatchedInputEventReceiver {
        ...
        /* JADX WARNING: Removed duplicated region for block: B:16:0x0035 A:{Catch:{ all -> 0x02a0, all -> 0x01e8, Exception -> 0x00fc, all -> 0x0173 }} */
        /* JADX WARNING: Removed duplicated region for block: B:16:0x0035 A:{Catch:{ all -> 0x02a0, all -> 0x01e8, Exception -> 0x00fc, all -> 0x0173 }} */
                ...
                switch (motionEvent.getAction()) {
                    ...
                    case 2:
                        ...
                        synchronized (TaskPositioner.this.mService.mWindowMap) {
                            try {
                                WindowManagerService.boostPriorityForLockedSection();
                                if (TaskPositioner.this.mTask == null || TaskPositioner.this.mTask.mStack != null) {
                                    ...
                                    TaskPositioner.this.mDragEnded = false;
                                    TaskPositioner.this.mService.mH.sendEmptyMessage(40);
                                    WindowManagerService.resetPriorityAfterLockedSection();
                                }
                            } catch (Exception e3) {
    ...
    void register(Display display) {
        ...
        if (this.mMaxWidth > screenWidth) {
            this.mMaxWidth = screenWidth - this.mDelta;
        }
        if (this.mMaxHeight > screenWidth) {
            int i = screenWidth - this.mDelta;
            this.mMaxHeight = i;
            this.mMaxWidth = i;
        }
