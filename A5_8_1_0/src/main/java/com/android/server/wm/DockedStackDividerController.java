...
public class DockedStackDividerController implements DimLayerUser {
    ...
    private static final Interpolator IME_ADJUST_ENTRY_INTERPOLATOR = new PathInterpolator(0.2f, OppoBrightUtils.MIN_LUX_LIMITI, 0.1f, 1.0f);
    ...
    private void loadDimens() {
        ...
        this.mDividerWindowWidth = context.getResources().getDimensionPixelSize(17105019);
        this.mDividerInsets = context.getResources().getDimensionPixelSize(17105018);
        this.mDividerWindowWidthInactive = WindowManagerService.dipToPixel(2, this.mDisplayContent.getDisplayMetrics());
        this.mTaskHeightInMinimizedMode = context.getResources().getDimensionPixelSize(17105264);
    ...
    private void resetDragResizingChangeReported() {
        this.mDisplayContent.forAllWindows((Consumer) -$Lambda$-ShbHzWzMvKATSUwSngPXEFkvyU.$INST$4, true);
    }
    ...
    void reevaluateVisibility(boolean force) {
        if (this.mWindow != null) {
            ...
            if (!(focusApp == null || focusApp.getTask() == null || !focusApp.getTask().getFillsParent())) {
                visible = false;
    ...
    void registerDockedStackListener(IDockedStackListener listener) {
        ...
        notifyDockedStackExistsChanged(this.mDisplayContent.getDockedStackIgnoringVisibility() != null);
    ...
    private void setMinimizedDockedStack(boolean minimizedDock, boolean animate) {
        ...
        if (minimizedDock != wasMinimized) {
            ...
            if (isHomeStackResizable()) {
                ...
                if (animate) {
                    startAdjustAnimation(OppoBrightUtils.MIN_LUX_LIMITI, 1.0f);
                } else {
                    minimizedChange = setMinimizedDockedStack(true);
                }
            } else if (animate) {
                startAdjustAnimation(1.0f, OppoBrightUtils.MIN_LUX_LIMITI);
            } else {
    ...
    private void startImeAdjustAnimation(boolean adjustedForIme, boolean adjustedForDivider, WindowState imeWin) {
        ...
        this.mService.mDockedForDrawnCallback = new -$Lambda$HFbYE8vr4-NA3Y6wUONVd47OH54(adjustedForIme, adjustedForDivider, this);
    }

    /* renamed from: lambda$-com_android_server_wm_DockedStackDividerController_32585 */
    /* synthetic */ void m246lambda$-com_android_server_wm_DockedStackDividerController_32585(boolean adjustedForIme, boolean adjustedForDivider) {
    ...
    private boolean setMinimizedDockedStack(boolean minimized) {
        ...
        return stack.setAdjustedForMinimizedDock(minimized ? 1.0f : OppoBrightUtils.MIN_LUX_LIMITI);
    }
    ...
    private float getClipRevealMeetFraction(TaskStack stack) {
        ...
        return ((1.0f - Math.max(OppoBrightUtils.MIN_LUX_LIMITI, Math.min(1.0f, ((((float) Math.abs(this.mService.mAppTransition.getLastClipRevealMaxTranslation())) / ((float) stack.getMinimizeDistance())) - CLIP_REVEAL_MEET_FRACTION_MIN) / CLIP_REVEAL_MEET_FRACTION_MIN))) * 0.39999998f) + CLIP_REVEAL_MEET_EARLIEST;
    }
