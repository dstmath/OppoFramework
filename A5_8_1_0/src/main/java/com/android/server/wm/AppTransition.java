...
import com.color.util.ColorRecentsUtil;
...
public class AppTransition implements Dump {
    ...
    protected static final int THUMBNAIL_TRANSITION_ENTER_SCALE_DOWN = 2;
    protected static final int THUMBNAIL_TRANSITION_ENTER_SCALE_UP = 0;
    protected static final int THUMBNAIL_TRANSITION_EXIT_SCALE_DOWN = 3;
    protected static final int THUMBNAIL_TRANSITION_EXIT_SCALE_UP = 1;
    ...
    protected boolean mNextAppTransitionScaleUp;
    ...
    protected final Interpolator mThumbnailFadeInInterpolator;
    protected final Interpolator mThumbnailFadeOutInterpolator;
    ...
    protected static float computePivot(int startPos, float finalScale) {
    ...
    protected void getDefaultNextAppTransitionStartRect(Rect rect) {
    ...
    protected Animation createCurvedMotion(float fromX, float toX, float fromY, float toY) {
    ...
    Animation createAspectScaledThumbnailEnterExitAnimationLocked(int thumbTransitState, int uiMode, int orientation, int transit, Rect containingFrame, Rect contentInsets, Rect surfaceInsets, Rect stableInsets, boolean freeform, int taskId) {
        Animation a = createLauncherThumbnailEnterExitAnimationLocked(thumbTransitState, containingFrame, transit, taskId);
        if (a != null) {
            return a;
        }
        int appWidth = containingFrame.width();
        ...
        if (WindowManagerDebugConfig.DEBUG_APP_TRANSITIONS || WindowManagerDebugConfig.DEBUG_ANIM) {
            Slog.d(TAG, "createAspectScaledThumbnailEnterExitAnimationLocked thumbTransitState:" + thumbTransitState + " uiMode:" + uiMode + " orientation:" + orientation + " transit:" + transit + " containingFrame:" + containingFrame + " contentInsets:" + contentInsets + " surfaceInsets:" + surfaceInsets + " stableInsets:" + stableInsets + " freeform:" + freeform + " taskId:" + taskId);
        }
            ...
            case 3:
                ...
                if (freeform && scaleUp) {
                    ...
                    if (shouldScaleDownThumbnailTransition(uiMode, orientation)) {
                        ...
                        boolean isNeedStretch = ColorRecentsUtil.isThumbnailNeedStretch((appWidth - contentInsets.left) - contentInsets.right, (appHeight - contentInsets.top) - contentInsets.bottom, this.mService.mPolicy.getNavigationBarStatus(), displayInfo.logicalWidth, displayInfo.logicalHeight, this.mService.mNavigationBarHeightDefault, this.mService.mStatusBarHeightDefault);
                        if (WindowManagerDebugConfig.DEBUG_APP_TRANSITIONS || WindowManagerDebugConfig.DEBUG_ANIM) {
                            Slog.d(TAG, "createAspectScaledThumbnailEnterExitAnimationLocked isNeedStretch:" + isNeedStretch);
                        }
                        if (!isNeedStretch) {
                            if (orientation == 1) {
                                scaleY = scaleX;
                        ...
                        if (scaleUp) {
                            animationSet = new ClipRectAnimation(this.mTmpFromClipRect, this.mTmpToClipRect);
                        } else {
                            animationSet = new ClipRectAnimation(this.mTmpToClipRect, this.mTmpFromClipRect);
                        }
                        if (scaleUp) {
    ...
    /* renamed from: lambda$-com_android_server_wm_AppTransition_99110 */
    /* synthetic */ void m201lambda$-com_android_server_wm_AppTransition_99110(IAppTransitionAnimationSpecsFuture future) {
