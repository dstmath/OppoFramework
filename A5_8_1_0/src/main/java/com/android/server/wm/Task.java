...
class Task extends WindowContainer<AppWindowToken> implements DimLayerUser {
    ...
    AppWindowToken getTopunFinishChild() {
        for (int i = this.mChildren.size() - 1; i >= 0; i--) {
            AppWindowToken token = (AppWindowToken) this.mChildren.get(i);
            if (token != null && (token.mFinishing ^ 1) != 0) {
                return token;
            }
        }
        return null;
    ...
    private int setBounds(Rect bounds, Configuration overrideConfig) {
        ...
        } else if (bounds == null || !Configuration.EMPTY.equals(overrideConfig)) {
        ...
        if (displayContent != null) {
            ...
            if (this.mRotation != newRotation) {
                ...
                if (StackId.isTaskResizeAllowed(this.mStack.mStackId)) {
                    ...
                    if (this.mTmpRect2 != null && this.mTmpRect2.top < 0 && inFreeformWorkspace()) {
                        this.mTmpRect2.bottom -= this.mTmpRect2.top;
                        this.mTmpRect2.top = 0;
                    }
