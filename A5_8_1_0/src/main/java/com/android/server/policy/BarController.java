...
public class BarController {
    ...
    public boolean setBarShowingLw(boolean show) {
        ...
        if (PhoneWindowManager.DEBUG_PANIC) {
            Slog.d(this.mTag, "setBarShowingLw: " + show + " change:" + change + " stateChanged:" + stateChanged + " Callers=" + Debug.getCallers(2));
        }
        if (!change) {
            z = stateChanged;
        }
    ...
    private boolean updateStateLw(final int state) {
        ...
        this.mState = state;
        if (PhoneWindowManager.DEBUG_PANIC) {
            Slog.d(this.mTag, "mState: " + StatusBarManager.windowStateToString(state));
        }
        this.mHandler.post(new Runnable() {
            public void run() {
                ...
                if (BarController.this.mStatusBarManagerId == 1 && BarController.this.mWindowManagerInternal != null) {
                    Bundle data = new Bundle();
                    data.putInt(BarController.SYSTEM_BAR_ID, BarController.this.mStatusBarManagerId);
                    data.putInt(BarController.SYSTEM_BAR_STATE, state);
                    BarController.this.mWindowManagerInternal.notifyWindowStateChange(data);
                }
            }
        });
        return true;
    ...
    public void setWindowManagerInternal(WindowManagerInternal windowManagerInternal) {
    ...
    private void setTransientBarState(int state) {
        if (this.mWin != null && state != this.mTransientBarState) {
            ...
            if (PhoneWindowManager.DEBUG_PANIC) {
                Slog.d(this.mTag, "mTransientBarState: " + transientBarStateToString(state));
            }
        }
    }
