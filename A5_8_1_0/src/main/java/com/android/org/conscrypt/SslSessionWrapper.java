...
abstract class SslSessionWrapper {
    ...
    private static final class Impl extends SslSessionWrapper {
        ...
        /* synthetic */ Impl(AbstractSessionContext context, SSL_SESSION ref, String host, int port, X509Certificate[] peerCertificates, byte[] peerOcspStapledResponse, byte[] peerSignedCertificateTimestamp, Impl -this7) {
            this(context, ref, host, port, peerCertificates, peerOcspStapledResponse, peerSignedCertificateTimestamp);
        }
    ...
    static SslSessionWrapper newInstance(SSL_SESSION ref, ActiveSession activeSession) throws SSLPeerUnverifiedException {
        ...
        if (!(context instanceof ClientSessionContext)) {
            return new Impl(context, ref, null, -1, null, null, null, null);
        }
        return new Impl(context, ref, activeSession.getPeerHost(), activeSession.getPeerPort(), activeSession.getPeerCertificates(), getOcspResponse(activeSession), activeSession.getPeerSignedCertificateTimestamp(), null);
    }
    ...
    static SslSessionWrapper newInstance(AbstractSessionContext context, byte[] data, String host, int port) {
        ...
        try {
            ...
            if (SessionType.isSupportedType(type)) {
                ...
                return new Impl(context, new SSL_SESSION(NativeCrypto.d2i_SSL_SESSION(sessionData)), host, port, peerCerts, ocspData, tlsSctData, null);
            }
