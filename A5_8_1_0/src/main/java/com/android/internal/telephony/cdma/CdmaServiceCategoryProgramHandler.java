...
public final class CdmaServiceCategoryProgramHandler extends WakeLockStateMachine {
    ...
    private final BroadcastReceiver mScpResultsReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            sendScpResults();
            CdmaServiceCategoryProgramHandler.this.-wrap0("mScpResultsReceiver finished");
        ...
        private void sendScpResults() {
            ...
            if (resultCode == -1 || resultCode == 1) {
                ...
                if (extras == null) {
                    CdmaServiceCategoryProgramHandler.this.-wrap1("SCP results error: missing extras");
                    return;
                }
                String sender = extras.getString("sender");
                if (sender == null) {
                    CdmaServiceCategoryProgramHandler.this.-wrap1("SCP results error: missing sender extra.");
                    return;
                }
                ArrayList<CdmaSmsCbProgramResults> results = extras.getParcelableArrayList("results");
                if (results == null) {
                    CdmaServiceCategoryProgramHandler.this.-wrap1("SCP results error: missing results extra.");
                    return;
                }
                ...
                try {
                    ...
                    CdmaServiceCategoryProgramHandler.this.-wrap2("exception creating SCP results PDU", e2);
                    try {
                        dos.close();
            ...
            CdmaServiceCategoryProgramHandler.this.-wrap1("SCP results error: result code = " + resultCode);
        }
    };
    ...
    protected boolean handleSmsMessage(Message message) {
        ...
        -wrap1("handleMessage got object of type: " + message.obj.getClass().getName());
        return false;
    }
    ...
    private boolean handleServiceCategoryProgramData(SmsMessage sms) {
        ...
        if (programDataList == null) {
            -wrap1("handleServiceCategoryProgramData: program data list is null!");
            return false;
        }
        Intent intent = new Intent("android.provider.Telephony.SMS_SERVICE_CATEGORY_PROGRAM_DATA_RECEIVED");
        intent.setPackage(this.mContext.getResources().getString(17039661));
