...
public class UsimPhoneBookManager extends Handler implements IccConstants {
    ...
    private void readEmailFileAndWait(int recId) {
        if (this.mPbrRecords.get(recId) == null) {
            Rlog.e(LOG_TAG, "PbrRecord is empty for recId:" + recId);
            return;
        }
        ...
        if (this.mPbrRecords.get(recId) != null) {
            ...
            while (i < numRecs) {
                try {
                    ...
                    if (!(email == null || email.equals(SpnOverride.MVNO_TYPE_NONE))) {
                        int adnEfid;
    ...
    private boolean buildType2EmailList(int recId) {
        ...
        for (int i = 0; i < numRecs; i++) {
            try {
                ...
                if (!(email == null || (email.equals(SpnOverride.MVNO_TYPE_NONE) ^ 1) == 0)) {
        ...
        switch (msg.what) {
            ...
            case 2:
                ...
                if (ar.exception == null) {
                    ...
                    log("can't load USIM ADN records due to exception");
                }
                obj = this.mLock;
                synchronized (obj) {
                    this.mLock.notify();
