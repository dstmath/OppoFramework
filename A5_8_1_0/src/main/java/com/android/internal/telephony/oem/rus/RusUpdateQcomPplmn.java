...
public final class RusUpdateQcomPplmn extends RusBase {
    ...
    private static final String TAG = "RusUpdateQcomPplmn";
    ...
    private static final String mPlmnIndex_end = "/QcomPplmn>";
    private static final String mPlmnIndex_start = "<QcomPplmn";
    private static final String mPlmnInfoFileName = "qual_update_plmn.xml";
    private static final String mSubPath = "/data/data/com.android.phone/";
    public final int FINAL_NV_LENGTH = 207;
    ...
    public void onSucceed(Message msg) {
        ...
        printPlmnByteArray(bytearray, len * 5);
        try {
    ...
    public int valiateAndUpatePlmn() {
        ...
        try {
            ...
            try {
                ...
                XmlUtils.beginDocument(confparser, "QcomPplmn");
    ...
    private void setPlmnList(int[][] plmnIdList, int len, int type) {
        ...
        byte[] finalplmndata = new byte[207];
        byte[] finalplmndata2 = new byte[207];
        ...
        if (n >= 1) {
            ...
            while (i < n) {
                ...
                finalplmndata = originalPlmn2FinalPlmn(plmndata, 0, 0);
                msg = obtainSucceedMsg();
                msg.obj = finalplmndata;
                sendMessage(msg);
                i++;
                pos2 = pos;
            }
        ...
        while (true) {
            ...
            if (i < m * 3) {
                ...
                finalplmndata2 = originalPlmn2FinalPlmn(plmndata2, 0, 0);
                msg = obtainSucceedMsg();
                msg.obj = finalplmndata2;
                sendMessage(msg);
                return;
            }
        }
    ...
    private byte[] originalPlmn2FinalPlmn(byte[] arrayOriginal, int data1, int data2) {
        int i;
        int lenOriginal = arrayOriginal.length;
        int arrow = (lenOriginal - 7) / 3;
        byte[] arrayNew = new byte[207];
        System.arraycopy(arrayOriginal, 0, arrayNew, 0, 7);
        int jstartOriginal = 7;
        int jstartNew = 7;
        for (i = 0; i < arrow; i++) {
            jstartNew = jstartOriginal + (i * 2);
            System.arraycopy(arrayOriginal, jstartOriginal, arrayNew, jstartNew, 3);
            jstartOriginal += 3;
            jstartNew += 3;
            int i2 = jstartNew + 1;
            arrayNew[jstartNew] = (byte) data1;
            jstartNew = i2 + 1;
            arrayNew[i2] = (byte) data2;
        }
        int lenRemain = (lenOriginal - 1) - jstartOriginal;
        for (i = 1; i <= lenRemain; i++) {
            arrayNew[jstartNew + i] = arrayOriginal[jstartOriginal + i];
        }
        return arrayNew;
    }
    ...
    void printPlmnByteArray(byte[] plmnarray, int len) {
        ...
        for (int i = 7; i < len + 7; i += 5) {
            printLog(TAG, "plmn_id=" + plmnarray[i] + " " + plmnarray[i + 1] + " " + plmnarray[i + 2] + " " + plmnarray[i + 3] + " " + plmnarray[i + 4]);
        }
    }
}
