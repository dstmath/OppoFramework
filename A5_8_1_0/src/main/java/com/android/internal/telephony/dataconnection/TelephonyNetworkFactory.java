...
public class TelephonyNetworkFactory extends NetworkFactory {
    ...
    private void onActivePhoneSwitch() {
        ...
        if (this.mIsDefault || mCurrentDefaultSubId == mCurrentSubId || this.mSubscriptionController.getPhoneId(mCurrentDefaultSubId) == this.mPhoneId) {
            boolean z2;
    ...
    private void onDefaultChange() {
        ...
        if (newIsDefault != this.mIsDefault) {
            ...
            if (this.mSubscriptionController.getActiveSubInfoCount(this.mContext.getOpPackageName()) == 1) {
                HashMap hashMap = this.mDefaultRequests;
                if (!this.mIsDefault) {
                    z = false;
                }
