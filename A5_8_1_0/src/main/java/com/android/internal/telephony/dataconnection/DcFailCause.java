...
public enum DcFailCause {
    ...
    public boolean isPermanentFailure(Context context, int subId) {
        HashSet<DcFailCause> permanentFailureSet;
        synchronized (sPermanentFailureCache) {
            permanentFailureSet = (HashSet) sPermanentFailureCache.get(Integer.valueOf(subId));
    ...
    public boolean isTelstraEsmCause() {
        if (this == INSUFFICIENT_RESOURCES || this == ACTIVATION_REJECT_UNSPECIFIED || this == SERVICE_OPTION_OUT_OF_ORDER || this == NETWORK_FAILURE) {
            return true;
        }
        return false;
    }
