...
public class ApnSetting {
    ...
    public static ApnSetting fromString(String data) {
        ...
        if (data.matches("^\\[ApnSettingV3\\]\\s*.*")) {
            version = 3;
            data = data.replaceFirst(V3_FORMAT_REGEX, SpnOverride.MVNO_TYPE_NONE);
        } else {
            if (data.matches("^\\[ApnSettingV2\\]\\s*.*")) {
                version = 2;
                data = data.replaceFirst(V2_FORMAT_REGEX, SpnOverride.MVNO_TYPE_NONE);
            } else {
                version = 1;
            }
        }
        ...
        int maxConnsTime = 0;
        int mtu = 0;
        String mvnoType = SpnOverride.MVNO_TYPE_NONE;
        String mvnoMatchData = SpnOverride.MVNO_TYPE_NONE;
        if (version == 1) {
    ...
    public static boolean mvnoMatches(IccRecords r, String mvnoType, String mvnoMatchData) {
        if (mvnoType.equalsIgnoreCase(SpnOverride.MVNO_TYPE_SPN)) {
            return r.getServiceProviderName() != null && r.getServiceProviderName().equalsIgnoreCase(mvnoMatchData);
        } else {
            if (mvnoType.equalsIgnoreCase(SpnOverride.MVNO_TYPE_IMSI)) {
                String imsiSIM = r.getIMSI();
                if (imsiSIM != null && imsiMatches(mvnoMatchData, imsiSIM)) {
                    return true;
                }
            } else if (mvnoType.equalsIgnoreCase(SpnOverride.MVNO_TYPE_GID)) {
