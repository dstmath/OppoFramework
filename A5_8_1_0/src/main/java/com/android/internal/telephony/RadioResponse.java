...
public class RadioResponse extends Stub {
    ...
    private void responseIccCardStatus(RadioResponseInfo responseInfo, CardStatus cardStatus) {
        ...
        if (rr != null) {
            ...
            if (cardStatus.cardState == 0) {
                Message msg = rr.mResult;
                if (!(msg == null || msg.obj == null || !(msg.obj instanceof Integer))) {
                    Integer slotId = msg.obj;
                    if (SubscriptionManager.isValidPhoneId(slotId.intValue())) {
                        TelephonyManager.getDefault();
                        if (TelephonyManager.getTelephonyProperty(slotId.intValue(), "gsm.sim.state", SpnOverride.MVNO_TYPE_NONE).equals(SpnOverride.MVNO_TYPE_NONE)) {
                            TelephonyManager.getDefault().setSimStateForPhone(slotId.intValue(), "ABSENT");
                        }
                    }
                }
            }
            iccCardStatus.mSimHotswapState = cardStatus.simHotswapState;
            this.mRil.riljLog("responseIccCardStatus: simHotswapState: " + iccCardStatus.mSimHotswapState);
