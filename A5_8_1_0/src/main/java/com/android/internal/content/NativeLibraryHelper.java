...
public class NativeLibraryHelper {
    ...
    private static native int nativeFindSupportedAbi(long j, boolean z, String[] strArr, boolean z2);
    ...
    public static int findSupportedAbi(Handle handle, String[] supportedAbis, boolean fromCopy) {
        ...
        for (long apkHandle : handle.apkHandles) {
            int res = nativeFindSupportedAbi(apkHandle, fromCopy, supportedAbis, handle.debuggable);
            if (res != -114) {
                if (res == -113) {
                    if (finalRes < 0) {
                        finalRes = -113;
