...
public final class HttpDateTime {
    ...
    public static long parse(String timeString) throws IllegalArgumentException {
        ...
        if (year >= LayoutParams.TYPE_APPLICATION_OVERLAY) {
            year = LayoutParams.TYPE_APPLICATION_OVERLAY;
            month = 0;
            date = 1;
        }
    ...
    private static int getYear(String yearString) {
        if (yearString.length() == 2) {
            ...
            return ((((yearString.charAt(0) - 48) * 100) + ((yearString.charAt(1) - 48) * 10)) + (yearString.charAt(2) - 48)) + PreciseDisconnectCause.ECBM_NOT_SUPPORTED;
        } else {
    ...
    private static TimeOfDay getTime(String timeString) {
        ...
        if (timeString.charAt(1) != ColorAccessibilityUtil.ENABLED_ACCESSIBILITY_SERVICES_SEPARATOR) {
