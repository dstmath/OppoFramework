...
public class PointerLocationView extends View implements InputDeviceListener, PointerEventListener {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mActivePointerId >= 0 && this.mActivePointerId < NP) {
    ...
    public void onPointerEvent(MotionEvent event) {
        ...
        if (action == 0 || (action & 255) == 5) {
            ...
            if (this.mActivePointerId < 0 || (((PointerState) this.mPointers.get(this.mActivePointerId)).mCurDown ^ 1) != 0) {
        ...
        for (int historyPos = 0; historyPos < N; historyPos++) {
            for (i = 0; i < NI; i++) {
                id = event.getPointerId(i);
                ps = (!this.mCurDown || id >= NP) ? null : (PointerState) this.mPointers.get(id);
