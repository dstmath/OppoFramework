...
class ByteBufferAsDoubleBuffer extends DoubleBuffer {
    ...
    static final /* synthetic */ boolean f52-assertionsDisabled = (ByteBufferAsDoubleBuffer.class.desiredAssertionStatus() ^ 1);
    ...
    public DoubleBuffer slice() {
        int pos = position();
        int lim = limit();
        if (f52-assertionsDisabled || pos <= lim) {
            int rem = pos <= lim ? lim - pos : 0;
            int off = (pos << 3) + this.offset;
            if (f52-assertionsDisabled || off >= 0) {
