...
class ByteBufferAsShortBuffer extends ShortBuffer {
    ...
    static final /* synthetic */ boolean f57-assertionsDisabled = (ByteBufferAsShortBuffer.class.desiredAssertionStatus() ^ 1);
    ...
    public ShortBuffer slice() {
        int pos = position();
        int lim = limit();
        if (f57-assertionsDisabled || pos <= lim) {
            int rem = pos <= lim ? lim - pos : 0;
            int off = (pos << 1) + this.offset;
            if (f57-assertionsDisabled || off >= 0) {
