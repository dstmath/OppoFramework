...
class ByteBufferAsFloatBuffer extends FloatBuffer {
    ...
    static final /* synthetic */ boolean f53-assertionsDisabled = (ByteBufferAsFloatBuffer.class.desiredAssertionStatus() ^ 1);
    ...
    public FloatBuffer slice() {
        int pos = position();
        int lim = limit();
        if (f53-assertionsDisabled || pos <= lim) {
            int rem = pos <= lim ? lim - pos : 0;
            int off = (pos << 2) + this.offset;
            if (f53-assertionsDisabled || off >= 0) {
