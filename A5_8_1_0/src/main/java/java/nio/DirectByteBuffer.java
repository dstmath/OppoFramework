...
public class DirectByteBuffer extends MappedByteBuffer implements DirectBuffer {
    ...
    static final /* synthetic */ boolean f54-assertionsDisabled = (DirectByteBuffer.class.desiredAssertionStatus() ^ 1);
    ...
    public final ByteBuffer slice() {
        if (this.memoryRef.isAccessible) {
            int pos = position();
            int lim = limit();
            if (f54-assertionsDisabled || pos <= lim) {
                int rem = pos <= lim ? lim - pos : 0;
                int off = pos + this.offset;
                if (f54-assertionsDisabled || off >= 0) {
    ...
    public final CharBuffer asCharBuffer() {
        ...
        int off = position();
        int lim = limit();
        if (f54-assertionsDisabled || off <= lim) {
