...
class ByteBufferAsIntBuffer extends IntBuffer {
    ...
    static final /* synthetic */ boolean f55-assertionsDisabled = (ByteBufferAsIntBuffer.class.desiredAssertionStatus() ^ 1);
    ...
    public IntBuffer slice() {
        int pos = position();
        int lim = limit();
        if (f55-assertionsDisabled || pos <= lim) {
            int rem = pos <= lim ? lim - pos : 0;
            int off = (pos << 2) + this.offset;
            if (f55-assertionsDisabled || off >= 0) {
