...
public final class MethodType implements Serializable {
    ...
    static final /* synthetic */ boolean f37-assertionsDisabled = (MethodType.class.desiredAssertionStatus() ^ 1);
        ...
        private static class WeakEntry<T> extends WeakReference<T> {
            ...
            public boolean equals(Object obj) {
                if (!(obj instanceof WeakEntry)) {
                    return MethodType.f37-assertionsDisabled;
                }
                Object that = ((WeakEntry) obj).get();
                Object mine = get();
                boolean equals = (that == null || mine == null) ? this == obj ? true : MethodType.f37-assertionsDisabled : mine.lambda$-java_util_function_Predicate_4628(that);
                return equals;
            }
    ...
    static void checkSlotCount(int count) {
        boolean z = f37-assertionsDisabled;
    ...
    public static MethodType methodType(Class<?> rtype, Class<?>[] ptypes) {
        return makeImpl(rtype, ptypes, f37-assertionsDisabled);
    }

    public static MethodType methodType(Class<?> rtype, List<Class<?>> ptypes) {
        return makeImpl(rtype, listToArray(ptypes), f37-assertionsDisabled);
    }
    ...
    public static MethodType genericMethodType(int objectArgCount) {
        return genericMethodType(objectArgCount, f37-assertionsDisabled);
    }
    ...
    MethodType asSpreaderType(Class<?> arrayType, int arrayLength) {
        if (f37-assertionsDisabled || parameterCount() >= arrayLength) {
            ...
            if (f37-assertionsDisabled || elemType != null) {
    ...
    MethodType asCollectorType(Class<?> arrayType, int arrayLength) {
        if (!f37-assertionsDisabled && parameterCount() < 1) {
            throw new AssertionError();
        } else if (f37-assertionsDisabled || lastParameterType().isAssignableFrom(arrayType)) {
            MethodType res;
            if (arrayType == Object[].class) {
    ...
    public boolean hasWrappers() {
        return unwrap() != this ? true : f37-assertionsDisabled;
    }
    ...
    boolean isGeneric() {
        return this == erase() ? hasPrimitives() ^ 1 : f37-assertionsDisabled;
    }
    ...
    private static MethodType wrapWithPrims(MethodType pt) {
        if (f37-assertionsDisabled || pt.hasPrimitives()) {
            MethodType wt = pt.wrapAlt;
            if (wt == null) {
                wt = MethodTypeForm.canonicalize(pt, 2, 2);
                if (f37-assertionsDisabled || wt != null) {
                    pt.wrapAlt = wt;
                } else {
                    throw new AssertionError();
                }
    ...
    private static MethodType unwrapWithNoPrims(MethodType wt) {
        if (f37-assertionsDisabled || !wt.hasPrimitives()) {
    ...
    public boolean equals(Object x) {
        if (this != x) {
            return x instanceof MethodType ? equals((MethodType) x) : f37-assertionsDisabled;
        } else {
            return true;
        }
    }
    ...
    boolean isConvertibleTo(MethodType newType) {
        ...
        if (argc <= 1) {
            return (argc != 1 || (canConvert(srcTypes[0], dstTypes[0]) ^ 1) == 0) ? true : f37-assertionsDisabled;
        } else {
            ...
            if (f37-assertionsDisabled || canConvertParameters(srcTypes, dstTypes)) {
                return true;
            }
            throw new AssertionError();
        }
    ...
    boolean explicitCastEquivalentToAsType(MethodType newType) {
        ...
        if (f37-assertionsDisabled || dstTypes.length == srcTypes.length) {
