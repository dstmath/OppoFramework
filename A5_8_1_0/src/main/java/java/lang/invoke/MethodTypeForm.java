...
final class MethodTypeForm {
    ...
    static final /* synthetic */ boolean f42-assertionsDisabled = (MethodTypeForm.class.desiredAssertionStatus() ^ 1);
    ...
    private boolean assertIsBasicType() {
        if (f42-assertionsDisabled || this.erasedType == this.basicType) {
            return true;
        }
    ...
    protected MethodTypeForm(MethodType erasedType) {
        ...
        if (ptypes == bpts && bt == rt) {
            ...
            if (lac != 0) {
                ...
                if (!(f42-assertionsDisabled || slot == 0)) {
            ...
            } else if (f42-assertionsDisabled || ptypeCount == pslotCount) {
                that = MethodType.genericMethodType(ptypeCount).form();
                if (f42-assertionsDisabled || this != that) {
            ...
            } else if (!f42-assertionsDisabled && this.basicType != erasedType) {
                throw new AssertionError();
            } else {
                return;
            }
    ...
    private static long pack(int a, int b, int c, int d) {
        if (f42-assertionsDisabled || ((((a | b) | c) | d) & -65536) == 0) {
    ...
    private static char unpack(long packed, int word) {
        if (f42-assertionsDisabled || word <= 3) {
    ...
    public boolean hasPrimitives() {
        return this.primCounts != 0 ? true : f42-assertionsDisabled;
    }

    public boolean hasNonVoidPrimitives() {
        boolean z = true;
        if (this.primCounts == 0) {
            return f42-assertionsDisabled;
        }
        if (primitiveParameterCount() != 0) {
            return true;
        }
        if (primitiveReturnCount() == 0 || returnCount() == 0) {
            z = f42-assertionsDisabled;
        }
        return z;
    }

    public boolean hasLongPrimitives() {
        return (longPrimitiveParameterCount() | longPrimitiveReturnCount()) != 0 ? true : f42-assertionsDisabled;
    }
