...
public class File implements Serializable, Comparable<File> {
    ...
    static final /* synthetic */ boolean f7-assertionsDisabled = (File.class.desiredAssertionStatus() ^ 1);
    ...
    final boolean isInvalid() {
        ...
        return this.status == PathStatus.INVALID ? true : f7-assertionsDisabled;
    }
    ...
    private File(String child, File parent) {
        if (!f7-assertionsDisabled && parent.path == null) {
            throw new AssertionError();
        } else if (f7-assertionsDisabled || !parent.path.equals("")) {
    ...
    public boolean isDirectory() {
        boolean z = f7-assertionsDisabled;
        SecurityManager security = System.getSecurityManager();
        if (security != null) {
            security.checkRead(this.path);
        }
    ...
    public File[] listFiles() {
        String[] ss = list();
        if (ss == null) {
            if (VMRuntime.getRuntime().getDisableCompatbilityFlag() || !separator.equals(this.path) || VMRuntime.getRuntime().getTargetSdkVersion() > 23) {
                return null;
            }
            System.logE("Can not read rootfs anymore, return a fake result!", new Throwable());
            ss = new String[]{separator + "sdcard"};
        }
        int n = ss.length;
        File[] fs = new File[n];
        for (int i = 0; i < n; i++) {
