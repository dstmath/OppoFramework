...
public class ArrayDeque<E> extends AbstractCollection<E> implements Deque<E>, Cloneable, Serializable {
    ...
    static final /* synthetic */ boolean f74-assertionsDisabled = (ArrayDeque.class.desiredAssertionStatus() ^ 1);
    ...
    private class DeqIterator implements Iterator<E> {
        ...
        public boolean hasNext() {
            return this.cursor != this.fence ? true : ArrayDeque.f74-assertionsDisabled;
        }
    ...
    static final class DeqSpliterator<E> implements Spliterator<E> {
        ...
        public boolean tryAdvance(Consumer<? super E> consumer) {
            ...
            int i = this.index;
            if (i == f) {
                return ArrayDeque.f74-assertionsDisabled;
            }
            E e = a[i];
            this.index = (i + 1) & m;
            if (e == null) {
    ...
    private void doubleCapacity() {
        if (f74-assertionsDisabled || this.head == this.tail) {
            int p = this.head;
    ...
    private void checkInvariants() {
        Object obj = 1;
        if (f74-assertionsDisabled || this.elements[this.tail] == null) {
            ...
            if (!f74-assertionsDisabled && this.elements[(this.head - 1) & (this.elements.length - 1)] != null) {
                throw new AssertionError();
            }
            return;
        }
    ...
    public boolean isEmpty() {
        return this.head == this.tail ? true : f74-assertionsDisabled;
    }
    ...
    public Object[] toArray() {
        int head = this.head;
        int tail = this.tail;
        boolean wrap = tail < head ? true : f74-assertionsDisabled;
