...
public class FileChannelImpl extends FileChannel {
    ...
    static final /* synthetic */ boolean f16-assertionsDisabled = (FileChannelImpl.class.desiredAssertionStatus() ^ 1);
    ...
    private static class SimpleFileLockTable extends FileLockTable {
        /* renamed from: -assertionsDisabled */
        static final /* synthetic */ boolean f17-assertionsDisabled = (SimpleFileLockTable.class.desiredAssertionStatus() ^ 1);
        ...
        private void checkList(long position, long size) throws OverlappingFileLockException {
            if (f17-assertionsDisabled || Thread.holdsLock(this.lockList)) {
        ...
        public void add(FileLock fl) throws OverlappingFileLockException {
            synchronized (this.lockList) {
                ...
                this.lockList.-java_util_stream_SpinedBuffer-mthref-0(fl);
            }
        }
        ...
        public void replace(FileLock fl1, FileLock fl2) {
            synchronized (this.lockList) {
                ...
                this.lockList.-java_util_stream_SpinedBuffer-mthref-0(fl2);
            }
        }
    }
    ...
    private static class Unmapper implements Runnable {
        /* renamed from: -assertionsDisabled */
        static final /* synthetic */ boolean f18-assertionsDisabled = (Unmapper.class.desiredAssertionStatus() ^ 1);
        ...
        private Unmapper(long address, long size, int cap, FileDescriptor fd) {
            if (f18-assertionsDisabled || address != 0) {
    ...
    public static FileChannel open(FileDescriptor fd, String path, boolean readable, boolean writable, Object parent) {
        return new FileChannelImpl(fd, path, readable, writable, f16-assertionsDisabled, parent);
    }
            ...
            synchronized (this.positionLock) {
                ...
                try {
                    ...
                    if (isOpen()) {
                        ...
                        if (n <= 0) {
                            z = f16-assertionsDisabled;
                        }
                        end(z);
                        if (f16-assertionsDisabled || IOStatus.check(n)) {
                        } else {
                            throw new AssertionError();
                        }
                    }
                    this.threads.remove(ti);
                    end(f16-assertionsDisabled);
                    if (f16-assertionsDisabled || IOStatus.check(0)) {
                    } else {
                        throw new AssertionError();
                    }
                } catch (Throwable th) {
        ...
        synchronized (this.positionLock) {
            ...
            try {
                ...
                if (isOpen()) {
                    ...
                    if (f16-assertionsDisabled || IOStatus.check(p)) {
                    } else {
                        throw new AssertionError();
                    }
                }
                ...
                if (f16-assertionsDisabled || IOStatus.check(-1)) {
                    return 0;
                }
                throw new AssertionError();
            } catch (Throwable th) {
        ...
        synchronized (this.positionLock) {
            ...
            try {
                ...
                if (isOpen()) {
                    ...
                    if (f16-assertionsDisabled || IOStatus.check(s)) {
                    } else {
                        throw new AssertionError();
                    }
                }
                ...
                if (isOpen()) {
                    ...
                    do {
                        try {
                            ...
                            this.threads.remove(ti);
                            end(rv > -1 ? true : f16-assertionsDisabled);
                            if (!f16-assertionsDisabled && !IOStatus.check(rv)) {
                                AssertionError assertionError = new AssertionError();
                            }
                        }
                    } while (isOpen());
                    if (isOpen()) {
                        ...
                        if (!isOpen()) {
                            ...
                            throw new AssertionError();
                        } else if (f16-assertionsDisabled || p >= 0) {
                            if (newSize < size) {
                                do {
    ...
    private long transferToDirectlyInternal(long position, int icount, WritableByteChannel target, FileDescriptor targetFD) throws IOException {
        if (f16-assertionsDisabled || !this.nd.transferToDirectlyNeedsPositionLock() || (Thread.holdsLock(this.positionLock) ^ 1) == 0) {
            long n = -1;
            int ti = -1;
            try {
                begin();
                ...
                if (isOpen()) {
                    ...
                    if (n == -6) {
                        if (target instanceof SinkChannelImpl) {
                            pipeSupported = f16-assertionsDisabled;
                        }
                        if (target instanceof FileChannelImpl) {
                            fileSupported = f16-assertionsDisabled;
                        }
                        this.threads.remove(ti);
                        if (n > -1) {
                            z = true;
                        ...
                        transferSupported = f16-assertionsDisabled;
                        this.threads.remove(ti);
                        if (n > -1) {
                            z = true;
                        } else {
                        ...
                        long normalize = IOStatus.normalize(n);
                        this.threads.remove(ti);
                        end(n > -1 ? true : f16-assertionsDisabled);
                        return normalize;
                    }
                }
                this.threads.remove(ti);
    ...
    private long transferToTrustedChannel(long position, long count, WritableByteChannel target) throws IOException {
        ...
        while (remaining > 0) {
            ...
            try {
                ...
                if (f16-assertionsDisabled || n >= 0) {
                    remaining -= (long) n;
                    if (isSelChImpl) {
                        unmap(dbb);
                        break;
                    } else if (f16-assertionsDisabled || n > 0) {
                        position += (long) n;
                        unmap(dbb);
                    } else {
                        throw new AssertionError();
                ...
                if (f16-assertionsDisabled || !target.isOpen()) {
                    try {
                        close();
    ...
    private int readInternal(ByteBuffer dst, long position) throws IOException {
        boolean z = true;
        if (f16-assertionsDisabled || !this.nd.needsPositionLock() || (Thread.holdsLock(this.positionLock) ^ 1) == 0) {
            int n = 0;
            int ti = -1;
            try {
                begin();
    ...
    public MappedByteBuffer map(MapMode mode, long position, long size) throws IOException {
        ...
        if (mode == null) {
            ...
            if (!f16-assertionsDisabled && imode < 0) {
                throw new AssertionError();
                ...
                if (isOpen()) {
                    ...
                    if (isOpen()) {
                        ...
                        if (size == 0) {
                            ...
                            boolean z = (!this.writable || imode == 0) ? true : f16-assertionsDisabled;
                        ...
                        if (!f16-assertionsDisabled && !IOStatus.checkAll(addr)) {
                            throw new AssertionError();
                        } else if (f16-assertionsDisabled || addr % allocationGranularity == 0) {
                            int isize = (int) size;
                            Unmapper um = new Unmapper(addr, mapSize, isize, mfd, null);
                            long j = addr + ((long) pagePosition);
                            boolean z2 = (!this.writable || imode == 0) ? true : f16-assertionsDisabled;
    ...
    public FileLock lock(long position, long size, boolean shared) throws IOException {
        ...
        if (shared && (this.readable ^ 1) != 0) {
            ...
            flt.add(fli);
            boolean completed = f16-assertionsDisabled;
            int ti = -1;
            begin();
            ti = this.threads.add();
            if (isOpen()) {
                ...
                if (isOpen()) {
                    if (n == 1) {
                        if (f16-assertionsDisabled || shared) {
                            FileLockImpl fli2 = new FileLockImpl((FileChannel) this, position, size, (boolean) f16-assertionsDisabled);
    ...
    public FileLock tryLock(long position, long size, boolean shared) throws IOException {
        ...
        if (shared && (this.readable ^ 1) != 0) {
            ...
            try {
                ensureOpen();
                int result = this.nd.lock(this.fd, f16-assertionsDisabled, position, size, shared);
                if (result == -1) {
                    flt.remove(fli);
                    this.threads.remove(ti);
                    return null;
                    ...
                    this.threads.remove(ti);
                    return fli;
                } else if (f16-assertionsDisabled || shared) {
                    FileLockImpl fli2 = new FileLockImpl((FileChannel) this, position, size, (boolean) f16-assertionsDisabled);
                    flt.replace(fli, fli2);
                    this.threads.remove(ti);
                    return fli2;
    ...
    void release(FileLockImpl fli) throws IOException {
        ...
        try {
            ...
            if (f16-assertionsDisabled || this.fileLockTable != null) {
