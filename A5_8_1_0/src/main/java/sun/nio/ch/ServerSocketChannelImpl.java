...
class ServerSocketChannelImpl extends ServerSocketChannel implements SelChImpl {
    ...
    static final /* synthetic */ boolean f266-assertionsDisabled = (ServerSocketChannelImpl.class.desiredAssertionStatus() ^ 1);
    ...
    public boolean isBound() {
        boolean z;
        synchronized (this.stateLock) {
            z = this.localAddress != null ? true : f266-assertionsDisabled;
        }
        return z;
    }
    ...
    public SocketChannel accept() throws IOException {
        SocketChannel sc;
        boolean z = true;
        boolean z2 = f266-assertionsDisabled;
        synchronized (this.lock) {
            if (!isOpen()) {
                throw new ClosedChannelException();
            } else if (isBound()) {
                ...
                try {
                    ...
                    if (isOpen()) {
                        ...
                        if (!f266-assertionsDisabled && !IOStatus.check(n)) {
                        ...
                        this.thread = 0;
                        end(f266-assertionsDisabled);
                        if (f266-assertionsDisabled || IOStatus.check(0)) {
                        } else {
                            throw new AssertionError();
                        }
                    }
    ...
    public void kill() throws IOException {
        synchronized (this.stateLock) {
            if (this.state == 1) {
                ...
                this.state = 1;
            } else if (f266-assertionsDisabled || !(isOpen() || isRegistered())) {
                nd.close(this.fd);
                this.state = 1;
            } else {
                throw new AssertionError();
    ...
    public boolean translateReadyOps(int ops, int initialOps, SelectionKeyImpl sk) {
        ...
        if ((Net.POLLNVAL & ops) != 0) {
            return f266-assertionsDisabled;
        }
    ...
    int poll(int events, long timeout) throws IOException {
        boolean z = f266-assertionsDisabled;
        if (f266-assertionsDisabled || (Thread.holdsLock(blockingLock()) && !isBlocking())) {
            synchronized (this.lock) {
                long isOpen;
                try {
                    begin();
