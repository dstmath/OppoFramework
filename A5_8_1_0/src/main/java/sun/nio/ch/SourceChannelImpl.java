...
class SourceChannelImpl extends SourceChannel implements SelChImpl {
    ...
    static final /* synthetic */ boolean f268-assertionsDisabled = (SourceChannelImpl.class.desiredAssertionStatus() ^ 1);
    ...
    public void kill() throws IOException {
        synchronized (this.stateLock) {
            if (this.state == 1) {
                ...
                this.state = 1;
            } else if (f268-assertionsDisabled || !(isOpen() || isRegistered())) {
                nd.close(this.fd);
                this.state = 1;
            } else {
        ...
        synchronized (this.lock) {
            try {
                ...
                if (isOpen()) {
                    ...
                    if (f268-assertionsDisabled || IOStatus.check(n)) {
                    } else {
                        throw new AssertionError();
                    }
                }
                this.thread = 0;
                end(f268-assertionsDisabled);
                if (f268-assertionsDisabled || IOStatus.check(0)) {
                } else {
                    throw new AssertionError();
                }
            } finally {
