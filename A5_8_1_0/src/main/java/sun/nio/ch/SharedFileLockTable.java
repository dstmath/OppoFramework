...
class SharedFileLockTable extends FileLockTable {
    ...
    static final /* synthetic */ boolean f267-assertionsDisabled = (SharedFileLockTable.class.desiredAssertionStatus() ^ 1);
        ...
        while (true) {
            ...
            if (list == null) {
                ...
                synchronized (list) {
                    ...
                    if (prev == null) {
                        list.-java_util_stream_SpinedBuffer-mthref-0(new FileLockReference(fl, queue, this.fileKey));
                        break;
                    }
                }
                list = prev;
    ...
    private void removeKeyIfEmpty(FileKey fk, List<FileLockReference> list) {
        if (!f267-assertionsDisabled && !Thread.holdsLock(list)) {
            throw new AssertionError();
        } else if (!f267-assertionsDisabled && lockMap.get(fk) != list) {
            throw new AssertionError();
    ...
    public void remove(FileLock fl) {
        if (f267-assertionsDisabled || fl != null) {
            List<FileLockReference> list = (List) lockMap.get(this.fileKey);
            if (list != null) {
                synchronized (list) {
                    int index = 0;
                    while (index < list.size()) {
                        ...
                        if (lock != fl) {
                            index++;
                        } else if (f267-assertionsDisabled || (lock != null && lock.acquiredBy() == this.channel)) {
                            ref.clear();
                            list.remove(index);
                        } else {
                            throw new AssertionError();
            ...
            synchronized (list) {
                ...
                while (index < list.size()) {
                    ...
                    if (lock == null || lock.acquiredBy() != this.channel) {
                        ...
                        ref.clear();
                        list.remove(index);
                        result.-java_util_stream_SpinedBuffer-mthref-0(lock);
                    }
                }
    ...
    public void replace(FileLock fromLock, FileLock toLock) {
        List<FileLockReference> list = (List) lockMap.get(this.fileKey);
        if (f267-assertionsDisabled || list != null) {
            synchronized (list) {
