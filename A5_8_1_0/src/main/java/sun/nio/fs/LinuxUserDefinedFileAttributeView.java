...
class LinuxUserDefinedFileAttributeView extends AbstractUserDefinedFileAttributeView {
    ...
    static final /* synthetic */ boolean f282-assertionsDisabled = (LinuxUserDefinedFileAttributeView.class.desiredAssertionStatus() ^ 1);
        ...
        for (int pos = 0; pos < size; pos++) {
            if (unsafe.getByte(((long) pos) + address) == (byte) 0) {
                ...
                if (s.startsWith(USER_NAMESPACE)) {
                    list.-java_util_stream_SpinedBuffer-mthref-0(s.substring(USER_NAMESPACE.length()));
                }
                start = pos + 1;
            }
        }
    ...
    public List<String> list() throws IOException {
        if (System.getSecurityManager() != null) {
            checkAccess(this.file.getPathForPermissionCheck(), true, f282-assertionsDisabled);
        }
    ...
    public int read(String name, ByteBuffer dst) throws IOException {
        ...
        int lim = dst.limit();
        if (f282-assertionsDisabled || pos <= lim) {
            NativeBuffer nb;
            long address;
    ...
    public int write(String name, ByteBuffer src) throws IOException {
        if (System.getSecurityManager() != null) {
            checkAccess(this.file.getPathForPermissionCheck(), f282-assertionsDisabled, true);
        }
