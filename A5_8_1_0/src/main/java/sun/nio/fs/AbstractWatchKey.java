...
abstract class AbstractWatchKey implements WatchKey {
    ...
    static final /* synthetic */ boolean f271-assertionsDisabled = (AbstractWatchKey.class.desiredAssertionStatus() ^ 1);
    ...
    final void signalEvent(Kind<?> kind, Object context) {
        boolean isModify = kind == StandardWatchEventKinds.ENTRY_MODIFY ? true : f271-assertionsDisabled;
        synchronized (this) {
            ...
            if (size > 0) {
                ...
                if (!this.lastModifyEvents.isEmpty()) {
                    if (isModify) {
                        ...
                        if (ev != null) {
                            if (f271-assertionsDisabled || ev.kind() == StandardWatchEventKinds.ENTRY_MODIFY) {
                                ((Event) ev).increment();
                                return;
                            }
                ...
                if (size >= 512) {
                    ...
                    isModify = f271-assertionsDisabled;
                    context = null;
                }
            }
            ...
            this.events.-java_util_stream_SpinedBuffer-mthref-0(ev2);
            signal();
        }
    }
