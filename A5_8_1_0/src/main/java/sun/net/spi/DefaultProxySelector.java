...
public class DefaultProxySelector extends ProxySelector {
    ...
    public List<Proxy> select(URI uri) {
        ...
        proxyl.-java_util_stream_SpinedBuffer-mthref-0((Proxy) AccessController.doPrivileged(new PrivilegedAction<Proxy>() {
            public Proxy run() {
                String phost = null;
                int i = 0;
    ...
    static Pattern toPattern(String mask) {
        ...
        for (String disjunct : mask.split("\\|")) {
            if (!disjunct.isEmpty()) {
                ...
                joiner.-java_util_stream_Collectors-mthref-8(disjunctToRegex(disjunct.toLowerCase()));
            }
        }
        return disjunctionEmpty ? null : Pattern.compile(joiner.-java_util_stream_Collectors-mthref-10());
    }
