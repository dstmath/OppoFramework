...
public class LanguageTag {
    ...
    private boolean parseExtlangs(StringTokenIterator itr, ParseStatus sts) {
        ...
        while (!itr.isDone()) {
            ...
            this.extlangs.-java_util_stream_SpinedBuffer-mthref-0(s);
            sts.parseLength = itr.currentEnd();
            itr.next();
    ...
    private boolean parseVariants(StringTokenIterator itr, ParseStatus sts) {
        ...
        while (!itr.isDone()) {
            ...
            this.variants.-java_util_stream_SpinedBuffer-mthref-0(s);
            sts.parseLength = itr.currentEnd();
            itr.next();
        }
        return found;
    ...
    private boolean parseExtensions(StringTokenIterator itr, ParseStatus sts) {
        ...
        while (!itr.isDone()) {
            ...
            this.extensions.-java_util_stream_SpinedBuffer-mthref-0(sb.-java_util_stream_Collectors-mthref-7());
            found = true;
        }
        return found;
    }
    ...
    private boolean parsePrivateuse(StringTokenIterator itr, ParseStatus sts) {
        ...
        if (isPrivateusePrefix(s)) {
            ...
            if (sts.parseLength <= start) {
                ...
                this.privateuse = sb.-java_util_stream_Collectors-mthref-7();
                found = true;
            }
        }
        return found;
    ...
    public static LanguageTag parseLocale(BaseLocale baseLocale, LocaleExtensions localeExtensions) {
        ...
        if (variant.length() > 0) {
            ...
            while (!stringTokenIterator.isDone()) {
                ...
                variants.-java_util_stream_SpinedBuffer-mthref-0(var);
            ...
            if (!stringTokenIterator.isDone()) {
                ...
                if (buf.length() > 0) {
                    privuseVar = buf.-java_util_stream_Collectors-mthref-7();
                }
            }
        }
        List list = null;
        ...
        if (localeExtensions != null) {
            for (Character locextKey : localeExtensions.getKeys()) {
                ...
                if (isPrivateusePrefixChar(locextKey.charValue())) {
                    ...
                    list.-java_util_stream_SpinedBuffer-mthref-0(locextKey.toString() + SEP + ext.getValue());
                }
            }
        }
        if (list != null) {
