...
public final class InternalLocaleBuilder {
    ...
    static final /* synthetic */ boolean f157-assertionsDisabled = (InternalLocaleBuilder.class.desiredAssertionStatus() ^ 1);
    ...
    public InternalLocaleBuilder setExtensions(String subtags) throws LocaleSyntaxException {
        ...
        while (!itr.isDone()) {
            ...
            extensions.-java_util_stream_SpinedBuffer-mthref-0(sb.-java_util_stream_Collectors-mthref-7());
        }
        if (!itr.isDone()) {
            s = itr.current();
    ...
    public InternalLocaleBuilder setLocale(BaseLocale base, LocaleExtensions localeExtensions) throws LocaleSyntaxException {
        ...
        if (language.equals("ja") && region.equals("JP") && variant.equals("JP")) {
            if (f157-assertionsDisabled || "japanese".equals(localeExtensions.getUnicodeLocaleType("ca"))) {
                variant = "";
            } else {
                throw new AssertionError();
            }
        } else if (language.equals("th") && region.equals("TH") && variant.equals("TH")) {
            if (f157-assertionsDisabled || "thai".equals(localeExtensions.getUnicodeLocaleType("nu"))) {
                variant = "";
            } else {
                throw new AssertionError();
            }
    ...
    static String removePrivateuseVariant(String privuseVal) {
        ...
        if (f157-assertionsDisabled || prefixStart == 0 || prefixStart > 1) {
    ...
    private void setUnicodeLocaleExtension(String subtags) {
        ...
        while (!itr.isDone()) {
            ...
            if (key != null) {
                ...
                } else if (f157-assertionsDisabled || typeStart == -1 || typeEnd != -1) {
            ...
            if (itr.hasNext()) {
                ...
                if (f157-assertionsDisabled || typeStart == -1 || typeEnd != -1) {
                    type = typeStart == -1 ? "" : subtags.substring(typeStart, typeEnd);
                    if (this.ukeywords == null) {
                        this.ukeywords = new HashMap(4);
                    }
