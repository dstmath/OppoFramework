...
public enum Wrapper {
    ...
    private static abstract class Format {
        ...
        static {
            f41-assertionsDisabled = Format.class.desiredAssertionStatus() ^ 1;
        }
        ...
        static int format(int kind, int size, int slots) {
            Object obj = 1;
            if (!f41-assertionsDisabled && ((kind >> 12) << 12) != kind) {
                throw new AssertionError();
            } else if (f41-assertionsDisabled || ((size - 1) & size) == 0) {
    ...
    private static boolean checkConvertibleFrom() {
        ...
        while (i < length) {
            Wrapper w = values[i];
            if (!f40-assertionsDisabled && !w.isConvertibleFrom(w)) {
                throw new AssertionError();
            } else if (f40-assertionsDisabled || VOID.isConvertibleFrom(w)) {
                if (w != VOID) {
                    if (!f40-assertionsDisabled && !OBJECT.isConvertibleFrom(w)) {
                        throw new AssertionError();
                    } else if (!f40-assertionsDisabled && w.isConvertibleFrom(VOID)) {
                        throw new AssertionError();
                    }
                }
                if (w != CHAR) {
                    if (!f40-assertionsDisabled && CHAR.isConvertibleFrom(w)) {
                        throw new AssertionError();
                    } else if (!(w.isConvertibleFrom(INT) || f40-assertionsDisabled || !w.isConvertibleFrom(CHAR))) {
                        throw new AssertionError();
                    }
                }
                if (w != BOOLEAN) {
                    if (!f40-assertionsDisabled && BOOLEAN.isConvertibleFrom(w)) {
                        throw new AssertionError();
                    } else if (!(w == VOID || w == OBJECT || f40-assertionsDisabled || !w.isConvertibleFrom(BOOLEAN))) {
                        throw new AssertionError();
                    }
                }
                if (w.isSigned()) {
                    for (Wrapper x : values()) {
                        if (w != x) {
                            if (x.isFloating()) {
                                if (!f40-assertionsDisabled && w.isConvertibleFrom(x)) {
                                    throw new AssertionError();
                                }
                            } else if (!x.isSigned()) {
                                continue;
                            ...
                            } else if (!(f40-assertionsDisabled || w.isConvertibleFrom(x))) {
                                throw new AssertionError();
                            }
                        }
                    }
                ...
                if (w.isFloating()) {
                    for (Wrapper x2 : values()) {
                        if (w != x2) {
                            if (x2.isSigned()) {
                                if (!(f40-assertionsDisabled || w.isConvertibleFrom(x2))) {
                                    throw new AssertionError();
                                }
                            ...
                            } else if (!(f40-assertionsDisabled || w.isConvertibleFrom(x2))) {
                                throw new AssertionError();
                            }
                        }
                    }
    ...
    private static int hashWrap(Class<?> x) {
        String xn = x.getName();
        if (!f40-assertionsDisabled && 10 != "java.lang.".length()) {
            throw new AssertionError();
    ...
    private <T> T convert(Object x, Class<T> type, boolean isCast) {
        ...
        if (this != OBJECT) {
            ...
            return result;
        } else if (f40-assertionsDisabled || !type.isPrimitive()) {
            if (!type.isInterface()) {
                type.cast(x);
            }
            result = x;
    ...
    public void copyArrayBoxing(Object a, int apos, Object[] values, int vpos, int length) {
        ...
        while (i < length) {
            ...
            if (f40-assertionsDisabled || value.getClass() == this.wrapperType) {
                values[i + vpos] = value;
                i++;
            } else {
                throw new AssertionError();
