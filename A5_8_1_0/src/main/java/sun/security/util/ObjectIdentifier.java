...
public final class ObjectIdentifier implements Serializable {
    ...
    static final /* synthetic */ boolean f79-assertionsDisabled = (ObjectIdentifier.class.desiredAssertionStatus() ^ 1);
    private static final long serialVersionUID = 8697030238860181294L;
    private int componentLen = -1;
    private Object components = null;
    private transient boolean componentsCalculated = f79-assertionsDisabled;
    ...
    private static byte[] pack(byte[] in, int ioffset, int ilength, int iw, int ow) {
        if (!f79-assertionsDisabled && (iw <= 0 || iw > 8)) {
            throw new AssertionError((Object) "input NUB must be between 1 and 8");
        } else if (!f79-assertionsDisabled && (ow <= 0 || ow > 8)) {
