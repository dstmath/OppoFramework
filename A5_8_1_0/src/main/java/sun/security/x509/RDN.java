...
public class RDN {
    ...
    public RDN(String name, Map<String, String> keywordMap) throws IOException {
        ...
        while (nextPlus >= 0) {
            ...
            if (!(nextPlus <= 0 || name.charAt(nextPlus - 1) == '\\' || quoteCount == 1)) {
                ...
                avaVec.-java_util_stream_SpinedBuffer-mthref-0(new AVA(new StringReader(avaString), (Map) keywordMap));
        ...
        if (format.equalsIgnoreCase(X500Principal.RFC2253)) {
            ...
            while (nextPlus >= 0) {
                if (nextPlus > 0 && name.charAt(nextPlus - 1) != '\\') {
                    ...
                    avaVec.-java_util_stream_SpinedBuffer-mthref-0(new AVA(new StringReader(avaString), 3, keywordMap));
                    avaOffset = nextPlus + 1;
                }
    ...
    private String toRFC2253StringInternal(boolean canonical, Map<String, String> oidMap) {
        ...
        for (AVA ava : toOutput) {
            ...
            sj.-java_util_stream_Collectors-mthref-8(toRFC2253CanonicalString2);
        }
        return sj.-java_util_stream_Collectors-mthref-10();
    }
}
