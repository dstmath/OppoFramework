...
public class X509CertImpl extends X509Certificate implements DerEncoder {
    ...
    public synchronized void verify(PublicKey key, String sigProvider) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, NoSuchProviderException, SignatureException {
        ...
        if (this.verifiedPublicKey != null && this.verifiedPublicKey.lambda$-java_util_function_Predicate_4628(key) && sigProvider.equals(this.verifiedProvider)) {
    ...
    public int getVersion() {
        ...
        try {
            return ((Integer) this.info.get("version.number")).lambda$-java_util_stream_IntPipeline_14709() + 1;
        } catch (Exception e) {
            return -1;
        }
    }
    ...
    public int getBasicConstraints() {
        try {
            ...
            if (certExt != null && ((Boolean) certExt.get(BasicConstraintsExtension.IS_CA)).booleanValue()) {
                return ((Integer) certExt.get(BasicConstraintsExtension.PATH_LEN)).lambda$-java_util_stream_IntPipeline_14709();
            }
            return -1;
        } catch (Exception e) {
            return -1;
    ...
    private static Collection<List<?>> makeAltNames(GeneralNames names) {
        ...
        for (GeneralName gname : names.names()) {
            ...
            nameEntry.-java_util_stream_SpinedBuffer-mthref-0(Integer.valueOf(name.getType()));
            switch (name.getType()) {
                case 1:
                    nameEntry.-java_util_stream_SpinedBuffer-mthref-0(((RFC822Name) name).getName());
                    break;
                case 2:
                    nameEntry.-java_util_stream_SpinedBuffer-mthref-0(((DNSName) name).getName());
                    break;
                case 4:
                    nameEntry.-java_util_stream_SpinedBuffer-mthref-0(((X500Name) name).getRFC2253Name());
                    break;
                case 6:
                    nameEntry.-java_util_stream_SpinedBuffer-mthref-0(((URIName) name).getName());
                    break;
                case 7:
                    try {
                        nameEntry.-java_util_stream_SpinedBuffer-mthref-0(((IPAddressName) name).getName());
                        break;
                    } catch (IOException ioe) {
                        throw new RuntimeException("IPAddress cannot be parsed", ioe);
                    }
                case 8:
                    nameEntry.-java_util_stream_SpinedBuffer-mthref-0(((OIDName) name).getOID().toString());
                    break;
                default:
                    DerOutputStream derOut = new DerOutputStream();
                    try {
                        name.encode(derOut);
                        nameEntry.-java_util_stream_SpinedBuffer-mthref-0(derOut.toByteArray());
                        break;
                    } catch (IOException ioe2) {
                        throw new RuntimeException("name cannot be encoded", ioe2);
                    }
            }
            newNames.-java_util_stream_SpinedBuffer-mthref-0(Collections.unmodifiableList(nameEntry));
        }
    ...
    private static Collection<List<?>> cloneAltNames(Collection<List<?>> altNames) {
        ...
        for (Collection nameEntry2 : altNames) {
            ...
            if (nameObject instanceof byte[]) {
                ...
                namesCopy.-java_util_stream_SpinedBuffer-mthref-0(Collections.unmodifiableList(nameEntryCopy));
            } else {
                namesCopy.-java_util_stream_SpinedBuffer-mthref-0(nameEntry2);
            }
        }
    ...
    /* JADX WARNING: Removed duplicated region for block: B:7:0x0025 A:{ExcHandler: java.security.NoSuchAlgorithmException (e java.security.NoSuchAlgorithmException), Splitter: B:1:0x0003} */
