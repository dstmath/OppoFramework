        ...
        while (true) {
            if (nextComma >= 0 || nextSemiColon >= 0) {
                ...
                if (!(rdnEnd < 0 || quoteCount == 1 || (escaped(rdnEnd, searchOffset, input) ^ 1) == 0)) {
                    dnVector.-java_util_stream_SpinedBuffer-mthref-0(new RDN(input.substring(dnOffset, rdnEnd), (Map) keywordMap));
                    dnOffset = rdnEnd + 1;
                    quoteCount = 0;
                }
                ...
                dnVector.-java_util_stream_SpinedBuffer-mthref-0(new RDN(input.substring(dnOffset), (Map) keywordMap));
    ...
    private void parseRFC2253DN(String dnString) throws IOException {
        ...
        while (rdnEnd >= 0) {
            if (rdnEnd > 0 && (escaped(rdnEnd, searchOffset, dnString) ^ 1) != 0) {
                dnVector.-java_util_stream_SpinedBuffer-mthref-0(new RDN(dnString.substring(dnOffset, rdnEnd), X500Principal.RFC2253));
                dnOffset = rdnEnd + 1;
            }
            searchOffset = rdnEnd + 1;
            rdnEnd = dnString.indexOf(44, searchOffset);
        }
        dnVector.-java_util_stream_SpinedBuffer-mthref-0(new RDN(dnString.substring(dnOffset), X500Principal.RFC2253));
        Collections.reverse(dnVector);
        this.names = (RDN[]) dnVector.toArray(new RDN[dnVector.size()]);
    }
