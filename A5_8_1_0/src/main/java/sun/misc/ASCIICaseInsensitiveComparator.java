...
public class ASCIICaseInsensitiveComparator implements Comparator<String> {
    ...
    static final /* synthetic */ boolean f60-assertionsDisabled = (ASCIICaseInsensitiveComparator.class.desiredAssertionStatus() ^ 1);
    ...
    public int compare(String s1, String s2) {
        ...
        while (i < minLen) {
            char c1 = s1.charAt(i);
            char c2 = s2.charAt(i);
            if (f60-assertionsDisabled || (c1 <= 127 && c2 <= 127)) {
                if (c1 != c2) {
                    c1 = (char) toLower(c1);
                    c2 = (char) toLower(c2);
                    if (c1 != c2) {
