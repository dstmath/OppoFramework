...
class FmRxControls {
    ...
    public void muteControl(int fd, boolean on) {
        int err;
        if (on) {
            err = FmReceiverJNI.setControlNative(fd, V4L2_CID_AUDIO_MUTE, 3);
        } else {
            err = FmReceiverJNI.setControlNative(fd, V4L2_CID_AUDIO_MUTE, 0);
        }
    }
    ...
    public int[] stationList(int fd) {
        ...
        for (int i = 0; i < station_num; i++) {
            ...
            if (real_freq < 1000.0f * lowBand || real_freq > 1000.0f * highBand) {
    ...
    public int updateSpurTable(int fd, int freq, int rmssi, boolean enable) {
        int re = FmReceiverJNI.setControlNative(fd, V4L2_CID_PRIVATE_SPUR_FREQ, freq);
        if (re < 0) {
    ...
    public boolean enableSoftMute(int fd, int enable) {
        ...
        if (FmReceiverJNI.enableSoftMute(fd, enable) == 0) {
            return true;
        }
        return false;
    }
