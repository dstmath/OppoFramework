...
public class FmRxRdsData {
    ...
    public int rdsGrpOptions(int grpMask, int buffSize, boolean rdsFilter) {
        int rdsFilt;
        byte rds_group_mask = (byte) (((byte) FmReceiverJNI.getControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC)) & 254);
        if (rdsFilter) {
            rdsFilt = 1;
        } else {
            rdsFilt = 0;
        ...
        re = FmReceiverJNI.setControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSD_BUF, buffSize);
        if (re != 0) {
            return re;
        }
    ...
    public int enableAFjump(boolean AFenable) {
        ...
        Log.d(LOGTAG, "Currently set rds_group_mask : " + FmReceiverJNI.getControlNative(this.mFd, V4L2_CID_PRIVATE_TAVARUA_RDSGROUP_PROC));
        if (AFenable) {
