...
public abstract class CollationIterator {
    ...
    static final /* synthetic */ boolean f78-assertionsDisabled = (CollationIterator.class.desiredAssertionStatus() ^ 1);
    ...
    private static final class SkippedState {
        /* renamed from: -assertionsDisabled */
        static final /* synthetic */ boolean f79-assertionsDisabled = (SkippedState.class.desiredAssertionStatus() ^ 1);
        ...
        void incBeyond() {
            if (f79-assertionsDisabled || !hasNext()) {
                this.pos++;
                return;
            }
    ...
    public final long nextCE() {
        ...
        } else if (f78-assertionsDisabled || this.cesIndex == this.ceBuffer.length) {
    ...
    final void setCurrentCE(long ce) {
        if (f78-assertionsDisabled || this.cesIndex > 0) {
        ...
        while (Collation.isSpecialCE32(ce32)) {
            ...
            switch (Collation.tagFromCE32(ce32)) {
                ...
                case 12:
                    ...
                    return;
                case 13:
                    if (!f78-assertionsDisabled && !forward) {
                        throw new AssertionError();
                    } else if (f78-assertionsDisabled || isLeadSurrogate(c)) {
    ...
    private final int nextCE32FromDiscontiguousContraction(CollationData d, CharsTrie suffixes, int ce32, int lookAhead, int c) {
        ...
        if (f78-assertionsDisabled || fcd16 > 255) {
            int nextCp = nextSkippedCodePoint();
            if (nextCp < 0) {
                backwardNumSkipped(1);
                return ce32;
    ...
    private final long previousCEUnsafe(int c, UVector32 offsets) {
        ...
        this.cesIndex = 0;
        if (f78-assertionsDisabled || this.ceBuffer.length == 0) {
            int offset = getOffset();
            while (this.numCpFwd > 0) {
                this.numCpFwd--;
                nextCE();
                if (f78-assertionsDisabled || this.ceBuffer.get(this.ceBuffer.length - 1) != Collation.NO_CE) {
                    this.cesIndex = this.ceBuffer.length;
                    if (f78-assertionsDisabled || offsets.size() < this.ceBuffer.length) {
    ...
    private final void appendNumericSegmentCEs(CharSequence digits) {
        ...
        if (!f78-assertionsDisabled && (1 > length || length > 254)) {
            throw new AssertionError();
        } else if (f78-assertionsDisabled || length == 1 || digits.charAt(0) != 0) {
            ...
            if (f78-assertionsDisabled || length >= 7) {
                int pair;
                int pos;
