...
public class Keyboard {
    ...
    private static final int MODE_HOVER_NAVIGATIONBAR_SHOW_SUSPENDED = 0;
            ...
            try {
                ...
                if (event == 1) {
                    ...
                    if (TAG_ROW.equals(tag)) {
                        inRow = true;
                        x = 0;
                        if (this.mSecurityType == 1 && this.mContext.getPackageManager().hasSystemFeature("com.oppo.feature.screen.heteromorphism")) {
                            x = handleLandHookface(context, 0);
                        }
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "Suying.You@ROM.SDK, 2018-01-16 : Add for secure keyboard", property = OppoRomType.ROM)
    public Keyboard(Context context, int xmlLayoutResId, int modeId, int securityType) {
        ...
        if (securityType == 2) {
            if (this.mDisplayWidth > this.mDisplayHeight) {
                Log.d("Keyboard", "displayWidth is bigger than displayHeight");
                int value = this.mDisplayWidth;
                this.mDisplayWidth = this.mDisplayHeight;
                this.mDisplayHeight = value;
            }
            Log.d("Keyboard", "defaultDisplay : " + this.mDisplayWidth + ", " + this.mDisplayHeight);
        }
        if (securityType == 1 && context.getPackageManager().hasSystemFeature("com.oppo.feature.screen.heteromorphism")) {
            int orientation = wm.getDefaultDisplay().getOrientation();
            if (context.getResources().getConfiguration().orientation == 2) {
                this.mDisplayWidth -= 160;
            }
        }
        this.mDefaultHorizontalGap = 0;
        this.mDefaultWidth = this.mDisplayWidth / 10;
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "Suying.You@Plf.SDK, 2018-02-06 : Modify for BUG:1270117", property = OppoRomType.ROM)
    private int handleLandHookface(Context context, int x) {
        WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        int orientation = wm.getDefaultDisplay().getOrientation();
        if (context.getResources().getConfiguration().orientation != 2) {
            return x;
