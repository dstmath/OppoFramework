...
public final class SQLiteSession {
    ...
    static final /* synthetic */ boolean f69-assertionsDisabled = (SQLiteSession.class.desiredAssertionStatus() ^ 1);
    ...
    public void endTransaction(CancellationSignal cancellationSignal) {
        throwIfNoTransaction();
        if (f69-assertionsDisabled || this.mConnection != null) {
    ...
    private void acquireConnection(String sql, int connectionFlags, CancellationSignal cancellationSignal) {
        if (this.mConnection == null) {
            if (f69-assertionsDisabled || this.mConnectionUseCount == 0) {
    ...
    private void releaseConnection() {
        if (!f69-assertionsDisabled && this.mConnection == null) {
            throw new AssertionError();
        } else if (f69-assertionsDisabled || this.mConnectionUseCount > 0) {
