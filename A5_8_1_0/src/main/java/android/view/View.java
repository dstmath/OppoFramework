...
import com.color.util.ColorReflectWidget;
...
public class View implements Callback, KeyEvent.Callback, ColorLongshotViewInt, AccessibilityEventSource {
    ...
    private ColorReflectData mReflectData;
    private ColorReflectWidget mReflectWidget;
    ...
    int mViewFlags;
    @OppoHook(level = OppoHookType.NEW_FIELD, note = "JianHui.Yu@ROM.SDK, 2016-12-15 : Add for ViewHooks", property = OppoRomType.ROM)
    ...
    private final class CheckForLongPress implements Runnable {
        ...
        public void run() {
            ...
            } else if (!View.this.getRootView().isLongPressSwipe() && View.this.performLongClick(this.mX, this.mY)) {
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2016-12-15 : Modify for ViewHooks", property = OppoRomType.ROM)
    public View(Context context) {
        ...
        this.mHasFieldText = false;
        this.mReflectData = null;
        this.mReflectWidget = null;
        this.mHasInit = false;
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2016-12-12 : Modify for ScrollBar Effect", property = OppoRomType.ROM)
    ...
    public AccessibilityNodeInfo createAccessibilityNodeInfo() {
        ...
        if (this.mAccessibilityDelegate != null) {
            info = this.mAccessibilityDelegate.createAccessibilityNodeInfo(this);
        } else {
    ...
    /* renamed from: lambda$-android_view_View_425589 */
    static /* synthetic */ boolean m2lambda$-android_view_View_425589(int id, View t) {
    ...
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        ...
        if ((focusableMode & 1) != 1 || (isFocusableInTouchMode() ^ 1) == 0) {
            views.add(this);
        }
    }
    ...
    public boolean isImportantForAccessibility() {
        ...
        if (mode == 1 || isActionableForAccessibility() || hasListenersForAccessibility() || getAccessibilityNodeProvider() != null || getAccessibilityLiveRegion() != 0) {
            z = true;
        } else {
            z = AccessibilityManager.getInstance(this.mContext).isColorDirectEnabled();
        }
        return z;
    }
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2016-12-13 : Modify for ScrollBar Effect", property = OppoRomType.ROM)
                ...
                if (p != null && ai != null && l < r && t < b) {
                    ...
                    if (this.mIgnoreOnDescendantInvalidated && (p instanceof ViewGroup)) {
                        ...
                        if (parentViewGroup != null) {
                            parentViewGroup.mIgnoreOnDescendantInvalidated = true;
                        }
                    }
    ...
    void assignParent(ViewParent parent) {
        if (parent == null && isDebugVersion()) {
            this.mAssignNullStack = "mParent:" + this.mParent + " parent:" + parent + " " + Debug.getCallers(8);
        }
        if (this.mParent == null) {
            this.mParent = parent;
        } else if (parent == null) {
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "QiaoYi.Luo@ROM.SDK, 2017-09-14 : Modify for rtl layout", property = OppoRomType.ROM)
    ...
    public void requestLayout() {
        ...
        this.mPrivateFlags |= Integer.MIN_VALUE;
        if (!(this.mParent == null || (this.mParent.isLayoutRequested() ^ 1) == 0)) {
            try {
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2016-12-13 : Modify for ScrollBar Effect; JianHui.Yu@ROM.SDK, 2017-01-07 : Modify for Longshot", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2017-01-07 : Modify for Longshot", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JianHui.Yu@ROM.SDK, 2016-12-15 : Add for oppo style", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JianHui.Yu@ROM.SDK, 2016-02-04 : Add for Longshot", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JianHui.Yu@ROM.SDK, 2016-03-04 : Add for Longshot", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JianHui.Yu@ROM.SDK, 2016-02-25 : Add for Longshot", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JianHui.Yu@ROM.SDK, 2018-03-01 : Add for Longshot 5.0", property = OppoRomType.ROM)
    ...
    protected boolean hookOverScrollBy(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX, int scrollRangeY, int maxOverScrollX, int maxOverScrollY, boolean isTouchEvent) {
        boolean isLongshotConnected = this.mViewHooks.getLongshotController().isLongshotConnected();
        if (isLongshotConnected) {
            maxOverScrollY = 0;
        }
        boolean result = overScrollBy(deltaX, deltaY, scrollX, scrollY, scrollRangeX, scrollRangeY, maxOverScrollX, maxOverScrollY, isTouchEvent);
        if (!isLongshotConnected) {
            return result;
        }
        return this.mViewHooks.getLongshotController().overScrollBy(deltaX, deltaY, scrollX, scrollY, scrollRangeX, scrollRangeY, maxOverScrollX, maxOverScrollY, isTouchEvent, this.mScrollY, result);
    }
    ...
    private void initReflectData() {
        this.mHasInit = true;
        this.mReflectData = ColorReflectDataUtils.getInstance().getInitData(this.mContext);
        if (this.mReflectData != null && this.mReflectData.isReflectEnable()) {
            String name = getClass().getName();
            this.mReflectWidget = this.mReflectData.findWidget(this.mContext, this.mContext.getPackageName(), getClass().getName());
            if (this.mReflectWidget != null) {
                this.mHasFieldText = true;
                try {
                    int level = this.mReflectWidget.getFieldLevel();
                    String fieldName = this.mReflectWidget.getField();
                    if (level == 0) {
                        this.mFieldText = getClass().getDeclaredField(fieldName);
                    } else if (level == 1) {
                        this.mFieldText = getClass().getSuperclass().getDeclaredField(fieldName);
                    } else if (level == 2) {
                        this.mFieldText = getClass().getSuperclass().getSuperclass().getDeclaredField(fieldName);
                    }
                    if (this.mFieldText != null) {
                        this.mFieldText.setAccessible(true);
                        return;
                    } else {
                        this.mHasFieldText = false;
                        return;
