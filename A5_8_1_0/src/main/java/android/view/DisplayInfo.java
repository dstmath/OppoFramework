...
public final class DisplayInfo implements Parcelable {
    ...
    private void getMetricsWithSize(DisplayMetrics outMetrics, CompatibilityInfo compatInfo, Configuration configuration, int width, int height) {
        ...
        int compatHeight = ((this.logicalHeight < this.logicalWidth ? this.logicalHeight : this.logicalWidth) * 16) / 9;
        if (currentPackageName == null || !ColorDisplayCompatUtils.getInstance().shouldCompatAdjustForPkg(currentPackageName)) {
            ...
            if (ColorDisplayCompatUtils.getInstance().hasHeteromorphismFeature()) {
                width = PreciseDisconnectCause.SIP_SERVER_ERROR;
                outMetrics.widthPixels = PreciseDisconnectCause.SIP_SERVER_ERROR;
                outMetrics.noncompatWidthPixels = PreciseDisconnectCause.SIP_SERVER_ERROR;
                outMetrics.heightPixels = height;
                outMetrics.noncompatHeightPixels = height;
            } else {
                outMetrics.widthPixels = compatHeight;
                outMetrics.noncompatWidthPixels = compatHeight;
                outMetrics.heightPixels = height;
                outMetrics.noncompatHeightPixels = height;
            }
        } else if (ColorDisplayCompatUtils.getInstance().hasHeteromorphismFeature()) {
            outMetrics.widthPixels = width;
            outMetrics.noncompatWidthPixels = width;
            height = PreciseDisconnectCause.SIP_SERVER_ERROR;
            outMetrics.heightPixels = PreciseDisconnectCause.SIP_SERVER_ERROR;
            outMetrics.noncompatHeightPixels = PreciseDisconnectCause.SIP_SERVER_ERROR;
        } else {
            outMetrics.widthPixels = width;
            outMetrics.noncompatWidthPixels = width;
            outMetrics.heightPixels = compatHeight;
            outMetrics.noncompatHeightPixels = compatHeight;
        }
        if (ColorDisplayCompatUtils.getInstance().hasHeteromorphismFeature() && currentPackageName != null && ColorDisplayCompatUtils.getInstance().shouldNonImmersiveAdjustForPkg(currentPackageName)) {
            if (width > height) {
                width = outMetrics.widthPixels - 53;
                outMetrics.widthPixels = width;
                outMetrics.noncompatWidthPixels = width;
            } else {
                height = outMetrics.heightPixels - 53;
                outMetrics.heightPixels = height;
                outMetrics.noncompatHeightPixels = height;
            }
        }
