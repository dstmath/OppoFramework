...
public class SurfaceControl {
    ...
    public void destroy() {
        if (DEBUG_SFC) {
            Slog.v(TAG, "SurfaceControl destroy: " + System.identityHashCode(this) + " this " + this);
        }
        if (this.mNativeObject != 0) {
    ...
    public void detachChildren() {
        if (this.mName != null && (this.mName.contains("com.oppo.camera") || this.mName.contains("com.coloros.compass") || this.mName.contains("com.coloros.speechassist"))) {
            return;
        }
        if (this.mName != null && this.mName.contains("com.coloros.video")) {
            return;
        }
        if (this.mName == null || !this.mName.contains("com.android.wallpaper.livepicker")) {
    ...
    public void hide() {
        if (DEBUG_SFC) {
            Slog.v(TAG, "SurfaceControl hide: " + System.identityHashCode(this) + " this " + this);
        }
        checkNotReleased();
    ...
    public void show() {
        if (DEBUG_SFC) {
            Slog.v(TAG, "SurfaceControl show : " + System.identityHashCode(this) + " this " + this);
        }
        checkNotReleased();
    ...
    public static GraphicBuffer screenshotToBuffer(Rect sourceCrop, int width, int height, int minLayer, int maxLayer, boolean useIdentityTransform, int rotation) {
        IBinder displayToken = getBuiltInDisplay(0);
        if ((rotation == 0 || rotation == 2) && OppoScreenDragUtil.isOffsetState()) {
            float scale = OppoScreenDragUtil.getScale();
            int sourceBottom = sourceCrop.bottom;
            int sourceRight = sourceCrop.right;
            int sourceLeft = sourceCrop.left;
            int sourceTop = sourceCrop.top + ((int) (((float) sourceCrop.height()) * (1.0f - scale)));
            if (OppoScreenDragUtil.getOffsetX() > 0) {
                sourceLeft += (int) (((float) sourceCrop.width()) * (1.0f - scale));
            } else {
                sourceRight -= (int) (((float) sourceCrop.width()) * (1.0f - scale));
            }
            sourceCrop.set(sourceLeft, sourceTop, sourceRight, sourceBottom);
        }
