...
public final class ViewRootImpl implements ViewParent, Callbacks, DrawCallbacks {
    ...
    private static final String FULLSCREENTASK = "com.oppo.feature.fullscreenmultitasking";
    private static final String HATEEROMORPHISM = "com.oppo.feature.screen.heteromorphism";
    private static final String KEYGUARD_PACKAGENAME = "com.android.keyguard";
    private static final boolean LOCAL_LOGV = false;
    @OppoHook(level = OppoHookType.NEW_FIELD, note = "XiaoKang.Feng@ROM.SDK, 2016-10-28 : Add for Surface detection", property = OppoRomType.ROM)
    ...
    private static final String RECENTTS_PACKAGE = "com.coloros.recents";
    private static final String SYSTEMUI_PACKAGENAME = "com.android.systemui";
    ...
    boolean mIsLuckyMoneyView = false;
    boolean mIsPerfLockAcquired;
    private boolean mIsRencent;
    private boolean mIsSystemUI;
    boolean mIsWeixinLauncherUI = false;
    ...
    private boolean mRetryLayout = false;
    ...
    @OppoHook(level = OppoHookType.NEW_FIELD, note = "JianHui.Yu@ROM.SDK, 2017-04-13 : Add for Longshot", property = OppoRomType.ROM)
            ...
            if (ViewRootImpl.this.mView == null || (ViewRootImpl.this.mAdded ^ 1) != 0) {
                ...
                if (ViewRootImpl.isTerminalInputEvent(q.mEvent)) {
                    ...
                    if (ViewRootImpl.DEBUG_PANIC) {
                        Slog.w(ViewRootImpl.this.mTag, "Cancelling event due to no window focus: " + q.mEvent + ", hasFocus:" + ViewRootImpl.this.mAttachInfo.mHasWindowFocus + ", mStopped:" + ViewRootImpl.this.mStopped + ", mPausedForTransition:" + ViewRootImpl.this.mPausedForTransition);
                    }
                    return false;
                }
                if (ViewRootImpl.DEBUG_PANIC) {
                    Slog.w(ViewRootImpl.this.mTag, "Dropping event due to no window focus: " + q.mEvent + ", hasFocus:" + ViewRootImpl.this.mAttachInfo.mHasWindowFocus + ", mStopped:" + ViewRootImpl.this.mStopped + ", mPausedForTransition:" + ViewRootImpl.this.mPausedForTransition);
                }
                return true;
            }
        }
    ...
    final class EarlyPostImeInputStage extends InputStage {
        ...
        protected int onProcess(QueuedInputEvent q) {
            ...
            if (OppoScreenDragUtil.isOffsetState() && q != null && q.mEvent != null && (q.mEvent instanceof MotionEvent)) {
                if (ViewRootImpl.this.needScale()) {
                    OppoScreenDragUtil.setEventLocationScale((MotionEvent) q.mEvent, ViewRootImpl.this.mCompactScale);
                } else {
                    OppoScreenDragUtil.setEventLocation((MotionEvent) q.mEvent);
                }
            }
            if (q != null && q.mEvent != null && (q.mEvent instanceof MotionEvent) && ViewRootImpl.this.swipeFromBottom((MotionEvent) q.mEvent)) {
                if (InputLog.DEBUG) {
                    Slog.v(ViewRootImpl.TAG, "swipe input: " + ViewRootImpl.this.mIgnoring);
                }
                return 1;
            ...
            if (ViewRootImpl.this.mLastWasImTarget && (ViewRootImpl.this.isInLocalFocusMode() ^ 1) != 0) {
                ...
                if (imm != null) {
                    ...
                    if (ViewRootImpl.DEBUG_IMF) {
                        Log.v(ViewRootImpl.this.mTag, "Sending input event to IME: " + event);
                    }
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        private int processKeyEvent(QueuedInputEvent q) {
            KeyEvent event = q.mEvent;
            if (InputLog.DEBUG) {
                InputLog.d(ViewRootImpl.this.mTag, " processKeyEvent, " + event);
            }
            if (ViewRootImpl.this.mView.dispatchKeyEvent(event)) {
                if (InputLog.DEBUG) {
                    InputLog.d(ViewRootImpl.this.mTag, event.getKeyCode() + " was FINISH_HANDLED by  mView ");
                }
                return 1;
            } else if (shouldDropInputEvent(q)) {
                return 2;
        ...
        @OppoHook(level = OppoHookType.CHANGE_CODE, note = "Jun.Zhang@Plf.Framework,2013.02.19:add for three pointers move shot screen; Jianhua.Lin@ROM.SDK, 2017-02-18 : Modify for accidentally touch", property = OppoRomType.ROM)
        private int processPointerEvent(QueuedInputEvent q) {
            int i = 0;
            MotionEvent event = q.mEvent;
            boolean tmpOppoScreenShot = OppoScreenShotUtil.checkPauseDeliverPointer();
            if (!this.startOppoScreenShot && tmpOppoScreenShot) {
                ViewRootImpl.this.mHandler.removeCallbacks(ViewRootImpl.this.mProcessCpuTrackerRunnable);
                ViewRootImpl.this.mHandler.postDelayed(ViewRootImpl.this.mProcessCpuTrackerRunnable, 2000);
            }
            ...
            MotionEvent eventWrap = ColorAccidentallyTouchUtils.getInstance().updatePointerEvent(event, ViewRootImpl.this.mView, ViewRootImpl.this.mLastConfigurationFromResources);
            if (eventWrap == null) {
                return 1;
            }
            event = eventWrap;
            boolean handled = ViewRootImpl.this.mView.dispatchPointerEvent(eventWrap);
            int action = eventWrap.getActionMasked();
            if (action == 2) {
                ViewRootImpl.this.mHaveMoveEvent = true;
            } else if (action == 1) {
                ViewRootImpl.this.mHaveMoveEvent = false;
                ViewRootImpl.this.mIsPerfLockAcquired = false;
            }
    ...
    final class ViewRootHandler extends Handler {
        ...
        /* JADX WARNING: Missing block: B:31:0x00d9, code:
            if (r38.this$0.mDisplay.getDisplayId() == r14.argi3) goto L_0x0007;
     */
        /* Code decompiled incorrectly, please refer to instructions dump. */
        public void handleMessage(Message msg) {
            SomeArgs args;
            ...
            switch (msg.what) {
                ...
                case 6:
                    if (ViewRootImpl.this.mAdded) {
                        ...
                        if (ViewRootImpl.DEBUG_PANIC) {
                            Log.d(ViewRootImpl.TAG, "MSG_WINDOW_FOCUS_CHANGED, hasWindowFocus:" + ViewRootImpl.this.mAttachInfo.mHasWindowFocus);
                        }
                ...
                case 29:
                    ...
                    break;
                case 101:
                    ViewRootImpl.this.mRetryLayout = true;
                    break;
            }
        }
    }
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2016-03-03 : Modify for Longshot", property = OppoRomType.ROM)
    public ViewRootImpl(Context context, Display display) {
        ...
        this.mStatusBarHeight = context.getResources().getInteger(202178561);
        mHeteromorphism = !context.getPackageManager().hasSystemFeature(HATEEROMORPHISM) ? context.getPackageManager().hasSystemFeature(FULLSCREENTASK) : true;
        if (mHeteromorphism) {
            mHeteromorphismHeight = context.getResources().getInteger(202178562);
        }
        this.mIsSystemUI = "com.android.systemui".equals(this.mBasePackageName);
        this.mIsRencent = RECENTTS_PACKAGE.equals(this.mBasePackageName);
    ...
    /* JADX WARNING: Missing block: B:117:0x0533, code:
            return;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
        ...
        if (DEBUG_DISABLEHW) {
            ...
            if (((attrs.flags & 16777216) != 0) && ThreadedRenderer.isAvailable()) {
                ...
                if (fakeHwAccelerated) {
                    ...
                    boolean translucent = attrs.format == -1 ? hasSurfaceInsets : true;
    ...
    void setWindowStopped(boolean stopped) {
        if (DEBUG_PANIC) {
            Log.d(TAG, "setWindowStopped, stopped:" + stopped);
        }
    ...
    private static boolean shouldUseDisplaySize(LayoutParams lp) {
        if (lp.type == LayoutParams.TYPE_STATUS_BAR_PANEL || lp.type == LayoutParams.TYPE_INPUT_METHOD || lp.type == LayoutParams.TYPE_VOLUME_OVERLAY) {
            return true;
        }
        return false;
    }
    ...
    /* JADX WARNING: Missing block: B:405:0x0a78, code:
            if (r57 == false) goto L_0x05c3;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "XiaoKang.Feng@ROM.SDK, 2016-09-20: Add for Surface detection", property = OppoRomType.ROM)
    private void performTraversals() {
        ...
        if (host != null && (this.mAdded ^ 1) == 0) {
            ...
            if (this.mFirst || windowShouldResize || insetsChanged || viewVisibilityChanged || params != null || this.mForceNextWindowRelayout || this.mRetryLayout) {
                    ...
                    if (hadSurface) {
                        ...
                        if (this.mAttachInfo.mThreadedRenderer != null) {
                            synchronized (this.mGraphicLock) {
                                try {
                                    z = this.mAttachInfo.mThreadedRenderer.initialize(this.mSurface);
                ...
                if (threadedRenderer != null && threadedRenderer.isEnabled() && (z || this.mWidth != threadedRenderer.getWidth() || this.mHeight != threadedRenderer.getHeight() || this.mNeedsRendererSetup)) {
            ...
            if (this.mFirst && sAlwaysAssignFocus) {
            ...
            if (changedVisibility || regainedFocus) {
                boolean isToast = this.mWindowAttributes == null ? false : this.mWindowAttributes.type == LayoutParams.TYPE_TOAST;
                if (!isToast) {
            ...
            if (hasWindowFocus && (isInLocalFocusMode() ^ 1) != 0) {
                ...
                if (imTarget != this.mLastWasImTarget) {
                    ...
                    if (imm != null && imTarget) {
                        if (DEBUG_IMF) {
                            Log.d(TAG, "notify imeMgr focus change. imTarget:" + imTarget);
                        }
                        ...
                        Log.d(TAG, "do not notify imeMgr focus change. imTarget:" + imTarget);
                    }
                }
            }
                ...
                if (this.mLayoutRequesters.size() > 0) {
                    ...
                    if (validLayoutRequesters != null) {
                        ...
                        for (int i = 0; i < numValidRequests; i++) {
                            ((View) validLayoutRequesters.get(i)).requestLayout();
                        }
    ...
    private void performDraw() {
        if (this.mAttachInfo.mDisplayState != 1 || (this.mReportNextDraw ^ 1) == 0) {
            if (this.mView == null) {
                return;
            }
        } else if (this.mIsKeyguard && shouldOpenAllframeDraw()) {
    ...
    private void draw(boolean fullRedrawNeeded) {
        ...
        if (surface.isValid()) {
            ...
            if (this.mAttachInfo.mViewScrollChanged) {
                if (this.mHaveMoveEvent && (this.mIsPerfLockAcquired ^ 1) != 0) {
                    this.mIsPerfLockAcquired = true;
                    if (this.mPerf == null) {
                        this.mPerf = new BoostFramework();
                    }
                    if (this.mPerf != null) {
                        this.mPerf.perfHint(BoostFramework.VENDOR_HINT_SCROLL_BOOST, this.mContext.getPackageName(), -1, 4);
                    }
                }
            ...
            if (!dirty.isEmpty() || this.mIsAnimating || accessibilityFocusDirty) {
                if (this.mAttachInfo.mThreadedRenderer != null && this.mAttachInfo.mThreadedRenderer.isEnabled()) {
                    ...
                    synchronized (this.mGraphicLock) {
                        synchronized (this.mDestroyLock) {
                            if (this.mAttachInfo.mThreadedRenderer == null || !this.mAttachInfo.mThreadedRenderer.isEnabled()) {
                                Log.w(TAG, "mThreadedRenderer have been destroy");
                            } else {
    ...
    private boolean getAccessibilityFocusedRect(Rect bounds) {
        ...
        if (!manager.isEnabled() || (manager.isOppoTouchExplorationEnabled() ^ 1) != 0) {
            return false;
        }
    ...
    private void performConfigurationChange(MergedConfiguration mergedConfiguration, boolean force, int newDisplayId) {
        ...
        if (!ci.equals(CompatibilityInfo.DEFAULT_COMPATIBILITY_INFO)) {
            Configuration globalConfig2 = new Configuration(globalConfig);
            ci.applyToConfiguration(this.mNoncompatDensity, globalConfig2);
    ...
    private static boolean isNavigationKey(KeyEvent keyEvent) {
        switch (keyEvent.getKeyCode()) {
            ...
            case 123:
                return true;
            case 66:
                if (!(ActivityThread.currentPackageName() == null || (ActivityThread.currentPackageName().toLowerCase().contains("camera") ^ 1) == 0)) {
                    return true;
                }
        }
        return false;
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "XiaoKang.Feng@ROM.SDK, 2016-10-28 : Add for Surface detection", property = OppoRomType.ROM)
    ...
    public boolean swipeFromBottom(MotionEvent event) {
        switch (event.getAction()) {
            case 0:
                ...
                if (!(this.mCompactScale == 1.0f || this.mCompactScale == -1.0f || !needScale())) {
                    downY = (int) ((this.mCompactScale * ((float) downY)) + 0.5f);
                    downX = (int) ((this.mCompactScale * ((float) downX)) + 0.5f);
                }
                if (ori == 0.0f || ori == EVENT_OTHER) {
                    if (downY < this.mScreenHeight - this.mStatusBarHeight) {
                        ...
                        this.mIgnoring = true;
                        Slog.d(TAG, "swipeFromBottom mIgnoring " + this.mIgnoring + " downY " + downY + " mScreenHeight " + this.mScreenHeight + " mStatusBarHeight " + this.mStatusBarHeight + " globalScale " + this.mCompactScale);
                    }
                } else if (ori == EVENT_90) {
                    if (!mHeteromorphism && downX < this.mScreenHeight - this.mStatusBarHeight) {
                        this.mIgnoring = false;
                    } else if (!mHeteromorphism || downX <= mHeteromorphismHeight || downX >= this.mScreenHeight - this.mStatusBarHeight) {
                        this.mIgnoring = true;
                        Slog.d(TAG, "ViewRoomtImpl swipeRight 90 mIgnoring " + this.mIgnoring + " downX " + downX + " mScreenWidth " + this.mScreenWidth + " mStatusBarHeight " + this.mStatusBarHeight);
                    } else {
                        this.mIgnoring = false;
                    }
                } else if (ori != EVENT_270) {
                    this.mIgnoring = false;
                } else if (!mHeteromorphism && downX > this.mStatusBarHeight) {
                    this.mIgnoring = false;
                } else if (!mHeteromorphism || downX >= this.mScreenHeight - mHeteromorphismHeight || downX <= this.mStatusBarHeight) {
                    this.mIgnoring = true;
                    Slog.d(TAG, "ViewRoomtImpl swipeLeft 270 mIgnoring " + this.mIgnoring + " downX " + downX + " mScreenWidth " + this.mScreenWidth + " mStatusBarHeight " + this.mStatusBarHeight);
                } else {
                    this.mIgnoring = false;
                }
    ...
    private boolean needScale() {
        ...
        if (compatibilityInfo == null || !compatibilityInfo.isScalingRequired() || (compatibilityInfo.supportsScreen() ^ 1) == 0 || this.mNoncompatDensity == this.mDensity) {
            return false;
        }
        return true;
    }
