        ...
        if (this.mAuthority.equals(authority)) {
            ...
            if (DocumentsContract.METHOD_IS_CHILD_DOCUMENT.equals(method)) {
                ...
                if (!SystemProperties.getBoolean("persist.sys.permission.enable", false)) {
                    granted = true;
                } else if (authority == null || !authority.equals(DocumentsContract.EXTERNAL_STORAGE_PROVIDER_AUTHORITY)) {
                    granted = true;
                } else {
                    ...
                    if (mimeType == null || !(mimeType.startsWith("image") || mimeType.startsWith("video"))) {
                        ...
                        try {
                            IPermissionController controller = Stub.asInterface(ServiceManager.getService("permission"));
                            if (controller != null) {
                                granted = controller.checkPermission("android.permission.ACCESS_MEDIA_PROVIDER", Binder.getCallingPid(), Binder.getCallingUid());
                            } else {
                                granted = true;
                            }
                        } catch (Exception e) {
                ...
                if (isTreeUri) {
                    if (!Objects.equals(path.getPath().get(0), parentDocumentId)) {
                        Log.wtf(TAG, "Provider doesn't return path from the tree root. Expected: " + parentDocumentId + " found: " + ((String) path.getPath().get(0)));
                        LinkedList<String> docs = new LinkedList(path.getPath());
                        while (docs.size() > 1 && (Objects.equals(docs.getFirst(), parentDocumentId) ^ 1) != 0) {
                            docs.removeFirst();
                        }
