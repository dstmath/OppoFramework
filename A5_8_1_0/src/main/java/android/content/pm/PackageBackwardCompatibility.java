...
public class PackageBackwardCompatibility {
    ...
    public static void modifySharedLibraries(Package pkg) {
        ...
        if (ArrayUtils.contains(usesLibraries, ANDROID_TEST_RUNNER) && (androidTestMockPresent ^ 1) != 0) {
            usesLibraries.add(ANDROID_TEST_MOCK);
        }
        if (ArrayUtils.contains(usesOptionalLibraries, ANDROID_TEST_RUNNER) && (androidTestMockPresent ^ 1) != 0) {
