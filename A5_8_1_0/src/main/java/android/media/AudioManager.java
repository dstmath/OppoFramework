...
public class AudioManager {
                            ...
                            case 0:
                                ...
                                if (fri != null) {
                                    ...
                                    if (listener != null) {
                                        ...
                                        Log.d(AudioManager.TAG, "dispatching onAudioFocusChange end ");
                                        return;
                                    }
                                    return;
                                }
    ...
    public AudioManager(Context context) {
        ...
        this.mPreviousPorts = new ArrayList();
        setContext(context);
        this.mUseVolumeKeySounds = getContext().getResources().getBoolean(17957054);
        this.mUseFixedVolume = getContext().getResources().getBoolean(17957051);
    ...
    public int getRingerMode() {
        try {
            int mode = getService().getRingerModeExternal();
            if (mDebugLog) {
                Log.d(TAG, "getRingerMode mode=" + mode);
            }
            return mode;
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
    ...
    public int getStreamVolume(int streamType) {
        ...
        if (streamType == 3) {
            try {
                state = OppoMultimediaManager.getInstance(getContext()).getParameters("get_do_mute_music");
        ...
        String state2 = OppoMultimediaManager.getInstance(getContext()).getParameters("check_daemon_listinfo_byname=oppo-getstreamvolume=" + packageName);
        if ((packageName == null || !packageName.equals("com.android.bluetooth")) && ((state == null || !state.equalsIgnoreCase("true")) && (state2 == null || !state2.equalsIgnoreCase("true") || (isStreamMute(3) ^ 1) == 0))) {
    ...
    public void setStreamVolume(int streamType, int index, int flags) {
        ...
        try {
            ...
            if (streamType == 3) {
                String result = OppoMultimediaManager.getInstance(getContext()).getParameters("check_daemon_listinfo_byname=control=setStreamVolume");
    ...
    public void setSpeakerphoneOn(boolean on) {
        IAudioService service = getService();
        try {
            Log.d(TAG, "setSpeakerphoneOn on=" + on + " from " + getContext().getOpPackageName());
    ...
    public void startBluetoothScoVirtualCall() {
        IAudioService service = getService();
        try {
            Log.d(TAG, "startBluetoothScoVirtualCall from " + getContext().getOpPackageName());
    ...
    public void setBluetoothScoOn(boolean on) {
        IAudioService service = getService();
        try {
            Log.d(TAG, "setBluetoothScoOn on=" + on + " from " + getContext().getOpPackageName());
            String result = OppoMultimediaManager.getInstance(getContext()).getParameters("check_daemon_listinfo_byname=control=setBluetoothScoOn");
            if (result != null && result.equalsIgnoreCase("true")) {
    ...
    public void setMicrophoneMute(boolean on) {
        ...
        try {
            ...
            String result = OppoMultimediaManager.getInstance(getContext()).getParameters("check_daemon_listinfo_byname=control=setMicrophoneMute");
            if (result != null && result.equalsIgnoreCase("true")) {
                String keys = "get_device_change_authority=" + getContext().getOpPackageName();
                String mAuthoriyStr = OppoMultimediaManager.getInstance(getContext()).getParameters(keys);
    ...
    public void setParameters(String keyValuePairs) {
        if (keyValuePairs.startsWith(DEBUG_LOG)) {
            IAudioService service = getService();
            try {
                String[] s = keyValuePairs.split("=");
                if (s != null && s.length == 2) {
                    boolean on = s[1].equals("on");
                    service.setDebugLog(on);
                    mDebugLog = on;
                }
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
    ...
    public int setBluetoothA2dpDeviceConnectionState(BluetoothDevice device, int state, int profile) {
        IAudioService service = getService();
        try {
            Log.d(TAG, "setBluetoothA2dpDeviceConnectionState device=" + device + " state=" + state + " profile=" + profile);
    ...
    public String getProperty(String key) {
        ...
        if (PROPERTY_OUTPUT_SAMPLE_RATE.equals(key)) {
            ...
            if (PROPERTY_SUPPORT_AUDIO_SOURCE_UNPROCESSED.equals(key)) {
                return String.valueOf(getContext().getResources().getBoolean(17957026));
            }
            return null;
        }
    }
