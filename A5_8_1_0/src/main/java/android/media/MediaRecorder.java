...
public class MediaRecorder {
    ...
    public void setProfile(CamcorderProfile profile) {
        ...
        if ((profile.quality < 10002 || profile.quality > 10003) && profile.audioCodec >= 0) {
    ...
    public void prepare() throws IllegalStateException, IOException {
        if ("off".equals(SystemProperties.get("oppo.camera.hypnus.contrl", "on"))) {
        ...
        if (this.mPath != null) {
            file = new RandomAccessFile(this.mPath, "rws");
            try {
            ...
            file = new RandomAccessFile(this.mFile, "rws");
            try {
                _setOutputFile(file.getFD());
            } finally {
                file.close();
