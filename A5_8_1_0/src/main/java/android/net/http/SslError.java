...
public class SslError {
    ...
    static final /* synthetic */ boolean f90-assertionsDisabled = (SslError.class.desiredAssertionStatus() ^ 1);
    ...
    public SslError(int error, SslCertificate certificate, String url) {
        if (!f90-assertionsDisabled && certificate == null) {
            throw new AssertionError();
        } else if (f90-assertionsDisabled || url != null) {
            addError(error);
    ...
    public static SslError SslErrorFromChromiumErrorCode(int error, SslCertificate cert, String url) {
        if (!f90-assertionsDisabled && (error < -299 || error > -200)) {
            throw new AssertionError();
        } else if (error == -200) {
