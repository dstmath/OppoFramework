...
public final class LinkProperties implements Parcelable {
    ...
    public boolean removeDnsServer(InetAddress dnsServer) {
        ...
        boolean ret;
        synchronized (this.mDnses) {
            ret = this.mDnses.remove(dnsServer);
        }
        return ret;
    }
    ...
    public boolean removeRoute(RouteInfo route) {
        ...
        synchronized (this.mRoutes) {
            if (route != null) {
                if (Objects.equals(this.mIfaceName, route.getInterface())) {
                    remove = this.mRoutes.remove(route);
                }
            }
            remove = false;
        }
    ...
    public List<RouteInfo> getRoutes() {
        List<RouteInfo> unmodifiableList;
        synchronized (this.mRoutes) {
            unmodifiableList = Collections.unmodifiableList(this.mRoutes);
        }
        return unmodifiableList;
    }
    ...
    public boolean isIdenticalRoutes(LinkProperties target) {
        ...
        synchronized (this.mRoutes) {
            containsAll = this.mRoutes.size() == targetRoutes.size() ? this.mRoutes.containsAll(targetRoutes) : false;
        }
        return containsAll;
    }
