...
public final class BluetoothLeAdvertiser {
    ...
    public void stopAdvertisingSet(AdvertisingSetCallback callback) {
        try {
            int state = this.mBluetoothAdapter.getLeState();
            Log.d(TAG, "stopAdvertisingSet(): " + BluetoothAdapter.nameForState(state));
            if (this.mBluetoothAdapter == null || !(state == 12 || state == 15 || state == 13 || state == 11)) {
            ...
            Log.e(TAG, "Failed to stop Advertisement:", e2);
        }
    }
