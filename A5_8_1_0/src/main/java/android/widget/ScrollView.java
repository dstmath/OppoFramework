...
public class ScrollView extends FrameLayout {
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "LiangJing.Fu@Plf.SDK, 2013-01-21 : Modify for initialization oppo property", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JinPeng@Plf.SDK : Modify for mScroller of OppoOverScroller", property = OppoRomType.ROM)
    private void initScrollView() {
        this.mScroller = OppoOverScroller.newInstance(getContext(), true);
        setFocusable(true);
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JinPeng@ROM.SDK : Modify for re-calculate the overscroll distance; JianHui.Yu@ROM.SDK, 2018-03-01 : Modify for Longshot 5.0", property = OppoRomType.ROM)
            ...
            case 2:
                ...
                if (activePointerIndex != -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        deltaY = calcRealOverScrollDist(deltaY);
                        if (-wrap2(0, deltaY, 0, this.mScrollY, 0, range, 0, this.mOverscrollDistance, true) && (hasNestedScrollingParent() ^ 1) != 0) {
                            this.mVelocityTracker.clear();
                        }
            ...
            case 8:
                ...
                if (delta != 0) {
                    ...
                    if (newScrollY != oldScrollY) {
                        super.-wrap17(this.mScrollX, newScrollY);
                        return true;
                    }
                }
                break;
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2018-03-01 : Modify for Longshot 5.0", property = OppoRomType.ROM)
    public void computeScroll() {
        if (this.mScroller.computeScrollOffset()) {
            ...
            if (!(oldX == x && oldY == y)) {
                ...
                int overscrollMode = getOverScrollMode();
                boolean canOverscroll = overscrollMode != 0 ? overscrollMode == 1 && range > 0 : true;
                -wrap2(x - oldX, y - oldY, oldX, oldY, 0, range, 0, this.mOverflingDistance, false);
                onScrollChanged(this.mScrollX, this.mScrollY, oldX, oldY);
                if (canOverscroll) {
                    if (y < 0 && oldY >= 0) {
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JinPeng@Plf.SDK : Modify for can scroll for spring effect", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.CHANGE_CODE, note = "LiangJing.Fu@Plf.SDK : Modify for ignore EdgeEffect if is oppoStyle", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JinPeng@Plf.SDK : Add for initialization oppo property", property = OppoRomType.ROM)
    ...
    @OppoHook(level = OppoHookType.NEW_METHOD, note = "JinPeng@Plf.SDK : Add for re-calculate the overscroll distance", property = OppoRomType.ROM)
    private int calcRealOverScrollDist(int dist) {
        if (!isColorStyle() || (this.mScrollY >= 0 && this.mScrollY <= getScrollRange())) {
            return dist;
        }
        int overScrollY = this.mScrollY;
        if (this.mScrollY > getScrollRange()) {
            overScrollY = this.mScrollY - getScrollRange();
        }
        return (int) ((((float) dist) * (1.0f - ((((float) Math.abs(overScrollY)) * 1.0f) / ((float) this.mOverscrollDistance)))) / 3.0f);
    }
}
