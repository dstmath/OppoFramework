...
public class Editor {
    ...
    private class EasyEditPopupWindow extends PinnedPopupWindow implements OnClickListener {
        private static final int POPUP_TEXT_LAYOUT = 17367283;
        ...
        protected void initContentView() {
            ...
            this.mDeleteTextView = (TextView) inflater.inflate(17367283, null);
    ...
    private class SelectionHandleView extends HandleView {
        ...
        @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@Plf.SDK, 2016-12-24 : Modify for the x-position of start and end Selecion", property = OppoRomType.ROM)
        protected int getHotspotX(Drawable drawable, boolean isRtlRun) {
        ...
        /* JADX WARNING: Missing block: B:81:0x0203, code:
            if (android.widget.Editor.-get14(r26.this$0).canScrollHorizontally(r6 ? -1 : 1) == false) goto L_0x0205;
     */
        ...
        protected void updatePosition(float x, float y, boolean fromTouchScreen) {
            ...
            if (isLvlBoundary || ((rtlAtCurrentOffset && (atRtl ^ 1) != 0) || (!rtlAtCurrentOffset && atRtl))) {
                ...
                if (Editor.this.mTextView.getHorizontallyScrolling() && positionNearEdgeOfScrollingView(x, atRtl)) {
                    ...
                    if ((isExpanding && ((isStartHandle() && offset < currentOffset) || (!isStartHandle() && offset > currentOffset))) || (isExpanding ^ 1) != 0) {
                        int nextOffset;
                        this.mTouchWordDelta = 0.0f;
                        if (atRtl == isStartHandle()) {
    ...
    public class SuggestionsPopupWindow extends PinnedPopupWindow implements OnItemClickListener {
        ...
        @OppoHook(level = OppoHookType.CHANGE_CODE, note = "JiFeng.Tan@Plf.SDK : Modify for show as ColorOS style", property = OppoRomType.ROM)
        protected void initContentView() {
            ...
            this.mSuggestionListView.setBackground(new ColorDrawable(0));
    ...
    private class TextActionModeCallback extends Callback2 {
        ...
        private void populateMenuWithItems(Menu menu) {
            if (Editor.this.mTextView.canCut()) {
                menu.add(0, (int) R.id.cut, 4, (int) R.string.cut).setAlphabeticShortcut(StateProperty.TARGET_X).setShowAsAction(2);
            }
            if (Editor.this.mTextView.canCopy()) {
                menu.add(0, (int) R.id.copy, 5, (int) R.string.copy).setAlphabeticShortcut('c').setShowAsAction(2);
            }
            if (Editor.this.mTextView.canPaste()) {
                menu.add(0, (int) R.id.paste, 6, (int) R.string.paste).setAlphabeticShortcut('v').setShowAsAction(2);
            }
            if (Editor.this.mTextView.canShare()) {
                menu.add(0, (int) R.id.shareText, 7, (int) R.string.share).setShowAsAction(1);
            }
            if (Editor.this.mTextView.canRequestAutofill()) {
                String selected = Editor.this.mTextView.getSelectedText();
                if (selected == null || selected.isEmpty()) {
                    menu.add(0, (int) R.id.autofill, 10, (int) R.string.autofill).setShowAsAction(Integer.MIN_VALUE);
                }
            }
            if (Editor.this.mTextView.canPasteAsPlainText()) {
                menu.add(0, (int) R.id.pasteAsPlainText, 11, (int) R.string.paste_as_plain_text).setShowAsAction(1);
            }
            updateSelectAllItem(menu);
            updateReplaceItem(menu);
            updateAssistMenuItem(menu);
            Editor.this.mColorEditorUtils.updateItems(menu);
        }
        ...
        private void updateSelectAllItem(Menu menu) {
            ...
            if (canSelectAll && (selectAllItemExists ^ 1) != 0) {
                menu.add(0, (int) R.id.selectAll, 8, (int) R.string.selectAll).setShowAsAction(1);
            } else if (!canSelectAll && selectAllItemExists) {
                menu.removeItem(R.id.selectAll);
            }
            boolean canSelect = (!Editor.this.mTextView.canSelectText() || (Editor.this.mTextView.hasPasswordTransformationMethod() ^ 1) == 0) ? false : Editor.this.mTextView.getSelectionStart() == Editor.this.mTextView.getSelectionEnd();
            boolean selectItemExists = menu.findItem(201458768) != null;
            if (canSelect && (selectItemExists ^ 1) != 0) {
                menu.add(0, 201458768, 11, 201589766).setShowAsAction(1);
            } else if (!canSelect && selectItemExists) {
                menu.removeItem(201458768);
            }
        }
        ...
        private void updateReplaceItem(Menu menu) {
            ...
            if (canReplace && (replaceItemExists ^ 1) != 0) {
                menu.add(0, (int) R.id.replaceText, 9, (int) R.string.replace).setShowAsAction(1);
        ...
        private void updateAssistMenuItem(Menu menu) {
            ...
            if (canAssist()) {
                menu.add((int) R.id.textAssist, (int) R.id.textAssist, 0, textClassification.getLabel()).setIcon(textClassification.getIcon()).setShowAsAction(2);
        ...
        @OppoHook(level = OppoHookType.NEW_METHOD, note = "Jianhua.Lin@Plf.SDK : Modify for oppo style", property = OppoRomType.ROM)
        public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
            ...
            if (R.id.textAssist != item.getItemId() || textClassification == null) {
                handleItemClicked(item.getItemId());
                return Editor.this.mTextView.onTextContextMenuItem(item.getItemId());
            }
        ...
        @OppoHook(level = OppoHookType.NEW_METHOD, note = "Jianhua.Lin@Plf.SDK : Add for oppo style", property = OppoRomType.ROM)
        private void handleItemClicked(int id) {
            Context context = Editor.this.mTextView.getContext();
            switch (id) {
                case R.id.cut /*16908320*/:
                    Toast.makeText(context, context.getString(201590069), 0).show();
                    return;
                case R.id.copy /*16908321*/:
                    Toast.makeText(context, context.getString(201590068), 0).show();
                    return;
                case 201458768:
                    Editor.this.getSelectionController().setMinMaxOffset(Editor.this.mTextView.getSelectionStart());
    ...
    void onCreateContextMenu(ContextMenu menu) {
        if (!this.mIsBeingLongClicked && !Float.isNaN(this.mContextMenuAnchorX) && !Float.isNaN(this.mContextMenuAnchorY)) {
            ...
            if (offset != -1) {
                ...
                menu.add(0, (int) R.id.undo, 2, (int) R.string.undo).setAlphabeticShortcut(DateFormat.TIME_ZONE).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener).setEnabled(this.mTextView.canUndo());
                menu.add(0, (int) R.id.redo, 3, (int) R.string.redo).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener).setEnabled(this.mTextView.canRedo());
                menu.add(0, (int) R.id.cut, 4, (int) R.string.cut).setAlphabeticShortcut(StateProperty.TARGET_X).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener).setEnabled(this.mTextView.canCut());
                menu.add(0, (int) R.id.copy, 5, (int) R.string.copy).setAlphabeticShortcut('c').setOnMenuItemClickListener(this.mOnContextMenuItemClickListener).setEnabled(this.mTextView.canCopy());
                menu.add(0, (int) R.id.paste, 6, (int) R.string.paste).setAlphabeticShortcut('v').setEnabled(this.mTextView.canPaste()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
                menu.add(0, (int) R.id.pasteAsPlainText, 11, (int) R.string.paste_as_plain_text).setEnabled(this.mTextView.canPasteAsPlainText()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
                menu.add(0, (int) R.id.shareText, 7, (int) R.string.share).setEnabled(this.mTextView.canShare()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
                menu.add(0, (int) R.id.selectAll, 8, (int) R.string.selectAll).setAlphabeticShortcut(DateFormat.AM_PM).setEnabled(this.mTextView.canSelectAllText()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
                menu.add(0, (int) R.id.autofill, 10, (int) R.string.autofill).setEnabled(this.mTextView.canRequestAutofill()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
                this.mPreserveSelection = true;
            }
        }
    }
