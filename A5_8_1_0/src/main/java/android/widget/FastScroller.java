...
class FastScroller {
    ...
    private final Runnable mDeferHide = new Runnable() {
        @OppoHook(level = OppoHookType.CHANGE_CODE, note = "XiaoKang.Feng@Plf.SDK, 2017-04-21 : Modify for color fastscroller", property = OppoRomType.ROM)
        public void run() {
    ...
    public void onItemCountChanged(int childCount, int itemCount) {
        if (this.mOldItemCount != itemCount || (this.mOldChildCount != childCount && (this.mIsOppoStyle ^ 1) != 0)) {
    ...
    private void transitionToHidden() {
        ...
        Animator slideOut = groupAnimatorOfFloat(View.TRANSLATION_X, offset, this.mThumbImage, this.mTrackImage).setDuration(300);
        ...
        if (this.mIsOppoStyle) {
            ...
            this.mDecorAnimation.addListener(new AnimatorListenerAdapter() {
                public void onAnimationEnd(Animator animation) {
                    if (FastScroller.this.mState == 3) {
                        FastScroller.this.mState = 0;
                    }
                }
            });
        } else {
        ...
        switch (ev.getActionMasked()) {
            case 0:
                if (isPointInside(ev.getX(), ev.getY())) {
                    if (this.mList.isInScrollingContainer() || (this.mState == 0 && (this.mIsOppoStyle ^ 1) == 0)) {
    ...
    public boolean onTouchEvent(MotionEvent me) {
        ...
        switch (me.getActionMasked()) {
            case 0:
                if (!(!isPointInside(me.getX(), me.getY()) || this.mList.isInScrollingContainer() || (this.mState == 0 && (this.mIsOppoStyle ^ 1) == 0))) {
                    beginDrag();
                    return true;
                }
            case 1:
