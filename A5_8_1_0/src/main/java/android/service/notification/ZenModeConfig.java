...
public class ZenModeConfig implements Parcelable {
    ...
    public static Uri toEventConditionId(EventInfo event) {
        return new Builder().scheme("condition").authority(SYSTEM_AUTHORITY).appendPath(EVENT_PATH).appendQueryParameter("userId", Long.toString((long) event.userId)).appendQueryParameter("calendar", event.calendar != null ? event.calendar : "").appendQueryParameter("reply", Integer.toString(event.reply)).build();
    }
    ...
    public static EventInfo tryParseEventConditionId(Uri conditionId) {
        boolean isEvent;
        if (conditionId != null && conditionId.getScheme().equals("condition") && conditionId.getAuthority().equals(SYSTEM_AUTHORITY) && conditionId.getPathSegments().size() == 1) {
            isEvent = ((String) conditionId.getPathSegments().get(0)).equals(EVENT_PATH);
        } else {
            isEvent = false;
        }
        if (!isEvent) {
