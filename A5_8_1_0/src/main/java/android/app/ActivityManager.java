...
public class ActivityManager {
    ...
    public static final int START_FORBIDDEN_FOR_FREQUENCY_CONTROL = -80;
    ...
    public static boolean isHighEndGfx() {
        if (WifiEnterpriseConfig.ENGINE_ENABLE.equals(SystemProperties.get("persist.sys.force_sw_gles", "0")) || (isLowRamDeviceStatic() ^ 1) == 0) {
            return false;
        }
    ...
    public static boolean supportsMultiWindow(Context context) {
        ...
        if (!isLowRamDeviceStatic() || isWatch) {
            return Resources.getSystem().getBoolean(17957035);
        }
        return false;
    }
    ...
    public static void dumpPackageStateStatic(FileDescriptor fd, String packageName) {
        ...
        dumpService(pw, fd, TvContract.PARAM_PACKAGE, new String[]{packageName});
        pw.println();
        dumpService(pw, fd, Context.ACTIVITY_SERVICE, new String[]{"-a", TvContract.PARAM_PACKAGE, packageName});
        pw.println();
    ...
    public boolean addDisallowedRunningApp(List<String> appPkgNamesList) {
        if (appPkgNamesList == null || appPkgNamesList.size() <= 0) {
            Log.w(TAG, "addDisallowedRunningApp:appPkgNamesList is empty!");
            return false;
        }
        try {
            return ActivityManagerNative.getDefault().addDisallowedRunningApp(appPkgNamesList);
        } catch (RemoteException e) {
            Log.w(TAG, "addDisallowedRunningApp failed.");
            return false;
        }
    }
    ...
    public boolean removeDisallowedRunningApp(List<String> appPkgNamesList) {
        if (appPkgNamesList == null || appPkgNamesList.size() <= 0) {
            Log.w(TAG, "removeDisallowedRunningApp:appPkgNamesList is empty!");
            return false;
        }
        try {
            return ActivityManagerNative.getDefault().removeDisallowedRunningApp(appPkgNamesList);
        } catch (RemoteException e) {
            Log.w(TAG, "removeDisallowedRunningApp failed.");
            return false;
        }
    }
    ...
    public List<String> getDisallowedRunningApp() {
        try {
            ...
            Log.w(TAG, "getDisallowedRunningApp failed.");
            return null;
        }
    }
    ...
    public void disableSdcard() {
        try {
            ...
            Log.w(TAG, "disableSdcard failed.");
        }
    }
    ...
    public List<String> getAllTopPkgName() {
        try {
            return ActivityManagerNative.getDefault().getAllTopPkgName();
        } catch (RemoteException e) {
            Log.w(TAG, "getAllTopPkgName failed.");
            return null;
        }
    }
    ...
    public ApplicationInfo getFreeFormAppInfo() {
        try {
            return ActivityManagerNative.getDefault().getFreeFormAppInfo();
        } catch (RemoteException e) {
            Log.w(TAG, "getFreeFormAppInfo failed.");
            return null;
        }
    }
}
