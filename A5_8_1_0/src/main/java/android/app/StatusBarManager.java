...
public class StatusBarManager {
    ...
    public static final String PACKAGE_NAME_LOCK_SCREEN = "com.coloros.onekeylockscreen";
    public static final int SHORTCUTS_PANEL_STATE_DISABLE = 1;
    public static final int SHORTCUTS_PANEL_STATE_ENABLE = 0;
    public static final int SHORTCUTS_PANEL_STATE_HIDE = 2;
    public static final int SHORTCUTS_PANEL_STATE_SHOW = 3;
    public static final int STATUS_BAR_FUNCTION_CODE_LANCHER_DRAG = 2;
    ...
    public void setShortcutsPanelState(int state) {
        try {
            IStatusBarService svc = getService();
            if (svc != null) {
                svc.setShortcutsPanelState(state);
            }
        } catch (RemoteException ex) {
    ...
    public boolean setStatusBarFunction(int functionCode) {
        try {
            String pkgName = this.mContext.getPackageName();
            if (1 == functionCode && (pkgName.equals(PACKAGE_NAME_LOCK_SCREEN) ^ 1) != 0) {
                return false;
            }
            IStatusBarService svc = getService();
            if (svc != null) {
                return svc.setStatusBarFunction(functionCode, pkgName);
            }
            return false;
        } catch (RemoteException ex) {
            throw new RuntimeException(ex);
