...
public class Activity extends ContextThemeWrapper implements Factory2, Callback, KeyEvent.Callback, OnCreateContextMenuListener, ComponentCallbacks2, OnWindowDismissedCallback, WindowControllerCallback, AutofillClient {
    ...
    private static boolean DEBUG_BACK_KEY = SystemProperties.getBoolean("persist.sys.assert.panic", false);
    ...
    final String[] mActivitiesToImprove = new String[]{"com.tencent.mm/com.tencent.mm.plugin.gallery.ui.AlbumPreviewUI", "com.tencent.mm/com.tencent.mm.plugin.chatroom.ui.SeeRoomMemberUI", "com.tencent.mm/com.tencent.mm.ui.chatting.gallery.ImageGalleryUI"};
    final ArrayList<String> mActivitiesToImproveList = new ArrayList(Arrays.asList(this.mActivitiesToImprove));
    ...
    private String mDebugStrActName = "";
    private boolean mDebugWeChatUI = SystemProperties.getBoolean("persist.sys.assert.panic", false);
    View mDecor = null;
    ...
    private boolean mIsWeChatUIActivity = false;
    ...
    boolean mResumed;
    private int mSaveInstanceStateReason = 0;
    private SearchEvent mSearchEvent;
    private SearchManager mSearchManager;
    private boolean mShouldBoostAnimation = false;
    private boolean mShouldDelayFinish = false;
    ...
    public void startActivityForResult(Intent intent, int requestCode, Bundle options) {
        if (!(intent == null || intent.getComponent() == null || !this.mActivitiesToImproveList.contains(intent.getComponent().flattenToString()))) {
            ...
            if (mHM != null) {
                mHM.hypnusSetSignatureAction(12, 2000, Hypnus.getLocalSignature());
            }
        }
    ...
    public void startActivity(Intent intent, Bundle options) {
        if (!(!OppoAutoInstallManager.isInnerVersion.booleanValue() || intent == null || intent.getComponent() == null || intent.getComponent().getPackageName() == null)) {
            String pkgName = intent.getComponent().getPackageName();
            if (OppoAutoInstallManager.isNeedLeader(pkgName).booleanValue()) {
                if (!OppoAutoInstallManager.grExists().booleanValue()) {
                    ...
                    if (OppoAutoInstallManager.canShowDialog(appPkgName).booleanValue()) {
                        OppoAutoInstallManager.doGr(str, str2, appPkgName, OppoAutoInstallManager.DO_GR_DOWN_INSTALL);
                        OppoAutoInstallManager.exit(appPkgName);
                    } else {
                        if (OppoAutoInstallManager.DEBUG_GR) {
                    ...
                    return;
                } else if (pkgName != null && pkgName.equals("com.google.android.apps.maps")) {
                    OppoAutoInstallManager.doGr(null, null, null, OppoAutoInstallManager.DO_GR_CHECK_INTERNET);
                }
            }
        }
        if (options != null) {
    ...
    private void finish(final int finishTask) {
        int i = 0;
        if (this.mIsFinishBoost) {
            if (mHM == null) {
                mHM = HypnusManager.getHypnusManager();
            }
            mHM.hypnusSetAction(12, 10);
        }
        if (this.mShouldDelayFinish) {
            this.mShouldDelayFinish = false;
            boolean userBehavior = false;
            StackTraceElement[] sts = Thread.currentThread().getStackTrace();
            int length = sts.length;
            while (i < length) {
                if (sts[i].getClassName().contains("StartupDirector")) {
                    userBehavior = true;
                    break;
                }
                i++;
            ...
            if (!userBehavior) {
                int tempFinishTask = finishTask;
                this.mHandler.postDelayed(new Runnable() {
                    public void run() {
                        Activity.this.finish(finishTask);
                    }
                }, 150);
                return;
            }
        ...
        if (this.mParent == null) {
            ...
            if (DEBUG_BACK_KEY) {
                Log.v(TAG, "Finishing self: token=" + this.mToken + WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER + Debug.getCallers(8));
            }
            if (resultData != null) {
                try {
    ...
    final void attach(Context context, ActivityThread aThread, Instrumentation instr, IBinder token, int ident, Application application, Intent intent, ActivityInfo info, CharSequence title, Activity parent, String id, NonConfigurationInstances lastNonConfigurationInstances, Configuration config, String referrer, IVoiceInteractor voiceInteractor, Window window, ActivityConfigCallback activityConfigCallback) {
        ...
        if (this.mActivityInfo.name != null) {
            String actName = this.mActivityInfo.name;
            if (actName.equals("com.tencent.mm/.ui.LauncherUI") || actName.equals("com.tencent.mm/.plugin.sns.ui.SnsTimeLineUI")) {
                if (this.mDebugWeChatUI) {
                    Log.d(TAG, "attach:is debug target.");
                }
                this.mIsWeChatUIActivity = true;
                this.mDebugStrActName = actName;
                boolean openDebugFlag = isDebugActivityTarget();
                this.mMainThread.mIsDebugTarget = openDebugFlag;
                if (this.mFragments != null) {
                    this.mFragments.setIsDebugTarget(openDebugFlag);
                }
            }
        }
    }

    private boolean isDebugActivityTarget() {
        return this.mDebugWeChatUI ? this.mIsWeChatUIActivity : false;
    }
    ...
    final void performStart() {
        ...
        if (isDebugActivityTarget()) {
            Log.d(TAG, "callActivityOnStart:" + this.mDebugStrActName);
        }
        ...
        if (this.mCalled) {
            ...
            if ((isAppDebuggable && !VMRuntime.getRuntime().getDisableCompatbilityFlag() && DEBUG_BACK_KEY) || isDlwarningEnabled) {
    ...
    final void performResume() {
        ...
        if (this.mCalled) {
            if (isDebugActivityTarget()) {
                Log.d(TAG, "performResume:" + this.mDebugStrActName);
            }
            if (!(this.mVisibleFromClient || (this.mFinished ^ 1) == 0)) {
                Log.w(TAG, "An activity without a UI must call finish() before onResume() completes");
                if (getApplicationInfo().targetSdkVersion > 22) {
                    if (VMRuntime.getRuntime().getDisableCompatbilityFlag() || "true".equals(SystemProperties.get("persist.sys.assert.panic"))) {
                        throw new IllegalStateException("Activity " + this.mComponent.toShortString() + " did not call finish() prior to onResume() completing");
                    }
                    Log.w(TAG, "Finish an activity without a UI before onResume() completing, if it did not do this");
                    finish();
                }
            }
            this.mCalled = false;
    ...
    final void performPause() {
        ...
        if (this.mCalled || getApplicationInfo().targetSdkVersion < 9) {
            this.mResumed = false;
            if (isDebugActivityTarget()) {
                Log.d(TAG, "performPause:" + this.mDebugStrActName);
                return;
            }
            return;
        }
    ...
    final void performStop(boolean preserveWindow) {
        ...
        if (isDebugActivityTarget()) {
            Log.d(TAG, "performStop, mStopped:" + this.mStopped + ", " + this.mDebugStrActName);
        }
        if (!this.mStopped) {
            if (this.mWindow != null) {
