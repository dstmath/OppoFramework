...
@OppoHook(level = OppoHookType.CHANGE_CODE, note = "JianHui.Yu@Plf.SDK, 2015-05-20 : Modify for coloros service", property = OppoRomType.ROM)
final class SystemServiceRegistry {
    ...
    static {
        ...
        registerService(Context.SECRECY_SERVICE, SecrecyManager.class, new CachedServiceFetcher<SecrecyManager>() {
            public SecrecyManager createService(ContextImpl ctx) {
                return new SecrecyManager(ctx.getOuterContext(), ISecrecyService.Stub.asInterface(ServiceManager.getService(Context.SECRECY_SERVICE)));
            }
        });
        registerService(Context.FINGERPRINT_SERVICE, FingerprintManager.class, new CachedServiceFetcher<FingerprintManager>() {
            public FingerprintManager createService(ContextImpl ctx) throws ServiceNotFoundException {
                IBinder binder;
                if (ctx.getApplicationInfo().targetSdkVersion < 26 || (ActivityThread.inCptWhiteList(708, ctx.getApplicationInfo().processName) ^ 1) == 0) {
        ...
        registerService(Context.FACE_SERVICE, FaceManager.class, new CachedServiceFetcher<FaceManager>() {
            public FaceManager createService(ContextImpl ctx) {
                return new FaceManager(ctx.getOuterContext(), IFaceService.Stub.asInterface(ServiceManager.getService(Context.FACE_SERVICE)));
            }
        });
        ...
        SystemServiceRegistry.registerServices();
    }
    ...
    @OppoHook(level = OppoHookType.CHANGE_ACCESS, note = "JianHui.Yu@Plf.SDK, 2016-05-20 : [-private] Modify for coloros service", property = OppoRomType.ROM)
