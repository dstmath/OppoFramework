...
public final class ServerOperation implements Operation, BaseStream {
    ...
    public synchronized boolean continueOperation(boolean sendEmpty, boolean inStream) throws IOException {
        if (V) {
            Log.v(TAG, "continueOperation");
        }
        if (this.mGetOperation) {
            if (V) {
                Log.v(TAG, "Get continueOperation ");
            }
            sendReply(ResponseCodes.OBEX_HTTP_CONTINUE);
            return true;
            ...
            if (sendEmpty) {
                sendReply(ResponseCodes.OBEX_HTTP_CONTINUE);
                if (V) {
                    Log.v(TAG, "continueOperation:ServerSet SRM sendEmpty clause");
                }
                ...
                sendReply(ResponseCodes.OBEX_HTTP_CONTINUE);
                if (V) {
                    Log.v(TAG, "continueOperation: Server setting SRM");
                }
            }
        }
    }
    ...
    public synchronized boolean sendReply(int type) throws IOException {
        ...
        if (V) {
            Log.v(TAG, "mMaxPcKLen :" + this.mMaxPacketLength + " headerArryLen :" + headerArray.length);
        }
