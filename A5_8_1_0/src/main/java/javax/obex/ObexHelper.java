...
public final class ObexHelper {
    public static final int A2DP_SCO_OBEX_MAX_CLIENT_PACKET_SIZE = 8192;
    ...
    public static final boolean VDBG = Log.isLoggable(LOG_TAG, 2);
            ...
            try {
                ...
                switch (headerID & 192) {
                    ...
                    case 64:
                        ...
                        index++;
                        int length = ((headerArray[index] & 255) << 8) + (headerArray[index + 1] & 255);
                        index += 2;
                        if (length >= 3) {
                            length -= 3;
    ...
    public static byte[] createHeader(HeaderSet head, boolean nullOut) {
        ...
        try {
            ...
            if (value != null) {
                if (VDBG) {
                    Log.v(TAG, " Add APP PARAM Header = 76");
                }
                out.write(76);
                length = value.length + 3;
                lengthArray[0] = (byte) ((length >> 8) & 255);
            ...
            for (i = 0; i < 16; i++) {
                ...
                if (stringHeader != null) {
                    out.write(((byte) i) + 48);
                    value2 = convertToUnicodeByteArray(stringHeader);
                    ...
                    out.write(value2);
                    if (VDBG) {
                        Log.v(TAG, " Add Unicode String value = " + value2);
                    }
                    if (nullOut) {
                        head.setHeader(i + 48, null);
                    }
                }
                value2 = (byte[]) head.getHeader(i + 112);
                if (value2 != null) {
                    ...
                    out.write(value2);
                    if (VDBG) {
                        Log.v(TAG, " Add ByteSeq value = " + value2);
                    }
                    if (nullOut) {
            ...
            if (head.mAuthChall != null) {
                ...
                if (VDBG) {
                    Log.v(TAG, " Add mAuthChall value = " + head.mAuthChall);
                }
                if (nullOut) {
            ...
            if (head.mAuthResp != null) {
                ...
                if (VDBG) {
                    Log.v(TAG, " Add mAuthChall value = " + head.mAuthResp);
                }
                if (nullOut) {
                    head.mAuthResp = null;
                }
