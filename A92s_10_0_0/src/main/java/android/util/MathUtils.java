...
public final class MathUtils {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: android.util.MathUtils.constrain(float, float, float):float
     arg types: [float, int, int]
     candidates:
      android.util.MathUtils.constrain(int, int, int):int
      android.util.MathUtils.constrain(long, long, long):long
      android.util.MathUtils.constrain(float, float, float):float */
    ...
    public static void fitRect(Rect outToResize, int largestSide) {
        if (!outToResize.isEmpty()) {
            outToResize.scale(((float) largestSide) / ((float) Math.max(outToResize.width(), outToResize.height())));
        }
    }
}
