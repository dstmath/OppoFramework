...
public class ListView extends AbsListView {
    ...
    /* JADX INFO: Multiple debug info for r1v17 int: [D('focusLayoutRestoreDirectChild' android.view.View), D('firstPosition' int)] */
    /* access modifiers changed from: protected */
                    ...
                    if (this.mItemCount != 0) {
                        ...
                        if (sel != null) {
                            if (!this.mItemsCanFocus || !hasFocus() || sel.hasFocus()) {
                                ...
                                if (!((sel == focusLayoutRestoreDirectChild2 && focusLayoutRestoreView != null && focusLayoutRestoreView.requestFocus()) || sel.requestFocus())) {
    ...
    private View makeAndAddView(int position, int y, boolean flow, int childrenLeft, boolean selected) {
        ...
        if (this.mDataChanged || (activeView = this.mRecycler.getActiveView(position)) == null) {
            if (LMManager.getMode() == 2 && LMManager.MM_PACKAGENAME.equals(getContext().getPackageName()) && position == this.mItemCount - 1 && !LMManager.getLastViewItem()) {
                LMManager.setLastViewItem(true);
            }
            View child = obtainView(position, this.mIsScrap);
            if (position == this.mItemCount - 1 && LMManager.MM_PACKAGENAME.equals(getContext().getPackageName())) {
                LMManager.getLMManager().noteObtainedLastViewItem(child);
            }
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft, boolean selected, boolean isAttachedToWindow) {
        ...
        if ((!isAttachedToWindow || p.forceAdd) && (!p.recycledHeaderFooter || p.viewType != -2)) {
            ...
            addViewInLayout(child, i2, p, true);
    ...
    /* JADX INFO: Multiple debug info for r13v14 int: [D('itemCount' int), D('itemIndex' int)] */
    /* JADX INFO: Multiple debug info for r8v9 int: [D('start' int), D('previousIndex' int)] */
    ...
    /* JADX INFO: Multiple debug info for r9v8 'adapter'  android.widget.ListAdapter: [D('drawDividers' boolean), D('adapter' android.widget.ListAdapter)] */
    /* access modifiers changed from: protected */
                        ...
                        if (!drawDividers2 || bottom2 >= listBottom2) {
                            ...
                            if (adapter2.isEnabled(itemIndex)) {
                                if (headerDividers || (!isHeader && listBottom3 >= headerCount)) {
                                    if (isLastItem || (adapter2.isEnabled(listBottom3) && (footerDividers2 || (!isFooter && listBottom3 < footerLimit)))) {
                                        bounds.top = bottom2;
                    ...
                    if ((headerDividers || !isHeader2) && (footerDividers2 || !isFooter2)) {
                        ...
                        if (drawDividers2) {
                            ...
                            if (top > effectivePaddingTop2) {
                                ...
                                if (!adapter.isEnabled(itemIndex2)) {
                                    footerDividers = footerDividers2;
                                } else if (!headerDividers && (isHeader2 || previousIndex < headerCount)) {
                                    footerDividers = footerDividers2;
                                } else if (isFirstItem || (adapter.isEnabled(previousIndex) && (footerDividers2 || (!isFooter2 && previousIndex < footerLimit)))) {
                                    footerDividers = footerDividers2;
    ...
    /* JADX INFO: Multiple debug info for r2v1 android.widget.ListAdapter: [D('dividerHeight' int), D('adapter' android.widget.ListAdapter)] */
    /* JADX INFO: Multiple debug info for r0v4 int: [D('start' int), D('previousIndex' int)] */
    private boolean shouldAdjustHeightForDivider(int itemIndex) {
        ...
        if (!adapter.isEnabled(itemIndex)) {
            z = true;
        } else if (!headerDividers && (isHeader || previousIndex < headerCount)) {
            z = true;
        } else if (isFirstItem) {
            return true;
        } else {
