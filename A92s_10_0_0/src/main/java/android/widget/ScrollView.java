...
public class ScrollView extends FrameLayout {
                ...
                if (actionMasked == 2) {
                    ...
                    if (activePointerIndex == -1) {
                        ...
                        if (this.mIsBeingDragged) {
                            ...
                            int deltaY2 = this.mColorViewConfigHelper.calcRealOverScrollDist(deltaY, this.mScrollY, getScrollRange());
                            ...
                            if (dispatchNestedScroll(0, scrolledDeltaY, 0, deltaY2 - scrolledDeltaY, this.mScrollOffset)) {
                                ...
                                if (pulledToY < 0) {
                                    this.mEdgeGlowTop.onPull(((float) deltaY2) / ((float) getHeight()), ev.getX(activePointerIndex) / ((float) getWidth()));
                                    if (!this.mEdgeGlowBottom.isFinished()) {
                                        this.mEdgeGlowBottom.onRelease();
                                    }
                                } else if (pulledToY > range) {
                                    this.mEdgeGlowBottom.onPull(((float) deltaY2) / ((float) getHeight()), 1.0f - (ev.getX(activePointerIndex) / ((float) getWidth())));
    ...
    public void draw(Canvas canvas) {
        ...
        int width2;
        super.draw(canvas);
        if (shouldDisplayEdgeEffects() && !isColorStyle()) {
            int scrollY = this.mScrollY;
