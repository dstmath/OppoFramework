...
public class ScrollBarDrawable extends Drawable implements Drawable.Callback {
    ...
    @OppoHook(level = OppoHook.OppoHookType.NEW_FIELD, note = "JianHui.Yu@ROM.SDK, 2016-12-12 : Add for ScrollBar Effect", property = OppoHook.OppoRomType.ROM)
    private final IColorScrollBarEffect mEffect;
    private int mExtent;
    ...
    @OppoHook(level = OppoHook.OppoHookType.CHANGE_CODE, note = "JianHui.Yu@ROM.SDK, 2016-12-13 : Modify for ScrollBar Effect", property = OppoHook.OppoRomType.ROM)
    public void draw(Canvas canvas) {
        ...
        if (!canvas.quickReject((float) r.left, (float) r.top, (float) r.right, (float) r.bottom, Canvas.EdgeType.AA)) {
            ...
            if (drawThumb) {
                ...
                int thumbLength = this.mEffect.getThumbLength(scrollBarLength, vertical ? r.width() : r.height(), extent, range);
    ...
    @OppoHook(level = OppoHook.OppoHookType.NEW_METHOD, note = "JianHui.Yu@ROM.SDK, 2016-12-12 : Add for ScrollBar Effect", property = OppoHook.OppoRomType.ROM)
    public ScrollBarDrawable() {
        this(ColorScrollBarEffect.NO_EFFECT);
    }

    @OppoHook(level = OppoHook.OppoHookType.NEW_METHOD, note = "JianHui.Yu@ROM.SDK, 2016-12-12 : Add for ScrollBar Effect", property = OppoHook.OppoRomType.ROM)
    public ScrollBarDrawable(IColorScrollBarEffect effect) {
        this.mAlpha = 255;
        this.mEffect = effect;
    }
}
