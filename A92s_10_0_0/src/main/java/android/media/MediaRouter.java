...
public class MediaRouter {
    ...
    public static final boolean DEBUG = (Log.isLoggable(TAG, 3) || !"user".equals(Build.TYPE));
    ...
    static class Static implements DisplayManager.DisplayListener {
        ...
        /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
         method: android.media.MediaRouter.RouteCategory.<init>(int, int, boolean):void
         arg types: [?, int, int]
         candidates:
          android.media.MediaRouter.RouteCategory.<init>(java.lang.CharSequence, int, boolean):void
          android.media.MediaRouter.RouteCategory.<init>(int, int, boolean):void */
    ...
    public void addCallback(int types, Callback cb, int flags) {
        if (DEBUG) {
            Log.d(TAG, "addCallback: " + cb);
        }
        synchronized (MediaRouter.class) {
    ...
    public void removeCallback(Callback cb) {
        int index;
        synchronized (MediaRouter.class) {
            index = findCallbackInfo(cb);
            if (DEBUG) {
                Log.d(TAG, "removeCallback: " + cb + ", index: " + index);
            }
            if (index >= 0) {
    ...
    private static void updateWifiDisplayRoute(RouteInfo route, WifiDisplay display, WifiDisplayStatus wfdStatus, boolean disconnected) {
        ...
        if (changed2 || route.setRealStatusCode(getWifiDisplayStatusCode(display, wfdStatus))) {
            dispatchRouteChanged(route);
        }
            ...
            if ((this.mSupportedTypes & 2) != 0) {
                ...
                if (this.mPresentationDisplayId >= 0) {
                    ...
                    while (i < length2) {
                        ...
                        if (display2.getType() == 3 && this.mDeviceAddress.equals(display2.getAddress().toString())) {
                            return display2;
                        }
                        i++;
                    }
