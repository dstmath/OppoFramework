...
public class AudioRecord implements AudioRouting, MicrophoneDirection, AudioRecordingMonitor, AudioRecordingMonitorClient {
    ...
    public static int getMinBufferSize(int sampleRateInHz, int channelConfig, int audioFormat) {
        ...
        if (!(channelConfig == 1 || channelConfig == 2)) {
            ...
            if (size <= 0) {
                OppoAtlasManager.getInstance(null).setEvent(257, "EventID,5,sampleRate," + sampleRateInHz + ",channel," + channelConfig + ",format," + audioFormat + ",pid," + Binder.getCallingPid() + ",error," + size);
            }
            if (size != 0) {
                return -2;
            }
    ...
    public void startRecording() throws IllegalStateException {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(150, Os.getuid());
        if (SystemProperties.getBoolean("persist.sys.permission.enable", false)) {
            try {
                if (!IPermissionController.Stub.asInterface(ServiceManager.getService("permission")).checkPermission(Manifest.permission.RECORD_AUDIO, Process.myPid(), Process.myUid())) {
    ...
    public void stop() throws IllegalStateException {
        if (this.mState == 1) {
            ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(151, Os.getuid());
            synchronized (this.mRecordingStateLock) {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: android.media.AudioRecord.addOnRoutingChangedListener(android.media.AudioRouting$OnRoutingChangedListener, android.os.Handler):void
     arg types: [android.media.AudioRecord$OnRoutingChangedListener, android.os.Handler]
     candidates:
      ...
      android.media.AudioRecord.addOnRoutingChangedListener(android.media.AudioRouting$OnRoutingChangedListener, android.os.Handler):void */
    @Deprecated
