...
public class Html {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: android.text.TextUtils.indexOf(java.lang.CharSequence, char, int, int):int
     arg types: [android.text.Spanned, int, int, int]
     candidates:
      android.text.TextUtils.indexOf(java.lang.CharSequence, java.lang.CharSequence, int, int):int
      android.text.TextUtils.indexOf(java.lang.CharSequence, char, int, int):int */
    private static void withinBlockquoteIndividual(StringBuilder out, Spanned text, int start, int end) {
        ...
        for (int i = start; i <= end; i = next + 1) {
            next = TextUtils.indexOf((CharSequence) text, 10, i, end);
            if (next < 0) {
                next = end;
            }
            if (next == i) {
                ...
                String tagType = isListItem ? "li" : TtmlUtils.TAG_P;
                out.append("<");
                out.append(tagType);
        ...
        while (i < end) {
            ...
            for (int j = 0; j < style.length; j++) {
                ...
                if (style[j] instanceof RelativeSizeSpan) {
                    out.append(String.format("<span style=\"font-size:%.2fem;\">", Float.valueOf(((RelativeSizeSpan) style[j]).getSizeChange())));
                }
                if (style[j] instanceof ForegroundColorSpan) {
                    out.append(String.format("<span style=\"color:#%06X;\">", Integer.valueOf(((ForegroundColorSpan) style[j]).getForegroundColor() & 16777215)));
                }
                if (style[j] instanceof BackgroundColorSpan) {
                    out.append(String.format("<span style=\"background-color:#%06X;\">", Integer.valueOf(16777215 & ((BackgroundColorSpan) style[j]).getBackgroundColor())));
                }
            }
