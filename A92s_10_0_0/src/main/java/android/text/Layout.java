...
public abstract class Layout {
    ...
    /* JADX INFO: Multiple debug info for r11v0 int: [D('spanEnd' int), D('length' int)] */
    ...
    /* JADX INFO: Multiple debug info for r4v2 int: [D('spansLength' int), D('start' int)] */
    @UnsupportedAppUsage
                        ...
                        if (start != end || start == 0) {
                            ...
                            for (int j = 0; j < this.mLineBackgroundSpans.numberOfSpans; j++) {
                                if (this.mLineBackgroundSpans.spanStarts[j] < end && this.mLineBackgroundSpans.spanEnds[j] > start) {
                                    spans3 = (ParagraphStyle[]) GrowingArrayUtils.append(spans3, spansLength3, this.mLineBackgroundSpans.spans[j]);
                                    spansLength3++;
                                }
                            }
                            spans = spans3;
    ...
    /* JADX INFO: Multiple debug info for r2v4 int: [D('x' int), D('tabStops' android.text.Layout$TabStops)] */
    ...
    private void addSelection(int line, int start, int end, int top, int bottom, SelectionRectangleConsumer consumer) {
        ...
        int lineend = getLineEnd(line);
        Directions dirs = getLineDirections(line);
        if (lineend > linestart && layout2.mText.charAt(lineend - 1) == 10) {
            lineend--;
        }
        int i2 = 0;
