...
public class TextLine {
    ...
    public void justify(float justifyWidth) {
        ...
        float addedWidth = ((ITextJustificationHooks) OppoFeatureCache.getOrCreate(ITextJustificationHooks.DEFAULT, new Object[0])).calculateAddedWidth(justifyWidth, width, spaces, 0, end, this.mCharsValid, this.mChars, this.mText, this.mStart);
        if (addedWidth > 0.0f) {
            this.mAddedWidthForJustify = addedWidth;
            this.mIsJustifying = true;
        } else if (spaces != 0) {
            this.mAddedWidthForJustify = (justifyWidth - width) / ((float) spaces);
            this.mIsJustifying = true;
        }
    }
    ...
    /* JADX INFO: Multiple debug info for r1v41 int: [D('prevRunLevel' int), D('runStart' int)] */
    /* access modifiers changed from: package-private */
                        ...
                        if (cursor == runStart5) {
                            ...
                            while (true) {
                                ...
                                if (pos >= prevRunStart) {
                                    ...
                                    if (pos < prevRunLimit) {
                                        runStart3 = runStart5;
                                        int runStart6 = (runs2[prevRunIndex2 + 1] >>> 26) & 63;
                                        if (runStart6 < runLevel2) {
