...
class TimeFormatter {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:154:0x0316, code lost:
        return false;
     */
    /* JADX WARNING: Incorrect type for immutable var: ssa=int, code=?, for r14v2, types: [boolean, int] */
    private boolean handleToken(CharBuffer formatBuffer, ZoneInfo.WallTime wallTime, ZoneInfo zoneInfo) {
                                        ...
                                        default:
                                            switch (currentChar) {
                                                ...
                                                case 87:
                                                    ...
                                                    this.numberFormatter.format(getFormat(modifier, "%02d", "%2d", "%d", "%02d"), Integer.valueOf((yearDay - i) / 7));
                                                    return false;
                                                case 88:
        ...
        if (currentChar == 86) {
            ...
            Object[] objArr = new Object[r14];
            objArr[0] = Integer.valueOf(w);
            formatter.format(format, objArr);
            return false;
        } else if (currentChar == 103) {
            outputYear(year, false, r14, modifier);
            return false;
        } else {
            outputYear(year, r14, r14, modifier);
            return false;
        }
    }
