...
public class SpannableStringBuilder implements CharSequence, GetChars, Spannable, Editable, Appendable, GraphicsOperations {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: android.text.SpannableStringBuilder.replace(int, int, java.lang.CharSequence, int, int):android.text.SpannableStringBuilder
     arg types: [int, int, java.lang.CharSequence, int, int]
     candidates:
      android.text.SpannableStringBuilder.replace(int, int, java.lang.CharSequence, int, int):android.text.Editable
      android.text.Editable.replace(int, int, java.lang.CharSequence, int, int):android.text.Editable
      android.text.SpannableStringBuilder.replace(int, int, java.lang.CharSequence, int, int):android.text.SpannableStringBuilder */
        ...
        for (int i5 = this.mSpanCount - 1; i5 >= 0; i5--) {
            ...
            if ((this.mSpanFlags[i5] & 51) == 51) {
                ...
                if (spanStart > start && spanStart <= end) {
                    spanStart = end;
                    while (spanStart < clen && (spanStart <= end || charAt(spanStart - 1) != 10)) {
                        spanStart++;
                    }
                }
                if (spanEnd2 <= start || spanEnd2 > end) {
                    ...
                    int spanEnd3 = end;
                    while (spanEnd3 < clen && (spanEnd3 <= end || charAt(spanEnd3 - 1) != 10)) {
                        spanEnd3++;
                    }
                    spanEnd = spanEnd3;
                }
    ...
    /* JADX INFO: Multiple debug info for r11v2 int: [D('origLen' int), D('i' int)] */
    @Override // android.text.Editable
    public SpannableStringBuilder replace(int start, int end, CharSequence tb, int tbstart, int tbend) {
        ...
        if (adjustSelection) {
            ...
            if (selectionStart <= start || selectionStart >= end) {
                ...
                int selectionStart2 = start + Math.toIntExact((((long) newLen) * ((long) (selectionStart - start))) / ((long) i2));
                textWatchers = textWatchers2;
            ...
            if (selectionEnd <= start || selectionEnd >= end) {
                ...
                int selectionEnd3 = start + Math.toIntExact((((long) newLen) * ((long) (selectionEnd - start))) / ((long) i2));
                changed = true;
    ...
    public <T> T[] getSpans(int queryStart, int queryEnd, Class<T> kind, boolean sortByInsertionOrder) {
        ...
        T[] ret = (Object[]) Array.newInstance((Class<?>) kind, count);
            ...
            if (spanMax >= queryStart) {
                ...
                if (spanEnd < queryStart || (!(spanStart == spanEnd || queryStart == queryEnd || (spanStart != queryEnd && spanEnd != queryStart)) || (Object.class != kind && !kind.isInstance(this.mSpans[i])))) {
                    ...
                    if (sort) {
                        ...
                        int j = 0;
                        while (j < count2 && spanPriority <= (getSpanFlags(ret[j]) & Spanned.SPAN_PRIORITY)) {
                            j++;
                        }
