...
public class DynamicLayout extends Layout {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: android.text.TextUtils.indexOf(java.lang.CharSequence, char, int):int
    ...
    public void reflow(CharSequence s, int where, int before, int after) {
        ...
        if (s == this.mBase) {
            ...
            int n2 = reflowed2.getLineCount();
            if (where2 + after3 == len || reflowed2.getLineStart(n2 - 1) != where2 + after3) {
                n = n2;
            } else {
                n = n2 - 1;
            }
            ...
            while (i2 < n) {
                ...
                int end = i2 == n + -1 ? where2 + after3 : reflowed2.getLineStart(i2 + 1);
    ...
    public void updateBlocks(int startLine, int endLine, int newLineCount) {
        ...
        while (true) {
            if (i2 >= this.mNumberOfBlocks) {
                break;
            } else if (this.mBlockEndLines[i2] >= startLine) {
                firstBlock = i2;
                break;
            } else {
                i2++;
        ...
        while (true) {
            if (i3 >= this.mNumberOfBlocks) {
                break;
            } else if (this.mBlockEndLines[i3] >= endLine) {
                lastBlock = i3;
                break;
            } else {
                i3++;
