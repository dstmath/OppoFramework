...
public final class LocationAccessPolicy {
    ...
    private static final String[] mNLPPackages = {"com.baidu.map.location", "com.tencent.android.location", "com.amap.android.location", "com.amap.android.ams"};
    ...
    private static LocationPermissionResult checkAppLocationPermissionHelper(Context context, LocationPermissionQuery query, String permissionToCheck) {
        ...
        if (minSdkVersion > 10000) {
            ...
            return null;
        } else if (isAppAtLeastSdkVersion(context, query.callingPackage, minSdkVersion, query.callingUid)) {
            return LocationPermissionResult.DENIED_HARD;
        } else {
    ...
    public static LocationPermissionResult checkLocationPermission(Context context, LocationPermissionQuery query) {
        ...
        if (!checkSystemLocationAccess(context, query.callingUid, query.callingPid) && !isNLP(query.callingPackage)) {
            return LocationPermissionResult.DENIED_SOFT;
        }
    ...
    private static boolean isCurrentProfile(Context context, int uid) {
        ...
        try {
            ...
            if (currentUser == 0 && ColorMultiAppManager.getInstance().isMultiAppUserId(callingUserId)) {
                return true;
            }
    ...
    private static boolean isAppAtLeastSdkVersion(Context context, String pkgName, int sdkVersion, int pkgUid) {
        try {
            if (context.getPackageManager().getApplicationInfoAsUser(pkgName, 0, UserHandle.getUserId(pkgUid)).targetSdkVersion >= sdkVersion) {
                return true;
            }
            return false;
        } catch (PackageManager.NameNotFoundException e) {
