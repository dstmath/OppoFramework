...
public class CellBroadcastMessage implements Parcelable {
    ...
    /* JADX INFO: Multiple debug info for r2v26 android.telephony.SmsCbCmasInfo: [D('cmasCertaintyColumn' int), D('cmasInfo' android.telephony.SmsCbCmasInfo)] */
    @UnsupportedAppUsage
    public static CellBroadcastMessage createFromCursor(Cursor cursor) {
        ...
        if (cmasMessageClassColumn2 == -1 || cursor.isNull(cmasMessageClassColumn2)) {
            ...
            cmasInfo = makeSmsCbCmasInfo(messageClass, cmasCategory, responseType, severity, urgency, certainty);
        }
    ...
    private static SmsCbCmasInfo makeSmsCbCmasInfo(int messageClass, int category, int responseType, int severity, int urgency, int certainty) {
        ...
        try {
            return Class.forName("mediatek.telephony.MtkSmsCbCmasInfo").getConstructor(Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE, Long.TYPE).newInstance(Integer.valueOf(messageClass), Integer.valueOf(category), Integer.valueOf(responseType), Integer.valueOf(severity), Integer.valueOf(urgency), Integer.valueOf(certainty), 0L);
        } catch (Exception e) {
            return new SmsCbCmasInfo(messageClass, category, responseType, severity, urgency, certainty);
        }
    }
