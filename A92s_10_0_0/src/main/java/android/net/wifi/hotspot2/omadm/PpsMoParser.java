...
public final class PpsMoParser {
    ...
    private static long[] parseRoamingConsortiumOI(String oiStr) throws ParsingException {
        String[] oiStrArray = oiStr.split(SmsManager.REGEX_PREFIX_DELIMITER);
        ...
        if (!node.isLeaf()) {
            ...
            for (PPSNode child : node.getChildren()) {
                ...
                if (c == 0) {
                    ...
                    String[] fqdnMatchArray = fqdnMatch.split(SmsManager.REGEX_PREFIX_DELIMITER);
    ...
    private static Map<String, byte[]> parseAAAServerTrustRootList(PPSNode node) throws ParsingException {
        if (!node.isLeaf()) {
            ...
            for (PPSNode child : node.getChildren()) {
                ...
                certList.put(certTuple.first, (byte[]) certTuple.second);
            }
            return certList;
        }
