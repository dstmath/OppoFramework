...
public class Paint {
    ...
    public Paint(Paint paint) {
        ...
        injectedByColorOS(paint.mTypeface);
    }
    ...
    public void set(Paint src) {
        ...
        injectedByColorOS(src.mTypeface);
    }
    ...
    public long getNativeInstance() {
        ...
        long newNativeColorFilter = 0;
        long newNativeShader = shader == null ? 0 : shader.getNativeInstance();
        ...
        if (colorFilter != null) {
            newNativeColorFilter = colorFilter.getNativeInstance();
        }
        ...
        nSetForceZgFont(this.mNativePaint, ((IColorBurmeseZgHooks) ColorFrameworkFactory.getInstance().getFeature(IColorBurmeseZgHooks.DEFAULT, new Object[0])).getZgFlag());
    ...
    public void setColor(long color) {
        nSetColor(this.mNativePaint, Color.colorSpace(color).getNativeInstance(), color);
        this.mColor = color;
    }
    ...
    public void setAlpha(int a) {
        this.mColor = Color.pack(Color.red(this.mColor), Color.green(this.mColor), Color.blue(this.mColor), ((float) a) * 0.003921569f, Color.colorSpace(this.mColor));
    ...
    private Xfermode installXfermode(Xfermode xfermode) {
        ...
        if (newMode != (xfermode2 != null ? xfermode2.porterDuffMode : Xfermode.DEFAULT)) {
    ...
    public void setShadowLayer(float radius, float dx, float dy, long shadowColor) {
        nSetShadowLayer(this.mNativePaint, radius, dx, dy, Color.colorSpace(shadowColor).getNativeInstance(), shadowColor);
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: android.graphics.Paint.setShadowLayer(float, float, float, int):void
    ...
    public float getTextRunAdvances(char[] chars, int index, int count, int contextIndex, int contextCount, boolean isRtl, float[] advances, int advancesIndex) {
        int i;
        if (chars != null) {
            int length = index | count | contextIndex | contextCount | advancesIndex | (index - contextIndex) | (contextCount - count) | ((contextIndex + contextCount) - (index + count)) | (chars.length - (contextIndex + contextCount));
            if (advances == null) {
                i = 0;
            } else {
    ...
    public void replaceTypeface(Typeface typeface) {
        Typeface tf = ((IColorFontManager) OppoFeatureCache.getOrCreate(IColorFontManager.DEFAULT, new Object[0])).flipTypeface(typeface);
        if (tf != null) {
    ...
    private void injectedByColorOS(Typeface typeface) {
