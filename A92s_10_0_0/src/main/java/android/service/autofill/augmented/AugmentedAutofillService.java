                ...
                if (this.mSmartSuggestion == null || !this.mFocusedId.equals(this.mLastShownId)) {
                    try {
                        ...
                        if (rect != null) {
                            ...
                            PresentationParams.SystemPopupPresentationParams systemPopupPresentationParams = this.mSmartSuggestion;
                            return systemPopupPresentationParams;
                        } else if (AugmentedAutofillService.sDebug) {
                            String access$900 = AugmentedAutofillService.TAG;
                            Log.d(access$900, "getViewCoordinates(" + this.mFocusedId + ") returned null");
                        }
                    } catch (RemoteException e) {
                        String access$9002 = AugmentedAutofillService.TAG;
                        Log.w(access$9002, "Could not get coordinates for " + this.mFocusedId);
                        return null;
                    }
                } else {
                    PresentationParams.SystemPopupPresentationParams systemPopupPresentationParams2 = this.mSmartSuggestion;
                    return systemPopupPresentationParams2;
                }
            }
        }
            ...
            if (event == 1) {
                ...
                if (this.mFirstOnSuccessTime == 0) {
                    ...
                    if (AugmentedAutofillService.sDebug) {
                        String access$9002 = AugmentedAutofillService.TAG;
                        Log.d(access$9002, "Service responded nothing in " + TimeUtils.formatDuration(duration));
                    }
                }
                try {
                ...
                Log.w(access$9005, "invalid event reported: " + event);
            } else {
                type = 2;
                if (this.mUiFirstDestroyedTime == 0) {
                    ...
                    if (AugmentedAutofillService.sDebug) {
                        ...
                        Log.d(access$9006, "UI destroyed in " + TimeUtils.formatDuration(duration));
                    }
                }
            }
