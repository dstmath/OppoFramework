...
public abstract class MediaBrowserService extends Service {
    ...
    public static final boolean DBG = (!"user".equals(Build.TYPE));
    @UnsupportedAppUsage
    ...
    public void performLoadChildren(final String parentId, final ConnectionRecord connection, final Bundle options) {
        Result<List<MediaBrowser.MediaItem>> result = new Result<List<MediaBrowser.MediaItem>>(parentId) {
            ...
            public void onResultSent(List<MediaBrowser.MediaItem> list, int flag) {
                ...
                } else if (MediaBrowserService.DBG) {
                    Log.d(MediaBrowserService.TAG, "Not sending onLoadChildren result for connection that has been disconnected. pkg=" + connection.pkg + " id=" + parentId);
                }
            }
        };
            ...
            public void onResultSent(MediaBrowser.MediaItem item, int flag) {
                if (MediaBrowserService.this.mConnections.get(connection.callbacks.asBinder()) != connection) {
                    if (MediaBrowserService.DBG) {
                        Log.d(MediaBrowserService.TAG, "Not sending onLoadItem result for connection that has been disconnected. pkg=" + connection.pkg + " id=" + itemId);
                    }
                } else if ((flag & 2) != 0) {
