...
public final class AccessibilityIterators {
    ...
    static class ParagraphTextSegmentIterator extends AbstractTextSegmentIterator {
        ...
        private boolean isStartBoundary(int index) {
            return this.mText.charAt(index) != 10 && (index == 0 || this.mText.charAt(index + -1) == 10);
        }

        private boolean isEndBoundary(int index) {
            return index > 0 && this.mText.charAt(index + -1) != 10 && (index == this.mText.length() || this.mText.charAt(index) == 10);
        }
    }
}
