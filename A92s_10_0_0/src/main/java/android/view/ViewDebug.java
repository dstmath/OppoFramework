        ...
        while (i < count) {
            ...
            try {
                        ...
                        if (type == String[].class) {
                            ...
                            if (property.hasAdjacentMapping() && array2 != null) {
                                ...
                                while (j < array2.length) {
                                    if (array2[j] != null) {
                                        array = array2;
                                        writeEntry(out, categoryPrefix2 + prefix, array2[j], "", array2[j + 1] == null ? "null" : array2[j + 1]);
                                    } else {
                                        array = array2;
                                    }
                                    j += 2;
                            ...
                            writeEntry(out, categoryPrefix2 + prefix, field.getName(), "", fieldValue);
                        } else {
                ...
                if (!property.resolveId() || context == null) {
                    FlagToString[] flagsMapping = property.flagMapping();
                    if (flagsMapping.length > 0) {
                        exportUnrolledFlags(out, flagsMapping, field.getInt(view), categoryPrefix2 + prefix + field.getName() + '_');
                    }
                    IntToString[] mapping = property.mapping();
                    if (mapping.length > 0) {
