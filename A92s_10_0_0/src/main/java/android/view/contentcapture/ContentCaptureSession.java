        ...
        synchronized (this.mLock) {
            if (!this.mDestroyed) {
                ...
                if (ContentCaptureHelper.sVerbose) {
                    String str = TAG;
                    Log.v(str, "destroy(): state=" + getStateAsString(this.mState) + ", mId=" + this.mId);
                }
                if (this.mChildren != null) {
                    int numberChildren = this.mChildren.size();
                    if (ContentCaptureHelper.sVerbose) {
                        String str2 = TAG;
                        Log.v(str2, "Destroying " + numberChildren + " children first");
                    }
                    for (int i = 0; i < numberChildren; i++) {
                        try {
                            ...
                            String str3 = TAG;
                            Log.w(str3, "exception destroying child session #" + i + ": " + e);
                        }
                    }
                }
            } else if (ContentCaptureHelper.sDebug) {
                String str4 = TAG;
                Log.d(str4, "destroy(" + this.mId + "): already destroyed");
            }
        }
    }
