    ...
    public boolean isCurrectViewSetEnableAndInCpt(View currectView) {
        if (currectView != null && !currectView.isEnabled()) {
            if (!this.mInCptWhiteList && !this.mAlreadyInitCpt) {
                if (OppoMirrorActivityThread.inCptWhiteList != null) {
                    this.mInCptWhiteList = OppoMirrorActivityThread.inCptWhiteList.call(null, Integer.valueOf((int) MetricsProto.MetricsEvent.ACTION_PERMISSION_REQUEST_SEND_SMS), currectView.getContext().getPackageName()).booleanValue();
                }
