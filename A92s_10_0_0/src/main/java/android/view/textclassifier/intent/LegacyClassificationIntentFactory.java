...
public final class LegacyClassificationIntentFactory implements ClassificationIntentFactory {
    ...
    private static List<LabeledIntent> createForAddress(Context context, String text) {
        ...
        try {
            ...
            actions.add(new LabeledIntent(context.getString(R.string.map), null, context.getString(R.string.map_desc), null, new Intent("android.intent.action.VIEW").setData(Uri.parse(String.format("geo:0,0?q=%s", encText))), 0));
