...
public class TelecomManager implements IOppoTelecomManagerEx {
    ...
    @Override // android.telecom.IOppoTelecomManagerEx
    public String colorInteractWithTelecomService(int command, String ext) {
        Log.i(TAG, "service colorInteractWithTelecomService which = " + command);
        String ret = null;
        try {
            if (isServiceConnected()) {
                ret = getTelecomService().colorInteractWithTelecomService(command, ext);
            }
        } catch (RemoteException e) {
            Log.e(TAG, "Error calling ITelecomService#colorInteractWithTelecomService", e);
        }
        Log.i(TAG, "colorInteractWithTelecomService ret = " + ret);
        return ret;
    }

    @Override // android.telecom.IOppoTelecomManagerEx
    public void addNewOutgoingCall(Intent intent) {
        Log.i(TAG, "addNewOutgoingCall intent = " + intent);
        try {
            Log.i(TAG, "addNewOutgoingCall packageName = " + this.mContext.getOpPackageName());
            intent.putExtra(PACKAGE_NAME_KEY, this.mContext.getOpPackageName());
            if (isServiceConnected()) {
                getTelecomService().addNewOutgoingCall(intent);
            }
        } catch (RemoteException e) {
            Log.e(TAG, "Error calling ITelecomService#addNewOutgoingCall", e);
        }
    }

    @Override // android.telecom.IOppoTelecomManagerEx
    public void oppoCancelMissedCallsNotification(Bundle extras) {
        Log.i(TAG, "oppoCancelMissedCallsNotification intent = " + extras);
        ITelecomService service = getTelecomService();
        if (service != null) {
            try {
                service.oppoCancelMissedCallsNotification(this.mContext.getOpPackageName(), extras);
            } catch (RemoteException e) {
                Log.e(TAG, "Error calling ITelecomService#oppoCancelMissedCallsNotification", e);
            }
        }
    }
}
