...
public class Location implements Parcelable {
    ...
    /* JADX INFO: Multiple debug info for r6v10 double: [D('cosSigma' double), D('lat2' double)] */
    private static void computeDistanceAndBearing(double lat1, double lon1, double lat2, double lon2, BearingDistanceCache results) {
        ...
        while (true) {
            ...
            deltaSigma = B * sinSigma3 * (cos2SM + ((B / 4.0d) * ((((cos2SMSq * 2.0d) - 4.0d) * lat23) - ((((B / 6.0d) * cos2SM) * (((sinSigma3 * 4.0d) * sinSigma3) - 1.5d)) * ((4.0d * cos2SMSq) - 1.5d)))));
            lambda = L + ((1.0d - C) * f * sinAlpha * (sigma + (C * sinSigma3 * (cos2SM + (C * lat23 * (((2.0d * cos2SM) * cos2SM) - 4.0d))))));
        ...
        float unused = results.mDistance = (float) (6356752.3142d * A * (sigma - deltaSigma));
        float unused2 = results.mInitialBearing = (float) (((double) ((float) Math.atan2(cosU2 * sinLambda, (cosU1 * sinU2) - ((sinU1 * cosU2) * cosLambda)))) * 57.29577951308232d);
        float unused3 = results.mFinalBearing = (float) (((double) ((float) Math.atan2(cosU1 * sinLambda, ((-sinU1) * cosU2) + (cosU1 * sinU2 * cosLambda)))) * 57.29577951308232d);
        double unused4 = results.mLat1 = lat12;
        double unused5 = results.mLat2 = lat22;
        double unused6 = results.mLon1 = lon12;
        double unused7 = results.mLon2 = lon22;
    }
