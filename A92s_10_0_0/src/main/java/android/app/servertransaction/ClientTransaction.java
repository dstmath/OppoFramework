...
public class ClientTransaction implements Parcelable, ObjectPoolItem {
    ...
    public void dump(String prefix, PrintWriter pw) {
        ...
        if (size > 0) {
            pw.println();
            for (int i = 0; i < size; i++) {
                pw.append((CharSequence) prefix).append((CharSequence) "    ").println(this.mActivityCallbacks.get(i).toString());
            }
            pw.append((CharSequence) prefix).println("  ]");
        } else {
            pw.println("]");
        }
        PrintWriter append = pw.append((CharSequence) prefix).append((CharSequence) "  stateRequest=");
        ActivityLifecycleItem activityLifecycleItem = this.mLifecycleStateRequest;
    ...
    public String toString() {
        ...
        ActivityLifecycleItem activityLifecycleItem = this.mLifecycleStateRequest;
        if (activityLifecycleItem != null) {
            sb.append(activityLifecycleItem.getClass().getSimpleName());
        }
        return "ClientTransaction " + sb.toString();
    }
}
