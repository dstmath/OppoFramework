...
public abstract class OppoBaseBatteryStatsImpl extends BatteryStats {
    ...
    class ScreenoffBatteryStats {
        ...
        private void updateWifiSignalLevels(long rawRealtime, int which) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                if (time > 0) {
                    if (1 != 0) {
                        level = SignalStrength.SIGNAL_STRENGTH_NAMES[i];
                    } else {
                        level = "level" + i;
                    }
                    if (OppoBaseBatteryStatsImpl.DEBUG_DETAIL) {
    ...
    /* JADX INFO: Multiple debug info for r2v6 long: [D('usePhoneSignalStr' boolean), D('timeDelta' long)] */
    ...
    /* JADX INFO: Multiple debug info for r12v3 long: [D('androidWakelocksBak' java.util.HashMap<java.lang.String, com.android.internal.os.OppoBaseBatteryStatsImpl$WakeLockEntry>), D('heldTime' long)] */
    ...
    /* JADX INFO: Multiple debug info for r14v3 long: [D('heldTime' long), D('NSE' int)] */
    ...
    public String formatBytesLocal(long bytes) {
