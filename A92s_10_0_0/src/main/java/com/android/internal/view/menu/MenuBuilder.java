    ...
    public void findItemsWithShortcutForKey(List<MenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (event.getKeyData(possibleChars) || keyCode == 67) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (((modifierState & Menu.SUPPORTED_MODIFIERS_MASK) == (69647 & (qwerty ? item.getAlphabeticModifiers() : item.getNumericModifiers()))) && shortcutChar != 0 && ((shortcutChar == possibleChars.meta[0] || shortcutChar == possibleChars.meta[2] || (qwerty && shortcutChar == 8 && keyCode == 67)) && item.isEnabled())) {
                    items.add(item);
                }
            }
        }
