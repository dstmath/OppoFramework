...
public class IccProvider extends ContentProvider {
    ...
    private static IccInternalInterface makeIccProvider(UriMatcher urlMatcher, Context context) {
        if (SystemProperties.get("ro.vendor.mtk_telephony_add_on_policy", OppoModemLogManager.DEFAULT_MODEMDUMP_POSTBACK).equals(OppoModemLogManager.DEFAULT_MODEMDUMP_POSTBACK)) {
            try {
                Class<?> clazz = Class.forName("com.mediatek.internal.telephony.phb.MtkIccProvider", false, ClassLoader.getSystemClassLoader());
                Rlog.d(TAG, "class = " + clazz);
                Constructor clazzConstructfunc = clazz.getConstructor(UriMatcher.class, Context.class);
    ...
    public Cursor query(Uri url, String[] projection, String selection, String[] selectionArgs, String sort) {
        ...
        if (iccInternalInterface != null) {
            return iccInternalInterface.query(url, projection, selection, selectionArgs, sort);
        }
        log("query");
    ...
    public Uri insert(Uri url, ContentValues initialValues) {
        ...
        IccInternalInterface iccInternalInterface = sMtkIccProvider;
        if (iccInternalInterface != null) {
            Uri resultUri = iccInternalInterface.insert(url, initialValues);
        ...
        buf.append(0);
        Uri resultUri2 = Uri.parse(buf.toString());
        getContext().getContentResolver().notifyChange(url, null);
        return resultUri2;
    }
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        IccInternalInterface iccInternalInterface = sMtkIccProvider;
        if (iccInternalInterface != null) {
            int result = iccInternalInterface.delete(url, where, whereArgs);
            if (result <= 0) {
                return result;
            }
            getContext().getContentResolver().notifyChange(url, null);
    ...
    public int update(Uri url, ContentValues values, String where, String[] whereArgs) {
        ...
        IccInternalInterface iccInternalInterface = sMtkIccProvider;
        if (iccInternalInterface != null) {
            int result = iccInternalInterface.update(url, values, where, whereArgs);
            if (result <= 0) {
                return result;
            }
            getContext().getContentResolver().notifyChange(url, null);
