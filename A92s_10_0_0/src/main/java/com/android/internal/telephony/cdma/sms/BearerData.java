...
public class BearerData {
    ...
    public static int countAsciiSeptetsPublic(CharSequence msg, boolean force) {
        return countAsciiSeptets(msg, force);
    }
    ...
    public static GsmAlphabet.TextEncodingDetails calcTextEncodingDetailsOem(CharSequence msg, boolean force7BitEncoding, boolean isEntireMsg, int encodingType) {
        return OppoBearerData.calcTextEncodingDetailsOem(msg, force7BitEncoding, isEntireMsg, encodingType);
    }
    ...
    public static byte[] encode(BearerData bData) {
        UserData userData2 = bData.userData;
        bData.hasUserDataHeader = (userData2 == null || userData2.userDataHeader == null) ? false : true;
        try {
    ...
    private static boolean decodeUserData(BearerData bData, BitwiseInputStream inStream) throws BitwiseInputStream.AccessException {
        ...
        userData2.msgType = 0;
        int consumedBits = 5;
        if (userData2.msgEncoding == 1 || bData.userData.msgEncoding == 10) {
    ...
    private static void decodeUserDataPayload(UserData userData2, boolean hasUserDataHeader2) throws CodingException {
        ...
        if (hasUserDataHeader2) {
            int udhLen = userData2.payload[0] & 255;
            offset = 0 + udhLen + 1;
        ...
        int udhLen2 = userData2.msgEncoding;
        if (udhLen2 == 0) {
    ...
    /* JADX INFO: Multiple debug info for r6v2 int: [D('remainingBits' int), D('consumedBits' byte)] */
        ...
        if (inStream.available() >= 13) {
            ...
            if (inStream.available() >= paramBits) {
                ...
                while (paramBits >= 48) {
                    ...
                    int language2 = inStream.read(textBits);
                    ...
                    if (paramBits2 >= textBits2) {
                        ...
                        userData2.payload = inStream.readByteArray(textBits2);
                        paramBits = paramBits2 - textBits2;
                        decodeUserDataPayload(userData2, false);
                        programDataList.add(new CdmaSmsCbProgramData(operation, category, language2, maxMessages, alertOption, userData2.payloadStr));
                        decodeSuccess = true;
                        textBits = 8;
                    } else {
    ...
    public static void decodeCmasUserData(BearerData bData, int serviceCategory) throws BitwiseInputStream.AccessException, CodingException {
        int numFields;
        BitwiseInputStream inStream = new BitwiseInputStream(bData.userData.payload);
    ...
    private static void encodeOctetEms(UserData uData, byte[] udhData) {
        ...
        uData.msgEncoding = 0;
        uData.msgEncodingSet = true;
        uData.numFields = uData.payload.length + udhBytes;
