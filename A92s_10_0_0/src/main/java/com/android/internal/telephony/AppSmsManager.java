...
public class AppSmsManager {
    ...
    public boolean handleSmsReceivedIntent(Intent intent) {
        ...
        synchronized (this.mLock) {
            ...
            try {
                info.pendingIntent.send(this.mContext, 0, new Intent().putExtras(intent.getExtras()).putExtra("android.telephony.extra.STATUS", 0).putExtra("android.telephony.extra.SMS_MESSAGE", message).putExtra("android.telephony.extra.SIM_SUBSCRIPTION_ID", info.subId).addFlags(2097152));
    ...
    private void removeExpiredTokenLocked() {
        ...
        for (String token : this.mTokenMap.keySet()) {
            ...
            if (request.packageBasedToken && currentTimeMillis - TIMEOUT_MILLIS > request.timestamp) {
                try {
                    request.pendingIntent.send(this.mContext, 0, new Intent().putExtra("android.telephony.extra.STATUS", 1).addFlags(2097152));
                } catch (PendingIntent.CanceledException e) {
                }
    ...
    private void removeRequestLocked(AppRequestInfo info) {
        if (info != null) {
            Log.d(NotificationChannelController.CHANNEL_ID_SMS, "removeRequestLocked," + info.packageName);
        }
    ...
    private void addRequestLocked(AppRequestInfo info) {
        if (info != null) {
            Log.d(NotificationChannelController.CHANNEL_ID_SMS, "addRequestLocked," + info.packageName);
        }
