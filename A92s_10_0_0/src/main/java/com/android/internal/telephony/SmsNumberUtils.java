...
public class SmsNumberUtils {
        ...
        if (number == null) {
            ...
            if (nanpState != 4) {
                ...
                switch (internationalState) {
                    ...
                    case NP_CC_AREA_LOCAL /*{ENCODED_INT: 104}*/:
    ...
    private static int checkInternationalNumberPlan(Context context, NumberEntry numberEntry, ArrayList<String> allIDDs, String homeIDD) {
        ...
        if (number.startsWith(PLUS_SIGN)) {
            ...
            if (number.startsWith(OppoModemLogManager.DEFAULT_MODEMDUMP_POSTBACK) || (countryCode = getCountryCode(context, number)) <= 0) {
                return 0;
            }
    ...
    public static String filterDestAddr(Phone phone, String destAddr) {
        ...
        String networkMcc;
        if (DBG) {
            Rlog.d(TAG, "enter filterDestAddr. destAddr=\"" + Rlog.pii(TAG, destAddr) + "\"");
        }
        if (destAddr == null || !PhoneNumberUtils.isGlobalPhoneNumber(destAddr)) {
            Rlog.w(TAG, "destAddr" + Rlog.pii(TAG, destAddr) + " is not a global phone number! Nothing changed.");
            return destAddr;
        }
        ...
        if (DBG) {
            ...
            if (result != null) {
                str = Rlog.pii(TAG, result);
            } else {
                str = Rlog.pii(TAG, destAddr);
            }
            sb2.append(str);
            sb2.append("\"");
