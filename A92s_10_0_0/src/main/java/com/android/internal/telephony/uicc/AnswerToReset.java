...
public class AnswerToReset {
    ...
    private void checkIsEuiccSupported() {
        ...
        while (i < this.mInterfaceBytes.size() - 1) {
            if (this.mInterfaceBytes.get(i).getTD() == null || (this.mInterfaceBytes.get(i).getTD().byteValue() & 15) != 15 || this.mInterfaceBytes.get(i + 1).getTB() == null || (this.mInterfaceBytes.get(i + 1).getTB().byteValue() & 128) == 0 || (this.mInterfaceBytes.get(i + 1).getTB().byteValue() & 2) == 0) {
                i++;
            } else {
    ...
    private boolean parseAtrString(String atr) {
        ...
        if (atr == null) {
            ...
            if (atrBytes == null || (index = parseConventionByte(atrBytes, 0)) == -1 || (index2 = parseFormatByte(atrBytes, index)) == -1 || (index3 = parseInterfaceBytes(atrBytes, index2)) == -1 || (index4 = parseHistoricalBytes(atrBytes, index3)) == -1 || (index5 = parseCheckBytes(atrBytes, index4)) == -1) {
                return false;
            }
    ...
    public boolean isExtendedApduSupported() {
        ...
        if (historicalBytes == null || (cardCapabilities = historicalBytes.getValue(7)) == null || cardCapabilities.length < 3) {
            return false;
        }
        if (this.mIsDirectConvention) {
            ...
            return false;
        } else if ((cardCapabilities[2] & 2) > 0) {
            return true;
        } else {
            return false;
        }
