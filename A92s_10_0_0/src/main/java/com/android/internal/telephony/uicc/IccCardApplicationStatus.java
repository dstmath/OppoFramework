        ...
        switch (substate) {
            ...
            default:
                if (SystemProperties.get("ro.vendor.mtk_telephony_add_on_policy", OppoModemLogManager.DEFAULT_MODEMDUMP_POSTBACK).equals(OppoModemLogManager.DEFAULT_MODEMDUMP_POSTBACK)) {
                    Class<?> mtkIccUtilsEx = Class.forName("com.mediatek.internal.telephony.uicc.MtkIccUtilsEx");
                    if (mtkIccUtilsEx != null) {
                        newSubState2 = (PersoSubState) mtkIccUtilsEx.getMethod("PersoSubstateFromRILInt", Integer.TYPE).invoke(null, Integer.valueOf(substate));
                    } else {
                        newSubState2 = PersoSubState.PERSOSUBSTATE_UNKNOWN;
                        loge("MtkIccUtilsEx is null!");
                    }
                    newSubState = newSubState2;
                } else {
