...
public class OppoFeatureManager {
    ...
    public static <T extends IOppoCommonFeature> T getTraceMonitor(T real) {
        if (real != null) {
            ...
            if (!isTracing((IOppoCommonFeature) real)) {
                return real;
            }
            InvocationHandler handler = new DynamicProxy(real, index.name());
            return Proxy.newProxyInstance(handler.getClass().getClassLoader(), new Class[]{(Class) real.getDefault().getClass().getGenericInterfaces()[0]}, handler);
        }
