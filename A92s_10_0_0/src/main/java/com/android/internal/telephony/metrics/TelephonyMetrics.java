...
public class TelephonyMetrics {
    ...
    private synchronized void printAllMetrics(PrintWriter rawWriter) {
        ...
        for (TelephonyProto.TelephonyEvent event : this.mTelephonyEvents) {
            ...
            pw.println(PhoneConfigurationManager.SSSS);
        }
        pw.decreaseIndent();
        ...
        pw.decreaseIndent();
        pw.println("Hardware Version: " + SystemProperties.get("ro.boot.revision", PhoneConfigurationManager.SSSS));
    }
    ...
    private synchronized TelephonyProto.TelephonyLog buildProto() {
        ...
        log.hardwareRevision = SystemProperties.get("ro.boot.revision", PhoneConfigurationManager.SSSS);
    ...
    public synchronized void updateActiveSubscriptionInfoList(List<SubscriptionInfo> subInfos) {
        ...
        for (SubscriptionInfo info : subInfos) {
            ...
            inActivePhoneList.removeIf(new Predicate(phoneId) {
                /* class com.android.internal.telephony.metrics.$$Lambda$TelephonyMetrics$tQOsX1lKb2eTuPp1rpkeIAEOoY */
    ...
    private TelephonyProto.EmergencyNumberInfo convertEmergencyNumberToEmergencyNumberInfo(EmergencyNumber num) {
        ...
        emergencyNumberInfo.urns = (String[]) num.getEmergencyUrns().stream().toArray($$Lambda$TelephonyMetrics$fLmZDbNadlr6LF7zSJ6jCR1AAsk.INSTANCE);
    ...
    public void writeRilDial(int phoneId, GsmCdmaConnection conn, int clirMode, UUSInfo uusInfo) {
        ...
        calls[0].index = -1;
        convertConnectionToRilCall(conn, calls[0], PhoneConfigurationManager.SSSS);
    ...
    public void writeIncomingVoiceMailSms(int phoneId, String format) {
        ...
        writeIncomingSmsWithType(phoneId, 2, format, true);
    }

    public void writeIncomingSmsTypeZero(int phoneId, String format) {
        logv("Logged Type-0 SMS message.");
        writeIncomingSmsWithType(phoneId, 3, format, true);
    }
    ...
    private void writeIncomingSmsSessionWithType(int phoneId, int type, boolean smsOverIms, String format, long[] timestamps, boolean blocked, boolean success) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            long time = timestamps[i2];
            SmsSessionEventBuilder format2 = new SmsSessionEventBuilder(8).setFormat(convertSmsFormat(format));
            int i3 = 1;
            if (smsOverIms) {
                i = 3;
            } else {
            ...
            SmsSessionEventBuilder tech = format2.setTech(i);
            if (success) {
                i3 = 0;
            }
            smsSession.addEvent(time, tech.setErrorCode(i3).setSmsType(type).setBlocked(blocked));
        }
        finishSmsSession(smsSession);
    }

    public void writeIncomingWapPush(int phoneId, boolean smsOverIms, String format, long[] timestamps, boolean success) {
        writeIncomingSmsSessionWithType(phoneId, 4, smsOverIms, format, timestamps, false, success);
    }

    public void writeIncomingSmsSession(int phoneId, boolean smsOverIms, String format, long[] timestamps, boolean blocked) {
        writeIncomingSmsSessionWithType(phoneId, 0, smsOverIms, format, timestamps, blocked, true);
    }
    ...
    public void writeCarrierIdMatchingEvent(int phoneId, int version, int cid, String unknownMcmnc, String unknownGid1, CarrierResolver.CarrierMatchingRule simInfo) {
        ...
        if (simInfo.privilegeAccessRule != null) {
            carrierIdMatchingResult.privilegeAccessRule = (String[]) simInfo.privilegeAccessRule.stream().toArray($$Lambda$TelephonyMetrics$x2dJi76S2YQdpSTfY8RZ8qC_K6g.INSTANCE);
        }
