...
public abstract class IccRecords extends Handler implements IccConstants {
    ...
    protected int mMvnoDBState = 0;
    protected RegistrantList mMvnoRecordReadyRegistrants = new RegistrantList();
    protected int mMvnoSimState = (1 << MvnoType.DEFAULT.ordinal());
    ...
    public void handleRefresh(IccRefreshResponse refreshResponse) {
        if (refreshResponse == null) {
            ...
            if (refreshResponse.refreshResult != 0) {
    ...
    public void setEfGbabp(String gbabp, Message onComplete) {
        if (onComplete != null) {
            AsyncResult.forMessage(onComplete, (Object) null, new IccException("Default setEfGbabp exception."));
            onComplete.sendToTarget();
        }
    }
    ...
    public boolean is_test_card() {
        log("leon is_test_card: mIsTestCard:" + this.mIsTestCard + "/imsi:" + OemConstant.isTestCard(this.mImsi));
        return this.mIsTestCard || OemConstant.isTestCard(this.mImsi);
    }

    public void setMvnoDBState(int state) {
        this.mMvnoDBState = state;
    }

    public void resetMvnoState() {
        this.mMvnoSimState = 1 << MvnoType.DEFAULT.ordinal();
        this.mMvnoDBState = 0;
    }

    public void setMvnoSimState(MvnoType type) {
        this.mMvnoSimState |= 1 << type.ordinal();
    }

    public void setMvnoState(MvnoType type) {
        setMvnoSimState(type);
        if (!isMvnoReady()) {
            return;
        }
        if (this.mParentApp.getState() == IccCardApplicationStatus.AppState.APPSTATE_PIN || this.mParentApp.getState() == IccCardApplicationStatus.AppState.APPSTATE_PUK) {
            ...
            this.mMvnoRecordReadyRegistrants.notifyRegistrants();
        }
    }

    public boolean isMvnoReady() {
        int i = this.mMvnoDBState;
        return i > 0 && i == (this.mMvnoSimState & i);
    }

    public void registerForMvnoReady(Handler h, int what, Object obj) {
        if (!this.mDestroyed.get()) {
            Registrant r = new Registrant(h, what, obj);
            this.mMvnoRecordReadyRegistrants.add(r);
            if (isMvnoReady()) {
                r.notifyRegistrant(new AsyncResult((Object) null, (Object) null, (Throwable) null));
            }
        }
    ...
    public void unregisterForMvnoReady(Handler h) {
        this.mMvnoRecordReadyRegistrants.remove(h);
    }

    /* JADX WARNING: Removed duplicated region for block: B:25:0x0092 A[SYNTHETIC, Splitter:B:25:0x0092] */
    public static int oemMvnoParamState(Context context, String operator) {
        if (TextUtils.isEmpty(operator)) {
            return 0;
        }
        Cursor cursor = null;
        int dbState = 1 << MvnoType.DEFAULT.ordinal();
        try {
            Cursor cursor2 = context.getContentResolver().query(Uri.withAppendedPath(Telephony.Carriers.CONTENT_URI, "filtered"), null, "numeric = '" + operator + "'", null, HbpcdLookup.ID);
            ...
            MvnoType[] types = MvnoType.values();
            do {
                String mvnoType = cursor2.getString(cursor2.getColumnIndexOrThrow("mvno_type"));
                String mvnoMatchData = cursor2.getString(cursor2.getColumnIndexOrThrow("mvno_match_data"));
                if (!TextUtils.isEmpty(mvnoType) && !TextUtils.isEmpty(mvnoMatchData)) {
                    ...
                    while (true) {
                        ...
                        MvnoType m = types[i];
                        if (m.toString().equalsIgnoreCase(mvnoType)) {
                            dbState |= 1 << m.ordinal();
                            break;
                        }
                        i++;
