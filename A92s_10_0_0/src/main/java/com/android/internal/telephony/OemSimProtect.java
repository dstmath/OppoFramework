...
public class OemSimProtect {
    ...
    public static final Uri SIM_PROTECT_URI_CONTACT = Uri.withAppendedPath(SIM_PROTECT_URI_OBSERVER, SIM_PROTECT_CONTACT);
    ...
    /* access modifiers changed from: private */
    public static final Uri SIM_PROTECT_URI_SMS = Uri.withAppendedPath(SIM_PROTECT_URI_OBSERVER, "sms");
    private static OemSimProtect instance = new OemSimProtect();
    private boolean isReg = false;
    private SimObserver observer_contact = new SimObserver();
    /* access modifiers changed from: private */
    public HashMap<String, Boolean> sim_map_contact = new HashMap<>();
    /* access modifiers changed from: private */
    public HashMap<String, Boolean> sim_map_sms = new HashMap<>();
    ...
    public boolean isSimProtectContact(Context context, String packageName) {
        if (this.sim_map_contact.containsKey(packageName)) {
            return this.sim_map_contact.get(packageName).booleanValue();
        }
    ...
    public boolean isSimProtectSms(Context context, String packageName) {
        if (this.sim_map_sms.containsKey(packageName)) {
            return this.sim_map_sms.get(packageName).booleanValue();
        }
