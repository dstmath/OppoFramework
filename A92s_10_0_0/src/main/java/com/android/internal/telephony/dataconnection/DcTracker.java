...
public class DcTracker extends AbstractDcTracker {
    ...
    private class DctOnSubscriptionsChangedListener extends SubscriptionManager.OnSubscriptionsChangedListener {
        ...
        this.mPollNetStat = new Runnable() {
            ...
            public void run() {
                ...
                if (DcTracker.this.mIsScreenOn) {
                    DcTracker dcTracker = DcTracker.this;
                    int unused = dcTracker.mNetStatPollPeriod = Settings.Global.getInt(dcTracker.mResolver, "pdp_watchdog_poll_interval_ms", 1000);
                } else {
                    DcTracker dcTracker2 = DcTracker.this;
                    int unused2 = dcTracker2.mNetStatPollPeriod = Settings.Global.getInt(dcTracker2.mResolver, "pdp_watchdog_long_poll_interval_ms", DcTracker.POLL_NETSTAT_SCREEN_OFF_MILLIS);
                }
    ...
    private void startAlarmForReconnect(long delay, ApnContext apnContext) {
        ...
        PendingIntent alarmIntent = PendingIntent.getBroadcast(this.mPhone.getContext(), this.mPhone.getPhoneId() + 1, intent, 134217728);
    ...
    /* access modifiers changed from: protected */
    public void sendRequestNetworkCompleteMsg(Message message, boolean success, int transport, int requestType) {
        if (message != null) {
    ...
    private void onEnableApn(int apnType, int requestType, Message onCompleteMsg) {
        ...
        if (mtkIsAddRequestNetworkCompleteMsg(apnType, requestType) || trySetupData(apnContext, requestType)) {
            addRequestNetworkCompleteMsg(onCompleteMsg, apnType);
        } else {
            sendRequestNetworkCompleteMsg(onCompleteMsg, false, this.mTransportType, requestType);
        }
    ...
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r15v3, resolved type: android.net.ConnectivityManager} */
    ...
    private void onDataSetupComplete(ApnContext apnContext, boolean success, int cause, int requestType) {
        ...
        if (!mtkCanHandleOnDataSetupComplete(apnContext, success, cause, requestType)) {
            ...
            if (success) {
                ...
                cm.setProvisioningNotificationVisible(r10, r10, this.mProvisionActionName);
                ...
                if (("onDataSetupComplete: error apn=" + apn2) != null) {
                    str = apn2.getApnName();
                } else {
                    str = ((Object) null) + ", cause=" + cause + ", requestType=" + requestTypeToString(requestType);
                }
                log(str);
                ...
                String apnType = apnContext.getApnType();
                if (apn3 != null) {
                    str3 = apn3.getApnName();
                }
                phone.notifyPreciseDataConnectionFailed(apnType, str3, cause);
    ...
    private class DataStallRecoveryHandler {
        ...
        public void doRecovery() {
            if (DcTracker.this.getOverallState() == DctConstants.State.CONNECTED) {
                ...
                long unused = DcTracker.this.mSentSinceLastRecv = 0;
    ...
    public void startDataStallAlarm(boolean suspectedStall) {
        ...
        if (super.checkIfNeedDataStall() && this.mDsRecoveryHandler.isNoRxDataStallDetectionEnabled() && getOverallState() == DctConstants.State.CONNECTED) {
            ...
            SubscriptionManager.putPhoneIdAndSubIdExtra(intent, this.mPhone.getPhoneId());
            this.mDataStallAlarmIntent = PendingIntent.getBroadcast(this.mPhone.getContext(), this.mPhone.getPhoneId() + 1, intent, 134217728);
    ...
    private void startProvisioningApnAlarm() {
        ...
        this.mProvisioningApnAlarmIntent = PendingIntent.getBroadcast(this.mPhone.getContext(), this.mPhone.getPhoneId() + 1, intent, 134217728);
