        ...
        if ((deactivated || clippingParameters.isClippingEnablingAllowed(transformedView)) && (transformedView.getParent() instanceof ViewGroup)) {
            ...
            while (true) {
                if (deactivated || clippingParameters.isClippingEnablingAllowed(transformedView)) {
                    ArraySet<View> clipSet = (ArraySet) parent.getTag(16908812);
                    if (clipSet == null) {
                        clipSet = new ArraySet<>();
                        parent.setTagInternal(16908812, clipSet);
                    }
                    Boolean clipChildren = (Boolean) parent.getTag(16908813);
                    if (clipChildren == null) {
                        clipChildren = Boolean.valueOf(parent.getClipChildren());
                        parent.setTagInternal(16908813, clipChildren);
                    }
                    Boolean clipToPadding = (Boolean) parent.getTag(16908815);
                    if (clipToPadding == null) {
                        clipToPadding = Boolean.valueOf(parent.getClipToPadding());
                        parent.setTagInternal(16908815, clipToPadding);
                    }
                    if (!deactivated) {
                        ...
                        if (clipSet.isEmpty()) {
                            ...
                            parent.setTagInternal(16908812, null);
