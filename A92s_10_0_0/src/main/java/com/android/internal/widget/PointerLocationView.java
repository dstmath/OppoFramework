...
public class PointerLocationView extends View implements InputManager.InputDeviceListener, WindowManagerPolicyConstants.PointerEventListener {
    ...
    /* access modifiers changed from: protected */
    /* JADX WARNING: Removed duplicated region for block: B:63:0x054b  */
    /* JADX WARNING: Removed duplicated region for block: B:64:0x0569  */
    @Override // android.view.View
    ...
    public void onPointerEvent(MotionEvent event) {
        ...
        int i2 = 1;
        if (event.getAction() == 0 && ViewDebugManager.debugHighFrameRateTouchLowLatency) {
            Log.d("View", "onPointerEvent");
            if (getViewRootImpl() != null) {
                getViewRootImpl().postTouchPointerViewMessage(true);
            }
        }
        if (action == 0 || (action & 255) == 5) {
            ...
            if (i5 < 0 || (i5 < NP2 && !this.mPointers.get(i5).mCurDown)) {
        ...
        while (historyPos2 < N2) {
            ...
            while (i6 < NI) {
                ...
                PointerState ps5 = (!this.mCurDown || id3 >= NP) ? null : this.mPointers.get(id3);
        ...
        for (int i7 = 0; i7 < NI; i7++) {
            ...
            PointerState ps6 = (!this.mCurDown || id4 >= NP) ? null : this.mPointers.get(id4);
            MotionEvent.PointerCoords coords4 = ps6 != null ? ps6.mCoords : this.mTempCoords;
            ...
            if (ps != null) {
                ...
                if (velocityTracker3 != null) {
                    float unused6 = ps.mAltXVelocity = velocityTracker3.getXVelocity(id);
                    float unused7 = ps.mAltYVelocity = this.mAltVelocity.getYVelocity(id);
                    this.mAltVelocity.getEstimator(id, ps.mAltEstimator);
                }
                int unused8 = ps.mToolType = event.getToolType(i7);
                if (ps.mHasBoundingBox) {
                    c = ' ';
                    float unused9 = ps.mBoundingLeft = event.getAxisValue(32, i7);
                    float unused10 = ps.mBoundingTop = event.getAxisValue(33, i7);
                    float unused11 = ps.mBoundingRight = event.getAxisValue(34, i7);
                    float unused12 = ps.mBoundingBottom = event.getAxisValue(35, i7);
                } else {
                    c = ' ';
                }
            } else {
