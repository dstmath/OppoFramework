    ...
    public void dump(PrintWriter pw, String prefix) {
        synchronized (this.mLock) {
            ...
            for (int i = 0; i < this.mAllCallingUids.size(); i++) {
                ...
                pw.print(prefix);
                pw.println(String.format("  %-9d %6d  %6d %6d %6d  %6d %6d %6d %6d %12.3f", Integer.valueOf(uid), Long.valueOf(this.mQueryStats.get(uid)), Long.valueOf(this.mInsertStats.get(uid)), Long.valueOf(this.mUpdateStats.get(uid)), Long.valueOf(this.mDeleteStats.get(uid)), Long.valueOf(this.mBatchStats.get(uid)), Long.valueOf(this.mInsertInBatchStats.get(uid)), Long.valueOf(this.mUpdateInBatchStats.get(uid)), Long.valueOf(this.mDeleteInBatchStats.get(uid)), Double.valueOf(((double) this.mOperationDurationMillis.get(uid)) / 1000.0d)));
            }
            pw.println();
        }
    }
