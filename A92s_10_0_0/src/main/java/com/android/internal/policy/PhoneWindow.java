...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    /* JADX INFO: Multiple debug info for r1v18 int: [D('res' android.util.TypedValue), D('layoutResource' int)] */
    /* access modifiers changed from: protected */
    public ViewGroup generateLayout(DecorView decor) {
        ...
        int flagsToUpdate = (~getForcedWindowFlags()) & OppoBaseNotificationChannel.USER_LOCKED_CHANGEABLE_SHOW_ICON;
        if (this.mIsFloating) {
            setLayout(-2, -2);
            setFlags(0, flagsToUpdate);
        } else {
            setFlags(OppoBaseNotificationChannel.USER_LOCKED_CHANGEABLE_SHOW_ICON, flagsToUpdate);
        }
        ...
        if (!this.mForcedNavigationBarColor) {
            this.mNavigationBarColor = ColorStatusBarController.getInstance().caculateSystemBarColor(context.getPackageName(), getContext().getClass().getName(), a.getColor(36, -1), 1);
            z = false;
        ...
        if (!targetPreQ) {
            this.mEnsureStatusBarContrastWhenTransparent = a.getBoolean(52, z);
            this.mEnsureNavigationBarContrastWhenTransparent = a.getBoolean(53, true);
        }
        WindowManager.LayoutParams params = getAttributes();
        if (!this.mIsFloating) {
            if (ColorStatusBarController.getInstance().shouldDrawSystemBarBackgroundColor(this.mIsStartingWindow, context.getPackageName()) || (!targetPreL && a.getBoolean(34, false))) {
