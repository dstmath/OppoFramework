...
public class DefaultPermissionGrantPolicy extends OppoBaseDefaultPermissionGrantPolicy {
    ...
    /* access modifiers changed from: private */
    public final PermissionManagerService mPermissionManager;
    ...
    DefaultPermissionGrantPolicy(Context context, Looper looper, PermissionManagerService permissionManager) {
        ...
        this.mColorDefaultPermissionGrantPolicyInner = new ColorDefaultPermissionGrantPolicyInner();
    }
    ...
    /* JADX INFO: Multiple debug info for r2v58 java.lang.String: [D('useOpenWifiPackageName' java.lang.String), D('dialerAppPackageNames' java.lang.String[])] */
    private void grantDefaultSystemHandlerPermissions(int userId) {
        ...
        grantPermissionsToSystemPackage(getDefaultProviderAuthorityPackage("com.android.contacts", userId), userId, CONTACTS_PERMISSIONS, PHONE_PERMISSIONS, STORAGE_PERMISSIONS);
    ...
    public void grantRuntimePermissions(PackageInfo pkg, Set<String> permissionsWithoutSplits, boolean systemFixed, boolean ignoreSystemPackage, boolean whitelistRestrictedPermissions, int userId) {
    ...
    public final class ColorDefaultPermissionGrantPolicyInner implements IColorDefaultPermissionGrantPolicyInner {
        public ColorDefaultPermissionGrantPolicyInner() {
        }

        @Override // com.android.server.pm.permission.IColorDefaultPermissionGrantPolicyInner
        public void grantRuntimePermissions(PackageInfo pkg, Set<String> permissions, boolean systemFixed, int userId) {
            DefaultPermissionGrantPolicy.this.grantRuntimePermissions(pkg, permissions, systemFixed, userId);
        }

        @Override // com.android.server.pm.permission.IColorDefaultPermissionGrantPolicyInner
        public void grantRuntimePermissions(PackageInfo pkg, Set<String> permissions, boolean systemFixed, boolean ignoreSystemPackage, boolean whitelistRestrictedPermissions, int userId) {
            DefaultPermissionGrantPolicy.this.grantRuntimePermissions(pkg, permissions, systemFixed, ignoreSystemPackage, whitelistRestrictedPermissions, userId);
        }

        @Override // com.android.server.pm.permission.IColorDefaultPermissionGrantPolicyInner
        public PermissionManagerService getPermissionManagerService() {
            return DefaultPermissionGrantPolicy.this.mPermissionManager;
        }

        @Override // com.android.server.pm.permission.IColorDefaultPermissionGrantPolicyInner
        public BasePermission getPermission(String permission) {
            return DefaultPermissionGrantPolicy.this.mPermissionManager.getPermission(permission);
        }
    }
                ...
                if (doesPackageSupportRuntimePermissions(pi)) {
                    if (pi.requestedPermissions.length != 0) {
                        ...
                        for (int i = 0; i < pi.requestedPermissions.length; i++) {
                            ...
                            BasePermission bp = this.mPermissionManager.getPermission(permission);
                            if (bp != null && bp.isRuntime() && CtaManagerFactory.getInstance().makeCtaManager().isCtaOnlyPermission(bp.getName())) {
                                permissions.add(permission);
                            }
                        }
                        if (!permissions.isEmpty()) {
                            grantRuntimePermissions(pi, permissions, isSysComponentOrPersistentPlatformSignedPrivApp(pi), userId);
                        }
                    }
                }
            }
