...
public class PackageManagerServiceCompilerMapping {
    ...
    public static final String[] REASON_STRINGS = {"first-boot", "boot", "install", "bg-dexopt", "ab-ota", "inactive", "shared", "core-app"};

    static {
        String[] strArr = REASON_STRINGS;
        if (8 != strArr.length) {
            throw new IllegalStateException("REASON_STRINGS not correct");
        } else if (!"core-app".equals(strArr[7])) {
    ...
    private static boolean isFilterAllowedForReason(int reason, String filter) {
        return reason != 7 || !DexFile.isProfileGuidedCompilerFilter(filter);
    }
