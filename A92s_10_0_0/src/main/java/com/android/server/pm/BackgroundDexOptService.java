...
public class BackgroundDexOptService extends JobService {
    ...
    public static void schedule(Context context) {
        if (!isBackgroundDexoptDisabled()) {
            ...
            js.schedule(new JobInfo.Builder(JOB_POST_BOOT_UPDATE, sDexoptServiceName).setMinimumLatency(TimeUnit.MINUTES.toMillis(1)).setOverrideDeadline(TimeUnit.MINUTES.toMillis(1)).setRequiresDeviceIdle(true).build());
        ...
        new Thread("BackgroundDexOptService_IdleOptimization") {
            ...
            public void run() {
                ...
                if (!BackgroundDexOptService.mUserUnlockedOnce && deviceUprading && BackgroundDexOptService.abortIdleDexoptCounter < 2) {
                    abortBgDexopt = true;
                    BackgroundDexOptService.access$208();
                }
                if (abortBgDexopt) {
