...
public class UsageStatsService extends SystemService implements UserUsageStatsService.StatsUpdatedListener {
                ...
                if (i == 1) {
                    ...
                    synchronized (UsageStatsService.this.mUidToKernelCounter) {
                        if (newCounter != UsageStatsService.this.mUidToKernelCounter.get(uid, 0)) {
                            ...
                            try {
                                FileUtils.stringToFile(UsageStatsService.KERNEL_COUNTER_FILE, uid + StringUtils.SPACE + newCounter);
                            } catch (IOException e) {
    ...
    private final class LocalService extends UsageStatsManagerInternal {
        ...
        public void updateAutoPowerSaveEnable(boolean enable) {
            if (enable != UsageStatsService.this.mAppStandby.getRunAutoPowerSaveValue()) {
                UsageStatsService.this.mAppStandby.setRunAutoPowerSaveValue(enable);
                UsageStatsService.this.mAppStandby.postOneTimeCheckIdleStates();
                Slog.d(UsageStatsService.TAG, "updateAutoPowerSaveEnable: mRusAutoPowerSaveEnable=" + UsageStatsService.this.mAppStandby.getRunAutoPowerSaveValue());
            }
        }
    }
}
