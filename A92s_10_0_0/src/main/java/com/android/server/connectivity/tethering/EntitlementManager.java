...
public class EntitlementManager {
    ...
    public boolean isTetherProvisioningRequired(TetheringConfiguration config) {
        if (this.mSystemProperties.getBoolean(DISABLE_PROVISIONING_SYSPROP_KEY, false) || config.provisioningApp.length == 0 || carrierConfigAffirmsEntitlementCheckNotRequired(config) || config.provisioningApp.length != 2) {
            return false;
        }
        return true;
    }
