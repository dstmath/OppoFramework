...
public class LockTaskController {
    ...
    static {
        ...
        STATUS_BAR_FLAG_MAP_LOCKED.append(8, new Pair<>(Integer.valueOf((int) DumpState.DUMP_SERVICE_PERMISSIONS), 0));
    ...
    LockTaskController(Context context, ActivityStackSupervisor supervisor, Handler handler) {
        ...
        OppoFeatureCache.get(IColorLockTaskController.DEFAULT).init(this.mContext, this.mSupervisor, this, this.mLockTaskModeTasks, this.mLockTaskPackages);
    }
    ...
    private boolean isLockTaskModeViolationInternal(TaskRecord task, boolean isNewClearTask) {
        if (OppoFeatureCache.get(IColorLockTaskController.DEFAULT).isLockDeviceMode()) {
            return (task.mLockTaskAuth == 3 || task.mLockTaskAuth == 2) ? false : true;
        }
        ...
        if (task.mLockTaskAuth != 1 || task.intent == null || !"android.intent.action.CHOOSER".equals(task.intent.getAction()) || !"coloros_multiapp_chooser".equals(task.affinity)) {
            return !isTaskWhitelisted(task) && !this.mLockTaskModeTasks.isEmpty();
        }
        return false;
    }
    ...
    public void stopLockTaskMode(TaskRecord task, boolean isSystemCaller, int callingUid) {
        if (this.mLockTaskModeState == 0 || OppoFeatureCache.get(IColorLockTaskController.DEFAULT).isLockDeviceMode()) {
            return;
        }
        if (isSystemCaller) {
    ...
    public void clearLockedTasks(String reason) {
        if (ActivityTaskManagerDebugConfig.DEBUG_LOCKTASK) {
            ...
            Slog.i(str, "clearLockedTasks: " + reason);
        }
        if (!this.mLockTaskModeTasks.isEmpty()) {
    ...
    private void removeLockedTask(TaskRecord task) {
        if (this.mLockTaskModeTasks.remove(task)) {
            if (ActivityTaskManagerDebugConfig.DEBUG_LOCKTASK) {
                String str = TAG_LOCKTASK;
                Slog.d(str, "removeLockedTask: removed " + task);
            }
            if (this.mLockTaskModeTasks.isEmpty()) {
                if (OppoFeatureCache.get(IColorLockTaskController.DEFAULT).isLockDeviceMode()) {
                    OppoFeatureCache.get(IColorLockTaskController.DEFAULT).stopLockDeviceModeBySystem();
                }
                if (ActivityTaskManagerDebugConfig.DEBUG_LOCKTASK) {
                    String str2 = TAG_LOCKTASK;
                    Slog.d(str2, "removeLockedTask: task=" + task + " last task, reverting locktask mode. Callers=" + Debug.getCallers(3));
                }
    ...
    public void startLockTaskMode(TaskRecord task, boolean isSystemCaller, int callingUid) {
        ...
        if (ActivityTaskManagerDebugConfig.DEBUG_LOCKTASK) {
            Slog.w(TAG_LOCKTASK, isSystemCaller ? "Locking pinned" : "Locking fully");
        }
    ...
    private void setLockTaskMode(TaskRecord task, int lockTaskModeState, String reason, boolean andResume) {
        if (task.mLockTaskAuth == 0) {
        ...
        } else if (OppoFeatureCache.get(IColorLockTaskController.DEFAULT).isLockDeviceMode() && (this.mLockTaskModeTasks.contains(task) || (this.mLockTaskModeTasks.isEmpty() && !task.mRootLockDeviceTask))) {
            ...
            if (ActivityTaskManagerDebugConfig.DEBUG_LOCKTASK) {
                String str = TAG_LOCKTASK;
                Slog.w(str, "setLockTaskMode: Locking to " + task + " Callers=" + Debug.getCallers(4));
            }
    ...
    public void dump(PrintWriter pw, String prefix) {
        ...
        for (int i = 0; i < this.mLockTaskModeTasks.size(); i++) {
            pw.println(prefix2 + "  #" + i + StringUtils.SPACE + this.mLockTaskModeTasks.get(i));
        }
        ...
        OppoFeatureCache.get(IColorLockTaskController.DEFAULT).dump(pw, prefix2);
    }
