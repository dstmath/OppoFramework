...
public final class InputMonitor {
    ...
    public void populateInputWindowHandle(InputWindowHandle inputWindowHandle, WindowState child, int flags, int type, boolean isVisible, boolean hasFocus, boolean hasWallpaper) {
        ...
        if (child.mYoffset < 0) {
            inputWindowHandle.frameTop = frame.top + child.mYoffset;
            inputWindowHandle.frameBottom = frame.bottom + child.mYoffset;
            inputWindowHandle.touchableRegion.op(inputWindowHandle.frameLeft, inputWindowHandle.frameTop, inputWindowHandle.frameRight, inputWindowHandle.frameBottom, Region.Op.REPLACE);
        }
        OppoFeatureCache.get(IColorZoomWindowManager.DEFAULT).adjustInputWindowHandle(this, child, inputWindowHandle);
        ...
        if (WindowManagerDebugConfig.DEBUG_INPUT) {
            Slog.d(WmsExt.TAG, "addInputWindowHandle: " + child + ", " + inputWindowHandle);
        }
        if (hasFocus) {
    ...
    public void setInputFocusLw(WindowState newWindow, boolean updateInputWindows) {
        if (WindowManagerDebugConfig.DEBUG_FOCUS_LIGHT || WindowManagerDebugConfig.DEBUG_INPUT) {
            Slog.d(WmsExt.TAG, "Input focus has changed to " + newWindow);
        }
    ...
    public final class UpdateInputForAllWindowsConsumer implements Consumer<WindowState> {
        ...
        /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
         ...
         arg types: [com.android.server.wm.InputMonitor$UpdateInputForAllWindowsConsumer, int]
         candidates:
        ...
        public void accept(WindowState w) {
            ...
            } else if (w.mWinAnimator.hasSurface()) {
