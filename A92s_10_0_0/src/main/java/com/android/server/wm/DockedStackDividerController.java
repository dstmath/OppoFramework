...
public class DockedStackDividerController {
    ...
    private void loadDimens() {
        ...
        this.mDividerWindowWidth = context.getResources().getDimensionPixelSize(17105142);
        this.mDividerInsets = context.getResources().getDimensionPixelSize(17105141);
        this.mDividerWindowWidthInactive = WindowManagerService.dipToPixel(2, this.mDisplayContent.getDisplayMetrics());
        this.mTaskHeightInMinimizedMode = context.getResources().getDimensionPixelSize(17105432);
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     ...
     arg types: [com.android.server.wm.-$$Lambda$vhwCX-wzYksBgFM46tASKUCeQRc, int]
     candidates:
    ...
    public void reevaluateVisibility(boolean force) {
        if (this.mWindow != null) {
            ...
            if (this.mLastVisibility != visible || force) {
                ...
                if (!visible) {
                    setResizeDimLayer(false, 0, OppoBrightUtils.MIN_LUX_LIMITI);
                }
            }
        }
    }
    ...
    static boolean isDockSideAllowed(int dockSide, int originalDockSide, int navBarPosition, boolean navigationBarCanMove) {
        ...
        if (navigationBarCanMove) {
            ...
            if ((dockSide == 1 || dockSide == 3) && originalDockSide == 2) {
                return true;
            }
            return false;
        }
    ...
    private void startImeAdjustAnimation(boolean adjustedForIme, boolean adjustedForDivider, WindowState imeWin) {
        ...
        if (!this.mService.mWaitingForDrawn.isEmpty()) {
            ...
            this.mService.mDockedForDrawnCallback = new Runnable(adjustedForIme, adjustedForDivider) {
    ...
    private boolean setMinimizedDockedStack(boolean minimized) {
        ...
        if (stack.setAdjustedForMinimizedDock(minimized ? 1.0f : OppoBrightUtils.MIN_LUX_LIMITI)) {
            return true;
        }
        return false;
    }
    ...
    private float getClipRevealMeetFraction(TaskStack stack) {
        ...
        return ((1.0f - Math.max((float) OppoBrightUtils.MIN_LUX_LIMITI, Math.min(1.0f, ((((float) Math.abs(this.mDisplayContent.mAppTransition.getLastClipRevealMaxTranslation())) / ((float) stack.getMinimizeDistance())) - CLIP_REVEAL_MEET_FRACTION_MIN) / CLIP_REVEAL_MEET_FRACTION_MIN))) * 0.39999998f) + CLIP_REVEAL_MEET_EARLIEST;
    }
