...
public class WindowContainer<E extends WindowContainer> extends ConfigurationContainer<E> implements Comparable<WindowContainer>, SurfaceAnimator.Animatable {
    ...
    WindowContainer(WindowManagerService wms) {
        ...
        this.mSurfaceAnimator = new SurfaceAnimator(this, new Runnable() {
            /* class com.android.server.wm.$$Lambda$yVRF8YoeNdTa8GR1wDStVsHu8xM */
    ...
    public int getOrientation(int candidate) {
        ...
        for (int i2 = this.mChildren.size() - 1; i2 >= 0; i2--) {
            ...
            if (wc == null || !(wc instanceof TaskStack) || !((TaskStack) wc).inZoomWindowingMode()) {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     ...
     arg types: [com.android.server.wm.WindowContainer<E>$ForAllWindowsConsumerWrapper, boolean]
     candidates:
    ...
    /* access modifiers changed from: package-private */
    public int getWindowNum(Predicate<WindowState> callback) {
        int wc = 0;
        for (int i = this.mChildren.size() - 1; i >= 0; i--) {
            if (this.mChildren.get(i).getWindow(callback) != null) {
                wc++;
            }
        }
        return wc;
    ...
    public boolean shouldMagnify() {
        ...
        for (int i = 0; i < this.mChildren.size(); i++) {
            if (!this.mChildren.get(i).shouldMagnify(this.mTmpOffset)) {
                return false;
            }
        }
        return true;
    ...
    /* access modifiers changed from: package-private */
    public boolean shouldMagnify(int offset) {
        this.mTmpOffset = offset;
    ...
    public void applyMagnificationSpec(SurfaceControl.Transaction t, MagnificationSpec spec) {
        if (spec != null) {
            this.mTmpOffset = (int) spec.offsetY;
        }
        if (shouldMagnify()) {
            t.setMatrix(this.mSurfaceControl, spec.scale, OppoBrightUtils.MIN_LUX_LIMITI, OppoBrightUtils.MIN_LUX_LIMITI, spec.scale).setPosition(this.mSurfaceControl, spec.offsetX, spec.offsetY);
            return;
        }
        for (int i = 0; i < this.mChildren.size(); i++) {
    ...
    public void startAnimation(SurfaceControl.Transaction t, AnimationAdapter anim, boolean hidden) {
        if (WindowManagerDebugConfig.DEBUG_ANIM) {
            Slog.v("WindowManager", "Starting animation on " + this + ": " + anim);
        }
