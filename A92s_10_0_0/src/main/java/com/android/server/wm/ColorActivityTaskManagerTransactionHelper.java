...
public final class ColorActivityTaskManagerTransactionHelper extends ColorActivityTaskManagerCommonHelper implements IColorActivityTaskManager {
    ...
    private List<ColorAppInfo> getAllTopAppInfosLocked() {
        ...
        int displayId2;
        ColorActivityTaskManagerTransactionHelper colorActivityTaskManagerTransactionHelper = this;
        List<ColorAppInfo> list = new ArrayList<>();
        int displayId3 = 0;
        ActivityDisplay display2 = colorActivityTaskManagerTransactionHelper.mAtms.mRootActivityContainer.getActivityDisplay(0);
        if (display2 != null) {
            boolean primary = false;
            boolean second = false;
            boolean fullscreen = false;
            boolean zoomscreen = false;
            int stackNdx = display2.getChildCount() - 1;
            while (true) {
                if (stackNdx < 0) {
                    break;
                }
                    ...
                    if (windowingMode == 2) {
                        ...
                        if (windowingMode == 1) {
                            if (ActivityTaskManagerDebugConfig.DEBUG_STACK || ActivityTaskManagerDebugConfig.DEBUG_TASKS) {
                                ...
                                sb.append("getAllTopAppInfos fullscreen top = ");
                                sb.append(top);
                        ...
                        moreVisAppInfo = colorActivityTaskManagerTransactionHelper.getMoreVisibleAppInfoLocked(stack, top);
                ...
                stackNdx--;
                colorActivityTaskManagerTransactionHelper = this;
                displayId3 = displayId;
            }
            if (ActivityTaskManagerDebugConfig.DEBUG_STACK || ActivityTaskManagerDebugConfig.DEBUG_TASKS) {
                Slog.i(TAG, "getAllTopAppInfos have done");
            }
        }
        return list;
    }
