...
class ActivityMetricsLogger {
    ...
    public void lambda$logAppTransitionMultiEvents$1$ActivityMetricsLogger(WindowingModeTransitionInfoSnapshot info) {
        if (info.type == 8 || info.type == 7) {
            ...
            MtkSystemServer instance = MtkSystemServer.getInstance();
            instance.addBootEvent("AP_Launch: " + info.launchedActivityShortComponentName + StringUtils.SPACE + info.windowsDrawnDelayMs + "ms");
        }
    }
    ...
    public WindowingModeTransitionInfoSnapshot logAppTransitionReportedDrawn(ActivityRecord r, boolean restoredFromBundle) {
        ...
        Log.p("Quality", "LaunchTime: " + r.shortComponentName + StringUtils.SPACE + startupTimeMs + StringUtils.SPACE + startupTimeMs);
