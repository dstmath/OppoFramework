...
public class AppTransition implements DumpUtils.Dump {
    ...
    /* JADX DEBUG: Multi-variable search result rejected for TypeSearchVarInfo{r12v8, resolved type: android.view.animation.AnimationSet} */
    /* JADX WARN: Multi-variable type inference failed */
        ...
        if (!freeform || !scaleUp) {
            if (freeform) {
                ...
                if (shouldScaleDownThumbnailTransition(uiMode, orientation)) {
                    ...
                    if (scaleUp) {
                        translateAnim2 = createCurvedMotion(scale2, OppoBrightUtils.MIN_LUX_LIMITI, startY - ((float) contentInsets.top), OppoBrightUtils.MIN_LUX_LIMITI);
                    } else {
                        translateAnim2 = createCurvedMotion(OppoBrightUtils.MIN_LUX_LIMITI, scale2, OppoBrightUtils.MIN_LUX_LIMITI, startY - ((float) contentInsets.top));
                    }
