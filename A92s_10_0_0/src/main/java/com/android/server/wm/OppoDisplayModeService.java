...
public class OppoDisplayModeService extends IOppoScreenModeHook {
    ...
    public DisplayInfo mDefaultDisplayInfo;
    ...
    /* access modifiers changed from: private */
    public final HashMap<String, Item> mMap;
    private SettingsObserver mSettingsObserver;
    /* access modifiers changed from: private */
    public final ArrayList<String> mTPBoostList;
    private RefreshRateConfigUpdater mUpdater;
    /* access modifiers changed from: private */
    public SparseArray<Vote> mVotes;
            ...
            if (0 < args.length) {
                ...
                if ("set_enable".equals(cmd)) {
                    ...
                    if (opti4 < args.length) {
                        OppoRefreshRateConstants.DEBUG = args[opti4].equals("1");
                    }
                } else if ("simulate_ratingapp".equals(cmd)) {
                    int opti5 = 0 + 1;
                    if (opti5 < args.length) {
                        boolean open = args[opti5].equals("1");
                        int rateId = 0;
                        rateId = 0;
                        if (open) {
                            try {
                    ...
                    setHighTemperatureStatus(args[opti].equals("1") ? 1 : 0, 0);
                }
            } else if (!this.mEnable) {
                pw.println("Disabled");
            } else {
    ...
    /* access modifiers changed from: package-private */
    public boolean canOverrideRefreshRate(WindowState w) {
        if (!this.mEnable) {
            return false;
        }
        Item item = findPolicyItem(w);
        if (item == null || !item.mDisableViewOverride) {
            return true;
        }
        return false;
    }
    ...
    public int getPreferredModeId(int windowPreferredId, DisplayInfo info) {
        ...
        if (windowPreferredId > 0) {
            synchronized (this.mLock) {
                updateVoteLocked(0, Vote.forRefreshRate(windowPreferredId));
    ...
    private Item findPolicyItem(WindowState w) {
        ...
        String actName = w.mAppToken != null ? w.mAppToken.mActivityComponent.getClassName() : null;
    ...
    private int[] getDisplayModeId(Display.Mode[] supportedModes, int width, int height, float minRefreshRate, float maxRefreshRate) {
        ...
        for (Display.Mode mode : supportedModes) {
            if (mode.getPhysicalWidth() == width && mode.getPhysicalHeight() == height) {
                ...
                if (refreshRate >= minRefreshRate - 0.001f && refreshRate <= maxRefreshRate + 0.001f) {
    ...
    /* access modifiers changed from: private */
    public int getFinalDisplayModeIdLocked(SparseArray<Vote> votes, DisplayInfo info) {
        int refreshRateId = -1;
        int priority = 3;
        while (true) {
            if (priority >= 0) {
    ...
    /* access modifiers changed from: private */
    public void notifyDisplayManager(int displayModeId) {
    ...
    private static class LazyHolder {
        /* access modifiers changed from: private */
        public static final OppoDisplayModeService INSTANCE = new OppoDisplayModeService();
    ...
    public static class Item {
        ...
        String mPkgName;
        /* access modifiers changed from: private */
        public OppoRefreshRateConstants.PreferredRefreshRateData mPreferredRefreshRateId;

        Item(String pkgName, String actName, String refreshRateStr) {
            this(pkgName, actName, refreshRateStr, false);
        }

        Item(String pkgName, String actName, String refreshRateStr, boolean disableOverride) {
        ...
        public String toString() {
            return "Item{mPkgName='" + this.mPkgName + '\'' + ", mActivityName='" + this.mActivityName + '\'' + ", mPreferredRefreshRateId=" + this.mPreferredRefreshRateId + ", mDisableViewOverride=" + this.mDisableViewOverride + '}';
        }
    }
    ...
    private class RefreshRateConfigUpdater extends RomUpdateHelper {
        ...
        private static final String SYS_FILE_DIR = "system/etc/refresh_rate_config.xml";
        ...
        public void init() {
            String data_config_content = getFileContent(DATA_FILE_DIR);
            int data_config_version = getConfigVersion(data_config_content);
            String system_config_content = getFileContent(SYS_FILE_DIR);
        ...
        /* access modifiers changed from: private */
        public Item restoreRow(XmlPullParser parser, int depth) throws IOException {
            return new Item(XmlUtils.readStringAttribute(parser, "packageName"), XmlUtils.readStringAttribute(parser, ATTR_ACTNAME), XmlUtils.readStringAttribute(parser, ATTR_PREFERREDREFRESH), XmlUtils.readBooleanAttribute(parser, ATTR_DISABLEOVERRIDE, false));
        }
                ...
                if (!TextUtils.isEmpty(content)) {
                    try {
                        ...
                        if (version > RefreshRateConfigUpdater.this.CURRENT_VERSION) {
                            ...
                            int unused = RefreshRateConfigUpdater.this.CURRENT_VERSION = version;
