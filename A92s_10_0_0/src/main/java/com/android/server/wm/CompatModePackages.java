...
public final class CompatModePackages {
                ...
                if ("compat-packages".equals(parser.getName())) {
                    ...
                    do {
                        if (eventType2 == 2) {
                            ...
                            if (parser.getDepth() == 2 && "pkg".equals(tagName) && (pkg = parser.getAttributeValue(null, Settings.ATTR_NAME)) != null) {
    ...
    public CompatibilityInfo compatibilityInfoForPackageLocked(ApplicationInfo ai) {
        float scale = WindowManagerService.isDisplayCompat(ai.packageName, ai.uid);
        boolean z = false;
        if (scale <= OppoBrightUtils.MIN_LUX_LIMITI || scale == 1.0f) {
            ai.overrideDensity = 0;
            ai.privateFlags &= Integer.MAX_VALUE;
            ai.setNewAppScale(1.0f);
        } else {
            ai.overrideDensity = (int) (((float) DisplayMetrics.DENSITY_DEVICE) / scale);
            ai.privateFlags |= Integer.MIN_VALUE;
            ai.setNewAppScale(scale);
            Slog.i(TAG, "DisplayCompat: compatibilityInfoForPackageLocked for " + ai);
        }
