...
public class TaskStack extends OppoBaseTaskStack implements BoundsAnimationTarget, ConfigurationContainerListener {
    ...
    private int setBounds(Rect existing, Rect bounds) {
        if (WindowManagerDebugConfig.DEBUG_STACK) {
            Slog.d(WmsExt.TAG, "setBounds bound = " + bounds + ", stackId = " + this.mStackId, new Throwable("setBounds"));
        }
    ...
    public void positionChildAt(Task child, int position) {
        if (WindowManagerDebugConfig.DEBUG_STACK) {
            Slog.i(WmsExt.TAG, "positionChildAt: positioning task=" + child + " at " + position);
        }
        if (child != null) {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: com.android.server.wm.TaskStack.positionChildAt(int, com.android.server.wm.Task, boolean):void
     arg types: [int, com.android.server.wm.Task, boolean]
     candidates:
      com.android.server.wm.TaskStack.positionChildAt(int, com.android.server.wm.Task, boolean):void
    ...
    private void positionChildAt(int position, Task child, boolean includingParents, boolean showForAllUsers) {
        ...
        if (WindowManagerDebugConfig.DEBUG_TASK_MOVEMENT) {
            Slog.d(WmsExt.TAG, "positionTask: task=" + this + " position=" + position);
        }
        EventLog.writeEvent((int) EventLogTags.WM_TASK_MOVED, Integer.valueOf(child.mTaskId), Integer.valueOf(targetPosition == this.mChildren.size() - 1 ? 1 : 0), Integer.valueOf(targetPosition));
    }
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     ...
     arg types: [com.android.server.wm.-$$Lambda$TaskStack$PVMhxGhbT6eBbe3ARm5uodEqxDE, int]
     candidates:
    ...
    @Override // com.android.server.wm.WindowContainer, com.android.server.wm.OppoBaseTaskStack, com.android.server.wm.ConfigurationContainer
        ...
        if (dockedBounds != null) {
            if (!dockedBounds.isEmpty()) {
                ...
                if (isActivityTypeHome()) {
                    ...
                    Slog.e(WmsExt.TAG, "Failed to get valid docked side for docked stack");
    ...
    public void setAdjustedForIme(WindowState imeWin, boolean keepLastAmount) {
        ...
        if (!this.mAdjustedForIme || keepLastAmount) {
            ...
            float adjustDividerAmount = OppoBrightUtils.MIN_LUX_LIMITI;
    ...
    public void resetAdjustedForIme(boolean adjustBoundsNow) {
        if (adjustBoundsNow) {
            this.mImeWin = null;
            this.mImeGoingAway = false;
            this.mAdjustImeAmount = OppoBrightUtils.MIN_LUX_LIMITI;
            this.mAdjustDividerAmount = OppoBrightUtils.MIN_LUX_LIMITI;
    ...
    public void applyAdjustForImeIfNeeded(Task task) {
        if (this.mMinimizeAmount == OppoBrightUtils.MIN_LUX_LIMITI && this.mAdjustedForIme && !this.mAdjustedBounds.isEmpty()) {
    ...
    public boolean isAdjustedForMinimizedDockedStack() {
        return this.mMinimizeAmount != OppoBrightUtils.MIN_LUX_LIMITI;
    }
    ...
    public void dump(PrintWriter pw, String prefix, boolean dumpAll) {
        ...
        if (this.mMinimizeAmount != OppoBrightUtils.MIN_LUX_LIMITI) {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: com.android.server.wm.WindowContainer.forAllWindows(java.util.function.Consumer<com.android.server.wm.WindowState>, boolean):void
     arg types: [com.android.server.wm.-$$Lambda$TaskStack$NPerlV3pAikqmRCCx3JO0qCLTyw, int]
     candidates:
      com.android.server.wm.WindowContainer.forAllWindows(com.android.internal.util.ToBooleanFunction<com.android.server.wm.WindowState>, boolean):boolean
      com.android.server.wm.WindowContainer.forAllWindows(java.util.function.Consumer<com.android.server.wm.WindowState>, boolean):void */
    ...
    public void animateResizePinnedStack(Rect toBounds, Rect sourceHintBounds, int animationDuration, boolean fromFullscreen) {
        ...
        if (inPinnedWindowingMode()) {
            ...
            if (intendedAnimationType2 == 1) {
                if (fromFullscreen) {
                    setPinnedStackAlpha(OppoBrightUtils.MIN_LUX_LIMITI);
                }
