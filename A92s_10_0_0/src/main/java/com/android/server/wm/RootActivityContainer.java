...
public class RootActivityContainer extends ConfigurationContainer implements DisplayManager.DisplayListener {
    ...
    public boolean startHomeOnDisplay(int userId, String reason, int displayId, boolean allowInstrumenting, boolean fromHomeKey) {
        ...
        if (Settings.Secure.getIntForUser(this.mService.mContext.getContentResolver(), "user_setup_complete", 0, userId) != 0) {
            try {
                ActivityInfo defaultLauncherInfo = resolveActivityInfo(homeIntent, 787456, userId);
                if (defaultLauncherInfo != null && !"com.oppo.launcher".equals(defaultLauncherInfo.applicationInfo.packageName)) {
                    UserManager userManager = (UserManager) this.mService.mContext.getSystemService(UserManager.class);
                    Slog.d(TAG, "startHomeOnDisplay: " + defaultLauncherInfo.applicationInfo.packageName + "," + defaultLauncherInfo.name);
                    if (userManager == null || !userManager.isUserUnlocked(userId)) {
                        try {
                            ActivityInfo info2 = AppGlobals.getPackageManager().getActivityInfo(new ComponentName("com.android.settings", "com.android.settings.FallbackHome"), 525312, userId);
                            if (info2 != null) {
                                aInfo = info2;
                            }
    ...
    private ActivityInfo resolveActivityInfo(Intent intent, int flags, int userId) {
        ...
        if (comp != null) {
            ...
            ResolveInfo info = AppGlobals.getPackageManager().resolveIntent(intent, intent.resolveTypeIfNeeded(this.mService.mContext.getContentResolver()), flags, userId);
            if (info != null) {
        ...
        if (activityDisplay != null) {
            ...
            if (stack != null) {
                ...
                if (currentDisplay == null) {
                    ...
                    Slog.e(TAG, "Can not move stack=" + stack + " to single task instance display=" + activityDisplay);
                }
            } else {
    ...
    public boolean moveTopStackActivityToPinnedStack(int stackId) {
        ...
        if (stack != null) {
            ...
            if (r == null) {
                Slog.w(TAG, "moveTopStackActivityToPinnedStack: No top running activity in stack=" + stack);
                return false;
            } else if (this.mService.mForceResizableActivities || r.supportsPictureInPicture()) {
                moveActivityToPinnedStack(r, null, OppoBrightUtils.MIN_LUX_LIMITI, "moveTopActivityToPinnedStack");
                return true;
            } else {
                Slog.w(TAG, "moveTopStackActivityToPinnedStack: Picture-In-Picture not supported for  r=" + r);
                return false;
            }
        } else {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     ...
     arg types: [com.android.server.wm.ActivityStack, int, int, int, int, int, java.lang.String]
     candidates:
    ...
    public boolean resumeFocusedStacksTopActivities(ActivityStack targetStack, ActivityRecord target, ActivityOptions targetOptions) {
        ...
        OppoFeatureCache.get(IColorZoomWindowManager.DEFAULT).pauseZoomWindowActivity();
        return result;
    }
    ...
    public <T extends ActivityStack> T getLaunchStack(ActivityRecord r, ActivityOptions options, TaskRecord candidateTask, boolean onTop) {
        return getLaunchStack(r, options, candidateTask, onTop, null);
    }
    ...
    public <T extends ActivityStack> T getLaunchStack(ActivityRecord r, ActivityOptions options, TaskRecord candidateTask, boolean onTop, LaunchParamsController.LaunchParams launchParams) {
        ...
        if (displayId != -1 && canLaunchOnDisplay(r, displayId)) {
            if (r != null && (stack3 = getValidLaunchStackOnDisplay(displayId, r, candidateTask, options, launchParams)) != null) {
                return stack3;
            }
    ...
    public boolean allResumedActivitiesIdle() {
        ...
        while (displayNdx >= 0) {
            ...
            if (display.isSleeping() || (stack = display.getFocusedStack()) == null || stack.numActivities() == 0 || ((resumedActivity = stack.getResumedActivity()) != null && resumedActivity.idle)) {
                ...
                Slog.d(TAG_STATES, "allResumedActivitiesIdle: stack=" + stack.mStackId + StringUtils.SPACE + resumedActivity + " not idle");
                return false;
            }
        }
        ...
        for (int displayNdx = this.mActivityDisplays.size() - 1; displayNdx >= 0; displayNdx--) {
            ...
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; stackNdx--) {
                ...
                if (r != null && !r.isState(ActivityStack.ActivityState.PAUSED, ActivityStack.ActivityState.STOPPED, ActivityStack.ActivityState.STOPPING)) {
                    ...
                    Slog.d(TAG_STATES, "allPausedActivitiesComplete: r=" + r + " state=" + r.getState());
                    pausing = false;
                }
            }
        }
