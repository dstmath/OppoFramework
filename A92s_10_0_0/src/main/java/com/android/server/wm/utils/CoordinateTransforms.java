...
public class CoordinateTransforms {
    ...
    public static void transformPhysicalToLogicalCoordinates(int rotation, int physicalWidth, int physicalHeight, Matrix out) {
        if (rotation == 0) {
            ...
            out.postTranslate(OppoBrightUtils.MIN_LUX_LIMITI, (float) physicalWidth);
        } else if (rotation == 2) {
            out.setRotate(180.0f);
            out.postTranslate((float) physicalWidth, (float) physicalHeight);
        } else if (rotation == 3) {
            out.setRotate(90.0f);
            out.postTranslate((float) physicalHeight, OppoBrightUtils.MIN_LUX_LIMITI);
        } else {
    ...
    public static void transformLogicalToPhysicalCoordinates(int rotation, int physicalWidth, int physicalHeight, Matrix out) {
        if (rotation == 0) {
            ...
            out.setRotate(90.0f);
            out.preTranslate(OppoBrightUtils.MIN_LUX_LIMITI, (float) (-physicalWidth));
        } else if (rotation == 2) {
            out.setRotate(180.0f);
            out.preTranslate((float) (-physicalWidth), (float) (-physicalHeight));
        } else if (rotation == 3) {
            out.setRotate(270.0f);
            out.preTranslate((float) (-physicalHeight), OppoBrightUtils.MIN_LUX_LIMITI);
        } else {
            throw new IllegalArgumentException("Unknown rotation: " + rotation);
        }
    }
