...
public class ColorStorageUtils {
    ...
    public static long getDataFreeSpace() {
        ...
        if (statFs == null) {
            ...
            long freeDataSpace2 = sDataLowThreshold + 629145600;
            sDataFreeSpace = freeDataSpace2;
            return freeDataSpace2;
        }
        try {
    ...
    public static String formatBytesLocked(long bytes) {
        ...
        if (bytes < 1024) {
            ...
            formatter.format("%.2fKB", Double.valueOf(((double) bytes) / 1024.0d));
            return formatBuilder.toString();
        } else if (bytes < GB_BYTES) {
            formatter.format("%.2fMB", Double.valueOf(((double) bytes) / 1048576.0d));
            return formatBuilder.toString();
        } else {
            formatter.format("%.2fGB", Double.valueOf(((double) bytes) / 1.073741824E9d));
            return formatBuilder.toString();
        }
    }
