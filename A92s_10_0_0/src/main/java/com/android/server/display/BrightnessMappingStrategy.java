...
public abstract class BrightnessMappingStrategy {
    ...
    public static BrightnessMappingStrategy create(Resources resources) {
        ...
        float[] brightnessLevelsNits = getFloatArray(resources.obtainTypedArray(17235988));
        float autoBrightnessAdjustmentMaxGamma = resources.getFraction(18022400, 1, 1);
        float[] nitsRange = getFloatArray(resources.obtainTypedArray(17236064));
        int[] backlightRange = resources.getIntArray(17236063);
        mOppoBrightUtils = OppoBrightUtils.getInstance();
        OppoBrightUtils oppoBrightUtils = mOppoBrightUtils;
        if (OppoBrightUtils.mOppoScreenBrightnessSupport) {
            return new OppoMappingStrategy();
        }
    ...
    private static boolean isValidMapping(float[] x, float[] y) {
        ...
        float prevX = x[0];
        float prevY = y[0];
        if (prevX < OppoBrightUtils.MIN_LUX_LIMITI || prevY < OppoBrightUtils.MIN_LUX_LIMITI || Float.isNaN(prevX) || Float.isNaN(prevY)) {
            return false;
        }
        for (int i = 1; i < N; i++) {
    ...
    private static boolean isValidMapping(float[] x, int[] y) {
        ...
        float prevX = x[0];
        int prevY = y[0];
        if (prevX < OppoBrightUtils.MIN_LUX_LIMITI || prevY < 0 || Float.isNaN(prevX)) {
            return false;
        }
        for (int i = 1; i < N; i++) {
    ...
    public float inferAutoBrightnessAdjustment(float maxGamma, float desiredBrightness, float currentBrightness) {
        ...
        } else if (desiredBrightness == OppoBrightUtils.MIN_LUX_LIMITI) {
            adjustment = -1.0f;
    ...
    static class PhysicalMappingStrategy extends BrightnessMappingStrategy {
        ...
        public PhysicalMappingStrategy(BrightnessConfiguration config, float[] nits, int[] backlight, float maxGamma) {
            ...
            Preconditions.checkArrayElementsInRange(nits, (float) OppoBrightUtils.MIN_LUX_LIMITI, Float.MAX_VALUE, "nits");
