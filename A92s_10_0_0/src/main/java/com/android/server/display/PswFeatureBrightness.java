...
public class PswFeatureBrightness implements IPswFeatureBrightness {
    ...
    public int putBrightnessTodatabase(int target) {
        if (OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getmScreenGlobalHBMSupport() && OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getsGlobalHbmSellMode() == 1) {
            target = target > OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getTEN_BITS_MAXBRIGHTNESS() ? OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getTEN_BITS_MAXBRIGHTNESS() : target;
        ...
        if (OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getmSetBrihgtnessSlide()) {
            if ((OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getmScreenGlobalHBMSupport() && target <= OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getTEN_BITS_MAXBRIGHTNESS()) || !OppoFeatureCache.getOrCreate(IOppoBrightness.DEFAULT, new Object[0]).getmScreenGlobalHBMSupport()) {
