...
public class BatterySaverStateMachine extends OppoBaseBatterySaverStateMachine {
    ...
    public void onBootCompleted() {
        ...
        OppoFeatureCache.get(IColorBatterySaveExtend.DEFAULT).onBatterySaveChanged(this.mContext.getContentResolver(), false);
    ...
    public /* synthetic */ void lambda$onBootCompleted$0$BatterySaverStateMachine() {
        ...
        synchronized (this.mLock) {
            ...
            OppoFeatureCache.get(IColorBatterySaveExtend.DEFAULT).setSystemStartup(true);
    ...
    private void doAutoBatterySaverLocked() {
        if (!OppoFeatureCache.get(IColorBatterySaveExtend.DEFAULT).isClosedSuperFirewall(this.mContext.getPackageManager())) {
            Slog.d(TAG, "doAutoBatterySaverLocked: ignore");
        } else if (this.mBootCompleted && this.mSettingsLoaded && this.mBatteryStatusSet) {
    ...
    public void setBatterySaverEnabledManually(boolean enabled) {
        int i;
        synchronized (this.mLock) {
            OppoFeatureCache.get(IColorBatterySaveExtend.DEFAULT).setSystemStartup(false);
            if (enabled) {
                i = 2;
            } else {
                i = 3;
    ...
    private void enableBatterySaverLocked(boolean enable, boolean manual, int intReason, String strReason) {
        if (this.mBatterySaverController.isFullEnabled() != enable) {
            if (!enable || !this.mIsPowered || !OppoFeatureCache.get(IColorBatterySaveExtend.DEFAULT).isClosedSuperFirewall(this.mContext.getPackageManager())) {
                ...
                OppoFeatureCache.get(IColorBatterySaveExtend.DEFAULT).onBatterySaveChanged(this.mContext.getContentResolver(), enable);
                Slog.d(TAG, "enableBatterySaver: Enabled=" + enable + " manual=" + manual + " reason=" + strReason + "(" + intReason + ")");
                if (intReason == 9) {
                    runOnBgThread(new Runnable() {
    ...
    public void triggerDynamicModeNotification() {
        ...
        ensureNotificationChannelExists(manager, DYNAMIC_MODE_NOTIF_CHANNEL_ID, 17039882);
        manager.notifyAsUser(TAG, DYNAMIC_MODE_NOTIFICATION_ID, buildNotification(DYNAMIC_MODE_NOTIF_CHANNEL_ID, this.mContext.getResources().getString(17039884), 17039883, "android.intent.action.POWER_USAGE_SUMMARY"), UserHandle.ALL);
    }
    ...
    private Notification buildNotification(String channelId, String title, int summaryId, String intentAction) {
        ...
        return new Notification.Builder(this.mContext, channelId).setSmallIcon(17302301).setContentTitle(title).setContentText(summary).setContentIntent(batterySaverIntent).setStyle(new Notification.BigTextStyle().bigText(summary)).setOnlyAlertOnce(true).setAutoCancel(true).build();
    }
