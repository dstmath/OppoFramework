...
final class UiModeManagerService extends SystemService {
    ...
    public final IUiModeManager.Stub mService = new IUiModeManager.Stub() {
        ...
        public void setNightMode(int mode) {
            if (isNightModeLocked() && UiModeManagerService.this.getContext().checkCallingOrSelfPermission("android.permission.MODIFY_DAY_NIGHT_MODE") != 0) {
                ...
                try {
                    synchronized (UiModeManagerService.this.mLock) {
                        int mode2 = OppoFeatureCache.getOrCreate(IColorDarkModeManager.DEFAULT, new Object[0]).hideAutoChangeUiMode(mode);
                        if (UiModeManagerService.this.mNightMode != mode2) {
                            if (!UiModeManagerService.this.mCarModeEnabled) {
                                Settings.Secure.putIntForUser(UiModeManagerService.this.getContext().getContentResolver(), "ui_night_mode", mode2, user);
                                if (UserManager.get(UiModeManagerService.this.getContext()).isPrimaryUser()) {
                                    SystemProperties.set(UiModeManagerService.SYSTEM_PROPERTY_DEVICE_THEME, Integer.toString(mode2));
                                }
                            }
                            int unused = UiModeManagerService.this.mNightMode = mode2;
                            UiModeManagerService.this.updateLocked(0, 0);
                        }
                    }
                } finally {
    ...
    public void onStart() {
        ...
        this.mEnableCarDockLaunch = res.getBoolean(17891435);
        this.mUiModeLocked = res.getBoolean(17891476);
        this.mNightModeLocked = res.getBoolean(17891475);
    ...
    public /* synthetic */ void lambda$onStart$0$UiModeManagerService(PowerSaveState state) {
        synchronized (this.mLock) {
            if (this.mPowerSave != state.batterySaverEnabled) {
                if (OppoFeatureCache.getOrCreate(IColorDarkModeManager.DEFAULT, new Object[0]).useForcePowerSave()) {
    ...
    public boolean updateNightModeFromSettings(Context context, Resources res, int userId) {
        ...
        this.mNightMode = OppoFeatureCache.getOrCreate(IColorDarkModeManager.DEFAULT, new Object[0]).hideAutoChangeUiMode(this.mNightMode);
        if (oldNightMode != this.mNightMode) {
            return true;
        }
        return false;
    }
    ...
    private void updateConfigurationLocked() {
        ...
        if (this.mPowerSave && !this.mCarModeEnabled && OppoFeatureCache.getOrCreate(IColorDarkModeManager.DEFAULT, new Object[0]).useForcePowerSave()) {
            uiMode = (uiMode & -17) | 32;
        }
    ...
    private void adjustStatusBarCarModeLocked() {
        ...
        if (this.mCarModeEnabled) {
            this.mNotificationManager.notifyAsUser(null, 10, new Notification.Builder(context, SystemNotificationChannels.CAR_MODE).setSmallIcon(17303509).setDefaults(4).setOngoing(true).setWhen(0).setColor(context.getColor(17170460)).setContentTitle(context.getString(17039644)).setContentText(context.getString(17039643)).setContentIntent(PendingIntent.getActivityAsUser(context, 0, new Intent(context, DisableCarModeActivity.class), 0, null, UserHandle.CURRENT)).build(), UserHandle.ALL);
            return;
        }
