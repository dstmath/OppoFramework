...
public class MediaSessionService extends SystemService implements Watchdog.Monitor {
    static final boolean DEBUG = (Log.isLoggable(TAG, 3) || "eng".equals(Build.TYPE) || "userdebug".equals(Build.TYPE));
    ...
    public void destroySessionLocked(MediaSessionRecord session) {
        ...
        if (this.mGlobalPrioritySession == session) {
            ...
            if (!(user == null || user.mLastMediaButtonReceiver == null || (componentName2 = user.mLastMediaButtonReceiver.getIntent().getComponent()) == null || !session.getPackageName().equals(componentName2.getPackageName()))) {
                PendingIntent unused = user.mLastMediaButtonReceiver = null;
            }
            if (!(user == null || user.mRestoredMediaButtonReceiver == null || !session.getPackageName().equals(user.mRestoredMediaButtonReceiver.getPackageName()))) {
                ComponentName unused2 = user.mRestoredMediaButtonReceiver = null;
            }
            if (session.isActive() && user != null) {
                user.pushAddressedPlayerChangedLocked();
            }
        } else if (user != null) {
            if (!(user.mLastMediaButtonReceiver == null || (componentName = user.mLastMediaButtonReceiver.getIntent().getComponent()) == null || !session.getPackageName().equals(componentName.getPackageName()))) {
                PendingIntent unused3 = user.mLastMediaButtonReceiver = null;
            }
            if (user.mRestoredMediaButtonReceiver != null && session.getPackageName().equals(user.mRestoredMediaButtonReceiver.getPackageName())) {
                ComponentName unused4 = user.mRestoredMediaButtonReceiver = null;
            }
    ...
    class SessionManagerImpl extends ISessionManager.Stub {
        ...
        public boolean isGlobalPriorityActive() {
            boolean access$1700;
            synchronized (MediaSessionService.this.mLock) {
                access$1700 = MediaSessionService.this.isGlobalPriorityActiveLocked();
            }
            return access$1700;
        }
        ...
        public void dispatchMediaKeyEventLocked(String packageName, int pid, int uid, boolean asSystemService, KeyEvent keyEvent, boolean needWakeLock) {
            ...
            if (session != null) {
                ...
                OppoFeatureCache.get(IColorHansManager.DEFAULT).hansMediaEventIfNeeded(session.getUid(), session.getPackageName());
                if (needWakeLock) {
                ...
                keyIntent.setPackage("com.oppo.music");
                if (needWakeLock) {
                ...
                try {
                    if (MediaSessionService.this.mCurrentFullUserRecord.mLastMediaButtonReceiver != null) {
                        ...
                        Context access$1900 = MediaSessionService.this.mContext;
                        if (needWakeLock) {
                            i = this.mKeyEventReceiver.mLastTimeoutId;
                        }
                        receiver.send(access$1900, i, mediaButtonIntent, this.mKeyEventReceiver, MediaSessionService.this.mHandler);
        ...
        class KeyEventWakeLockReceiver extends ResultReceiver implements Runnable, PendingIntent.OnFinished {
            private static final int MSG_RECEIVE_RESULT = 1;
            private final Handler mHandler;
            /* access modifiers changed from: private */
            public int mLastTimeoutId = 0;
            private final Handler mMyHandler = new Handler() {
                /* class com.android.server.media.MediaSessionService.SessionManagerImpl.KeyEventWakeLockReceiver.AnonymousClass1 */

                public void handleMessage(Message msg) {
                    if (msg.what == 1) {
                        KeyEventWakeLockReceiver.this.finishReceiveResult(msg.arg1, (Bundle) msg.obj);
                    }
                }
            };
            private int mRefCount = 0;

            public void finishReceiveResult(int resultCode, Bundle resultData) {
            ...
            public void onReceiveResult(int resultCode, Bundle resultData) {
                ...
                msg.obj = resultData;
                msg.what = 1;
                this.mMyHandler.sendMessage(msg);
            }
