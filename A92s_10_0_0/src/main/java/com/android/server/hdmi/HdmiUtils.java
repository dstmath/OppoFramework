...
final class HdmiUtils {
    ...
    static int twoBytesToInt(byte[] data) {
        return ((data[0] & OppoNfcChipVersion.NONE) << 8) | (data[1] & OppoNfcChipVersion.NONE);
    }

    static int twoBytesToInt(byte[] data, int offset) {
        return ((data[offset] & OppoNfcChipVersion.NONE) << 8) | (data[offset + 1] & OppoNfcChipVersion.NONE);
    }

    static int threeBytesToInt(byte[] data) {
        return ((data[0] & OppoNfcChipVersion.NONE) << UsbDescriptor.DESCRIPTORTYPE_CAPABILITY) | ((data[1] & OppoNfcChipVersion.NONE) << 8) | (data[2] & OppoNfcChipVersion.NONE);
    }
