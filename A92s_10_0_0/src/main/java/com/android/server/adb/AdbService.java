...
public class AdbService extends IAdbManager.Stub {
    ...
    /* access modifiers changed from: private */
    public final AdbHandler mHandler;
    /* access modifiers changed from: private */
    ...
    public final class AdbHandler extends Handler {
        AdbHandler(Looper looper) {
            super(looper);
            try {
                boolean unused = AdbService.this.mAdbEnabled = SystemProperties.getBoolean("ro.oppo.adb.secure", false) || containsFunction(SystemProperties.get(AdbService.USB_PERSISTENT_CONFIG_PROPERTY, ""), "adb") || Settings.Global.getInt(AdbService.this.mContentResolver, "adb_enabled", 0) > 0 || "enable".equals(SystemProperties.get("ro.boot.atm", ""));
