...
public abstract class IntentResolver<F extends IntentFilter, R> {
    ...
    /* JADX INFO: Multiple debug info for r12v16 'printer'  android.util.Printer: [D('printer' android.util.Printer), D('filter' F)] */
    /* access modifiers changed from: package-private */
    ...
    public boolean dump(PrintWriter out, String title, String prefix, String packageName, boolean printFilter, boolean collapseDuplicates) {
        ...
        String sepPrefix = StringUtils.LF + prefix;
        String curPrefix = title + StringUtils.LF + prefix;
    ...
    private void buildResolveList(Intent intent, FastImmutableArraySet<String> categories, boolean debug, boolean defaultOnly, String resolvedType, String scheme, F[] src, List<R> dest, int userId) {
        ...
        while (true) {
            if (i2 < N2) {
                ...
                if (debug) {
                    Slog.v(TAG, "Matching against filter " + ((Object) filter2));
                }
                if (!excludingStopped || !isFilterStopped(filter2, userId)) {
                    if (packageName2 == null || isPackageForFilter(packageName2, filter2)) {
                        if (filter2.getAutoVerify() && debug) {
                            ...
                            int z = 0;
                            for (int authorities = filter2.countDataAuthorities(); z < authorities; authorities = authorities) {
                                Slog.v(TAG, "   " + filter2.getDataAuthority(z).getHost());
                                z++;
                            }
                        }
                        if (allowFilterResult(filter2, dest)) {
                            action = filter;
                            i = i2;
                            N = N2;
                            data = data2;
                            ...
                            int match = filter2.match(filter, resolvedType, scheme, data2, categories, TAG);
                            if (match >= 0) {
                                if (debug) {
                                    Slog.v(TAG, "  Filter matched!  match=0x" + Integer.toHexString(match) + " hasDefault=" + filter2.hasCategory("android.intent.category.DEFAULT"));
                                }
                                if (!defaultOnly || filter2.hasCategory("android.intent.category.DEFAULT")) {
                                    R oneResult = newResult(filter2, match, userId);
                                    if (debug) {
                                    ...
                                    if (oneResult != null) {
                                        dest.add(oneResult);
                                        if (debug) {
                                            dumpFilter(logPrintWriter2, "    ", filter2);
                                            logPrintWriter2.flush();
                                            filter2.dump(logPrinter2, "    ");
                                        }
                                    }
                                } else {
                                    hasNonDefaults = true;
