...
import com.android.server.job.OppoBaseJobSchedulerService;
...
public class JobSchedulerService extends OppoBaseJobSchedulerService implements StateChangedListener, JobCompletedListener {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("android.intent.action.PACKAGE_CHANGED".equals(action)) {
                ...
                boolean doCancel = !OppoFeatureCache.get(IColorAlarmManagerHelper.DEFAULT).isFilterRemovePackage(pkgName);
    ...
    protected final List<StateController> mControllers;
    ...
    public static class Constants {
        ...
        public void updateConstantsLocked(String value) {
            ...
            this.MIN_LINEAR_BACKOFF_TIME = this.mParser.getDurationMillis(KEY_MIN_LINEAR_BACKOFF_TIME, 10000);
            this.MIN_EXP_BACKOFF_TIME = this.mParser.getDurationMillis(KEY_MIN_EXP_BACKOFF_TIME, 10000);
    ...
    public int scheduleAsPackage(JobInfo job, JobWorkItem work, int uId, String packageName, int userId, String tag) {
        ...
        minIntervalConstraint(job, uId);
        systemJobSameIdMonitor(job, uId);
        synchronized (this.mLock) {
            ...
            if (work == null || toCancel == null || !toCancel.getJob().equals(job)) {
                ...
                if (packageName == null && this.mJobs.countJobsForUid(uId) > 100) {
                    if (uId == 1000) {
                        systemMaxJobsMonitor(uId, this.mJobs);
                    } else {
                ...
                if (interceptScheduleJobLocked(jobStatus, work)) {
                    return 1;
                }
                if (work != null) {
    ...
    public void cancelJobImplLocked(JobStatus cancelled, JobStatus incomingJob, String reason) {
        ...
        if (!"frozen_pending_internal".equals(reason)) {
            interceptCancelJob(cancelled.getUid(), cancelled.getJobId());
        }
        reportActiveLocked();
    }
    ...
    public JobSchedulerService(Context context) {
        ...
        this.mColorJssEx = ColorServiceFactory.getInstance().getColorJobSchedulerServiceEx(context, this);
        onOppoStart();
        ...
        OppoFeatureCache.get(IColorJobScheduleManager.DEFAULT).initConstructor(this.mControllers, this);
        OppoFeatureCache.get(IBatteryIdleController.DEFAULT).addController(this.mControllers, this);
        ColorJobSchedulerHelper.getInstance().init(this);
        try {
            this.mColorJssInner = new OppoBaseJobSchedulerService.ColorJobSchedulerServiceInner();
            OppoFeatureCache.get(IColorStrictModeManager.DEFAULT).init(this.mHandler, this, this.mColorJssInner);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    @Override // com.android.server.SystemService, com.android.server.job.OppoBaseJobSchedulerService
    public void onStart() {
    ...
    public void onBootPhase(int phase) {
        if (500 == phase) {
            ...
            initArgs(getContext(), this.mHandler, this);
            synchronized (this.mLock) {
    ...
    /* access modifiers changed from: package-private */
    public boolean stopTrackingJobLocked(JobStatus jobStatus, JobStatus incomingJob, boolean writeBack) {
    ...
    /* JADX INFO: Multiple debug info for r1v9 long: [D('delayMillis' long), D('backoff' long)] */
    ...
    public JobStatus getRescheduleJobForFailureLocked(JobStatus failureToReschedule) {
        ...
        long delayMillis2 = Math.min(backoff, 18000000L);
        if (job.getOppoJob()) {
            long delayMillis3 = Math.min(delayMillis2, 600000L);
            Slog.i(TAG, "getRescheduleJobForFailureLocked set job delay " + delayMillis3);
            delayMillis = delayMillis3;
        } else {
            delayMillis = delayMillis2;
        }
    ...
    final class ReadyJobQueueFunctor implements Consumer<JobStatus> {
        ...
        public void accept(JobStatus job) {
            ...
            } else if (OppoFeatureCache.get(IColorJobScheduleManager.DEFAULT).needCheck()) {
                OppoFeatureCache.get(IColorJobScheduleManager.DEFAULT).updateParamterOnServiceContextLocked(job);
            } else {
    ...
    final class MaybeReadyJobQueueFunctor implements Consumer<JobStatus> {
        ...
        public void accept(JobStatus job) {
            if (JobSchedulerService.this.isReadyToBeExecutedLocked(job)) {
                ...
                JobSchedulerService.this.acceptForMaybeReadyJobQueueFunctor(job);
        ...
        private void reset() {
            ...
            this.contentCount = 0;
            JobSchedulerService.this.resetForMaybeReadyJobQueueFunctor();
    ...
    public boolean isReadyToBeExecutedLocked(JobStatus job) {
        ...
        if (!OppoFeatureCache.get(IColorHansManager.DEFAULT).hansJobIfNeeded(job.getSourceUid(), job.getSourcePackageName())) {
            return false;
        }
        boolean satisfied = isComponentUsable(job);
        String pkgName = null;
        ...
        return OppoFeatureCache.get(IColorJobScheduleManager.DEFAULT).isSatisfied(satisfied, job, pkgName);
    }
    ...
    public void maybeRunPendingJobsLocked() {
        ...
        reportActiveLocked();
        rcdBattIdleStart(this.mLock, this.mActiveServices);
    }
    ...
    final class LocalService implements JobSchedulerInternal {
        ...
        @Override // com.android.server.job.ColorJobSchedulerInternal
        public int pendingJobs(int uid) {
            return JobSchedulerService.this.pendingJobs(uid);
        }

        @Override // com.android.server.job.ColorJobSchedulerInternal
        public int restoreJobs(int uid) {
            return JobSchedulerService.this.restoreSpecialJobs(uid);
        }
    }
    ...
    public int executeRunCommand(String pkgName, int userId, int jobId, boolean force) {
        if (DEBUG) {
            Slog.v(TAG, "executeRunCommand(): " + pkgName + SliceClientPermissions.SliceAuthority.DELIMITER + userId + StringUtils.SPACE + jobId + " f=" + force);
        }
        try {
    ...
    public int executeTimeoutCommand(PrintWriter pw, String pkgName, int userId, boolean hasJobId, int jobId) {
        if (DEBUG) {
            Slog.v(TAG, "executeTimeoutCommand(): " + pkgName + SliceClientPermissions.SliceAuthority.DELIMITER + userId + StringUtils.SPACE + jobId);
        }
        synchronized (this.mLock) {
    ...
    public int executeCancelCommand(PrintWriter pw, String pkgName, int userId, boolean hasJobId, int jobId) {
        if (DEBUG) {
            Slog.v(TAG, "executeCancelCommand(): " + pkgName + SliceClientPermissions.SliceAuthority.DELIMITER + userId + StringUtils.SPACE + jobId);
        }
        int pkgUid = -1;
        try {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: com.android.server.job.controllers.JobStatus.dump(java.io.PrintWriter, java.lang.String, boolean, long):void
     arg types: [com.android.internal.util.IndentingPrintWriter, java.lang.String, int, long]
     candidates:
      com.android.server.job.controllers.JobStatus.dump(android.util.proto.ProtoOutputStream, long, boolean, long):void
      com.android.server.job.controllers.JobStatus.dump(java.io.PrintWriter, java.lang.String, boolean, long):void */
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: com.android.server.job.JobPackageTracker.dump(java.io.PrintWriter, java.lang.String, int):void
     arg types: [com.android.internal.util.IndentingPrintWriter, java.lang.String, int]
     candidates:
      com.android.server.job.JobPackageTracker.dump(android.util.proto.ProtoOutputStream, long, int):void
      com.android.server.job.JobPackageTracker.dump(java.io.PrintWriter, java.lang.String, int):void */
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: com.android.server.job.JobPackageTracker.dumpHistory(java.io.PrintWriter, java.lang.String, int):boolean
     arg types: [com.android.internal.util.IndentingPrintWriter, java.lang.String, int]
     candidates:
      com.android.server.job.JobPackageTracker.dumpHistory(android.util.proto.ProtoOutputStream, long, int):void
      com.android.server.job.JobPackageTracker.dumpHistory(java.io.PrintWriter, java.lang.String, int):boolean */
    /* access modifiers changed from: package-private */
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: com.android.server.job.controllers.JobStatus.dump(android.util.proto.ProtoOutputStream, long, boolean, long):void
     arg types: [android.util.proto.ProtoOutputStream, int, int, long]
     candidates:
      com.android.server.job.controllers.JobStatus.dump(java.io.PrintWriter, java.lang.String, boolean, long):void
      com.android.server.job.controllers.JobStatus.dump(android.util.proto.ProtoOutputStream, long, boolean, long):void */
    /* access modifiers changed from: package-private */
    ...
    public boolean stopTrackingJobExported(JobStatus jobStatus, JobStatus incomingJob, boolean writeBack) {
        return stopTrackingJobLocked(jobStatus, incomingJob, writeBack);
    }

    /* access modifiers changed from: package-private */
    @Override // com.android.server.job.OppoBaseJobSchedulerService
    public void colorCancelJobImplLocked(JobStatus cancelled, JobStatus incomingJob, String reason) {
        cancelJobImplLocked(cancelled, incomingJob, reason);
    }
}
