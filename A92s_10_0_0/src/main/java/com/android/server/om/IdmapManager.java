...
class IdmapManager {
    ...
    private int calculateFulfilledPolicies(PackageInfo targetPackage, PackageInfo overlayPackage, int userId) {
        ...
        int fulfilledPolicies2 = OppoFeatureCache.get(IColorLanguageManager.DEFAULT).checkSignaturesMatching(overlayPackage.packageName, targetPackage.packageName, fulfilledPolicies, 16);
        if (ai.isVendor()) {
            return fulfilledPolicies2 | 4;
        }
        if (ai.isProduct()) {
            return fulfilledPolicies2 | 8;
        }
        if (ai.isOdm()) {
            return fulfilledPolicies2 | 32;
        }
        if (ai.isOem()) {
            return fulfilledPolicies2 | 64;
        }
        if (!ai.isProductServices() && ai.isSystemApp()) {
            return fulfilledPolicies2 | 2;
        }
        return fulfilledPolicies2;
    }
}
