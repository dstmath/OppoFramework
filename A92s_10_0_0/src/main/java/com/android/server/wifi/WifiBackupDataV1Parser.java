...
class WifiBackupDataV1Parser implements WifiBackupDataParser {
    ...
    private WifiConfiguration parseWifiConfigurationFromXml(XmlPullParser in, int outerTagDepth, int minorVersion) throws XmlPullParserException, IOException {
        ...
        if (!configKeyParsed.equals(configKeyCalculated) && !transformConfigKeyParsed(configKeyParsed).equals(configKeyCalculated)) {
            Log.w(TAG, "Configuration key does not match. Retrieved: " + configKeyParsed + ", Calculated: " + configKeyCalculated);
        }
        return configuration;
    }

    private String transformConfigKeyParsed(String configKeyParsed) {
        StringBuilder sb = new StringBuilder();
        String ssid = null;
        String keyMgmt = null;
        int indexOfMgmt = -1;
        if (configKeyParsed.endsWith("WPA_PSK")) {
            indexOfMgmt = configKeyParsed.lastIndexOf("WPA_PSK");
        } else if (configKeyParsed.endsWith("WPA2_PSK")) {
            indexOfMgmt = configKeyParsed.lastIndexOf("WPA2_PSK");
        } else if (configKeyParsed.endsWith("WPA_EAP")) {
            indexOfMgmt = configKeyParsed.lastIndexOf("WPA_EAP");
        } else if (configKeyParsed.endsWith("NONE")) {
            indexOfMgmt = configKeyParsed.lastIndexOf("NONE");
        }
        if (indexOfMgmt != -1) {
            ssid = configKeyParsed.substring(0, indexOfMgmt);
            keyMgmt = configKeyParsed.substring(indexOfMgmt);
        }
        if (ssid == null || keyMgmt == null || ssid.endsWith("-")) {
            return configKeyParsed;
        }
        sb.append(ssid);
        sb.append("-");
        sb.append(keyMgmt);
