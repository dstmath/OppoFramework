...
public class WifiConfigurationUtil {
    ...
    public static boolean isConfigForOpenNetwork(WifiConfiguration config) {
        return !isConfigForWepNetwork(config) && !isConfigForPskNetwork(config) && !MtkWapi.isConfigForWapiNetwork(config) && !isConfigForEapNetwork(config) && !isConfigForSaeNetwork(config) && !isConfigForEapSuiteBNetwork(config);
    }
    ...
    public static boolean hasMacRandomizationSettingsChanged(WifiConfiguration existingConfig, WifiConfiguration newConfig) {
        ...
        if (ClientModeImpl.isNotChineseOperator()) {
            return newConfig.macRandomizationSetting != 1;
        }
        return newConfig.macRandomizationSetting != SystemProperties.getInt("persist.sys.wifi.mac_randomization", 0);
    }
    ...
    private static boolean validateSsid(String ssid, boolean isAdd) {
        ...
        if (ssid.startsWith("\"")) {
            ...
            if (GbkUtil.isGbkSsid(ssid)) {
                ssidBytes = GbkUtil.stringToByteArray(ssid);
            }
    ...
    private static boolean validateBssid(String bssid) {
        ...
        if (bssid.isEmpty()) {
            ...
            return false;
        } else if ("any".equalsIgnoreCase(bssid)) {
            return true;
        } else {
            try {
