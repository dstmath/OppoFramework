...
public class NetworkStatsCollection implements FileRotator.Reader {
    ...
    public void writeToBackupFile(BufferedWriter buffWriter, String prefix) throws IOException {
        ...
        while (it.hasNext()) {
            Key key = it.next();
            char[] tmpBuf2 = ("line:" + index).toCharArray();
            buffWriter.write(tmpBuf2, 0, tmpBuf2.length);
            buffWriter.newLine();
            char[] tmpBuf3 = ("ident=" + key.ident.toString() + ", uid=" + key.uid + ", set=" + NetworkStats.setToString(key.set) + ", tag=" + NetworkStats.tagToString(key.tag)).toCharArray();
            buffWriter.write(tmpBuf3, 0, tmpBuf3.length);
            buffWriter.newLine();
            StringBuilder builder = new StringBuilder();
            builder.append("histroy = ");
            builder.append(this.mStats.get(key).toString());
            char[] tmpBuf4 = builder.toString().toCharArray();
            buffWriter.write(tmpBuf4, 0, tmpBuf4.length);
            buffWriter.newLine();
            index++;
        }
        buffWriter.flush();
    ...
    private static class Key implements Comparable<Key> {
        ...
        public Key(NetworkIdentitySet ident2, int uid2, int set2, int tag2) {
            ...
            this.set = set2;
            this.tag = tag2;
            this.hashCode = Objects.hash(ident2, Integer.valueOf(uid2), Integer.valueOf(set2), Integer.valueOf(tag2));
        }
        ...
        public String toString() {
            return " Key:" + " uid=" + this.uid + " set=" + setToString(this.set) + " tag=" + tagToString(this.tag);
        }
