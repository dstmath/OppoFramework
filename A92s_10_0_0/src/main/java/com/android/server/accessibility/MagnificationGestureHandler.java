...
class MagnificationGestureHandler extends BaseEventStreamTransformation {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: com.android.server.accessibility.MagnificationController.resetIfNeeded(int, boolean):boolean
     arg types: [int, int]
     candidates:
      com.android.server.accessibility.MagnificationController.resetIfNeeded(int, int):boolean
      com.android.server.accessibility.MagnificationController.resetIfNeeded(int, boolean):boolean */
    ...
    final class PanningScalingState extends GestureDetector.SimpleOnGestureListener implements ScaleGestureDetector.OnScaleGestureListener, State {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (this.mScaling) {
                ...
                return true;
            } else if (this.mInitialScaleFactor < OppoBrightUtils.MIN_LUX_LIMITI) {
    ...
    final class DetectingState implements State, Handler.Callback {
        ...
        public void onMotionEvent(MotionEvent event, MotionEvent rawEvent, int policyFlags) {
            ...
            if (actionMasked == 0) {
                ...
                if (!MagnificationGestureHandler.this.mMagnificationController.magnificationRegionContains(MagnificationGestureHandler.this.mDisplayId, event.getX(), event.getY())) {
                    transitionToDelegatingStateAndClear();
                } else if (isMultiTapTriggered(3) && !OppoScreenDragUtil.isOffsetState()) {
                    onTripleTap(event);
    ...
    public void zoomOn(float centerX, float centerY) {
        this.mMagnificationController.setScaleAndCenter(this.mDisplayId, MathUtils.constrain(this.mMagnificationController.getPersistedScale(), 2.0f, 8.0f), centerX, centerY, true, 0);
    }
    ...
    private static final class MotionEventInfo {
        ...
        public static String toString(MotionEventInfo info) {
            ...
            return MotionEvent.actionToString(info.event.getAction()).replace("ACTION_", "") + StringUtils.SPACE + toString(info.mNext);
        }
    }
