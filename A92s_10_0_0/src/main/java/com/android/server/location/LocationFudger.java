...
public class LocationFudger {
    ...
    public Location getOrCreate(Location location) {
        synchronized (this.mLock) {
            ...
            if (coarse == null) {
                Location addCoarseLocationExtraLocked = addCoarseLocationExtraLocked(location);
                return addCoarseLocationExtraLocked;
            } else if (coarse.getAccuracy() >= this.mAccuracyInMeters) {
                return coarse;
            } else {
                Location addCoarseLocationExtraLocked2 = addCoarseLocationExtraLocked(location);
                return addCoarseLocationExtraLocked2;
            }
        }
    }
