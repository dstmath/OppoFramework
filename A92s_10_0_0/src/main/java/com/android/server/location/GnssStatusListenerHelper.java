...
public abstract class GnssStatusListenerHelper extends RemoteListenerHelper<IGnssStatusListener> {
    ...
    public /* synthetic */ void lambda$onNmeaReceived$4$GnssStatusListenerHelper(long timestamp, String nmea, IGnssStatusListener listener, CallerIdentity callerIdentity) throws RemoteException {
        ...
        if (systemNanoTime - this.mLastCheckTime >= 1000000000) {
            Log.d(TAG, "onNmeaReceived re-check permissions of listeners size = " + this.mPermissionAllowedList.size() + " mLastCheckTime = " + this.mLastCheckTime);
            this.mPermissionAllowedList.clear();
            this.mLastCheckTime = systemNanoTime;
        }
        IBinder binder = listener.asBinder();
        boolean permissionAllowedListContains = this.mPermissionAllowedList.contains(binder);
        if (DEBUG) {
            Log.d(TAG, "onNmeaReceived binder=" + binder + " permissionAllowedListContains=" + permissionAllowedListContains);
        }
        if (!permissionAllowedListContains) {
            ...
            this.mPermissionAllowedList.add(binder);
        }
