...
public final class NotificationRecord {
    ...
    private Light calculateLights() {
        ...
        if (!this.mPreChannelsNotification) {
            if (getChannel().shouldShowLights()) {
                light2 = new Light(channelLightColor, defaultLightOn, defaultLightOff);
            } else {
                light2 = null;
            }
            light3 = light2;
    ...
    private long[] calculateVibration() {
        long[] vibration;
        long[] vibration2;
        long[] defaultVibration = NotificationManagerService.getLongArray(this.mContext.getResources(), 17236008, 17, NotificationManagerService.DEFAULT_VIBRATE_PATTERN);
        if (this.mPreChannelsNotification || !getChannel().shouldVibrate()) {
            vibration = null;
        } else {
        ...
        if ((notification.defaults & 2) != 0) {
            vibration2 = defaultVibration;
        } else {
            vibration2 = notification.vibrate;
        }
    ...
    public void applyAdjustments() {
        ...
        synchronized (this.mAdjustments) {
            for (Adjustment adjustment : this.mAdjustments) {
                ...
                if (signals.containsKey("key_option_stow")) {
                    this.sbn.getNotification().extras.putInt("key_option_stow", signals.getInt("key_option_stow"));
                }
    ...
    private String getUserExplanation() {
        if (this.mUserExplanation == null) {
            this.mUserExplanation = this.mContext.getResources().getString(17040134);
        }
