...
public class ZenModeHelper {
    ...
    public static final int OPPO_MULTI_USER_ID = 999;
    ...
    public boolean removeAutomaticZenRule(String id, String reason) {
        synchronized (this.mConfig) {
            ...
            if (canManageAutomaticZenRule(rule)) {
                ...
                boolean configLocked = setConfigLocked(newConfig, reason, null, true);
                return configLocked;
            }
    ...
    private boolean setConfigLocked(ZenModeConfig config, String reason, ComponentName triggeringComponent, boolean setRingerMode) {
        ...
        if (config != null) {
            try {
                if (config.isValid()) {
                    if (DEBUG) {
                        Slog.v(TAG, "reason = " + reason + " config = " + config.toString() + "triggeringComponent = " + triggeringComponent + "setRingerMode =" + setRingerMode);
                    }
    ...
    private void updateDefaultAutomaticRuleNames() {
        for (ZenModeConfig.ZenRule rule : this.mDefaultConfig.automaticRules.values()) {
            if ("EVENTS_DEFAULT_RULE".equals(rule.id)) {
                rule.name = this.mContext.getResources().getString(17041327);
            } else if ("EVERY_NIGHT_DEFAULT_RULE".equals(rule.id)) {
                rule.name = this.mContext.getResources().getString(17041328);
            }
        }
    }

    /* JADX INFO: Multiple debug info for r6v3 int: [D('zenAlarmsOnly' boolean), D('usage' int)] */
    /* access modifiers changed from: protected */
    @VisibleForTesting
    ...
    public Notification createZenUpgradeNotification() {
        ...
        extras.putString("android.substName", this.mContext.getResources().getString(17040059));
        int title = 17041338;
        int content = 17041337;
        int drawable = 17302851;
