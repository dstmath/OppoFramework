...
public class ValidateNotificationPeople implements NotificationSignalExtractor {
        ...
        while (true) {
            ...
            if (!TextUtils.isEmpty(handle)) {
                synchronized (this.mPeopleCache) {
                    LookupResult lookupResult = this.mPeopleCache.get(getCacheKey(ActivityManager.getCurrentUser(), handle));
    ...
    public LookupResult searchContacts(Context context, Uri lookupUri) {
        ...
        try {
            try {
                c = context.getContentResolver().query(ContentProvider.maybeAddUserId(lookupUri, ActivityManager.getCurrentUser()), LOOKUP_PROJECTION, null, null, null);
                if (c == null) {
    ...
    private class PeopleRankingReconsideration extends RankingReconsideration {
        ...
        public void work() {
            ...
            while (it.hasNext()) {
                ...
                if (lookupResult != null) {
                    synchronized (ValidateNotificationPeople.this.mPeopleCache) {
                        ValidateNotificationPeople.this.mPeopleCache.put(ValidateNotificationPeople.this.getCacheKey(ActivityManager.getCurrentUser(), handle), lookupResult);
                    }
                    if (ValidateNotificationPeople.DEBUG) {
            ...
            if (this.mRecord != null) {
                ...
                boolean z = true;
                boolean z2 = this.mContactAffinity > 0.0f;
