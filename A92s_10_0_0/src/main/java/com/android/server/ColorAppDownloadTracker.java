...
public class ColorAppDownloadTracker extends ColorAppActionTracker {
    ...
    /* access modifiers changed from: private */
    public HashSet<Integer> mDownloadApps = new HashSet<>();
    ...
    /* access modifiers changed from: private */
    public long mTrafficUpdateTime = 0;
    private Runnable mTrafficUpdater = new Runnable() {
        ...
        public void run() {
            ...
            long unused = ColorAppDownloadTracker.this.mTrafficUpdateTime = SystemClock.elapsedRealtime();
            if (ColorAppDownloadTracker.this.mCallback != null && ColorAppDownloadTracker.this.updateDownloadUploadList()) {
    ...
    public boolean updateDownloadUploadList() {
    ...
    private boolean isAppDownloadBaseOnDiff(Pair<Long, Long> stats, long interval) {
        long speed = (((Long) stats.first).longValue() * 1000) / interval;
        if (DEBUG) {
            Slog.d(TAG, "bytes = " + stats.first + ", interval = " + stats.second + ", speed is " + speed);
        }
        if (speed <= 8192 || ((int) (((Long) stats.first).longValue() / ((Long) stats.second).longValue())) <= DOWNLOAD_SIZE_PER_PACKAGE) {
            return DEBUG;
        }
        return true;
    }
    ...
    private class TrafficRecord {
        ...
        public void updateWholeTraffic(long trafficBytes, long trafficPackages) {
            ...
            if (ColorAppDownloadTracker.DEBUG) {
                Slog.d(ColorAppDownloadTracker.TAG, " whole traffic, wholeTrafficBytes = " + this.wholeTrafficBytes + ", wholeTrafficPackages = " + this.wholeTrafficPackages);
            }
        }
        ...
        public void updateUidTraffic(int uid, long rtBytes, long rtPackages) {
            ...
            if (ColorAppDownloadTracker.DEBUG) {
                Slog.d(ColorAppDownloadTracker.TAG, " updateUidTraffic uid " + uid + " bytes = " + (preRTBytes + rtBytes) + ", pakcaegs = " + (preRTPackages + rtPackages));
            }
