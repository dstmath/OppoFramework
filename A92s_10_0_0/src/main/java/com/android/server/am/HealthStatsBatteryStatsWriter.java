...
public class HealthStatsBatteryStatsWriter {
    ...
    public void writeUid(HealthStatsWriter uidWriter, BatteryStats bs, BatteryStats.Uid uid) {
        uidWriter.addMeasurement((int) OppoScreenOffGestureManager.MSG_SCREEN_TURNED_OFF, bs.computeBatteryRealtime(this.mNowRealtimeMs * 1000, 0) / 1000);
        ...
        if (controller2 != null) {
            uidWriter.addMeasurement((int) ColorNotificationListenerService.REASON_COLOROS_FORCESTOP, controller2.getIdleTimeCounter().getCountLocked(0));
            uidWriter.addMeasurement((int) ColorNotificationListenerService.REASON_COLOROS_BPMSUSPEND, controller2.getRxTimeCounter().getCountLocked(0));
            long sum2 = 0;
