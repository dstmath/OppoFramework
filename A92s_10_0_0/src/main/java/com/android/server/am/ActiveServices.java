...
public final class ActiveServices extends OppoBaseActiveServices {
    ...
    static final boolean DEBUG_AGING_VERSION = "1".equals(SystemProperties.get("SPECIAL_OPPO_CONFIG", "0"));
    /* access modifiers changed from: private */
    public static boolean DEBUG_DELAYED_SERVICE = ActivityManagerDebugConfig.DEBUG_SERVICE;
    /* access modifiers changed from: private */
    public static boolean DEBUG_DELAYED_STARTS = DEBUG_DELAYED_SERVICE;
    ...
    /* access modifiers changed from: private */
    public static final String TAG_SERVICE = (TAG + ActivityManagerDebugConfig.POSTFIX_SERVICE);
    ...
    public final class ServiceMap extends Handler {
        ...
        public void ensureNotStartingBackgroundLocked(ServiceRecord r) {
            if (this.mStartingBackground.remove(r)) {
                if (ActiveServices.DEBUG_DELAYED_STARTS) {
                    String access$300 = ActiveServices.TAG_SERVICE;
                    Slog.v(access$300, "No longer background starting: " + r);
                }
                rescheduleDelayedStartsLocked();
            }
            if (this.mDelayedStartList.remove(r) && ActiveServices.DEBUG_DELAYED_STARTS) {
                String access$3002 = ActiveServices.TAG_SERVICE;
                Slog.v(access$3002, "No longer delaying start: " + r);
            }
        }
        ...
        public void rescheduleDelayedStartsLocked() {
            ...
            while (this.mDelayedStartList.size() > 0 && this.mStartingBackground.size() < ActiveServices.this.mMaxStartingBackground) {
                ...
                if (ActiveServices.DEBUG_DELAYED_STARTS) {
                    String access$300 = ActiveServices.TAG_SERVICE;
                    Slog.v(access$300, "REM FR DELAY LIST (exec next): " + r2);
                }
                if (ActiveServices.DEBUG_DELAYED_SERVICE && this.mDelayedStartList.size() > 0) {
                    ...
                    for (int i2 = 0; i2 < this.mDelayedStartList.size(); i2++) {
                        String access$3002 = ActiveServices.TAG_SERVICE;
                        Slog.v(access$3002, "  #" + i2 + ": " + this.mDelayedStartList.get(i2));
                    }
                }
                r2.delayed = false;
            ...
            if (this.mStartingBackground.size() > 0) {
                ...
                if (ActiveServices.DEBUG_DELAYED_SERVICE) {
                    String access$3003 = ActiveServices.TAG_SERVICE;
                    Slog.v(access$3003, "Top bg start is " + next + ", can delay others up to " + when);
                }
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     ...
     arg types: [java.lang.String, android.os.RemoteCallback]
     candidates:
                                    ...
                                    try {
                                        if (ActiveServices.this.mPendingServices.contains(s2)) {
                                            if (!ActiveServices.this.mAm.getPackageManagerInternalLocked().isPermissionsReviewRequired(s2.packageName, s2.userId)) {
                                                try {
                                                    String unused = ActiveServices.this.bringUpServiceLocked(s2, service2.getFlags(), callerFg2, false, false);
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            ...
                            Binder.restoreCallingIdentity(origId);
                            OppoFeatureCache.get(IColorCommonListManager.DEFAULT).updateAppInfo(service2.getAction(), s.appInfo.uid, s.appInfo.packageName, true);
                            return 1;
                        } catch (Throwable th5) {
                            th = th5;
                            Binder.restoreCallingIdentity(origId);
            ...
            try {
                ...
                if (clist.size() > 0) {
                    ...
                    if (r.binding.service.app != null) {
                        OppoFeatureCache.get(IColorCommonListManager.DEFAULT).updateAppInfo(r.binding.intent.intent.getIntent().getAction(), r.binding.service.appInfo.uid, r.binding.service.packageName, false);
    ...
    public final void killServicesLocked(ProcessRecord app, boolean allowRestart) {
        ...
        while (i3 >= 0) {
            ServiceRecord sr = app.services.valueAt(i3);
        ...
        for (int i4 = app.services.size() - 1; i4 >= 0; i4--) {
            ServiceRecord sr2 = app.services.valueAt(i4);
            if (!app.isPersistent()) {
                app.services.removeAt(i4);
        ...
        if (!allowRestart) {
            ...
            for (int i5 = this.mRestartingServices.size() - 1; i5 >= 0; i5--) {
                ServiceRecord r = this.mRestartingServices.get(i5);
                if (r.processName.equals(app.processName) && r.serviceInfo.applicationInfo.uid == app.info.uid) {
                    this.mRestartingServices.remove(i5);
                    clearRestartingIfNeededLocked(r);
                }
            }
