...
final class ActivityManagerConstants extends ContentObserver {
    ...
    ActivityManagerConstants(Context context, ActivityManagerService service, Handler handler) {
        ...
        if (Process.getTotalMemory() > 7516192768L && Process.getTotalMemory() <= 8589934592L) {
            this.MAX_CACHED_PROCESSES = 128;
            DEFAULT_MAX_CACHED_PROCESSES = 128;
        } else if (Process.getTotalMemory() > 8589934592L) {
            this.MAX_CACHED_PROCESSES = 64;
            DEFAULT_MAX_CACHED_PROCESSES = 64;
        }
    }
