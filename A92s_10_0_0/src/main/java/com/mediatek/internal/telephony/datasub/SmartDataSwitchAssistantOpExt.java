...
public class SmartDataSwitchAssistantOpExt implements ISmartDataSwitchAssistantOpExt {
    ...
    public void onSrvccStateChanged() {
        this.isSrvccDuringCall = true;
        logd("onSrvccStateChanged, isSrvccDuringCall=true");
    }
    ...
    public boolean checkIsSwitchAvailable(int phoneId) {
        ...
        if (dcHelper == null || dcHelper.getDsdaMode() != 1) {
            ...
            logd("checkIsSwitchAvailable() nwType=" + nwType + ", isCdma=" + ServiceState.isCdma(nwType) + ", isWifcCalling=" + isWifiCalling + ", isVoLteCalling=" + isVoLteCalling + ", isSrvccDuringCall=" + this.isSrvccDuringCall);
            if (ServiceState.isCdma(nwType) || nwType == 16 || isWifiCalling || (!isVoLteCalling && !this.isSrvccDuringCall)) {
