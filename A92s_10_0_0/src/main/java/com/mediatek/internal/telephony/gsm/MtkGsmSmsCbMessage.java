...
public class MtkGsmSmsCbMessage {
    ...
    public static SmsCbMessage createSmsCbMessage(Context context, SmsCbHeader header, SmsCbLocation location, byte[][] pdus) throws IllegalArgumentException {
        ...
        if (mtkHeader.isEmergencyMessage()) {
            priority = 3;
        } else {
            priority = 0;
        }
    ...
    private static byte[] getWacData(SmsCbHeader header, byte[] pdu) {
        ...
        if (offset < pdu.length - 1) {
            int length = ((pdu[offset + 1] & PplMessageManager.Type.INVALID) << 8) | (pdu[offset] & 255);
    ...
    /* JADX INFO: Multiple debug info for r0v4 'result'  java.util.ArrayList<com.mediatek.internal.telephony.gsm.cbutil.Shape>: [D('lati1' int), D('result' java.util.ArrayList<com.mediatek.internal.telephony.gsm.cbutil.Shape>)] */
        ...
        while (i < len) {
            ...
            if (tag != 1) {
                ...
                if (tag == 2) {
                    ...
                    int k = 0;
                    while (k < count) {
                        polygon.addVertex(new Vertex(((((double) ((((wacBytes2[i] & PplMessageManager.Type.INVALID) << 14) | ((wacBytes2[i + 1] & PplMessageManager.Type.INVALID) << 6)) | ((wacBytes2[i + 2] & 252) >>> b2))) * 180.0d) / 4194304.0d) - 90.0d, ((((double) (((((wacBytes2[i + 2] & b) << 20) | ((wacBytes2[i + 3] & PplMessageManager.Type.INVALID) << 12)) | ((wacBytes2[i + 4] & PplMessageManager.Type.INVALID) << 4)) | ((wacBytes2[i + 5] & 240) >>> 4))) * 360.0d) / 4194304.0d) - 180.0d));
                        polygon.addVertex(new Vertex(((((double) (((((wacBytes2[i + 5] & 15) << 18) | ((wacBytes2[i + 6] & PplMessageManager.Type.INVALID) << 10)) | ((wacBytes2[i + 7] & PplMessageManager.Type.INVALID) << 2)) | ((wacBytes2[i + 8] & 192) >>> 6))) * 180.0d) / 4194304.0d) - 90.0d, ((((double) ((((wacBytes2[i + 8] & 63) << PplControlData.STATUS_WIPE_REQUESTED) | ((wacBytes2[i + 9] & PplMessageManager.Type.INVALID) << 8)) | (wacBytes2[i + 10] & 255))) * 360.0d) / 4194304.0d) - 180.0d));
                        i += 11;
                        k++;
                        tag = tag;
                        polyLen = polyLen;
                    ...
                    result.add(new Circle(new Vertex(((((double) ((((wacBytes2[i + 2] & PplMessageManager.Type.INVALID) << 14) | ((wacBytes2[i + 3] & PplMessageManager.Type.INVALID) << 6)) | ((wacBytes2[i + 4] & 252) >>> 2))) * 180.0d) / 4194304.0d) - 90.0d, ((((double) (((((wacBytes2[i + 4] & 3) << 20) | ((wacBytes2[i + 5] & PplMessageManager.Type.INVALID) << 12)) | ((wacBytes2[i + 6] & PplMessageManager.Type.INVALID) << 4)) | ((wacBytes2[i + 7] & 240) >>> 4))) * 360.0d) / 4194304.0d) - 180.0d), ((double) ((((wacBytes2[i + 7] & 15) << PplControlData.STATUS_WIPE_REQUESTED) | ((wacBytes2[i + 8] & PplMessageManager.Type.INVALID) << 8)) | (wacBytes2[i + 9] & 255))) / 64.0d));
                    i += 10;
                    wacBytes = wacBytes2;
                }
            } else {
                wacBytes = wacBytes2;
                int maxWaitTime = wacBytes[i + 2] & 255;
        ...
        while (i < len) {
            ...
            if (tag == 1) {
                ...
                for (int k = 0; k < count; k++) {
                    singleList.add(new WhamTuple(0, ((whamBytes[i] & PplMessageManager.Type.INVALID) << 8) | (whamBytes[i + 1] & 255), ((whamBytes[i + 2] & PplMessageManager.Type.INVALID) << 8) | (whamBytes[i + 3] & 255)));
                    i2 = i + 4;
                }
            } else if (tag != 2) {
                Rlog.d(TAG, "not expected tag:" + tag);
                ...
                for (int k2 = 0; k2 < count2; k2++) {
                    commonList.add(new WhamTuple(1, ((whamBytes[i] & PplMessageManager.Type.INVALID) << 8) | (whamBytes[i + 1] & 255), ((whamBytes[i + 2] & PplMessageManager.Type.INVALID) << 8) | (whamBytes[i + 3] & 255)));
                    i += 4;
                }
