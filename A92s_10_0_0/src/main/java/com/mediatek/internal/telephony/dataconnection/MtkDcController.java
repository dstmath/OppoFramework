...
public class MtkDcController extends DcController {
    ...
    /* access modifiers changed from: private */
    public boolean isIpMatched(LinkProperties oldLp, LinkProperties newLp) {
    ...
    /* access modifiers changed from: package-private */
    public int getActiveDcTagSupportInternet() {
        HashMap<Integer, DataConnection> dcListActiveByCid;
        synchronized (this.mDcListAll) {
            dcListActiveByCid = new HashMap<>(this.mDcListActiveByCid);
        }
        for (DataConnection dc : dcListActiveByCid.values()) {
            List<ApnContext> apnContexts = dc.getApnContexts();
            if (apnContexts.size() != 0) {
                for (ApnContext apnContext : apnContexts) {
                    Iterator it = apnContext.getNetworkRequests().iterator();
                    while (true) {
                        if (it.hasNext()) {
                            if (((NetworkRequest) it.next()).hasCapability(12)) {
                                if (DBG) {
                                    log("getActiveDcTagSupportInternet: get dc.mTag=" + dc.mTag);
                                }
                                return dc.mTag;
                            }
                        }
                ...
                loge("getActiveDcTagSupportInternet: no connected apns, ignore");
            }
        }
        if (!DBG) {
            return 0;
        }
        log("getActiveDcTagSupportInternet: no actvie DC support internet");
        return 0;
    }

    /* access modifiers changed from: private */
    public boolean isDNSMatched(LinkProperties oldLp, LinkProperties newLp) {
    ...
    /* access modifiers changed from: private */
    public boolean isRouteMatched(LinkProperties oldLp, LinkProperties newLp) {
