...
public class MtkProxyController extends ProxyController {
    ...
    public int mRildMode;
    /* access modifiers changed from: private */
    public int mSetRafRetryCause;
    ...
    public void issueFinish(int sessionId) {
        ...
        synchronized (this.mSetRadioAccessFamilyStatus) {
            resetRadioAccessFamilyStatusCounter();
            for (int i2 = 0; i2 < this.mPhones.length; i2++) {
                mtkLogd("issueFinish: phoneId=" + i2 + " sessionId=" + sessionId + " mTransactionFailed=" + this.mTransactionFailed);
                int i3 = this.mOldRadioAccessFamily[i2];
                String str = this.mCurrentLogicalModemIds[i2];
                if (this.mTransactionFailed) {
                    i = 2;
                } else {
                    i = 1;
                }
                sendRadioCapabilityRequest(i2, sessionId, 4, i3, str, i, 4);
                if (this.mTransactionFailed) {
