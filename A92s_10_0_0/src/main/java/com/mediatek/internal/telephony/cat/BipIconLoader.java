...
class BipIconLoader extends Handler {
    ...
    /* JADX INFO: Multiple debug info for r2v3 byte: [D('valueIndex' int), D('currentByte' byte)] */
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     method: ClspMth{android.graphics.Color.rgb(int, int, int):int}
     arg types: [byte, byte, byte]
     candidates:
      ClspMth{android.graphics.Color.rgb(float, float, float):int}
      ClspMth{android.graphics.Color.rgb(int, int, int):int} */
