...
public class MtkImsManager extends ImsManager {
    ...
    public ArrayList<ImsMmTelManager.RegistrationCallback> mCallbacks = new ArrayList<>();
    ...
    /* access modifiers changed from: private */
    public MtkImsUt mMtkUt = null;
    /* access modifiers changed from: private */
    public boolean mNotifyOnly = DBG;
    ...
    public void updateVideoCallFeatureValue(CapabilityChangeRequest request) {
        ...
        boolean isVilteFeatureOn = (!available || !vilteEnabled || !isNonTty || !isCameraAvailable || (!ignoreDataEnabledChanged && ((!isDataEnabled || (!ignoreDataRoaming && !isDataRoamingEnable)) && !isTestSim()))) ? false : DBG;
        log("[" + this.mPhoneId + "] updateVideoCallFeatureValue ViLTE: available = " + available + ", vilteEnabled = " + vilteEnabled + ", nonTTY = " + isNonTty + ", ignoreDataEnabledChanged = " + ignoreDataEnabledChanged + ", data enabled = " + isDataEnabled + ", ignoreDataRoaming = " + ignoreDataRoaming + ", data roaming enabled = " + isDataRoamingEnable + ", is test sim = " + isTestSim() + ", camera support = " + isCameraAvailable);
        if (isVilteFeatureOn) {
    ...
    private class MtkImsServiceDeathRecipient implements IBinder.DeathRecipient {
        ...
        public void binderDied() {
            IMtkImsService unused = MtkImsManager.this.mMtkImsService = null;
            MtkImsUt unused2 = MtkImsManager.this.mMtkUt = null;
            boolean unused3 = MtkImsManager.this.mNotifyOnly = false;
    ...
    public void setLteFeatureValues(boolean turnOn) {
        ...
        if (isVolteEnabledByPlatform() && isVtEnabledByPlatform()) {
            ...
            boolean enableViLte = turnOn && isVtEnabledByUser() && isCameraAvailable && (ignoreDataEnabledChanged || ((isDataEnabled() && (ignoreDataRoaming || isDataRoamingEnable)) || isTestSim()));
    ...
    private boolean isFeatureEnabledByPlatformExt(int feature) {
        ...
        IImsManagerExt imsMgrExt = getImsManagerPluginInstance(this.mContext);
        if (imsMgrExt != null) {
            return imsMgrExt.isFeatureEnabledByPlatform(this.mContext, feature, this.mPhoneId);
        }
        logw("plugin null=" + imsMgrExt + ", return " + DBG);
        return DBG;
    }
        ...
        if (imsManager != null) {
            try {
                ...
                if (!isNonTtyOrTtyOnVolteEnabled(context, this.mPhoneId)) {
                    ...
                    enableViLte = e4genabled != 0 && isVtEnabledByUser() && (getBooleanCarrierConfig("ignore_data_enabled_changed_for_video_calls") || isDataEnabled() || isTestSim());
                }
                if (!vtenabled || !enableViLte) {
                    i = 0;
                }
                setComboFeatureValue(volte_value, i, -1);
                if (e4genabled != 0 || vtenabled) {
