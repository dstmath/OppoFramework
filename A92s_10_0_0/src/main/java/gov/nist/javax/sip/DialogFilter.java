...
class DialogFilter implements ServerRequestInterface, ServerResponseInterface {
    ...
    /* JADX WARN: Failed to insert an additional move for type inference into block B:37:0x00ff */
    /* JADX WARN: Failed to insert an additional move for type inference into block B:304:0x06cd */
    /* JADX WARN: Failed to insert an additional move for type inference into block B:470:0x09cc */
    /* JADX WARN: Failed to insert an additional move for type inference into block B:478:0x09f0 */
    /* JADX WARN: Failed to insert an additional move for type inference into block B:317:0x06ff */
    /* JADX DEBUG: Additional 10 move instruction added to help type inference */
    ...
    public void processRequest(SIPRequest sipRequest, MessageChannel incomingMessageChannel) {
        ...
        if (this.listeningPoint != null) {
            ...
            if (sipProvider != null) {
                ...
                dialog = dialog;
                if (dialog != null) {
                    SipProviderImpl sipProvider2 = dialog.getSipProvider();
                    dialog = dialog;
                    if (sipProvider != sipProvider2) {
                    ...
                    if (!sipRequest.getMethod().equals(Request.REFER) || dialog == null) {
                        ...
                        if (dialog == null && sipRequest.getMethod().equals("NOTIFY")) {
                            ...
                            if (sipProvider.isAutomaticDialogSupportEnabled() && pendingSubscribeClientTx == null && !sipStack2.deliverUnsolicitedNotify) {
                                ...
                                SIPDialog subscriptionDialog2 = pendingSubscribeClientTx.getDefaultDialog();
                                if (subscriptionDialog2 == null || subscriptionDialog2.getDialogId() == null || !subscriptionDialog2.getDialogId().equals(dialogId)) {
                                    if (subscriptionDialog2 == null || subscriptionDialog2.getDialogId() != null) {
                                        subscriptionDialog = pendingSubscribeClientTx.getDialog(dialogId);
                                    } else {
                                        subscriptionDialog2.setDialogId(dialogId);
                                        subscriptionDialog = subscriptionDialog2;
                                    }
                                    if (sipStack2.isLoggingEnabled()) {
                                    ...
                                    transaction.setDialog(subscriptionDialog2, dialogId);
                                    if (!transaction.isTransactionMapped()) {
                                        this.sipStack.mapTransaction(transaction);
                                        transaction.setPassToListener();
                                        try {
                                    ...
                                    sipStack2.putDialog(subscriptionDialog2);
                                    subscriptionDialog2.addTransaction(pendingSubscribeClientTx);
                                    pendingSubscribeClientTx.setDialog(subscriptionDialog2, dialogId);
                                    subscriptionDialog = subscriptionDialog2;
                                }
                                if (transaction.isTransactionMapped()) {
    ...
    public void processResponse(SIPResponse response, MessageChannel incomingMessageChannel, SIPDialog dialog) {
        ...
        if (this.listeningPoint == null) {
            ...
            if (sipProvider == null) {
                ...
                if (!(dialog == 0 || response.getStatusCode() == 100)) {
