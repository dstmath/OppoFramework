...
public class HTMLScanner implements Scanner, Locator {
    ...
    static {
        ...
        while (true) {
            ...
            int[] iArr2 = statetable;
            if (iArr2[i + 1] > maxChar) {
                maxChar = iArr2[i + 1];
            }
            i += 4;
        }
        statetableIndexMaxChar = maxChar + 1;
        statetableIndex = (short[][]) Array.newInstance(short.class, maxState + 1, maxChar + 3);
        for (int theState2 = 0; theState2 <= maxState; theState2++) {
            for (int ch = -2; ch <= maxChar; ch++) {
                int hit = -1;
                int action = 0;
                int i2 = 0;
                while (true) {
                    int[] iArr3 = statetable;
                    if (i2 >= iArr3.length) {
                        break;
                    }
                    if (theState2 != iArr3[i2]) {
                        if (action != 0) {
                            break;
                        }
                    } else if (iArr3[i2 + 1] == 0) {
                        hit = i2;
                        action = iArr3[i2 + 2];
                    } else if (iArr3[i2 + 1] == ch) {
                        hit = i2;
                        int action2 = iArr3[i2 + 2];
                        break;
                    }
                    i2 += 4;
                }
                statetableIndex[theState2][ch + 2] = (short) hit;
            }
        }
    }
    ...
    public void scan(Reader r0, ScanHandler h) throws IOException, SAXException {
        ...
        while (this.theState != 21) {
            ...
            if (ch >= 32 || ch == 10 || ch == 9 || ch == -1) {
                short s = statetableIndex[this.theState][((ch < -1 || ch >= statetableIndexMaxChar) ? -2 : ch) + 2];
                int action = 0;
                if (s != -1) {
                    int[] iArr = statetable;
                    action = iArr[s + 2];
                    this.theNextState = iArr[s + 3];
                }
                switch (action) {
                    case 0:
                    ...
                    case 12:
                        ...
                        if (i3 > 0) {
                            i = 0;
                            h.gi(this.theOutputBuffer, 0, i3);
                        } else {
                            i = 0;
                        }
                        this.theSize = i;
                        h.stage(this.theOutputBuffer, i, this.theSize);
                        break;
                    case 13:
                        mark();
                        char ch1 = (char) ch;
    ...
    private void save(int ch, ScanHandler h) throws IOException, SAXException {
        ...
        if (i >= cArr.length - 20) {
            int i2 = this.theState;
            if (i2 == 28 || i2 == 10) {
                h.pcdata(this.theOutputBuffer, 0, this.theSize);
                this.theSize = 0;
            } else {
                char[] newOutputBuffer = new char[(cArr.length * 2)];
                System.arraycopy(cArr, 0, newOutputBuffer, 0, i + 1);
