...
public final class Cookie {
    ...
    static Cookie parse(long currentTimeMillis, HttpUrl url, String setCookie) {
        ...
        while (pos < limit2) {
            ...
            String attributeValue = attributeEqualsSign < attributePairEnd ? Util.trimSubstring(str, attributeEqualsSign + 1, attributePairEnd) : "";
        ...
        if (deltaSeconds == Long.MIN_VALUE) {
            ...
            long expiresAt4 = currentTimeMillis + (deltaSeconds <= 9223372036854775L ? 1000 * deltaSeconds : Long.MAX_VALUE);
        ...
        if (domain3 == null) {
            domain2 = url.host();
        } else if (!domainMatch(url, domain3)) {
            return null;
        } else {
            domain2 = domain3;
        }
        if (path2 == null || !path2.startsWith("/")) {
            ...
            int lastSlash = encodedPath2.lastIndexOf(47);
            encodedPath = lastSlash != 0 ? encodedPath2.substring(0, lastSlash) : "/";
        } else {
            encodedPath = path2;
        }
        return new Cookie(cookieName, cookieValue, expiresAt2, domain2, encodedPath, secureOnly, httpOnly2, hostOnly2, persistent2);
    }
