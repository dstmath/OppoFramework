...
public class OppoStatistics {
    ...
    private static long mCalcStartTime = 0;
    private static long mDetalCalcTime = 28800000;
    private static int sAppId = 20120;
    private static int sCaclCount_1 = 0;
    private static int sCaclCount_2 = 0;
    private static int sCount = 0;
    private static int sMaxCaclCount = OppoCommonConstants.PSW_LAST_MESSAGE;
    ...
    public static void onCommon(Context context, String logTag, String eventId, Map<String, String> logMap, int needCountId) {
        if (mCalcStartTime == 0) {
            mCalcStartTime = System.currentTimeMillis();
        } else if (System.currentTimeMillis() - mCalcStartTime >= mDetalCalcTime) {
            sCaclCount_1 = 0;
            sCaclCount_2 = 0;
            mCalcStartTime = System.currentTimeMillis();
        }
        if (needCountId == 1 && sCaclCount_1 > sMaxCaclCount) {
            return;
        }
        if (needCountId != 2 || sCaclCount_2 <= sMaxCaclCount) {
            onCommon(context, sAppId, logTag, eventId, logMap, false);
            if (needCountId == 1) {
                sCaclCount_1++;
