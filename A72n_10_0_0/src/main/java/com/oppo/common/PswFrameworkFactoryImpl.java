...
public class PswFrameworkFactoryImpl extends PswFrameworkFactory {
    ...
    public <T extends IOppoCommonFeature> T getFeature(T def, Object... vars) {
        ...
        if (i == 1) {
            return (T) OppoFeatureManager.getTraceMonitor(getPswFeatureDemo(vars));
        }
        if (i == 2) {
            Log.i(TAG, "get feature:" + def.index().name());
            return (T) OppoFeatureManager.getTraceMonitor(getPswMediaPlayerUtils(vars));
        } else if (i == 3) {
            return (T) OppoFeatureManager.getTraceMonitor(getWifiRomUpdateHelper(vars));
        } else {
            if (i != 4) {
            ...
            Log.i(TAG, "get feature:" + def.index().name());
            return (T) OppoFeatureManager.getTraceMonitor(getPswScreenModeFeature(vars));
        }
    }
