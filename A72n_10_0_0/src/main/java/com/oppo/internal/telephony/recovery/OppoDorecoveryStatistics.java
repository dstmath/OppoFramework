...
public class OppoDorecoveryStatistics {
    ...
    private static ArrayList<PdpRecoveryStatisticsResult> mResultList = new ArrayList<>();
    ...
    public static void uploadLog(Context context) {
        ...
        synchronized (mLock) {
            Iterator<PdpRecoveryStatisticsResult> it = mResultList.iterator();
            while (it.hasNext()) {
                saveLog(context, OppoRIL.SYS_OEM_NW_DIAG_CAUSE_DATA_DORECOVERY_RESULT, it.next().toJson());
            }
            mResultList.clear();
    ...
    public static void updateLastResult(boolean fail) {
        long curTime = System.currentTimeMillis();
        Rlog.d(TAG, "updateLastResult " + fail + ", " + curTime);
        synchronized (mLock) {
            int size = mResultList.size();
            if (size > 0) {
                PdpRecoveryStatisticsResult result = mResultList.get(size - 1);
                if (result.mRecheckDelay == 0) {
                    result.mRecheckFail = fail;
                    result.mRecheckDelay = curTime - result.mCurTime;
                }
            }
        }
    }
    ...
    public static class PdpRecoveryStatisticsResult {
        ...
        public String toJson() {
            ...
            try {
                ...
                jw.name("mRecheckFail").value(this.mRecheckFail);
                jw.name("mRecheckDelay").value(this.mRecheckDelay);
                jw.endObject();
