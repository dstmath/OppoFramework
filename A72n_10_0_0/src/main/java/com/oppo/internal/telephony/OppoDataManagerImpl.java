...
public class OppoDataManagerImpl implements IOppoDataManager {
    ...
    public boolean handleDataBlockControl(Phone phone, DataConnection.ConnectionParams cp, ApnContext apnContext, DataConnection dc, int event) {
        ...
        if (OppoPolicyController.isPoliceVersion(phone)) {
            ...
            boolean isSpecialApn = !TextUtils.isEmpty(apntype) && ("ims".equals(apntype) || "emergency".equals(apntype) || "mms".equals(apntype));
            ...
            if (isDataAllow || isSpecialApn) {
                try {
                    if (!OppoPolicyController.isMmsAllow() && !TextUtils.isEmpty(apntype) && "mms".equals(apntype)) {
                ...
                DataCallResponse response2 = new DataCallResponse(65535, -1, 0, 0, 0, "", (List) null, (List) null, (List) null, (List) null, 0);
                Message msg2 = dc.obtainMessage(event, cp);
                msg2.getData().putParcelable("data_call_response", response2);
                dc.sendMessage(msg2);
                return true;
            }
        }
        return phone != null && OppoPolicyController.getMmsRestricted(phone.getPhoneId()) && apnContext != null && apnContext.getApnType().equals("mms");
    }
