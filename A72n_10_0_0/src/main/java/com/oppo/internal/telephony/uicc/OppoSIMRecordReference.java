...
public class OppoSIMRecordReference implements IOppoSIMRecords {
    ...
    public void fetchIMPI() {
        log("fetchIMPI()");
        String operator = SystemProperties.get(PROPERTY_OPERATOR);
        String regionMark = SystemProperties.get("ro.oppo.regionmark");
        String region = SystemProperties.get(PROPERTY_REGION);
        ...
        if ("O2".equals(operator) && "EUEX".equals(regionMark) && "GB".equals(region) && "GB".equals(country)) {
            AbstractSIMRecords tmpSimRecords = (AbstractSIMRecords) OemTelephonyUtils.typeCasting(AbstractSIMRecords.class, this.mSIMRecords);
            int recordsToLoad = tmpSimRecords.getRecordsToLoad();
            this.mFh.loadEFTransparent(28418, this.mSIMRecords.obtainMessage(100, new EfImpiLoaded()));
            tmpSimRecords.setRecordsToLoad(recordsToLoad + 1);
        }
    }

    private class EfImpiLoaded implements IccRecords.IccRecordLoaded {
        private EfImpiLoaded() {
        }
        ...
        public void onRecordLoaded(AsyncResult ar) {
            OppoSIMRecordReference.this.mImpi = IccUtils.bytesToHexString((byte[]) ar.result);
            SystemProperties.set("persist.sys.impi", OppoSIMRecordReference.this.mImpi);
            OppoSIMRecordReference oppoSIMRecordReference = OppoSIMRecordReference.this;
            oppoSIMRecordReference.log("EfImpiLoaded EF_IMPI=" + OppoSIMRecordReference.this.mImpi);
        }
    }
    ...
    public void setDataAndDataroamingForOperators(String gid1) {
        log("setDataAndDataroamingForOperators gid1=" + gid1);
        String simNumeric = this.mTelephonyManager.getSimOperatorNumericForPhone(this.mPhone.getPhoneId());
        if (("26201".equals(simNumeric) && matchGid1(gid1, "02")) || ("26002".equals(simNumeric) && (matchGid1(gid1, "21") || matchGid1(gid1, "22") || matchGid1(gid1, "23") || matchGid1(gid1, "24") || matchGid1(gid1, "26")))) {
            log("setUserDataEnabled false for DE TELECOM prepay SIM / PL T-mobile prepay SIM");
            this.mPhone.getDataEnabledSettings().setUserDataEnabled(false);
        }
        if ("26002".equals(simNumeric) || (("20416".equals(simNumeric) && matchGid1(gid1, "FF")) || ("20420".equals(simNumeric) && matchGid1(gid1, "FF")))) {
            log("setDataRoamingEnabled ON for PL T-mobile / NL T-mobile SIM");
            this.mPhone.setDataRoamingEnabled(true);
        }
    }

    private boolean matchGid1(String fullGid1, String targetGid1) {
        log("matchGID1 fullGid1=" + fullGid1 + " targetGid1=" + targetGid1);
        if (targetGid1 == null) {
            return false;
        }
        int length = targetGid1.length();
        if (fullGid1 == null || fullGid1.length() < length || !fullGid1.substring(0, length).equalsIgnoreCase(targetGid1)) {
            return false;
        }
        return true;
    }
