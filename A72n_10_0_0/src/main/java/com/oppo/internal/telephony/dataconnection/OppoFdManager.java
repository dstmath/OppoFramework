...
public class OppoFdManager extends Handler {
    ...
    private static String[] sTimerValueForUKThree = {"30", "30", "30", "30"};
    private static String[] sTimerValueForUK_O2 = {"50", "50", "50", "50"};
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (c == 0) {
                ...
                oppoFdManager.mIsTetheredMode = z;
    ...
    private boolean shouldEnableFd() {
        logd("mIsScreenOn = " + this.mIsScreenOn);
        if ((isFdEnabledOnlyWhenScreenOff() || isUKThree()) && this.mIsScreenOn) {
            return false;
        }
    ...
    private void updateFdModeIfNeeded() {
        try {
            if (isFdAllowed() && isOperatorSupport()) {
                ...
                logd("shouldEnableFd = " + shouldEnableFd());
                if (enable) {
                    logd("ENABLE_FASTDORMANCY");
    ...
    private void onScreenSwitched(boolean isScreenOn) {
        ...
        if (this.mIsScreenOn != isScreenOn) {
            ...
            if (isFdEnabledOnlyWhenScreenOff() || isUKThree()) {
                updateFdModeIfNeeded();
            }
        }
    }
    ...
    private void setFdConf() {
        try {
            if (isEuOrange()) {
                ...
                setFdTimerValue(sTimerValueForUK_O2);
            } else if (isUKThree()) {
                setFdTimerValue(sTimerValueForUKThree);
            }
    ...
    private boolean isEuTmobile() {
        try {
            IccRecords r = this.mPhone.getIccRecords();
            String operatorNumeric = r != null ? r.getOperatorNumeric() : "";
            if ("20416".equals(operatorNumeric) || "20420".equals(operatorNumeric) || "26002".equals(operatorNumeric) || "26034".equals(operatorNumeric) || "26201".equals(operatorNumeric) || "26206".equals(operatorNumeric)) {
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    private boolean isUKO2() {
        try {
            String regionmark = SystemProperties.get("ro.oppo.regionmark", "");
            String operator = SystemProperties.get("ro.oppo.operator", "");
            String country = SystemProperties.get("ro.oppo.euex.country", "");
            logd("regionmark=" + regionmark + ", operator=" + operator + ", country=" + country);
            if (TextUtils.isEmpty(regionmark) || !"EUEX".equals(regionmark) || TextUtils.isEmpty(operator) || !"O2".equals(operator) || TextUtils.isEmpty(country) || !"GB".equals(country)) {
                return false;
            }
            return true;
        } catch (Exception e) {
    ...
    private boolean isUKThree() {
        try {
            IccRecords r = this.mPhone.getIccRecords();
            if ("23420".equals(r != null ? r.getOperatorNumeric() : "")) {
                return true;
            }
            return false;
        } catch (Exception e) {
