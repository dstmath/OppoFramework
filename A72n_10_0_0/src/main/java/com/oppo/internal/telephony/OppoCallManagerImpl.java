...
public class OppoCallManagerImpl implements IOppoCallManager {
    ...
    /* JADX INFO: Multiple debug info for r12v3 boolean: [D('log_array' java.lang.String[]), D('imscall' boolean)] */
    ...
    public boolean isRestricted(int type) {
        if (type == 0) {
            return OppoPolicyController.getCallInRestricted();
        }
        if (type == 1) {
            return OppoPolicyController.getCallOutRestricted();
        }
        if (type == 2) {
            return OppoPolicyController.getSmsReceiveRestricted();
        }
        if (type == 3) {
            return OppoPolicyController.getSmsSendRestricted();
        }
        if (type != 4) {
            return false;
        }
