...
public class OppoSMSDispatcherReference implements IOppoSMSDispatcher {
    ...
    public boolean handleSmsSendControl(SMSDispatcher.SmsTracker smsTracker, Phone phone, Context context, int event) {
        boolean isPolicyMessageBlockList = false;
        try {
            if (OppoPolicyController.isPoliceVersion(phone)) {
                if (OppoSmsCommonUtils.oemIsMtSmsBlock(context, smsTracker.mDestAddress) && context.getPackageManager().hasSystemFeature("oppo.common.gov.black_or_white_contactlist")) {
                    isPolicyMessageBlockList = true;
                }
                ...
                if (context != null && (!isPolicyMessageSendEnable || isPolicyMessageBlockList)) {
                    smsTracker.onFailed(context, event, 0);
                    return true;
                }
            }
            if (phone != null && OppoPolicyController.getSmsSendRestricted(phone.getPhoneId()) && OppoPolicyController.isOppoSimLockVersion()) {
                OppoSimlockManagerImpl.broadcastSmsSentRestrict(context, phone.getPhoneId());
                if (context != null) {
                    smsTracker.onFailed(context, event, 0);
                    return true;
                }
