...
public class OppoRegSrvInfo {
    ...
    public static class RegSrvInfo {
        ...
        public void setDuration() {
            ...
            this.duration = this.duration / 1000.0f;
    ...
    public static boolean isCollectNotDoing(int sub) {
        ...
        if (sub < 0 || sub > 1) {
            ...
            return false;
        } else if (g_regsrv_info[sub].getCollectState() == 0) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isCollectDoing(int sub) {
        ...
        if (sub < 0 || sub > 1) {
            ...
            return false;
        } else if (g_regsrv_info[sub].getCollectState() == 1) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isCollectFinished(int sub) {
        ...
        if (sub < 0 || sub > 1) {
            ...
            return false;
        } else if (g_regsrv_info[sub].getCollectState() == 2) {
            return true;
        } else {
            return false;
        }
    ...
    public static boolean isCollectReported(int sub) {
        ...
        if (sub < 0 || sub > 1) {
            ...
            return false;
        } else if (g_regsrv_info[sub].getCollectState() == 3) {
            return true;
        } else {
            return false;
        }
    ...
    public static String getCollectInfo(int sub) {
        ...
        return ("kacao:" + g_regsrv_info[sub].getSubId()) + ("start_time:" + g_regsrv_info[sub].getStartTimeStr()) + ("duration:" + g_regsrv_info[sub].getDuration()) + ("plmn_list:" + g_regsrv_info[sub].getPlmnList().toString());
    }
        ...
        switch (msgtype) {
            case 72:
                if (isCollectNotDoing(sub) || isCollectReported(sub)) {
                    ...
                    StringBuffer plmnList = g_regsrv_info[sub].getPlmnList();
                    plmnList.append("|" + ratToString(code));
                }
                Rlog.d(LOG_TAG, "collectRegSrvInfo() REG_SRV_REQ msgtype:" + msgtype);
                break;
            case 73:
                if (isCollectDoing(sub)) {
                    StringBuffer plmnList2 = g_regsrv_info[sub].getPlmnList();
                    plmnList2.append("_" + String.valueOf(code));
                }
                Rlog.d(LOG_TAG, "collectRegSrvInfo() REG_SRV_MCC msgtype:" + msgtype);
                break;
            case 74:
                if (isCollectDoing(sub)) {
                    StringBuffer plmnList3 = g_regsrv_info[sub].getPlmnList();
                    plmnList3.append("_" + String.valueOf(code));
                }
