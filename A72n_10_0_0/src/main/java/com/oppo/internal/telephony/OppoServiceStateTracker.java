...
import com.oppo.internal.telephony.nrNetwork.OppoNrSaNsaMode;
...
public class OppoServiceStateTracker extends Handler implements IOppoServiceStateTracker {
    ...
    private boolean mIsOemRadioReseting = false;
    ...
    public OppoServiceStateTracker(ServiceStateTracker sst, GsmCdmaPhone phone) {
        ...
        OppoNrSaNsaMode.getInstance(this.mContext);
    }
        ...
        switch (msg.what) {
            ...
            case 3:
                ...
                if (true == this.mIsScreenOn) {
                    ...
                    this.mIsOemRadioReseting = false;
                }
    ...
    public int oppoOosDelayState(ServiceState mNewSS) {
        ...
        boolean isOos = ((getOemRegState(mNewSS) == 1 || getOemRegState(mNewSS) == 2) && this.mPhone.isRadioOn() && this.mSST.mSS.getState() != 3) || (this.mPhone.getRadioPowerState() == 0 && this.mDuplexModeChangeOnGoing) || ((getOemRegState(mNewSS) == 1 && (this.mIsModemRestarting || this.mIsOemRadioReseting)) || (getOemRegState(mNewSS) == 3 && (this.mIsModemRestarting || this.mIsOemRadioReseting)));
        ...
        logd("inSwitchingDss = " + inSwitchingDss + "  mSimState = " + mSimState + " isOos=" + isOos + " isOosInVowifi:" + isOosInVowifi);
        if (isOos || isOosInVowifi) {
            if (isOtherPhoneIncall()) {
                i = 1;
            } else if (this.mPhone.getState() != PhoneConstants.State.IDLE) {
                i = 1;
            } else if (!this.mIsModemRestarting && !this.mIsOemRadioReseting && (inSwitchingDss || mSimState != 5 || !twoCTcardNeedOosDelay())) {
        ...
        boolean isSimRemainSelPLMN = false;
        String simPlmn = TelephonyManager.getDefault().getSimOperatorNumeric();
        if (simPlmn != null && (simPlmn.equals("21403") || simPlmn.equals("21409") || simPlmn.equals("21411") || simPlmn.equals("21404") || simPlmn.equals("21405") || simPlmn.equals("21407"))) {
            isSimRemainSelPLMN = true;
        }
        logd("simPlmn = " + simPlmn + ", isSimRemainSelPLMN = " + isSimRemainSelPLMN);
        if (2 == this.mOosDelayState) {
            ...
            if (testMode == 0 && !isSimRemainSelPLMN) {
    ...
    public boolean onSignalStrengthResultEx(AsyncResult ar, SignalStrength oldSignalStrength) {
        ...
        if (getOemRegState(this.mSST.mSS) != 0 && !this.mIsModemRestarting && !this.mIsOemRadioReseting) {
        ...
        if (gsmCdmaPhone != null && gsmCdmaPhone.isPhoneTypeGsm()) {
            ...
            if (OppoServiceStateTrackerUtil.isVodafoneOperatorCheck(simCardMccMnc)) {
                ...
                if (OppoServiceStateTrackerUtil.isVodafoneHomePlmn(plmn, simCardMccMnc)) {
                    if (!TextUtils.isEmpty(spn)) {
                        pnnName2 = spn;
                        logd("getPnnName from spn, pnnName=" + pnnName2);
                    } else if (!(tmpSimRecords == null || tmpSimRecords.getEFpnnNetworkNames(0) == null)) {
                        pnnName2 = tmpSimRecords.getEFpnnNetworkNames(0).sFullName;
                        logd("getPnnName from pnn[0], pnnName=" + pnnName2);
                    }
                    ...
                    if (!(this.mSST.mSS.getVoiceRegState() == 0 || 18 != this.mSST.mSS.getRilDataRadioTechnology() || tmpSimRecords.getEFpnnNetworkNames(0) == null)) {
                        pnnName2 = tmpSimRecords.getEFpnnNetworkNames(0).sFullName;
                        logd("getPnnName from pnn[0] for IWLAN, pnnName=" + pnnName2);
                    }
                } else if (tmpSimRecords.getEFpnnNetworkNames(1) != null) {
                    pnnName2 = tmpSimRecords.getEFpnnNetworkNames(1).sFullName;
                    logd("getPnnName from pnn[1], pnnName=" + pnnName2);
                }
            }
            if (OppoServiceStateTrackerUtil.isGT4GSimCardCheck(simCardMccMnc)) {
                ...
                if (!TextUtils.isEmpty(spn)) {
                    ...
                    String pnnName3 = tmpSimRecords.getEFpnnNetworkNames(0).sFullName;
    ...
    public void setOemRadioReseting(boolean enable) {
        this.mIsOemRadioReseting = enable;
        if (!enable) {
            this.mSST.pollState();
        }
    }

    public boolean isNotNotifyMergeServiceOperator() {
        List<String> NOTNOTIFY_MERGE_SERVICE_LIST = Arrays.asList("50502", "50503", "50506", "52001", "52003", "28601", "28602", "45403");
        ...
        logd("isNotNotifyMergeServiceOperator current mccMnc = " + mccMnc);
        return NOTNOTIFY_MERGE_SERVICE_LIST.contains(mccMnc);
    }
}
