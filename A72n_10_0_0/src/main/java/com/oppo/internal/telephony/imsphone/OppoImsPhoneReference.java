...
public class OppoImsPhoneReference implements IOppoImsPhone {
    ...
    private Handler mHandler = new Handler(mConfigThread.getLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            boolean z = true;
            switch (msg.what) {
                case OppoImsPhoneReference.EVENT_DEFAULT_DATA_SUBSCRIPTION_CHANGED /* 1050 */:
                    OppoImsPhoneReference.this.evalUpdateImsConfig();
                    return;
                case OppoImsPhoneReference.EVENT_NETWORK_MODE_CHANGED /* 1051 */:
                    OppoImsPhoneReference.this.evalUpdateImsConfig(true);
                    return;
                case OppoImsPhoneReference.EVENT_CARRIER_CONFIG_CHANGED /* 1052 */:
                    ...
                    return;
                case OppoImsPhoneReference.EVENT_ROM_UPDATE_START /* 1053 */:
                    ...
                    return;
                case OppoImsPhoneReference.EVENT_RADIO_STATE_CHANGED /* 1054 */:
                    OppoImsPhoneReference.this.resetRadioAndIms(true);
                    return;
                case OppoImsPhoneReference.EVENT_DELAY_TO_UNREGISTER /* 1055 */:
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void startObserverNetworkMode() {
        ImsPhone imsPhone;
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void evalUpdateImsConfig(boolean withDualLTE) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void evalUpdateImsConfig() {
        ImsManager imsmanager = ImsManager.getInstance(this.mContext, this.mPhoneId);
        if (this.mPhone.getDefaultPhone() != null) {
            int subId = this.mPhone.getDefaultPhone().getSubId();
            if (imsmanager != null) {
                boolean isDualLTE = OemTelephonyUtils.getBooleanCarrierConfig(this.mContext, "config_oppo_dual_lte_available_bool", subId);
                logd("evalUpdateImsConfig isDualLTE " + isDualLTE);
                if (((TelephonyManager) this.mContext.getSystemService("phone")).getSimState(this.mPhoneId) == 10) {
                    imsmanager.updateImsServiceConfig(true);
                }
            }
        }
    ...
    public void setUserAgentToMd() {
        ...
        IccRecords iccRecords = this.mPhone.getDefaultPhone().getIccRecords();
        if (iccRecords != null) {
            String mccMnc = iccRecords.getOperatorNumeric();
            logd("setXcapUserAgentToMd value : " + value + " for mccMnc : " + mccMnc);
            if ("20416".equals(mccMnc) || "20420".equals(mccMnc)) {
                this.mPhone.getDefaultPhone().invokeOemRilRequestStrings(new String[]{"AT+EXCAPUAS=\"" + value + "\"", ""}, (Message) null);
            }
        }
    }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void loadPresetRusConfig() {
        ImsManager imsManager = ImsManager.getInstance(this.mContext, this.mPhoneId);
        SharedPreferences sharedPrefs = PreferenceManager.getDefaultSharedPreferences(this.mContext);
        if (imsManager == null) {
            return;
