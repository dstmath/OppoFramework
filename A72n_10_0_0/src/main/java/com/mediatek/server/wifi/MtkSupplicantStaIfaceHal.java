...
public class MtkSupplicantStaIfaceHal {
        ...
        synchronized (getLockForSupplicantStaIfaceHal()) {
            ...
            try {
                ...
                if (supplicantStaIfaceV21 != null) {
                    return checkStatusAndLogFailure(supplicantStaIfaceV21.registerCallback_2_1(callback), "registerMtkCallback");
                }
    ...
    private static ISupplicantIface getMtkIfaceV2_0(String ifaceName) {
        synchronized (getLockForSupplicantStaIfaceHal()) {
            ...
            try {
                ...
                Log.e(TAG, "ISupplicant.listInterfaces exception: " + e3);
                supplicantServiceDiedHandler(ifaceName);
                return null;
            } catch (IllegalArgumentException e4) {
                Log.e(TAG, "ISupplicant.listInterfaces IllegalArgumentException: " + e4);
                supplicantServiceDiedHandler(ifaceName);
                return null;
            }
        }
    ...
    /* access modifiers changed from: private */
    public static class SupplicantStaIfaceHalCallbackV2_1 extends ISupplicantStaIfaceCallback.Stub {
        ...
        @Override // vendor.mediatek.hardware.wifi.supplicant.V2_1.ISupplicantStaIfaceCallback
        public void onBigdataNotice(String data, int len) {
            Log.d(MtkSupplicantStaIfaceHal.TAG, "onBigdataNotice --> " + data + " len = " + len + " data.len = " + data.length());
            WifiInjector.getInstance().getClientModeImpl().notifyBigdataEvent(data);
        }
    }
