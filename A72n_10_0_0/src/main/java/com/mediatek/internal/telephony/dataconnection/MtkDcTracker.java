...
public class MtkDcTracker extends DcTracker {
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r3v0, resolved type: com.mediatek.internal.telephony.dataconnection.MtkDcTracker */
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r1v0, resolved type: com.mediatek.internal.telephony.dataconnection.MtkDcTracker */
    /* JADX WARN: Multi-variable type inference failed */
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r2v0, resolved type: com.mediatek.internal.telephony.dataconnection.MtkDcTracker */
    /* JADX WARN: Multi-variable type inference failed */
    /* access modifiers changed from: protected */
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r9v0, resolved type: com.mediatek.internal.telephony.dataconnection.MtkDcTracker */
    /* JADX WARN: Multi-variable type inference failed */
    /* access modifiers changed from: protected */
            ...
            for (ApnContext apnContext : this.mApnContexts.values()) {
                if (!hasMdAutoSetupImsCapability() || !"nwTypeChanged".equals(reason) || !"emergency".equals(apnContext.getApnType())) {
                    ...
                    if (currentApnSetting == null) {
                        ...
                        if (!canHandle || !(!isRestorePreferredApn() || apnContext.getWaitingApns() == null || waitingApns.size() == apnContext.getWaitingApns().size())) {
                            if (VDBG) {
        ...
        if (this.mWorkerHandler == null) {
            new Thread() {
                ...
                public void run() {
                    Looper.prepare();
                    MtkDcTracker mtkDcTracker = MtkDcTracker.this;
                    mtkDcTracker.mWorkerHandler = new WorkerHandler();
                    Looper.loop();
                }
            }.start();
        }
    ...
    public long mtkModifyInterApnDelay(long delay, ApnContext apnContext) {
        ...
        if (!"default".equals(apnContext.getApnType()) || !TextUtils.equals(apnContext.getReason(), DataFailCause.toString(3982))) {
            return delay;
        }
        return 100;
    }
    ...
    public boolean mtkAddOrSendRequestNetworkCompleteMsg(int apnType, int requestType, Message onCompleteMsg) {
        if (requestType != 2 || this.mTransportType != 2) {
            return false;
        }
        if (!this.mPhone.isWifiCallingEnabled()) {
            log("send complete msg with HANDOFF_PREFERENCE_CHANGED cause");
            sendRequestNetworkCompleteMsg(onCompleteMsg, false, this.mTransportType, requestType, MtkCharacterSets.WINDOWS_1251);
            return true;
        } else if (getDataRat() != 0) {
            return false;
        } else {
            log("add complete msg and remember the handover apn type");
    ...
    private boolean isRestorePreferredApn() {
        ...
        boolean bRestore = cursor.getCount() == 0;
        cursor.close();
        return bRestore;
    }
