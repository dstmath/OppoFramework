...
public class MtkWapPushOverSms extends WapPushOverSms {
    ...
    /* JADX INFO: Multiple debug info for r0v31 int: [D('intentData' byte[]), D('dataIndex' int)] */
    private DecodedResult decodeWapPdu(byte[] pdu, InboundSmsHandler handler) {
        ...
        int index = 0 + 1;
        try {
            int transactionId = pdu[0] & PplMessageManager.Type.INVALID;
            int index2 = index + 1;
            int pduType = pdu[index] & PplMessageManager.Type.INVALID;
            ...
            if (!(pduType == 6 || pduType == 7)) {
                ...
                if (index3 != -1) {
                    ...
                    index2 = index4 + 1;
                    pduType = pdu[index4] & PplMessageManager.Type.INVALID;
                    if (ENG) {
