...
public class BlockChecker {
    ...
    public static int getBlockStatus(Context context, String phoneNumber, Bundle extras) {
        ...
        try {
            ...
            if (blockStatus != 0) {
                OppoRlog.Rlog.d(TAG, "is blocked.");
            }
        } catch (Exception e) {
            OppoRlog.Rlog.e(TAG, "Exception checking for blocked number: " + e);
        }
        int durationMillis = (int) ((System.nanoTime() - startTimeNano) / 1000000);
        if (durationMillis > 500) {
            OppoRlog.Rlog.d(TAG, "Blocked number lookup took: " + durationMillis + " ms.");
        }
        return blockStatus;
    }
}
