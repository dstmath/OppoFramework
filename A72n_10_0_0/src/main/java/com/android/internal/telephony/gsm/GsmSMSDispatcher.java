...
public class GsmSMSDispatcher extends SMSDispatcher {
    ...
    public GsmSMSDispatcher(Phone phone, SmsDispatchersController smsDispatchersController, GsmInboundSmsHandler gsmInboundSmsHandler) {
        ...
        OppoRlog.Rlog.d(TAG, "GsmSMSDispatcher created");
    }
    ...
    public void sendSms(SMSDispatcher.SmsTracker tracker) {
        ...
        if (handleSmsSendControl(tracker, this.mPhone, this.mContext, 1)) {
            OppoRlog.Rlog.d(TAG, "gsm--sendSms, stop tracker.");
            return;
        }
    ...
    public UiccCardApplication getUiccCardApplication() {
        OppoRlog.Rlog.d(TAG, "GsmSMSDispatcher: subId = " + this.mPhone.getSubId() + " slotId = " + this.mPhone.getPhoneId());
    ...
    public void onUpdateIccAvailability() {
        ...
        if (this.mUiccController != null && (app = this.mUiccApplication.get()) != (newUiccApplication = getUiccCardApplication())) {
            if (app != null) {
                OppoRlog.Rlog.d(TAG, "Removing stale icc objects.");
            ...
            if (newUiccApplication != null) {
                OppoRlog.Rlog.d(TAG, "New Uicc application found");
