...
public abstract class SMSDispatcher extends AbstractSMSDispatcher {
    ...
    public void sendText(String destAddr, String scAddr, String text, PendingIntent sentIntent, PendingIntent deliveryIntent, Uri messageUri, String callingPkg, boolean persistMessage, int priority, boolean expectMore, int validityPeriod, boolean isForVvm) {
        ...
        String repairdDestAddr = repairSmsNumber(destAddr);
        SmsMessageBase.SubmitPduBase pdu = onSendText(repairdDestAddr, scAddr, text, sentIntent, deliveryIntent, messageUri, callingPkg, persistMessage, priority, expectMore, validityPeriod);
        if (pdu != null) {
            SmsTracker tracker = getSmsTracker(callingPkg, getSmsTrackerMap(repairdDestAddr, scAddr, text, pdu), sentIntent, deliveryIntent, getFormat(), messageUri, expectMore, text, true, persistMessage, priority, validityPeriod, isForVvm);
    ...
    public boolean checkDestination(SmsTracker tracker) {
        int event;
        if (!"VODAFONE_EEA".equals(SystemProperties.get("ro.oppo.operator", PhoneConfigurationManager.SSSS))) {
            OppoRlog.Rlog.d(TAG, "operator " + SystemProperties.get("ro.oppo.operator", PhoneConfigurationManager.SSSS));
            return true;
        } else if (this.mContext.checkCallingOrSelfPermission("android.permission.SEND_SMS_NO_CONFIRMATION") == 0 || tracker.mIsForVvm) {
            return true;
        } else {
            int rule = this.mPremiumSmsRule.get();
            int smsCategory = 0;
            if (rule == 1 || rule == 3) {
                String simCountryIso = this.mTelephonyManager.getSimCountryIsoForPhone(this.mPhone.getPhoneId());
                if (simCountryIso == null || simCountryIso.length() != 2) {
                    OppoRlog.Rlog.e(TAG, "Can't get SIM country Iso: trying network country Iso");
                    simCountryIso = this.mTelephonyManager.getNetworkCountryIsoForPhone(this.mPhone.getPhoneId());
                }
                smsCategory = this.mSmsDispatchersController.getUsageMonitor().checkDestination(tracker.mDestAddress, simCountryIso);
            }
            if (rule == 2 || rule == 3) {
                String networkCountryIso = this.mTelephonyManager.getNetworkCountryIsoForPhone(this.mPhone.getPhoneId());
                if (networkCountryIso == null || networkCountryIso.length() != 2) {
                    OppoRlog.Rlog.e(TAG, "Can't get Network country Iso: trying SIM country Iso");
                    networkCountryIso = this.mTelephonyManager.getSimCountryIsoForPhone(this.mPhone.getPhoneId());
                }
            ...
            if (Settings.Global.getInt(this.mResolver, "device_provisioned", 0) == 0) {
                OppoRlog.Rlog.e(TAG, "Can't send premium sms during Setup Wizard");
                return false;
            }
            ...
            if (premiumSmsPermission == 2) {
                OppoRlog.Rlog.w(TAG, "User denied this app from sending to premium SMS");
                ...
                OppoRlog.Rlog.d(TAG, "User approved this app to send to premium SMS");
                return true;
            }
        }
    }
    ...
    public void handleConfirmShortCode(boolean isPremium, SmsTracker tracker) {
        ...
        if (!denyIfQueueLimitReached(tracker)) {
            ...
            AlertDialog d = new AlertDialog.Builder(this.mContext, 201523207).setView(layout).setPositiveButton(r.getString(17041048), listener).setNegativeButton(r.getString(17041050), listener).setOnCancelListener(listener).create();
            d.getWindow().setType(TelephonyProto.TelephonyEvent.RilSetupDataCallResponse.RilDataCallFailCause.PDP_FAIL_MIP_FA_MOBILE_NODE_AUTHENTICATION_FAILURE);
            d.show();
    ...
    private String repairSmsNumber(String destAddr) {
        String repairdAddr = destAddr;
        String region = SystemProperties.get("persist.sys.oppo.region", "n");
        OppoRlog.Rlog.d(TAG, "Befor repairSmsNumber : " + destAddr + "region :" + region);
        if (!"CN".equalsIgnoreCase(region)) {
            return destAddr;
        }
        if (destAddr != null && destAddr.startsWith("861") && destAddr.length() == 13) {
            repairdAddr = "+" + destAddr;
        }
        OppoRlog.Rlog.d(TAG, "repairSmsNumber: destAddr = " + destAddr + " replaced to " + repairdAddr);
        return repairdAddr;
    }
}
