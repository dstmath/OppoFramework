...
public class GsmCdmaConnection extends Connection {
                ...
                switch (causeCode) {
                    ...
                    default:
                        ...
                        if (!isEmergencyCall()) {
                            ...
                            if (uiccAppState != IccCardApplicationStatus.AppState.APPSTATE_READY && (isPhoneTypeGsm() || phone.mCdmaSubscriptionSource == 0)) {
                                return 19;
                            }
                        }
                        if (isPhoneTypeGsm() && (causeCode == 65535 || causeCode == 260)) {
    ...
    public boolean update(DriverCall dc) {
        ...
        } else if (isIncoming() && !equalsBaseDialString(this.mAddress, dc.number) && (!this.mNumberConverted || !equalsBaseDialString(this.mConvertedNumber, dc.number))) {
    ...
    /* access modifiers changed from: protected */
    public int getAudioQualityFromDC(int audioQuality) {
        if (audioQuality == 2 || audioQuality == 8) {
            return 2;
        }
        return 1;
    }
    ...
    /* access modifiers changed from: protected */
    public void setPostDialState(Connection.PostDialState s) {
    ...
    @UnsupportedAppUsage
    protected static char findPOrWCharToAppend(String phoneNumber, int currPwIndex, int nextNonPwCharIndex) {
