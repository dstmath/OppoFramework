...
public class IccSmsInterfaceManager {
    ...
    public void sendMultipartTextWithOptions(String callingPackage, String destAddr, String scAddr, List<String> parts, List<PendingIntent> sentIntents, List<PendingIntent> deliveryIntents, boolean persistMessageForNonDefaultSmsApp, int priority, boolean expectMore, int validityPeriod) {
        ...
        for (int i2 = 0; i2 < parts.size(); i2++) {
            ...
            String singlePart2 = SmsMessage.shouldAppendPageNumberAsPrefix() ? String.valueOf(i2 + 1) + '/' + parts.size() + ' ' + singlePart : singlePart.concat(' ' + String.valueOf(i2 + 1) + '/' + parts.size());
            PendingIntent singleSentIntent = (sentIntents == null || sentIntents.size() <= i2) ? null : sentIntents.get(i2);
            ...
            this.mDispatchersController.sendText(destAddr2, scAddr, singlePart2, singleSentIntent, singleDeliveryIntent, null, callingPackage, persistMessageForNonDefaultSmsApp, priority, expectMore, validityPeriod, false);
        }
    }
