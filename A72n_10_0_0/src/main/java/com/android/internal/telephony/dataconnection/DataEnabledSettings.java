...
public class DataEnabledSettings {
    ...
    public synchronized void setUserDataEnabled(boolean enabled) {
        if (!isStandAloneOpportunistic(this.mPhone.getSubId(), this.mPhone.getContext()) || enabled) {
            int i = 0;
            if (((IOppoDataManager) OppoTelephonyFactory.getInstance().getFeature(IOppoDataManager.DEFAULT, new Object[0])).handleDataBlockControl(this.mPhone, enabled)) {
                log("---data-block-return---");
                return;
            }
            updateDataEnabled();
            ...
            if (GlobalSettingsHelper.setInt(context, "mobile_data", subId, i)) {
