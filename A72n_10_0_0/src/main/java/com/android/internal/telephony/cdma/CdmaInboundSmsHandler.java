...
public class CdmaInboundSmsHandler extends InboundSmsHandler {
    ...
    /* JADX INFO: Multiple debug info for r7v11 int: [D('index' int), D('destinationPort' int)] */
    private int processCdmaWapPdu(byte[] pdu, int referenceNumber, String address, String dispAddr, long timestamp) {
        ...
        if (segment == 0) {
            ...
            int index7 = index6 + 1;
            int destinationPort2 = index7 + 1;
            int destinationPort3 = ((pdu[index6] & 255) << 8) | (pdu[index7] & 255);
            if (!this.mCheckForDuplicatePortsInOmadmWapPush || !checkDuplicatePortOmadmWapPush(pdu, destinationPort2)) {
                destinationPort = destinationPort3;
                index = destinationPort2;
            } else {
                destinationPort = destinationPort3;
                index = destinationPort2 + 4;
            }
        } else {
            index = index4;
            destinationPort = 0;
