...
public class EmergencyNumberTracker extends Handler {
    ...
    private static final int EVENT_UPDATE_CARRIER_CONFIG_CAHNGED = 5;
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals("android.telephony.action.CARRIER_CONFIG_CHANGED")) {
                EmergencyNumberTracker.this.obtainMessage(5).sendToTarget();
    ...
    public void handleMessage(Message msg) {
        ...
        if (i == 1) {
            ...
            if (i != 3) {
                if (i != 4) {
                    if (i == 5) {
                        loge("EVENT_UPDATE_CARRIER_CONFIG_CAHNGED");
    ...
    private void cacheEmergencyDatabaseByCountry(String countryIso) {
        logd("ingnore cacheEmergencyDatabaseByCountry for ecc solution");
    }
    ...
    public boolean isEmergencyNumber(String number, boolean exactMatch) {
        ...
        if (!this.mEmergencyNumberListFromRadio.isEmpty()) {
            for (EmergencyNumber num : this.mEmergencyNumberList) {
                ...
                } else if (number2.startsWith("08")) {
                    if (number2.equals(num.getNumber())) {
                        return true;
                    }
    ...
    private boolean isEmergencyNumberFromEccList(String number, boolean useExactMatch) {
            ...
            for (String emergencyNum : split) {
                if (useExactMatch || this.mCountryIso.equals("br") || this.mCountryIso.equals("cl") || this.mCountryIso.equals("ni")) {
                    ...
                    for (String prefix2 : this.mEmergencyNumberPrefix) {
                        if (number.startsWith(prefix2 + emergencyNum)) {
                            return true;
                        }
                    }
                    continue;
        ...
        String[] split2 = (slotId < 0 ? "112,911,000,08,110,118,119,999" : "112,911").split(",");
        for (String emergencyNum2 : split2) {
            if (useExactMatch) {
                ...
                for (String prefix3 : this.mEmergencyNumberPrefix) {
                    if (number.equals(prefix3 + emergencyNum2)) {
                        return true;
                    }
                }
                continue;
                ...
                for (String prefix4 : this.mEmergencyNumberPrefix) {
                    if (number.equals(prefix4 + emergencyNum2)) {
                        return true;
                    }
                }
                continue;
        ...
        if (useExactMatch) {
            ...
            for (String prefix5 : this.mEmergencyNumberPrefix) {
                if (info.isEmergencyNumber(prefix5 + number, this.mCountryIso.toUpperCase())) {
                    return true;
                }
            }
            return false;
        } else if (info.connectsToEmergencyNumber(number, this.mCountryIso.toUpperCase())) {
            return true;
        } else {
            for (String prefix6 : this.mEmergencyNumberPrefix) {
                if (info.connectsToEmergencyNumber(prefix6 + number, this.mCountryIso.toUpperCase())) {
                    return true;
                }
            }
            return false;
