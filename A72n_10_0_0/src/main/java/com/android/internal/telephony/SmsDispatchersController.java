...
public class SmsDispatchersController extends AbstractSmsDispatchersController {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            OppoRlog.Rlog.d(SmsDispatchersController.TAG, "Received broadcast " + intent.getAction());
    ...
    public SmsDispatchersController(Phone phone, SmsStorageMonitor storageMonitor, SmsUsageMonitor usageMonitor) {
        OppoRlog.Rlog.d(TAG, "SmsDispatchersController created");
        ...
        this.mImsSmsDispatcher = telephonyComponentFactory.makeImsSmsDispatcher(phone, this);
        this.mCdmaDispatcher = telephonyComponentFactory.makeCdmaSMSDispatcher(phone, this);
        this.mGsmInboundSmsHandler = TelephonyComponentFactory.getInstance().inject(GsmInboundSmsHandler.class.getName()).makeGsmInboundSmsHandler(phone.getContext(), storageMonitor, phone);
        ...
        this.mGsmDispatcher = telephonyComponentFactory.makeGsmSMSDispatcher(phone, this, this.mGsmInboundSmsHandler);
        telephonyComponentFactory.makeSmsBroadcastUndelivered(phone.getContext(), this.mGsmInboundSmsHandler, this.mCdmaInboundSmsHandler);
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case 13:
                ...
                OppoRlog.Rlog.e(TAG, "IMS State query failed with exp " + ar.exception);
                return;
            case 14:
            case 17:
    ...
    private void updateImsInfo(AsyncResult ar) {
        ...
        this.mIms = z;
        OppoRlog.Rlog.d(TAG, "IMS registration state: " + this.mIms + " format: " + this.mImsSmsFormat);
    }
    ...
    public void injectSmsPdu(SmsMessage msg, String format, SmsInjectionCallback callback, boolean ignoreClass) {
        OppoRlog.Rlog.d(TAG, "SmsDispatchersController:injectSmsPdu");
        if (msg == null) {
            try {
                OppoRlog.Rlog.e(TAG, "injectSmsPdu: createFromPdu returned null");
                callback.onSmsInjectedResult(2);
            } catch (Exception e) {
                OppoRlog.Rlog.e(TAG, "injectSmsPdu failed: ", e);
                callback.onSmsInjectedResult(2);
            }
            ...
            if (format.equals("3gpp")) {
                OppoRlog.Rlog.i(TAG, "SmsDispatchersController:injectSmsText Sending msg=" + msg + ", format=" + format + "to mGsmInboundSmsHandler");
                this.mGsmInboundSmsHandler.sendMessage(7, ar);
            } else if (format.equals("3gpp2")) {
                OppoRlog.Rlog.i(TAG, "SmsDispatchersController:injectSmsText Sending msg=" + msg + ", format=" + format + "to mCdmaInboundSmsHandler");
                this.mCdmaInboundSmsHandler.sendMessage(7, ar);
            } else {
                OppoRlog.Rlog.e(TAG, "Invalid pdu format: " + format);
                callback.onSmsInjectedResult(2);
            }
        } else {
            OppoRlog.Rlog.e(TAG, "injectSmsPdu: not class 1");
            callback.onSmsInjectedResult(2);
        }
    }
    ...
    public void sendRetrySms(SMSDispatcher.SmsTracker tracker) {
        ...
        if (!oldFormat.equals(newFormat)) {
            ...
            if (!map.containsKey("scAddr") || !map.containsKey("destAddr") || (!map.containsKey("text") && (!map.containsKey("data") || !map.containsKey("destPort")))) {
                OppoRlog.Rlog.e(TAG, "sendRetrySms failed to re-encode per missing fields!");
            ...
            if (map.containsKey("text")) {
                OppoRlog.Rlog.d(TAG, "sms failed was text");
                String text = (String) map.get("text");
                if (isCdmaFormat(newFormat)) {
                    OppoRlog.Rlog.d(TAG, "old format (gsm) ==> new format (cdma)");
                    if (tracker.mDeliveryIntent == null) {
                        z = false;
                    }
                    pdu = com.android.internal.telephony.cdma.SmsMessage.getSubmitPdu(scAddr, destAddr, text, z, (SmsHeader) null);
                } else {
                    OppoRlog.Rlog.d(TAG, "old format (cdma) ==> new format (gsm)");
                    if (tracker.mDeliveryIntent == null) {
                        z = false;
                    }
                ...
                OppoRlog.Rlog.d(TAG, "sms failed was data");
            ...
            OppoRlog.Rlog.d(TAG, "old format matched new format (cdma)");
            this.mCdmaDispatcher.sendSms(tracker);
        } else {
            OppoRlog.Rlog.d(TAG, "old format matched new format (gsm)");
    ...
    public void sendData(String callingPackage, String destAddr, String scAddr, int destPort, byte[] data, PendingIntent sentIntent, PendingIntent deliveryIntent, boolean isForVvm) {
        oemSendData(this.mContext, callingPackage);
    ...
    @Override // com.android.internal.telephony.AbstractSmsDispatchersController
