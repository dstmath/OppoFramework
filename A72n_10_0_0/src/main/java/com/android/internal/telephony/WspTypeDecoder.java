...
public class WspTypeDecoder {
    ...
    public boolean decodeTextString(int startIndex) {
        ...
        if (bArr[startIndex] == Byte.MAX_VALUE) {
            this.mStringValue = new String(bArr, startIndex + 1, this.mDataLength - 2);
        } else {
            this.mStringValue = new String(bArr, startIndex, this.mDataLength - 1);
        }
        return true;
    }
    ...
    public boolean decodeShortInteger(int startIndex) {
        byte[] bArr = this.mWspData;
        if ((bArr[startIndex] & 128) == 0) {
            return false;
        }
        this.mUnsigned32bit = (long) (bArr[startIndex] & Byte.MAX_VALUE);
        this.mDataLength = 1;
        return true;
    }
    ...
    public boolean decodeUintvarInteger(int startIndex) {
        ...
        while (true) {
            byte[] bArr = this.mWspData;
            if ((bArr[index] & 128) == 0) {
                this.mUnsigned32bit = (this.mUnsigned32bit << 7) | ((long) (bArr[index] & Byte.MAX_VALUE));
                this.mDataLength = (index - startIndex) + 1;
                return true;
    ...
    public boolean decodeValueLength(int startIndex) {
        byte[] bArr = this.mWspData;
        if ((bArr[startIndex] & 255) > 31) {
            return false;
        }
        if (bArr[startIndex] < 31) {
            this.mUnsigned32bit = (long) bArr[startIndex];
            this.mDataLength = 1;
        } else {
