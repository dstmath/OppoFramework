...
public class CdmaSubscriptionSourceManager extends Handler {
    ...
    public CdmaSubscriptionSourceManager(Context context, CommandsInterface ci) {
        this.mCi = ci;
    ...
    public static CdmaSubscriptionSourceManager getInstance(Context context, CommandsInterface ci, Handler h, int what, Object obj) {
        synchronized (sReferenceCountMonitor) {
            if (sInstance == null) {
                sInstance = TelephonyComponentFactory.getInstance().inject(CdmaSubscriptionSourceManager.class.getName()).makeCdmaSubscriptionSourceManager(context, ci, h, what, obj);
            }
            sReferenceCount++;
        }
