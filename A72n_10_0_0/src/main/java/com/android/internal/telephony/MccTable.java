...
import com.android.internal.telephony.util.OppoLocaleChangedUtils;
...
public final class MccTable {
    ...
    public static void updateMccMncConfiguration(Context context, String mccmnc) {
        ...
        if (Build.IS_DEBUGGABLE) {
            String overrideMcc = SystemProperties.get(OppoLocaleChangedUtils.PROPERTY_OVERRIDE_MCC);
        ...
        if (!TextUtils.isEmpty(mccmnc)) {
            try {
                ...
                if (mcc != 0) {
                    ...
                    if (OppoLocaleChangedUtils.isNotNeedUpdateMccMncConfiguration()) {
                        return;
                    }
                }
                try {
                    ...
                    Locale locale = getLocaleFromMcc(context, mcc, null);
                    if (OppoLocaleChangedUtils.isNotNeedGetLocaleForLanguageCountry(context)) {
                        locale = null;
                    }
                    if (locale != null) {
                        ...
                        OppoLocaleChangedUtils.setPersistedLocale(locale);
                    }
                    if (updateConfig) {
    ...
    static {
        ...
        sTable.add(new MccEntry(AbstractSIMRecords.EVENT_GET_CPHSONS_DONE, "my", 2));
    ...
    private static Locale getLocaleForLanguageCountry(Context context, String language, String country) {
        ...
        Locale target = OppoLocaleChangedUtils.getLocaleForCountery(country);
        if (target == null) {
            target = new Locale(language, country);
        }
        try {
    ...
    private static void setTimezoneFromMccIfNeeded(Context context, int mcc) {
        String zoneId;
        if (!NewTimeServiceHelper.isTimeZoneSettingInitializedStatic() && (zoneId = defaultTimeZoneForMcc(mcc)) != null && zoneId.length() > 0) {
    ...
    public static void updateVdfPersistedLocale(Context context, String mccmnc) {
        if ((!Build.IS_DEBUGGABLE || TextUtils.isEmpty(SystemProperties.get(OppoLocaleChangedUtils.PROPERTY_OVERRIDE_MCC))) && !TextUtils.isEmpty(mccmnc)) {
            try {
                int mcc = Integer.parseInt(mccmnc.substring(0, 3));
                int mnc = Integer.parseInt(mccmnc.substring(3));
                try {
                    ...
                    if (!OppoLocaleChangedUtils.isVdfNeedUpdateLocale(context)) {
                        Configuration oldConfig = ActivityManager.getService().getConfiguration();
                        Slog.d(LOG_TAG, "updateVdfPersistedLocale updateConfig oldConfig=" + oldConfig);
                        if (oldConfig.locale != null) {
                            locale = oldConfig.locale;
                        }
                    }
                    if (locale != null) {
                        config.setLocale(locale);
                        updateConfig = true;
                        config.userSetLocale = true;
                        OppoLocaleChangedUtils.setVodafonePersisitedLocal(locale);
                    }
                    if (updateConfig) {
                        Slog.d(LOG_TAG, "updateVdfPersistedLocale updateConfig config=" + config);
                        ActivityManager.getService().updateConfiguration(config);
                        return;
                    }
                    Slog.d(LOG_TAG, "updateVdfPersistedLocale nothing to update");
                } catch (RemoteException e) {
                    Slog.e(LOG_TAG, "Can't update configuration", e);
                }
            } catch (NumberFormatException | StringIndexOutOfBoundsException ex) {
