...
public class LocaleTracker extends Handler {
    ...
    private void requestNextCellInfo(boolean succeeded) {
        if (this.mIsTracking) {
            int deepSleepStatus = Settings.System.getInt(this.mPhone.getContext().getContentResolver(), OemTelephonyUtils.DEEP_SLEEP_URI, 0);
            if (1 == deepSleepStatus) {
                log("[POWERSTATE] requestNextCellInfo deepSleepStatus = " + deepSleepStatus);
                return;
            }
            removeMessages(1);
            if (succeeded) {
    ...
    private synchronized void updateLocale() {
        ...
        this.mPhone.getServiceStateTracker().broadcastMccChange(mcc);
