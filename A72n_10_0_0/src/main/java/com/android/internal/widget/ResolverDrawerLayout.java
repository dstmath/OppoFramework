...
public class ResolverDrawerLayout extends ViewGroup {
    ...
    private boolean updateCollapseOffset(int oldCollapsibleHeight, boolean remainClosed) {
        ...
        if (isLaidOut()) {
            ...
            if (!remainClosed || oldCollapsibleHeight >= (i = this.mCollapsibleHeight) || this.mCollapseOffset != ((float) oldCollapsibleHeight)) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action != 0) {
            ...
            this.mOpenOnClick = isListChildUnderClipped(x2, y2) && this.mCollapseOffset > 0.0f;
        }
        if (this.mIsDragging) {
            abortAnimation();
        }
        ...
        if (action == 0) {
            ...
            if (!this.mIsDragging) {
                ...
                if (Math.abs(dy) > ((float) this.mTouchSlop) && findChildUnder(x2, y3) != null) {
                    ...
                    this.mLastTouchY = Math.max(f3 - ((float) i4), Math.min(f3 + dy, f3 + ((float) i4)));
                }
            }
            if (this.mIsDragging) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (getShowAtTop()) {
            ...
            this.mTopOffset = Math.max(i4, heightSize - heightUsed) + ((int) this.mCollapseOffset);
        }
