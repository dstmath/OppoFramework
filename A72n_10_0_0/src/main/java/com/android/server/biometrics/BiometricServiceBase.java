...
public abstract class BiometricServiceBase extends SystemService implements IHwBinder.DeathRecipient {
    ...
    public boolean canUseBiometric(String opPackageName, boolean requireForeground, int uid, int pid, int userId) {
        ...
        if ("com.oppo.autotest".equals(opPackageName) || "com.oppo.engineermode".equals(opPackageName) || "com.android.engineeringmode".equals(opPackageName) || "com.android.settings".equals(opPackageName) || "com.excelliance.dualaid".equals(opPackageName)) {
            return true;
        }
        if (this.mContext.getPackageManager().hasSystemFeature(OppoCustomizeNotificationHelper.Constants.FEATURE_BUSINESS_CUSTOM) && SystemProperties.getInt("persist.sys.ban.mode", 1) == 0) {
            Slog.w(getTag(), "canUseBiometric = false");
            return false;
        } else if (Binder.getCallingUid() == 1000 || isKeyguard(opPackageName)) {
            return true;
        } else {
