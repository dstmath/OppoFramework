...
public class BiometricService extends SystemService {
    private static final boolean BIOMETRIC_DEBUG = SystemProperties.getBoolean("persist.sys.assert.panic", false);
    ...
    private BiometricWakeupManagerService mBiometricWakeupManagerService = null;
    ...
    private final IBiometricServiceReceiverInternal mInternalReceiver = new IBiometricServiceReceiverInternal.Stub() {
        ...
        public void onAuthenticationFailed(int cookie, boolean requireConfirmation) throws RemoteException {
            BiometricService.this.mHandler.obtainMessage(3, BiometricService.this.getContext().getString(17039614)).sendToTarget();
        }
    ...
    public void systemReady() {
        BiometricWakeupManagerService biometricWakeupManagerService = this.mBiometricWakeupManagerService;
        if (biometricWakeupManagerService != null) {
            biometricWakeupManagerService.systemReady();
        } else {
            Slog.d(TAG, "mBiometricWakeupManagerService = null");
        }
    }
    ...
    public BiometricService(Context context) {
        ...
        this.mBiometricWakeupManagerService = BiometricWakeupManagerService.getBiometricWakeupManagerService(context);
    }
    ...
    public void onStart() {
        ...
        BiometricWakeupManagerService biometricWakeupManagerService = this.mBiometricWakeupManagerService;
        if (biometricWakeupManagerService != null) {
            publishLocalService(BiometricsManagerInternal.class, biometricWakeupManagerService.getBiometricsManagerLocalService());
        } else {
            Slog.w(TAG, "mBiometricWakeupManagerService  == null");
        }
    }
    ...
    private void logDialogDismissed(int reason) {
        ...
        if (reason == 1) {
            ...
            Slog.v(LoggableMonitor.TAG, "Confirmed! Modality: " + statsModality() + ", User: " + this.mCurrentAuthSession.mUserId + ", IsCrypto: " + this.mCurrentAuthSession.isCrypto() + ", Client: 2, RequireConfirmation: " + this.mCurrentAuthSession.mRequireConfirmation + ", State: 3, Latency: " + latency);
        ...
        Slog.v(LoggableMonitor.TAG, "Dismissed! Modality: " + statsModality() + ", User: " + this.mCurrentAuthSession.mUserId + ", IsCrypto: " + this.mCurrentAuthSession.isCrypto() + ", Action: 2, Client: 2, Error: " + error);
        ...
        try {
            ...
            if (!runningTasks.isEmpty()) {
                ...
                if (this.mCurrentAuthSession != null && !topPackage.contentEquals(this.mCurrentAuthSession.mOpPackageName)) {
                    if (BIOMETRIC_DEBUG) {
                        Slog.d(TAG, "handleTaskStackChanged currentClient:" + this.mCurrentAuthSession.mOpPackageName);
                    }
    ...
    private void handleAuthenticationSucceeded(boolean requireConfirmation, byte[] token) {
        try {
            ...
            if (BIOMETRIC_DEBUG) {
                Slog.d(TAG, "handleAuthenticationSucceeded requireConfirmation:" + requireConfirmation);
            }
    ...
    private void handleOnConfirmDeviceCredentialSuccess() {
        ...
        if (BIOMETRIC_DEBUG) {
            Slog.d(TAG, "handleOnConfirmDeviceCredentialSuccess");
        }
        try {
            this.mActivityTaskManager.unregisterTaskStackListener(this.mTaskStackListener);
    ...
    private void handleOnConfirmDeviceCredentialError(int error, String message) {
        if (this.mConfirmDeviceCredentialReceiver == null) {
            Slog.w(TAG, "onCDCAError null! Error: " + error + StringUtils.SPACE + message);
            return;
        }
        if (BIOMETRIC_DEBUG) {
            Slog.d(TAG, "handleOnConfirmDeviceCredentialError");
        }
        try {
            this.mActivityTaskManager.unregisterTaskStackListener(this.mTaskStackListener);
        ...
        try {
            if (this.mCurrentAuthSession == null || !this.mCurrentAuthSession.containsCookie(cookie)) {
                if (this.mPendingAuthSession == null || !this.mPendingAuthSession.containsCookie(cookie)) {
                    if (BIOMETRIC_DEBUG) {
                        Slog.d(TAG, "do nothing");
                    }
                } else if (this.mPendingAuthSession.mState == 1) {
                    if (BIOMETRIC_DEBUG) {
                        Slog.d(TAG, "mPendingAuthSession.mState=STATE_AUTH_CALLED message:" + message + " error:" + error);
                    }
                ...
                if (BIOMETRIC_DEBUG) {
                    Slog.d(TAG, "mCurrentAuthSession.mState=STATE_AUTH_STARTED message:" + message + " error:" + error);
                }
                ...
                if (BIOMETRIC_DEBUG) {
                    Slog.d(TAG, "mCurrentAuthSession.mState=STATE_AUTH_PAUSED message:" + message + " error:" + error);
                }
                this.mCurrentAuthSession.mClientReceiver.onError(error, message);
                this.mStatusBarService.onBiometricError(message);
                this.mActivityTaskManager.unregisterTaskStackListener(this.mTaskStackListener);
    ...
    private void handleOnDismissed(int reason) {
        ...
        if (BIOMETRIC_DEBUG) {
            Slog.d(TAG, "handleOnDismissed isFromConfirmDeviceCredential:" + this.mCurrentAuthSession.isFromConfirmDeviceCredential());
        }
    ...
    private void handleCancelAuthentication(IBinder token, String opPackageName) {
        ...
        if (authSession == null || authSession.mState != 6) {
            ...
            if (BIOMETRIC_DEBUG) {
                Slog.d(TAG, "handleCancelAuthentication mCurrentAuthSession.mState:" + this.mCurrentAuthSession.mState);
            }
            try {
