...
public class CameraView {
    ...
    public int startPreview() {
        ...
        Log.d(TAG, HealthState.START_PREVIEW);
        try {
    ...
    public int stopPreview() {
        ...
        try {
            ...
            try {
                ...
                Log.d(TAG, "From starPreview to stopPreview, time spent = " + (SystemClock.uptimeMillis() - this.mStarPreviewStartTime) + "ms");
                return 0;
            } catch (Throwable th) {
                this.mHealthMonitor.stop("stopPreview", session);
                throw th;
    ...
    public int releaseCamera() {
        if (this.mCamera != null) {
            Log.d(TAG, HealthState.RELEASE_CAMERA);
            try {
                long startTime = SystemClock.uptimeMillis();
                String session = String.valueOf(startTime);
                try {
                    ...
                    Log.d(TAG, "From openCamera to releaseCamera, time spent = " + (SystemClock.uptimeMillis() - this.mOpenCameraStartTime) + "ms");
                } catch (Throwable th) {
                    this.mHealthMonitor.stop(HealthState.RELEASE_CAMERA, session);
                    throw th;
                }
    ...
    public void updateCameraParameters(PrintWriter pw, String parameterArgs) {
        ...
        StringReader sr = new StringReader(parameterArgs.replace(',', '\r'));
