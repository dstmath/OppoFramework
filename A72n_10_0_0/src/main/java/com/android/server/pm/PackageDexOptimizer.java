...
public class PackageDexOptimizer {
    ...
    private int performDexOptLI(PackageParser.Package pkg, String[] targetInstructionSets, CompilerStats.PackageStats packageStats, PackageDexUsage.PackageUseInfo packageUseInfo, DexoptOptions options) {
        ...
        PackageParser.Package r15 = pkg;
        List<SharedLibraryInfo> sharedLibraries2 = r15.usesLibraryInfos;
        String[] dexCodeInstructionSets2 = InstructionSets.getDexCodeInstructionSets(targetInstructionSets != null ? targetInstructionSets : InstructionSets.getAppDexInstructionSets(r15.applicationInfo));
        List<String> paths2 = pkg.getAllCodePaths();
        int sharedGid3 = UserHandle.getSharedAppGid(r15.applicationInfo.uid);
        char c2 = 65535;
        if (sharedGid3 == -1) {
        ...
        pathsWithCode2[0] = (r15.applicationInfo.flags & 4) != 0;
        for (int i2 = 1; i2 < paths2.size(); i2++) {
            pathsWithCode2[i2] = (r15.splitFlags[i2 + -1] & 4) != 0;
        }
        String[] classLoaderContexts2 = DexoptUtils.getClassLoaderContexts(r15.applicationInfo, sharedLibraries2, pathsWithCode2);
            ...
            if (pathsWithCode2[i3]) {
                if (classLoaderContexts2[i3] != null) {
                    ...
                    if (options.getSplitName() == null || options.getSplitName().equals(new File(path).getName())) {
                        String str = null;
                        String profileName = ArtManager.getProfileName(i3 == 0 ? null : r15.splitNames[i3 - 1]);
                        ...
                        if (!options.isDexoptAsSharedLibrary()) {
                            if (!packageUseInfo.isUsedByOtherApps(path)) {
                                ...
                                String compilerFilter = packageDexOptimizer.getRealCompilerFilter(r15.applicationInfo, options.getCompilerFilter(), isUsedByOtherApps);
                                boolean profileUpdated = !options.isCheckForProfileUpdates() && packageDexOptimizer.isProfileUpdated(r15, sharedGid, profileName, compilerFilter);
                                int dexoptFlags = packageDexOptimizer.getDexFlags(r15, compilerFilter, options);
                        ...
                        String compilerFilter2 = packageDexOptimizer.getRealCompilerFilter(r15.applicationInfo, options.getCompilerFilter(), isUsedByOtherApps);
                        boolean profileUpdated2 = !options.isCheckForProfileUpdates() && packageDexOptimizer.isProfileUpdated(r15, sharedGid, profileName, compilerFilter2);
                        int dexoptFlags2 = packageDexOptimizer.getDexFlags(r15, compilerFilter2, options);
                        length = dexCodeInstructionSets2.length;
                        int result22 = result;
                        newResult = 0;
                        while (newResult < length) {
    ...
    private int dexOptSecondaryDexPathLI(ApplicationInfo info, String path, PackageDexUsage.DexUseInfo dexUseInfo, DexoptOptions options) {
        ...
        } else if (info.packageName == null || !info.packageName.equals("com.tencent.mm")) {
            compilerFilter = "extract";
            classLoaderContext = null;
        } else {
            Log.d(str4, "yac: SECONDARYDEX_FORCE_POLICY, force dexOptSecondaryDex this pkg " + info.packageName);
