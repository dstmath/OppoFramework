...
public abstract class PackageSettingBase extends SettingBase {
    ...
    private void doCopy(PackageSettingBase orig) {
        ...
        String[] strArr = orig.usesStaticLibraries;
        this.usesStaticLibraries = strArr != null ? (String[]) Arrays.copyOf(strArr, strArr.length) : null;
    ...
    public PackageUserState readUserState(int userId) {
        ...
        if (state != null) {
            ...
            return state;
        } else if (userId != 999 || OppoFeatureCache.get(IColorMultiAppManager.DEFAULT).isMultiUserInstallApp(this.name) || OppoFeatureCache.get(IColorMultiAppManager.DEFAULT).isMultiAllowedApp(this.name)) {
            return DEFAULT_USER_STATE;
        } else {
            PackageUserState ret = new PackageUserState();
            ret.installed = false;
            return ret;
        }
    }
    ...
    /* access modifiers changed from: package-private */
    public void setOppoFreezeState(int freezeState, int userId) {
        modifyUserState(userId).oppoFreezeState = freezeState;
    }

    /* access modifiers changed from: package-private */
    public int getOppoFreezeState(int userId) {
        return readUserState(userId).oppoFreezeState;
    }

    /* access modifiers changed from: package-private */
    public void setOppoFreezeFlag(int freezeState, int userId) {
        modifyUserState(userId).oppoFreezeFlag = freezeState;
    }

    /* access modifiers changed from: package-private */
    public int getOppoFreezeFlag(int userId) {
        return readUserState(userId).oppoFreezeFlag;
    }
}
