...
public class ColorDexMetadataCompileHelper {
    ...
    public class DexMetadataCompileReceiver extends BroadcastReceiver {
        ...
        private void runBackgroundDexOpt() {
            boolean mSuperPowerSaveMode = true;
            if (Settings.System.getIntForUser(ColorDexMetadataCompileHelper.this.mContext.getContentResolver(), ColorDexMetadataCompileHelper.SUPER_POWERSAVE_MODE_STATE, 0, 0) != 1) {
                mSuperPowerSaveMode = false;
            }
            if ((!ColorDexMetadataCompileHelper.this.mScreenOn || ColorDexMetadataCompileHelper.this.mDockIdle) && !mSuperPowerSaveMode) {
            ...
            if (ColorDexMetadataCompileHelper.this.checkBackgroundCompileEnable()) {
                if (action.equals("android.intent.action.SCREEN_OFF") || action.equals("android.intent.action.DREAMING_STARTED") || action.equals("android.intent.action.DOCK_IDLE")) {
                    if (!action.equals("android.intent.action.DOCK_IDLE")) {
                        ColorDexMetadataCompileHelper.this.mScreenOn = false;
                        ColorDexMetadataCompileHelper.this.mDockIdle = false;
                    } else if (ColorDexMetadataCompileHelper.this.mScreenOn) {
                        ColorDexMetadataCompileHelper.this.mDockIdle = true;
                    } else {
                        return;
                    }
                    ArraySet<String> pkgs = ColorDexMetadataManagerHelper.getInstance().getCompilePackageSet();
                    if (!pkgs.isEmpty()) {
                        ...
                        ColorDexMetadataCompileHelper.this.mAlarmSet = true;
                    } else if (ColorDexMetadataCompileHelper.this.mDebugSwitch) {
                    ...
                    if (!action.equals("android.intent.action.DOCK_ACTIVE")) {
                        ColorDexMetadataCompileHelper.this.mScreenOn = true;
                        ColorDexMetadataCompileHelper.this.mDockIdle = false;
                    } else if (ColorDexMetadataCompileHelper.this.mScreenOn) {
                        ColorDexMetadataCompileHelper.this.mDockIdle = false;
                    } else {
                    ...
                    ColorDexMetadataCompileHelper.this.mAlarmSet = false;
        ...
        /* access modifiers changed from: private */
        /* access modifiers changed from: public */
        private void startCompileTracking(Context context) {
            if (ColorDexMetadataCompileHelper.this.mDebugSwitch) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean checkBackgroundCompileEnable() {
