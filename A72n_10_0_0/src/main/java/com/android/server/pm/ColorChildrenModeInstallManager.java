        ...
        if (isInstall) {
            try {
                new Thread(new Runnable() {
                    /* class com.android.server.pm.ColorChildrenModeInstallManager.AnonymousClass1 */

                    public void run() {
                        Slog.w(ColorChildrenModeInstallManager.TAG, "prevent installation in children mode");
                        ColorChildrenModeInstallManager.this.mContext.sendBroadcastAsUser(new Intent("com.coloros.childrenspace.action.FORBID_INSTALL_PACKAGES"), user, "oppo.permission.OPPO_COMPONENT_SAFE");
                    }
                }).start();
            } catch (Throwable th) {
            ...
            new Thread(new Runnable() {
                /* class com.android.server.pm.ColorChildrenModeInstallManager.AnonymousClass2 */

                public void run() {
                    Slog.w(ColorChildrenModeInstallManager.TAG, "prevent uninstallation in children mode");
                    ColorChildrenModeInstallManager.this.mContext.sendBroadcastAsUser(new Intent("com.coloros.childrenspace.action.FORBID_DELETE_PACKAGES"), user, "oppo.permission.OPPO_COMPONENT_SAFE");
                }
            }).start();
        }
        Binder.restoreCallingIdentity(identity);
