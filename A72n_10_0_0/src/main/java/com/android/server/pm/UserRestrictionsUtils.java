...
public class UserRestrictionsUtils {
            ...
            for (String key : restrictions.keySet()) {
                if (!NON_PERSIST_USER_RESTRICTIONS.contains(key)) {
                    if (!USER_RESTRICTIONS.contains(key)) {
                        ...
                        serializer.attribute(null, key, TemperatureProvider.SWITCH_ON);
                    }
                }
            }
    ...
    private static void applyUserRestriction(Context context, int userId, String key, boolean newValue) {
        ...
        try {
            ...
            switch (c) {
                ...
                case 3:
                    if (newValue) {
                        Settings.Global.putStringForUser(context.getContentResolver(), "package_verifier_enable", NoFocusWindow.HUNG_CONFIG_ENABLE, userId);
                        Settings.Global.putStringForUser(context.getContentResolver(), "verifier_verify_adb_installs", NoFocusWindow.HUNG_CONFIG_ENABLE, userId);
                        break;
                    }
                    break;
                case 4:
    ...
    public static boolean isSettingRestrictedForUser(Context context, String setting, int userId, String value, int callingUid) {
        ...
        switch (c) {
            ...
            case 4:
            case 5:
                if (!NoFocusWindow.HUNG_CONFIG_ENABLE.equals(value)) {
