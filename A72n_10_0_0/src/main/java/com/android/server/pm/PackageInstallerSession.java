...
public class PackageInstallerSession extends IPackageInstallerSession.Stub {
    ...
    @GuardedBy({"mLock"})
    private float mClientProgress = OppoBrightUtils.MIN_LUX_LIMITI;
    @GuardedBy({"mLock"})
    ...
    @GuardedBy({"mLock"})
    private float mInternalProgress = OppoBrightUtils.MIN_LUX_LIMITI;
    ...
    @GuardedBy({"mLock"})
    private float mProgress = OppoBrightUtils.MIN_LUX_LIMITI;
    @GuardedBy({"mLock"})
    ...
    private boolean needToAskForPermissionsLocked() {
        ...
        String[] packageNames = this.mPm.getPackagesForUid(this.mInstallerUid);
        if (!this.mPm.hasSystemFeature("oppo.customize.function.silentinstall", 0) || packageNames == null || packageNames.length != 1 || !this.mPm.checkWhiteList(packageNames[0])) {
            ...
            boolean isInstallerSystem = UserHandle.getAppId(this.mInstallerUid) == 1000;
    ...
    public PackageInstallerSession(PackageInstallerService.InternalCallback callback, Context context, PackageManagerService pm, PackageSessionProvider sessionProvider, Looper looper, StagingManager stagingManager, int sessionId2, int userId2, String installerPackageName, int installerUid, PackageInstaller.SessionParams params2, long createdMillis2, File stageDir2, String stageCid2, boolean prepared, boolean committed, boolean sealed, int[] childSessionIds, int parentSessionId, boolean isReady, boolean isFailed, boolean isApplied, int stagedSessionErrorCode, String stagedSessionErrorMessage) {
        ...
        if (!params2.isMultiPackage) {
            if ((stageDir2 == null ? true : z) == (stageCid2 == null ? true : z)) {
    ...
    private void computeProgressLocked(boolean forcePublish) {
        this.mProgress = MathUtils.constrain(this.mClientProgress * 0.8f, (float) OppoBrightUtils.MIN_LUX_LIMITI, 0.8f) + MathUtils.constrain(this.mInternalProgress * 0.2f, (float) OppoBrightUtils.MIN_LUX_LIMITI, 0.2f);
        ...
        try {
            if (FileUtils.isValidExtFilename(name)) {
                ...
                try {
                    ...
                    FileDescriptor targetFd2 = Os.open(target.getAbsolutePath(), OsConstants.O_CREAT | OsConstants.O_WRONLY, TemperatureProvider.HIGH_TEMPERATURE_THRESHOLD);
                    Os.chmod(target.getAbsolutePath(), TemperatureProvider.HIGH_TEMPERATURE_THRESHOLD);
                    ...
                    if (incomingFd != null) {
                        ...
                        if (!(callingUid == 0 || callingUid == 1000 || callingUid == 2000)) {
                            if (this.mPm.checkUidPermission("android.permission.INSTALL_PACKAGES", Binder.getCallingUid()) == 0) {
                                Slog.d(TAG, "package " + Binder.getCallingUid() + " own INSTALL_PACKAGES permission, allow copy through incomingFd.");
                            } else {
    ...
    private void handleCommit() {
        ...
        if (this.params.isStaged) {
            ...
            destroyInternal();
            dispatchSessionFinished(-110, "APEX packages can only be installed using staged sessions.", null);
        } else {
            List<PackageInstallerSession> childSessions = getChildSessions();
            try {
                synchronized (this.mLock) {
    ...
    private PackageManagerService.ActiveInstallSession makeSessionActiveLocked() throws PackageManagerException {
        ...
        if (this.mRelinquished) {
            throw new PackageManagerException(-110, "Session relinquished");
        } else if (this.mDestroyed) {
    ...
    private static void maybeRenameFile(File from, File to) throws PackageManagerException {
        if (!from.equals(to) && !from.renameTo(to)) {
            throw new PackageManagerException(-110, "Could not rename file " + from + " to " + to);
        }
    }
    ...
    /* JADX INFO: Multiple debug info for r3v23 'libDirsToInherit'  java.util.List<java.io.File>: [D('libDirsToInherit' java.util.List<java.io.File>), D('archSubDir' java.io.File)] */
    /* JADX INFO: Multiple debug info for r3v24 'libDirsToInherit'  java.util.List<java.io.File>: [D('libDirsToInherit' java.util.List<java.io.File>), D('archSubDir' java.io.File)] */
    @GuardedBy({"mLock"})
    private void validateApkInstallLocked(PackageInfo pkgInfo) throws PackageManagerException {
        ...
        try {
            ...
            List<String> removeSplitList2 = new ArrayList<>();
            if (!ArrayUtils.isEmpty(removedFiles3)) {
                for (File removedFile : removedFiles3) {
                    String fileName = removedFile.getName();
                    removeSplitList2.add(fileName.substring(0, fileName.length() - REMOVE_SPLIT_MARKER_EXTENSION.length()));
                }
            }
            File[] addedFiles = this.mResolvedStageDir.listFiles(sAddedFilter);
            if (!ArrayUtils.isEmpty(addedFiles) || removeSplitList2.size() != 0) {
                    ...
                    try {
                        ...
                        if (stagedSplits.add(apk.splitName)) {
                            ...
                            if (FileUtils.isValidExtFilename(targetName)) {
                                ...
                                i++;
                                removeSplitList2 = removeSplitList;
                            } else {
    ...
    private static void copyFiles(List<File> fromFiles, File toDir) throws IOException {
        ...
        for (File fromFile : fromFiles) {
            ...
            if (FileUtils.copyFile(fromFile, tmpFile)) {
                try {
                    Os.chmod(tmpFile.getAbsolutePath(), TemperatureProvider.HIGH_TEMPERATURE_THRESHOLD);
