...
public class ColorRuntimePermGrantPolicyManager implements IColorRuntimePermGrantPolicyManager {
    ...
    private static final String SYS_PERMISSIONS_GRANTED_BY_WHITE_LIST_IN_CUSTOM = (OppoBaseEnvironment.getResourceDirectory().getAbsolutePath() + "/etc/config/default_grant_permissions_list.xml");
    ...
    private static ArrayMap<String, defaultPermissionInfo> mPermissionsGrantedByDefault = new ArrayMap<>();
    ...
    public void grantOppoNonFixRuntimePermssion(int userId, boolean supportRuntimeAlert) {
        ...
        if (!permInfoList3.isEmpty()) {
            ...
            if ("SOFTBANK".equals(mOperator) || "YMOBILE".equals(mOperator)) {
                HashMap<String, Boolean> softbankRequestMap = new HashMap<>();
                softbankRequestMap.put("com.android.cellbroadcastreceiver", true);
                softbankRequestMap.put("jp.softbank.mb.flcrlap", Boolean.valueOf((boolean) EXP_VERSION));
                grantOperatorsPackagesPerms(userId, softbankRequestMap);
            }
        }
    }
    ...
    public static void initRuntimeFilterInfos() {
        ...
        parseSysPermissionsGrantByDefaultWithFile(SYS_PERMISSIONS_GRANTED_BY_WHITE_LIST_IN_CUSTOM);
    }
    ...
    public static void parseSysPermissionsGrantByDefaultWithFile(String file) {
        int type;
        int i;
        File permFile = new File(file);
        if (!permFile.exists()) {
            Slog.w(TAG, "Couldn't find permissions file " + permFile);
            return;
        }
                    ...
                    while (true) {
                        ...
                        if ("default-grant-permissions".equals(parser.getName())) {
                            ...
                            if (TextUtils.isEmpty(packageName)) {
                                ...
                                while (XmlUtils.nextElementWithin(parser, depth)) {
                                    if ("permission".equals(parser.getName())) {
                                        String permission = parser.getAttributeValue(str, BrightnessConstants.AppSplineXml.TAG_NAME);
                                        if (TextUtils.isEmpty(permission)) {
                                            Slog.d(TAG, "skip package: " + packageName + " permission");
                                        } else {
                                            permInfo.addPermission(permission, XmlUtils.readBooleanAttribute(parser, "fixed"));
    ...
    public boolean onPermissionRevoked(ApplicationInfo applicationInfo, int userId) {
        ...
        OppoBasePackageManagerService basePms = typeCasting(this.mPms);
        if (!(basePms == null || basePms.mColorPmsInner == null)) {
            basePms.mColorPmsInner.onSystemAppPermissionRevoked(applicationInfo.uid, userId);
        }
        this.mPms.mHandler.post(new Runnable(applicationInfo, userId) {
            /* class com.android.server.pm.$$Lambda$ColorRuntimePermGrantPolicyManager$vMLRwDGbJiYaohcAdRMJonDxVQY */
            private final /* synthetic */ ApplicationInfo f$0;
            ...
            public final void run() {
                ColorRuntimePermGrantPolicyManager.lambda$onPermissionRevoked$0(this.f$0, this.f$1);
            }
        });
        return true;
    }

    static /* synthetic */ void lambda$onPermissionRevoked$0(ApplicationInfo applicationInfo, int userId) {
        Slog.i(TAG, "Kill " + applicationInfo.packageName + ": KILL_APP_REASON_PERMISSIONS_REVOKED,userId:" + userId);
        long identity = Binder.clearCallingIdentity();
        try {
            IActivityManager am = ActivityManager.getService();
            if (am != null) {
                try {
                    am.killUid(UserHandle.getAppId(applicationInfo.uid), userId, "permissions revoked");
                } catch (RemoteException e) {
                }
            }
        } catch (Exception e2) {
    ...
    private void grantOperatorsPackagesPerms(int userid, HashMap<String, Boolean> requestMap) {
        PackageInfo pkgInfo;
        if (!(requestMap == null || requestMap.entrySet() == null)) {
            for (Map.Entry<String, Boolean> singleEntry : requestMap.entrySet()) {
                ...
                if (singlePackageParser != null && doesPackageSupportRuntimePermissions(singlePackageParser) && !singlePackageParser.requestedPermissions.isEmpty() && (pkgInfo = this.mServiceInternal.getPackageInfo(singlePackageParser.packageName, (int) DEFAULT_PACKAGE_INFO_QUERY_FLAGS, (int) ColorFreeformManagerService.FREEFORM_CALLER_UID, userid)) != null) {
                    ...
                    if (!permissions.isEmpty()) {
                        if (singleEntry.getValue().booleanValue()) {
                            this.mColorDefaultPermissionGrantPolicyInner.grantRuntimePermissions(pkgInfo, permissions, true, userid);
                        } else {
                            this.mColorDefaultPermissionGrantPolicyInner.grantRuntimePermissions(pkgInfo, permissions, (boolean) EXP_VERSION, userid);
                            for (String perPermission : permissions) {
                                this.mPms.revokeRuntimePermission(singleEntry.getKey(), perPermission, userid);
                            }
                        }
                    }
                }
