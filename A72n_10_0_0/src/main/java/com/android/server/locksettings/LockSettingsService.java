...
public class LockSettingsService extends OppoBaseLockSettingsService {
    ...
    private static final int[] SYSTEM_CREDENTIAL_UIDS = {OppoUsageService.IntergrateReserveManager.READ_OPPORESEVE2_TYPE_MEDIA, OppoUsageService.IntergrateReserveManager.READ_OPPORESEVE2_TYPE_MEDIA_LOG_USAGE, 0, 1000};
    ...
    private static OppoLockSettingsServiceUtils mOppoLockSettingsServiceUtils = null;
    private static SyntheticPasswordManager mSyntheticPasswordManager = null;
    ...
    public static final class Lifecycle extends SystemService {
        ...
        /* JADX DEBUG: Multi-variable search result rejected for r4v0, resolved type: com.android.server.locksettings.LockSettingsService$Lifecycle */
        /* JADX WARN: Multi-variable type inference failed */
        /* JADX WARN: Type inference failed for: r0v2, types: [com.android.server.locksettings.LockSettingsService, android.os.IBinder] */
        /* JADX WARNING: Unknown variable types count: 1 */
        @Override // com.android.server.SystemService
        public void onStart() {
            AndroidKeyStoreProvider.install();
            this.mLockSettingsService = ColorServiceFactory.getInstance().getLockSettingsService(getContext());
            publishBinderService("lock_settings", this.mLockSettingsService);
            OppoLockSettingsServiceUtils unused = LockSettingsService.mOppoLockSettingsServiceUtils = new OppoLockSettingsServiceUtils(LockSettingsService.mSyntheticPasswordManager, this.mLockSettingsService, getContext());
        }
    ...
    public void tieManagedProfileLockIfNecessary(int managedUserId, byte[] managedUserPassword) {
        ...
        if (this.mUserManager.getUserInfo(managedUserId).isManagedProfile() && !this.mLockPatternUtils.isSeparateProfileChallengeEnabled(managedUserId) && !this.mStorage.hasChildProfileLock(managedUserId)) {
            ...
            if (isUserSecure(parentId)) {
                ...
                Slog.v(TAG, "Parent does not have a screen lock");
            }
        }
    }
            ...
            storage.setDatabaseOnCreateCallback(new LockSettingsStorage.Callback() {
                ...
                public void initialize(SQLiteDatabase db) {
                    if (SystemProperties.getBoolean("ro.lockscreen.disable.default", false)) {
                        storage.writeKeyValue(db, "lockscreen.disabled", NoFocusWindow.HUNG_CONFIG_ENABLE, 0);
                    }
                }
            });
            return storage;
    ...
    protected LockSettingsService(Injector injector) {
        ...
        mSyntheticPasswordManager = this.mSpManager;
        this.DEBUG_PANIC = SystemProperties.getBoolean("persist.sys.assert.panic", false);
        this.DEBUG = this.DEBUG_PANIC;
        this.mOppoLockSettingsInner = new LockSettingsInner();
    }
    ...
    private void showEncryptionNotificationForProfile(UserHandle user) {
        ...
        CharSequence message = r.getText(17040887);
        CharSequence detail = r.getText(17040886);
    ...
    private void showEncryptionNotification(UserHandle user, CharSequence title, CharSequence message, CharSequence detail, PendingIntent intent) {
        if (this.DEBUG) {
            Slog.v(TAG, "showing encryption notification, user: " + user.getIdentifier());
        }
        if (StorageManager.isFileEncryptedNativeOrEmulated()) {
            this.mNotificationManager.notifyAsUser(null, 9, new Notification.Builder(this.mContext, SystemNotificationChannels.DEVICE_ADMIN).setSmallIcon(17302832).setWhen(0).setOngoing(true).setTicker(title).setColor(this.mContext.getColor(17170460)).setContentTitle(title).setContentText(message).setSubText(detail).setVisibility(1).setContentIntent(intent).build(), user);
        }
    }
    ...
    private void hideEncryptionNotification(UserHandle userHandle) {
        if (this.DEBUG) {
            Slog.v(TAG, "hide encryption notification, user: " + userHandle.getIdentifier());
        }
    ...
    private void migrateOldData() {
        ...
        if (getString("migrated", null, 0) == null) {
            ...
            setString("migrated", TemperatureProvider.SWITCH_ON, 0);
        ...
        if (getString("migrated_user_specific", null, 0) == null) {
            ...
            i = i3;
            setString("migrated_user_specific", TemperatureProvider.SWITCH_ON, i);
        ...
        if (getString("migrated_biometric_weak", null, i) == null) {
            ...
            i2 = 0;
            setString("migrated_biometric_weak", TemperatureProvider.SWITCH_ON, 0);
        ...
        if (getString("migrated_lockscreen_disabled", null, i2) == null) {
            ...
            setString("migrated_lockscreen_disabled", TemperatureProvider.SWITCH_ON, 0);
        ...
        if (this.mContext.getPackageManager().hasSystemFeature("android.hardware.type.watch") && getString("migrated_wear_lockscreen_disabled", null, 0) == null) {
            ...
            setString("migrated_wear_lockscreen_disabled", TemperatureProvider.SWITCH_ON, 0);
    ...
    public void setBoolean(String key, boolean value, int userId) {
        checkWritePermission(userId);
        setStringUnchecked(key, userId, value ? NoFocusWindow.HUNG_CONFIG_ENABLE : "0");
    }
    ...
    public boolean getBoolean(String key, boolean defaultValue, int userId) {
        ...
        return value.equals(NoFocusWindow.HUNG_CONFIG_ENABLE) || value.equals(TemperatureProvider.SWITCH_ON);
    }
    ...
    public String getStringUnchecked(String key, String defaultValue, int userId) {
        if ("lock_pattern_autolock".equals(key)) {
            ...
            try {
                return this.mLockPatternUtils.isLockPatternEnabled(userId) ? NoFocusWindow.HUNG_CONFIG_ENABLE : "0";
            } finally {
    ...
    public byte[] getDecryptedPasswordForTiedProfile(int userId) throws KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException, CertificateException, IOException {
        if (this.DEBUG) {
            Slog.v(TAG, "Get child profile decrypted key");
        }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:25:0x006b, code lost:
        if (r22 != -1) goto L_0x0099;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:26:0x006d, code lost:
        if (r14 == null) goto L_0x0076;
     */
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:32:0x00a5, code lost:
        if (isManagedProfileWithUnifiedLock(r25) == false) goto L_0x00c3;
     */
    ...
    private void setLockCredentialInternal(byte[] credential, int credentialType, byte[] savedCredential, int requestedQuality, int userId, boolean allowUntrustedChange, boolean isLockTiedToParent) throws RemoteException {
        ...
        synchronized (this.mSpManager) {
            if (isSyntheticPasswordBasedCredentialLocked(userId)) {
                if (!OppoLockSettingsServiceUtils.isMemoryLow()) {
        ...
        synchronized (syntheticPasswordManager2) {
            try {
                if (shouldMigrateToSyntheticPasswordLocked(userId)) {
                    ...
                    try {
                        if (this.DEBUG) {
                            Slog.d(TAG, "setLockCredentialInternal: user=" + userId);
                        }
     ...
     */
    /* JADX WARNING: Removed duplicated region for block: B:24:0x013c A[ExcHandler: IOException | InvalidKeyException | KeyStoreException | NoSuchAlgorithmException | UnrecoverableKeyException | CertificateException | BadPaddingException | IllegalBlockSizeException | NoSuchPaddingException (r0v13 'e' java.lang.Exception A[CUSTOM_DECLARE]), Splitter:B:7:0x00d8] */
    @VisibleForTesting
    ...
    private void setAuthlessUserKeyProtection(int userId, byte[] key) throws RemoteException {
        if (this.DEBUG) {
            Slog.d(TAG, "setAuthlessUserKeyProtectiond: user=" + userId);
        }
    ...
    private void setUserKeyProtection(int userId, byte[] credential, VerifyCredentialResponse vcr) throws RemoteException {
        if (this.DEBUG) {
            Slog.d(TAG, "setUserKeyProtection: user=" + userId);
        }
        if (vcr == null) {
    ...
    private void clearUserKeyProtection(int userId) throws RemoteException {
        if (this.DEBUG) {
            Slog.d(TAG, "clearUserKeyProtection user=" + userId);
        }
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                ...
                while (i2 < length2) {
                    ...
                    i2++;
                    profileIdsWithDisabled = profileIdsWithDisabled;
                }
            }
    ...
    private VerifyCredentialResponse doVerifyCredential(byte[] credential, int credentialType, boolean hasChallenge, long challenge, int userId, ICheckCredentialProgressCallback progressCallback) throws RemoteException {
        ...
        if (userId != -9999 || Settings.Global.getInt(this.mContext.getContentResolver(), "device_provisioned", 0) == 0) {
            ...
            if (response != null) {
                resetTimeoutFlag(response);
                Slog.i(TAG, "doVerifyCredential response.getResponseCode()= " + response.getResponseCode() + " timeout:" + response.getTimeout());
    ...
    private void removeKeystoreProfileKey(int targetUserId) {
        if (this.DEBUG) {
            Slog.v(TAG, "Remove keystore profile key for user: " + targetUserId);
        }
        try {
            java.security.KeyStore keyStore = java.security.KeyStore.getInstance("AndroidKeyStore");
            keyStore.load(null);
    ...
    private VerifyCredentialResponse spBasedDoVerifyCredential(byte[] userCredential, int credentialType, boolean hasChallenge, long challenge, int userId, ICheckCredentialProgressCallback progressCallback) throws RemoteException {
        ...
        if (hasChallenge || !pm2.hasSystemFeature("android.hardware.biometrics.face") || !this.mInjector.hasEnrolledBiometrics()) {
            ...
            try {
                if (!isSyntheticPasswordBasedCredentialLocked(userId)) {
                    ...
                    if (response.getResponseCode() == 0) {
                        ...
                        if (progressCallback != null && OppoLockSettingsServiceUtils.isBootFromOTA()) {
                            progressCallback.onCredentialVerified();
                        }
    ...
    private long setLockCredentialWithAuthTokenLocked(byte[] credential, int credentialType, SyntheticPasswordManager.AuthenticationToken auth, int requestedQuality, int userId) throws RemoteException {
        ...
        if (this.DEBUG) {
            Slog.d(TAG, "setLockCredentialWithAuthTokenLocked: user=" + userId);
        }
    ...
    private void spBasedSetLockCredentialInternalLocked(byte[] credential, int credentialType, byte[] savedCredential, int requestedQuality, int userId, boolean allowUntrustedChange, boolean isLockTiedToParent) throws RemoteException {
        ...
        if (this.DEBUG) {
            Slog.d(TAG, "spBasedSetLockCredentialInternalLocked: user=" + userId);
        }
        if (isManagedProfileWithUnifiedLock(userId)) {
            try {
            ...
            if (auth2 == null) {
                ...
                OppoLockSettingsServiceUtils oppoLockSettingsServiceUtils = mOppoLockSettingsServiceUtils;
                if (oppoLockSettingsServiceUtils != null && userId == 0) {
                    oppoLockSettingsServiceUtils.writeSecretToTee(response, credential, credentialType, userId);
                }
    ...
    private long addEscrowToken(byte[] token, int userId, LockPatternUtils.EscrowTokenStateChangeCallback callback) throws RemoteException {
        long handle;
        if (this.DEBUG) {
            Slog.d(TAG, "addEscrowToken: user=" + userId);
        }
        synchronized (this.mSpManager) {
    ...
    private void activateEscrowTokens(SyntheticPasswordManager.AuthenticationToken auth, int userId) {
        if (this.DEBUG) {
            Slog.d(TAG, "activateEscrowTokens: user=" + userId);
        }
        synchronized (this.mSpManager) {
            disableEscrowTokenOnNonManagedDevicesIfNeeded(userId);
    ...
    @Override // com.android.server.locksettings.OppoBaseLockSettingsService
    public LockPatternUtils getLockPatternUtils() {
        return this.mLockPatternUtils;
    }

    public class LockSettingsInner implements IOppoLockSettingsInner {
        public LockSettingsInner() {
        }

        @Override // com.android.server.locksettings.IOppoLockSettingsInner
        public long getSyntheticPasswordHandle(int userId) {
            long syntheticPasswordHandleLocked;
            synchronized (LockSettingsService.this.mSpManager) {
                syntheticPasswordHandleLocked = LockSettingsService.this.getSyntheticPasswordHandleLocked(userId);
            }
            return syntheticPasswordHandleLocked;
        }

        @Override // com.android.server.locksettings.IOppoLockSettingsInner
        public boolean isSyntheticPasswordBasedCredential(int userId) {
            boolean isSyntheticPasswordBasedCredentialLocked;
            synchronized (LockSettingsService.this.mSpManager) {
                isSyntheticPasswordBasedCredentialLocked = LockSettingsService.this.isSyntheticPasswordBasedCredentialLocked(userId);
            }
