...
public class ColorCommonListManager implements IColorCommonListManager {
    ...
    private static final boolean LOG_DEBUG = SystemProperties.getBoolean(ColorAILog.OPPO_LOG_KEY, (boolean) LOG_DEBUG);
    ...
    public void updateAppInfo(int callingUid, String action, int uid, String pkgName, boolean isBind) {
    ...
    private void addConfigInfoList(String pkg, int uid, String cfgName) {
        ...
        synchronized (this.mCommonListMap) {
            ...
            if (lastMap == null) {
                ...
                if (cfgName == CONFIG_DEFAULT_INPUT) {
                    for (int i = 0; i < lastMap.size(); i++) {
                        if (userId == UserHandle.getUserId(lastMap.keyAt(i))) {
                            lastMap.removeAt(i);
                        }
                    }
                }
    ...
    private void updateInputMethod(boolean dispatch) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void updateDefaultDialerApp() {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void handleUpdateAppInfo(Message msg) {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:59:?, code lost:
        return;
     */
    private void handleUpdateWindowStateChange(Message msg) {
        if (msg != null && msg.getData() != null) {
            int uid = msg.arg1;
    ...
    public boolean isProtectedByTraffic(long usedTime, int uid, boolean isFgService, boolean isScreenOn, long screenOffFreezeTime, int lastResumedUid) {
        ...
        if (trafficUpdateTime == 0 || usedTime <= trafficUpdateTime) {
            ...
            if (!isFgService || !this.mTracker.isDownloadingApp(UserHandle.getAppId(uid))) {
                return LOG_DEBUG;
            }
