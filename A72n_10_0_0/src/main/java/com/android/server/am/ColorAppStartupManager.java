...
public class ColorAppStartupManager implements IColorAppStartupManager {
    ...
    private static final int NOTIF_SRC_SUPER_POWER_SAVE_MODE = 9;
    private static final int NOTIF_SRC_SUPER_POWER_SAVE_MODE_OFF = 39;
    private static final int NOTIF_SRC_SUPER_POWER_SAVE_MODE_ON = 38;
    ...
    private static String mGamePayAbnormalCallerCpn;
    private static String mGamePayAbnormalCallerPkg;
    private static String mGamePayAbnormalFirstCpn;
    ...
    private String SPS_BLACK_LIST;
    private String SPS_DESKTOP_LIST;
    private String SPS_LIST;
    private String SPS_MODE;
    private String SPS_WHITE_ACTION_LIST;
    private String SPS_WHITE_ACTIVITY_LIST;
    private String SPS_WHITE_BROADCAST_LIST;
    private String SPS_WHITE_PKG_LIST;
    private String SPS_WHITE_PROVIDER_LIST;
    private String SPS_WHITE_SERVICE_LIST;
    private boolean hypusModeOn;
    ...
    private List<ColorAppStartupMonitorInfo> mCollectBlackListInterceptList;
    private List<ColorAppStartupMonitorInfo> mCollectGamePayAbnormalList;
    private List<ColorAppStartupMonitorInfo> mCollectGamePayMonitorList;
    private List<ColorAppStartupMonitorInfo> mCollectProcessInfoList;
    ...
    private int mCpnUpperLimitCountInterval;
    private int mCpnUpperLimitCountStep;
    private List<Map<String, String>> mCpnUpperLimitMap;
    private boolean mCpnUpperLimitSwitch;
    private int mCpnUpperLimitThresholdConnected;
    ...
    private String mDefaultDialerApp;
    ...
    private List<String> mSPSActionWhiteList;
    private List<String> mSPSActivityWhiteList;
    private List<String> mSPSDeskTopList;
    private List<String> mSPSPkgBlackList;
    private List<String> mSPSPkgWhiteList;
    private List<String> mSPSProviderWhiteList;
    private List<String> mSPSReceiverWhiteList;
    private List<String> mSPSServiceWhiteList;
    ...
    private boolean mSuperPowerSavingEnable;
    ...
    private ColorAppStartupManager() {
        ...
        this.mSuperPowerSavingEnable = false;
        this.mSPSPkgWhiteList = new ArrayList();
        this.mSPSActivityWhiteList = new ArrayList();
        this.mSPSServiceWhiteList = new ArrayList();
        this.mSPSProviderWhiteList = new ArrayList();
        this.mSPSReceiverWhiteList = new ArrayList();
        this.mSPSActionWhiteList = new ArrayList();
        this.mSPSPkgBlackList = new ArrayList();
        this.mSPSDeskTopList = new ArrayList();
        this.SPS_LIST = "SPS_list";
        this.SPS_WHITE_PKG_LIST = "SPS_white_pkg_list";
        this.SPS_WHITE_ACTIVITY_LIST = "SPS_white_activity_list";
        this.SPS_WHITE_SERVICE_LIST = "SPS_white_service_list";
        this.SPS_WHITE_BROADCAST_LIST = "SPS_white_broadcast_list";
        this.SPS_WHITE_PROVIDER_LIST = "SPS_white_provider_list";
        this.SPS_WHITE_ACTION_LIST = "SPS_white_action_list";
        this.SPS_BLACK_LIST = "SPS_black_list";
        this.SPS_DESKTOP_LIST = "SPS_desktop_list";
        this.SPS_MODE = "SPS_mode";
        ...
        this.mDufaultInputMethodObserver = new ContentObserver(this.mHandler) {
            ...
            public void onChange(boolean selfChange) {
                ...
                colorAppStartupManager.mDefaultInputMethod = colorAppStartupManager.getDefaultInputMethod();
            }
        };
        this.mDefaultAppReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ColorAppStartupManager colorAppStartupManager = ColorAppStartupManager.this;
                colorAppStartupManager.mDefaultDialerApp = colorAppStartupManager.getDefaultDialerApp();
    ...
    public boolean shouldPreventProcessBroadcast(BroadcastRecord record, ResolveInfo info, String type) {
        ...
        if ("startProcess".equals(type)) {
            if (isInSuperPowerSavingMode()) {
                String cpnName = "";
                if (record.curComponent != null) {
                    cpnName = record.curComponent.getClassName();
                }
                SPSCase result = isAllowForSPS(record.intent, record.callingUid, info.activityInfo.packageName, cpnName, info.activityInfo.applicationInfo.uid, ColorHansManager.HANS_UFZ_REASON_BROADCAST, info.activityInfo.applicationInfo);
                if (result == SPSCase.TRUE) {
                    return false;
                }
                if (result == SPSCase.FALSE) {
                    if (this.mDebugSwitch) {
                        Log.i(ColorAppStartupMonitorInfo.TAG, "prevent start " + info.activityInfo.packageName + ", cmp " + cpnName + " by broadcast  callingUid " + record.callingUid);
                    }
                    return true;
                }
            }
    ...
    public void handleResumeActivity(IColorActivityRecordEx record, String processName, String pkgName) {
        if (isInSuperPowerSavingMode() && processName.equals(pkgName) && this.hypusModeOn) {
            this.mHypnus.hypnusSetNotification(9, (int) NOTIF_SRC_SUPER_POWER_SAVE_MODE_OFF);
            this.hypusModeOn = false;
            this.mHandler.postDelayed(new Runnable() {
                /* class com.android.server.am.ColorAppStartupManager.AnonymousClass2 */

                public void run() {
                    ColorAppStartupManager.this.mHypnus.hypnusSetNotification(9, (int) ColorAppStartupManager.NOTIF_SRC_SUPER_POWER_SAVE_MODE_ON);
                    ColorAppStartupManager.this.hypusModeOn = true;
                }
            }, 5000);
        }
        handleResumeActivity(record);
    }
    ...
    private boolean handleStartActivity(Intent intent, Intent ephemeralIntent, ActivityInfo aInfo, String callingPackage, int callingUid, int callingPid, String reason, int userId) {
            ...
            if (calledPackageName2 == null || !OppoListManager.getInstance().isAppStartForbidden(calledPackageName2)) {
                ...
                if (cpnClassName == null || !inBlackguardActivityList(cpnClassName)) {
                    ...
                    if (!OppoListManager.getInstance().getPreventRedundentStartSwitch() || cpnClassName == null || !OppoListManager.getInstance().isRedundentActivity(cpnClassName)) {
                        if (isInSuperPowerSavingMode()) {
                            if (typeCasting(intent) != null) {
                                calledUid = typeCasting(intent).getUid();
                            } else {
                                calledUid = -1;
                            }
                            SPSCase result = isAllowForSPS(intent, callingUid, calledPackageName, cpnClassName, calledUid, ColorHansManager.HANS_UFZ_REASON_ACTIVITY, aInfo.applicationInfo);
                            if (result == SPSCase.TRUE) {
                                return false;
                            }
                            if (result != SPSCase.FALSE) {
                                ...
                                Log.i(ColorAppStartupMonitorInfo.TAG, "prevent start " + calledPackageName + ", cmp " + cpnClassName + " by activity  callingUid " + callingUid);
                                return true;
                            }
                        } else {
                            str2 = ColorAppStartupMonitorInfo.TAG;
                        }
                        if (calledPackageName == null || !isInPreventIndulgeList(calledPackageName)) {
                            ...
                            if (this.mSwitchMonitor || ColorAppStartupManagerUtils.getInstance().getGamePayMonitorSwitch()) {
                                ComponentName topCpn2 = getTopComponentName();
                                if (topCpn2 != null) {
                                    str3 = str2;
                                    this.mHandler.post(new CollectGamePayRunnable(callingPackage, topCpn2.getClassName(), calledPackageName, cpnClassName, intent));
                                } else {
                                    str3 = str2;
                                }
                            } else {
                            ...
                            if ((this.mSwitchMonitor || ColorAppStartupManagerUtils.getInstance().getGamePayAbnormalSwitch()) && (topCpn = getTopComponentName()) != null) {
                                this.mHandler.post(new CollectGamePayAbnormalRunnable(callingPackage, topCpn.getClassName(), calledPackageName, cpnClassName, intent));
                            }
                            if (!this.mDynamicDebug) {
                                return false;
                            }
                            Log.d(str3, "shouldPreventStartActivity check_black cost time ==  " + (SystemClock.elapsedRealtime() - time2));
                            return false;
                        }
                        if (this.mDebugSwitch) {
                            Log.v(str2, "handleStartActivity prevent indulge: " + calledPackageName);
                        }
        ...
        Log.v(str, "handleStartActivity aInfo == null || aInfo.packageName == null return.");
        return false;
    }
    ...
    public boolean isInLruProcessesLocked(int uid) {
        ...
        try {
            return activityManagerService.mOomAdjuster.mActiveUids.get(uid) != null;
        } catch (Exception e) {
            Log.e(ColorAppStartupMonitorInfo.TAG, "getActiveUids failed!!");
            return false;
        }
    }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGamePayAbnormalBlackUrlKeyList(String url) {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:104:0x02f9  */
    /* JADX WARNING: Removed duplicated region for block: B:109:0x030c  */
    ...
    /* JADX WARNING: Removed duplicated region for block: B:99:0x02e7  */
    private boolean isAllowStartFromService(ProcessRecord callerApp, String callerPkg, int callingUid, ServiceRecord s, Intent intent, String type) {
        ...
        String str19;
        String str20;
        String calleePkg5 = s.appInfo.packageName;
        if (!isInSuperPowerSavingMode() || intent.getComponent() == null) {
            str2 = ColorAppStartupMonitorInfo.TAG;
            str = "prevent start ";
            str3 = " by service  callingUid ";
        } else {
            String className = intent.getComponent().getClassName();
            int i = s.appInfo.uid;
            ApplicationInfo applicationInfo = s.appInfo;
            str2 = ColorAppStartupMonitorInfo.TAG;
            str = "prevent start ";
            str3 = " by service  callingUid ";
            SPSCase result = isAllowForSPS(intent, callingUid, calleePkg5, className, i, "Service", applicationInfo);
            if (result == SPSCase.TRUE) {
                return true;
            }
            if (result == SPSCase.FALSE) {
                ...
                Log.i(str2, str + calleePkg5 + ", cmp " + intent.getComponent().getClassName() + str3 + callingUid);
                return false;
            }
        }
        if (isDefaultAllowStart(s.appInfo)) {
            ...
            if (callerApp == null) {
                str15 = type;
                if ("ssfa".equals(str15) || "sscn".equals(str15)) {
                    if (!inProtectWhiteList(calleePkg5)) {
                        str20 = str3;
                        if (!OppoListManager.getInstance().isInAutoBootWhiteList(calleePkg5, s.userId) && !OppoListManager.getInstance().isInstalledAppWidget(calleePkg5, s.appInfo.uid) && (!"sscn".equals(str15) || !OppoListManager.getInstance().isFromNotifyPkg(calleePkg5))) {
                            str6 = str15;
                            str7 = ", cmp ";
                            str5 = str2;
                            ...
                            if (actionStr != null) {
                                str8 = str6;
                                calleePkg2 = calleePkg5;
                                callerPkg2 = callerPkg;
                                actionStr2 = ", Type ";
                            } else if (actionStr.equals("android.media.browser.MediabrowserService")) {
                                if (callerApp.info.uid != 1002) {
                                    str8 = str6;
                                    calleePkg2 = calleePkg5;
                                    actionStr2 = ", Type ";
                                    calleePkg3 = str7;
                                    ...
                                    Log.i(str5, str + s.appInfo.packageName + str7 + cmpStr + " by " + callerPkg + " callingUid " + callingUid + ", Type " + str6);
                                    return false;
                                } else {
                                    str8 = str6;
                                    calleePkg2 = calleePkg5;
                                ...
                                if (!"bsfj".equals(str8) || "ssfa".equals(str8)) {
                                    callerPkg3 = callerPkg2;
                                    str11 = str8;
                                    str9 = str5;
                                    colorAppStartupManager2 = this;
                                    str10 = "ssfa";
                                } else if (!"sscn".equals(str8)) {
                                    colorAppStartupManager2 = this;
                                    if (colorAppStartupManager2.isSameApplication(callerApp, callingUid, s.appInfo)) {
                                        str11 = str8;
                                        callerPkg3 = callerPkg2;
                                        str9 = str5;
                                        str10 = "ssfa";
                                        ...
                                        str9 = str5;
                                        str10 = "ssfa";
                                        updateLaunchRecord(callerPkg2, calleePkg2, "1", "1", type);
                                        ...
                                        if (cpn == null) {
                                            if (colorAppStartupManager2.mSwitchMonitor) {
                                                colorAppStartupManager2.mHandler.post(new CollectAppInterceptRunnable(callerPkg3, calleePkg2, cpn.getClassName(), type));
                                            }
                                            Log.i(str9, str + calleePkg2 + calleePkg3 + cpn.toShortString() + " by " + callerPkg3 + " callingUid " + callingUid + actionStr2 + type);
                                            return false;
                                        }
                                        String action = intent.getAction();
                                        if (action == null) {
                                        ...
                                        if (this.mSwitchMonitor) {
                                            if (str11.equals("ss")) {
                                                ...
                                                this.mHandler.post(new CollectAppInterceptRunnable(callerPkg3, calleePkg2, action, "ssa"));
                                            } else {
                                                str12 = actionStr2;
                                                str13 = str9;
                                                colorAppStartupManager3 = this;
                                            ...
                                            if (str11.equals("bs")) {
                                                colorAppStartupManager3.mHandler.post(new CollectAppInterceptRunnable(callerPkg3, calleePkg2, action, "bsa"));
                                            }
                                        } else {
                                            str12 = actionStr2;
                                            str13 = str9;
                                        }
                                        Log.i(str13, str + calleePkg2 + "s.name " + s.name + ", Intent { act=" + action + " } by " + callerPkg3 + " callingUid " + callingUid + str12 + str11);
                                        return false;
                                    }
                                } else {
                                    callerPkg3 = callerPkg2;
                                ...
                                updateLaunchRecord(RECORD_CALLER_ANDROID, calleePkg2, "0", "1", type);
                                if (colorAppStartupManager2.mSwitchMonitor) {
                                }
                                if (str10.equals(str11)) {
                                }
                    ...
                    updateLaunchRecord(RECORD_CALLER_ANDROID, calleePkg5, RECORD_AUTO_LAUNCH_ALLOW_MODE, "0", type);
                    if (this.mDebugSwitch) {
                        Log.i(str2, "start " + calleePkg5 + str20 + callingUid + ", intent=" + intent);
                    }
                    return true;
                }
                str16 = str3;
            ...
            if ("bsfj".equals(str15)) {
                if (!inProtectWhiteList(calleePkg5)) {
                    str19 = str16;
                    if (!OppoListManager.getInstance().isInAutoBootWhiteList(calleePkg5, s.userId)) {
                        str7 = ", cmp ";
                        str5 = str14;
                        str6 = type;
                    }
                ...
                updateLaunchRecord(RECORD_CALLER_ANDROID, calleePkg5, RECORD_AUTO_LAUNCH_ALLOW_MODE, "0", type);
                if (this.mDebugSwitch) {
                    Log.i(str14, "start " + calleePkg5 + str19 + callingUid + ", intent=" + intent3);
                }
                return true;
            }
            str6 = str15;
            str7 = ", cmp ";
            str5 = str14;
            if (inAssociateStartWhiteList(calleePkg5, s.userId)) {
                str17 = str5;
                str18 = ", intent=";
                colorAppStartupManager4 = this;
                calleePkg4 = calleePkg5;
            } else if (inProtectWhiteList(calleePkg5)) {
                str17 = str5;
                str18 = ", intent=";
                colorAppStartupManager4 = this;
                calleePkg4 = calleePkg5;
            }
            updateLaunchRecord(RECORD_CALLER_ANDROID, calleePkg4, RECORD_ASSOCIATE_LAUNCH_ALLOW_MODE, "0", type);
            if (colorAppStartupManager4.mDebugSwitch) {
                Log.i(str17, "start " + calleePkg4 + str16 + callingUid + str18 + intent);
            }
            return true;
            actionStr = intent.getAction();
            if (actionStr != null) {
        ...
        if (colorAppStartupManager.mDebugSwitch) {
            Log.i(str2, "start " + calleePkg + " by service callingUid " + callingUid + str4 + intent2);
        }
        return true;
    }
    ...
    public boolean isAllowStartFromBroadCast(ProcessRecord callerApp, String callerPkg, int callingUid, Intent intent, ResolveInfo info) {
        ...
        if (isExpVersion()) {
            ...
            if (!isExGlobalBlackList(packageName)) {
        ...
        String packageName2 = info.activityInfo.applicationInfo.packageName;
        ...
        if (isDefaultAllowStart(info.activityInfo.applicationInfo)) {
            ...
            if (OppoListManager.getInstance().isInAutoBootWhiteList(packageName2, UserHandle.getUserId(uid))) {
                str3 = ColorAppStartupMonitorInfo.TAG;
            } else if (inProtectWhiteList(packageName2)) {
                str3 = ColorAppStartupMonitorInfo.TAG;
            } else if (inBroadCastWhiteList(packageName2)) {
                str3 = ColorAppStartupMonitorInfo.TAG;
            } else if (inBroadCastActionWhiteList(intent)) {
                str3 = ColorAppStartupMonitorInfo.TAG;
            } else {
                updateLaunchRecord(RECORD_CALLER_ANDROID, packageName2, "0", "1", "broadcast");
                ...
                if (cpn != null) {
                    if (this.mSwitchMonitor) {
                        str4 = "prevent start ";
                        str5 = " callingUid ";
                        this.mHandler.post(new CollectAppInterceptRunnable(callerPkg3, packageName2, cpn.getClassName(), "broadcast"));
                    } else {
                        str5 = " callingUid ";
                        str4 = "prevent start ";
                    }
                    Log.i(ColorAppStartupMonitorInfo.TAG, str4 + packageName2 + ", cmp " + cpn.toShortString() + " by broadcast " + callerPkg3 + str5 + callingUid);
                    return false;
                }
                String action = intent.getAction();
                if (action == null) {
                ...
                if (this.mSwitchMonitor) {
                    this.mHandler.post(new CollectAppInterceptRunnable(callerPkg3, packageName2, action, "broadcast_action"));
                }
                Log.i(ColorAppStartupMonitorInfo.TAG, "prevent start " + packageName2 + ", Intent { act=" + action + " } by broadcast " + callerPkg3 + " callingUid " + callingUid);
                return false;
            }
            updateLaunchRecord(RECORD_CALLER_ANDROID, packageName2, RECORD_AUTO_LAUNCH_ALLOW_MODE, "0", "broadcast");
            if (this.mDebugSwitch) {
                Log.d(str3, "start " + packageName2 + " by broadcast  callingUid " + callingUid + ", Intent=" + intent);
            }
            return true;
        }
        if (this.mDebugSwitch) {
            Log.d(str2, str + packageName2 + " by broadcast  callingUid " + callingUid + ", Intent=" + intent2);
        }
        return true;
    }
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r16v8, resolved type: boolean */
    ...
    public boolean isAllowStartFromProvider(ProcessRecord callerApp, ContentProviderRecord cpr, ApplicationInfo appInfo) {
        ...
        if (isInSuperPowerSavingMode()) {
            ComponentName cpn2 = cpr.name;
            if (cpn2 != null) {
                cpnName = cpn2.getClassName();
            } else {
                cpnName = "";
            }
            SPSCase result = isAllowForSPS(null, callingUid, packageName, cpnName, appInfo.uid, ColorHansManager.HANS_UFZ_REASON_PROVIDER, appInfo);
            if (result == SPSCase.TRUE) {
                return true;
            }
            if (result != SPSCase.FALSE) {
                str3 = ColorAppStartupMonitorInfo.TAG;
                str = " by provider  callingUid ";
                str2 = "prevent start ";
                i = 0;
            } else if (!this.mDebugSwitch) {
                return false;
            } else {
                Log.i(ColorAppStartupMonitorInfo.TAG, "prevent start " + packageName + ", cmp " + cpnName + " by provider  callingUid " + callingUid);
                return false;
            }
        } else {
            str3 = ColorAppStartupMonitorInfo.TAG;
        ...
        if (isDefaultAllowStart(appInfo)) {
            ...
            if (inAssociateStartWhiteList(packageName, UserHandle.getUserId(appInfo.uid))) {
                ...
                if (callerApp != null) {
                    ...
                    String callerPkg2 = pkgList.get(i);
                    if (callerPkg2 == null || !inProviderCallerPkgList(callerPkg2)) {
                        ...
                        Log.i(str3, "start " + packageName + str + callingUid + ", cpr=" + cpr.toShortString() + ", inProviderCallerPkgList");
                        return true;
                    }
                } else {
                    z = i;
                ...
                if (this.mSwitchMonitor || this.mDebugSwitch) {
                    ComponentName cpn3 = cpr.name;
                    if (cpn3 != null) {
                        if (this.mSwitchMonitor) {
                            cpn = cpn3;
                            this.mHandler.post(new CollectAppInterceptRunnable(callerPkg, packageName, cpn3.getClassName(), "provider"));
                        } else {
                            cpn = cpn3;
                        }
                        Log.i(str10, str2 + packageName + ", cmp " + cpn.toShortString() + " by contentprovider " + callerPkg + " callingUid " + callingUid);
                    }
                }
                return z;
            } else {
            ...
            Log.i(str8, str9 + packageName + str + callingUid + str7 + cpr.toShortString());
            return true;
        }
        if (!this.mDebugSwitch) {
            return true;
        }
        Log.i(str5, str6 + packageName + str + callingUid + str4 + cpr.toShortString());
        return true;
    }
    ...
    private boolean isDefaultAllowStart(ApplicationInfo info) {
        ...
        if (info.uid <= 10000 || (info.flags & 1) != 0 || (info.flags & ColorHansRestriction.HANS_RESTRICTION_BLOCK_BINDER) != 0 || inCustomizeWhiteList(info.packageName) || inGlobalWhiteList(info.packageName) || isCtsRunning()) {
            return true;
        }
        return false;
    }
    ...
    /* access modifiers changed from: private */
    public class CollectGamePayRunnable implements Runnable {
    ...
    /* access modifiers changed from: private */
    public class AppAbnormalMonitor implements Runnable {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void uploadCpnUpperLimit() {
        ActivityManagerService activityManagerService;
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void notifyPreventIndulge(String pkgName) {
    ...
    /* access modifiers changed from: private */
    public class PreventIndulgeRunnable implements Runnable {
    ...
    public void associateWhiteFileChanage(final int userId) {
        this.mHandler.postDelayed(new Runnable() {
            /* class com.android.server.am.ColorAppStartupManager.AnonymousClass7 */
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGamePayMonitorCpnList(String pkgName) {
        return ColorAppStartupManagerUtils.getInstance().getGamePayMonitorCpnList().contains(pkgName);
    }

    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGamePayMonitorFuzzyCpnList(String pkgName, String cpnName) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGameMonitorPackageList(String pkgName) {
    ...
    public boolean isCoreApp(ProcessRecord app) {
        if (isExpVersion() && !isInSuperPowerSavingMode()) {
            return true;
        }
        if (!(app == null || app.info == null || app.info.packageName == null)) {
            if (isInSuperPowerSavingMode()) {
                if (app.getCurProcState() <= 2) {
                    return true;
                }
                if (isDefaultAllowStart(app.info)) {
                    return !isInSuperPowerSavingBlackList(app.info.packageName);
                }
                if (!isInSPSDeskTopList(app.info.packageName)) {
                    return false;
                }
            }
            if (isDefaultAllowStart(app.info)) {
    ...
    /* access modifiers changed from: private */
    public class CollectGamePayAbnormalRunnable implements Runnable {
        private String mCalledCpn;
        private String mCalledPkgName;
        private String mCallerCpn;
        private String mCallerPkgName;
            ...
            if (this.mCallerPkgName != null && this.mCallerCpn != null && this.mCalledPkgName != null && this.mCalledCpn != null) {
                ...
                if (!ColorAppStartupManager.this.isInGamePayAbnormalWhiteList(this.mCallerPkgName, this.mCallerCpn, this.mCalledPkgName, this.mCalledCpn)) {
                    if (ColorAppStartupManager.this.isInGamePayAbnormalCallerBlackList(this.mCallerPkgName)) {
                        ...
                        synchronized (ColorAppStartupManager.this.mCollectGamePayAbnormalList) {
                            ColorAppStartupManager colorAppStartupManager = ColorAppStartupManager.this;
                            List list = ColorAppStartupManager.this.mCollectGamePayAbnormalList;
                            String str = ColorAppStartupManager.mGamePayAbnormalCallerPkg;
                            String str2 = ColorAppStartupManager.mGamePayAbnormalCallerCpn;
                            String str3 = this.mCalledPkgName;
                            colorAppStartupManager.collectProcessInfo(list, str, str2, str3, ColorAppStartupManager.mGamePayAbnormalFirstCpn + "," + this.mCalledCpn);
                            if (ColorAppStartupManager.this.mCollectGamePayAbnormalList.size() >= ColorAppStartupManagerUtils.getInstance().getGamePayAbnormalCheckCount()) {
                                ColorAppStartupManager.this.uploadProcessInfo(ColorAppStartupManager.this.mCollectGamePayAbnormalList, "game_pay");
                            }
                        }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void resetGamePayAbnormalStatus() {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGamePayAbnormalCalledBlackSecondMap(String calledPkgName, String calledFirstCpn, String calledSecondCpn, boolean isLookSecondCpn) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGamePayAbnormalCalledBlackFirstMap(String calledPkgName, String calledCpn) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGamePayAbnormalWhiteList(String callerPkg, String callerCpn, String calledPkg, String calledCpn) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean isInGamePayAbnormalCallerBlackList(String pkgName) {
    ...
    private boolean isInGamePayAbnormalPkgCpnMap(String pkgName, String cpnName, ArrayMap<String, List<String>> whitePkgCpnMap) {
        ...
        for (int i = 0; i < whitePkgCpnMap.size(); i++) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void uploadProcessInfo(List<ColorAppStartupMonitorInfo> infoList, String eventId) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void collectProcessInfo(List<ColorAppStartupMonitorInfo> infoList, String callerPkg, String callerCpn, String calledPkg, String calledCpn) {
    ...
    private boolean isSelfDevelopApp(String pkgName) {
        if (TextUtils.isEmpty(pkgName)) {
            return false;
        }
        if (pkgName.startsWith("com.oppo.") || pkgName.startsWith("com.coloros.") || pkgName.startsWith("com.nearme.") || pkgName.startsWith("com.heytap.")) {
            return true;
        }
        return false;
    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:59:0x0075, code lost:
        r3 = r4.mSPSActionWhiteList;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:60:0x0077, code lost:
        monitor-enter(r3);
     */
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:64:0x0080, code lost:
        if (r4.mSPSActionWhiteList.contains(r7) == false) goto L_0x0084;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:65:0x0082, code lost:
        monitor-exit(r3);
     */
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:68:0x0085, code lost:
        return false;
     */
    private boolean isInSuperPowerSavingWhiteList(String pkgName, String cpnName, String action, String cpnType) {
        boolean isCpnWhite = false;
        if (!(cpnType == null || cpnName == null)) {
            if (cpnType.equals(ColorHansManager.HANS_UFZ_REASON_ACTIVITY)) {
                synchronized (this.mSPSActivityWhiteList) {
                    isCpnWhite = this.mSPSActivityWhiteList.contains(cpnName);
                }
            } else if (cpnType.equals("Service")) {
                synchronized (this.mSPSServiceWhiteList) {
                    isCpnWhite = this.mSPSServiceWhiteList.contains(cpnName);
                }
            } else if (cpnType.equals(ColorHansManager.HANS_UFZ_REASON_PROVIDER)) {
                synchronized (this.mSPSProviderWhiteList) {
                    isCpnWhite = this.mSPSProviderWhiteList.contains(cpnName);
                }
            } else if (cpnType.equals(ColorHansManager.HANS_UFZ_REASON_BROADCAST)) {
                synchronized (this.mSPSReceiverWhiteList) {
                    isCpnWhite = this.mSPSReceiverWhiteList.contains(cpnName);
                }
            }
        }
        if (isCpnWhite) {
        ...
        synchronized (this.mSPSPkgWhiteList) {
            if (pkgName != null) {
                if (this.mSPSPkgWhiteList.contains(pkgName)) {
                    return true;
                }
            }
        }
    ...
    private boolean isInSuperPowerSavingBlackList(String pkgName) {
        boolean contains;
        synchronized (this.mSPSPkgBlackList) {
            contains = this.mSPSPkgBlackList.contains(pkgName);
        }
        return contains;
    }

    private boolean isInSPSDeskTopList(String pkgName) {
        boolean contains;
        synchronized (this.mSPSDeskTopList) {
            if (this.mDebugSwitch) {
                Log.d(ColorAppStartupMonitorInfo.TAG, " SPSDeskTopList: " + this.mSPSDeskTopList.toString());
            }
            contains = this.mSPSDeskTopList.contains(pkgName);
        }
        return contains;
    }

    private boolean isTopPkg(int callingUid, Intent intent, String cpnType) {
        if (OppoFeatureCache.get(IColorAppSwitchManager.DEFAULT) == null) {
            return false;
        }
        if (ColorHansManager.HANS_UFZ_REASON_ACTIVITY.equals(cpnType) && intent != null) {
            int uid = -1;
            if (typeCasting(intent) != null) {
                uid = typeCasting(intent).getCallingUid();
            }
            if (OppoFeatureCache.get(IColorAppSwitchManager.DEFAULT).getNextPkgUid() == uid) {
                return true;
            }
        }
        if (this.mDebugSwitch) {
            Log.d(ColorAppStartupMonitorInfo.TAG, " callingUid: " + callingUid + " topUid: " + OppoFeatureCache.get(IColorAppSwitchManager.DEFAULT).getNextPkgUid());
        }
        if (callingUid == OppoFeatureCache.get(IColorAppSwitchManager.DEFAULT).getNextPkgUid()) {
            return true;
        }
        return false;
    }
    ...
    private SPSCase isAllowForSPS(Intent intent, int callingUid, String pkgName, String cpnName, int uid, String cpnType, ApplicationInfo appInfo) {
        if (isInLruProcessesLocked(uid) && uid > 10000) {
            return SPSCase.TRUE;
        }
        if (!isDefaultAllowStart(appInfo)) {
            String action = null;
            ...
            if (isInSuperPowerSavingWhiteList(pkgName, cpnName, action, cpnType)) {
                return SPSCase.TRUE;
            }
            if (!isInSuperPowerSavingBlackList(pkgName) || isInSPSDeskTopList(pkgName)) {
                if (isSelfDevelopApp(pkgName)) {
                    if (inAssociateStartWhiteList(pkgName, UserHandle.getUserId(uid)) || OppoListManager.getInstance().isInAutoBootWhiteList(pkgName, UserHandle.getUserId(uid)) || isInSPSDeskTopList(pkgName)) {
                        return SPSCase.CANCLE;
                    }
                    if (isTopPkg(callingUid, intent, cpnType)) {
                        return SPSCase.TRUE;
                    }
                    return SPSCase.FALSE;
                } else if (isInSPSDeskTopList(pkgName)) {
                    return SPSCase.CANCLE;
                } else {
                    if (isTopPkg(callingUid, intent, cpnType)) {
                        return SPSCase.TRUE;
            ...
            } else if (isTopPkg(callingUid, intent, cpnType)) {
                return SPSCase.TRUE;
            } else {
                return SPSCase.FALSE;
            }
        } else if (!isInSuperPowerSavingBlackList(pkgName) || isInSPSDeskTopList(pkgName)) {
            return SPSCase.TRUE;
        } else {
            if (isTopPkg(callingUid, intent, cpnType)) {
                return SPSCase.TRUE;
