...
public final class NativeCrashListener extends Thread {
    ...
    static int unpackInt(byte[] buf, int offset) {
        return ((buf[offset] & OppoNfcChipVersion.NONE) << 24) | ((buf[offset + 1] & OppoNfcChipVersion.NONE) << 16) | ((buf[offset + 2] & OppoNfcChipVersion.NONE) << 8) | (buf[offset + 3] & OppoNfcChipVersion.NONE);
    }
    ...
    public void consumeNativeCrashData(FileDescriptor fd) {
        ...
        try {
            ...
            if (pid > 0) {
                ...
                if (pr != null) {
                    this.mAm.setCrashProcessRecord(pr);
                    if (!pr.isPersistent()) {
                        while (true) {
