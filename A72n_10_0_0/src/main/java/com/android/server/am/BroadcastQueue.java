...
public class BroadcastQueue extends OppoBaseBroadcastQueue {
            ...
            switch (msg.what) {
                case 200:
                    if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
                        String str = BroadcastQueue.TAG_BROADCAST;
                        Slog.v(str, "Received BROADCAST_INTENT_MSG [" + BroadcastQueue.this.mQueueName + "]");
                    }
    ...
    private final void processCurBroadcastLocked(BroadcastRecord r, ProcessRecord app, boolean skipOomAdj) throws RemoteException {
        ...
        } else if (!OppoFeatureCache.get(IColorHansManager.DEFAULT).hansBroadcastIfNeeded(r, r.curReceiver)) {
            skipReceiverLocked(r);
        } else {
    ...
    /* access modifiers changed from: package-private */
    /* JADX WARNING: Code restructure failed: missing block: B:501:0x127f, code lost:
        r0 = e;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:505:0x12cf, code lost:
        r0 = e;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:506:0x12d0, code lost:
        r10 = r6;
     */
    ...
    /* JADX WARNING: Removed duplicated region for block: B:501:0x127f A[ExcHandler: RuntimeException (e java.lang.RuntimeException), Splitter:B:488:0x1258] */
    /* JADX WARNING: Removed duplicated region for block: B:511:0x12f7  */
    /* JADX WARNING: Removed duplicated region for block: B:514:0x133b  */
    /* JADX WARNING: Removed duplicated region for block: B:516:0x1370  */
    /* JADX WARNING: Removed duplicated region for block: B:540:0x14d0 A[LOOP:2: B:69:0x024b->B:540:0x14d0, LOOP_END] */
    /* JADX WARNING: Removed duplicated region for block: B:548:0x041f A[SYNTHETIC] */
    /* JADX WARNING: Removed duplicated region for block: B:550:0x07d0 A[SYNTHETIC] */
    ...
    public final void processNextBroadcastLocked(boolean fromMsg, boolean skipOomAdj) {
        ...
        while (true) {
            ...
            OppoFeatureCache.get(IColorBroadcastManager.DEFAULT).adjustOrderedBroadcastReceiversQueue(r4, numReceivers == true ? 1 : 0);
                                            ...
                                            if (!sendResult) {
                                                try {
                                                    ...
                                                    try {
                                                        ...
                                                        Slog.w(TAG, "Failure [" + this.mQueueName + "] sending broadcast result of " + r2.intent, e2);
                                                        if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
                                                        }
                            ...
                            if (r == null) {
                                ...
                                if (brOptions2 != null && (info.activityInfo.applicationInfo.targetSdkVersion < brOptions2.getMinManifestReceiverApiLevel() || info.activityInfo.applicationInfo.targetSdkVersion > brOptions2.getMaxManifestReceiverApiLevel())) {
                                    ...
                                    if (this.mColorQueue != null) {
                                        skip2 = this.mColorQueue.isSkipThisStaticBroadcastReceivers(r.intent, info);
                                    }
                                }
                                if (!skip2 && !this.mService.validateAssociationAllowedLocked(r.callerPackage, r.callingUid, component.getPackageName(), info.activityInfo.applicationInfo.uid)) {
                                ...
                                if (!skip2 && (!this.mService.mIntentFirewall.checkBroadcast(r.intent, r.callingUid, r.callingPid, r.resolvedType, info.activityInfo.applicationInfo.uid))) {
                                ...
                                if (!skip2 && perm != 0) {
                                    ...
                                    skip = true;
                                } else if (skip2 || info.activityInfo.permission == null || (opCode = AppOpsManager.permissionToOpCode(info.activityInfo.permission)) == -1 || this.mService.mAppOpsService.noteOperation(opCode, r.callingUid, r.callerPackage) == 0) {
                                    skip = skip2;
                                } else {
                                ...
                                if (!skip) {
                                    ...
                                    try {
                                        ...
                                        Slog.w(TAG, "Exception getting recipient info for " + info.activityInfo.packageName, e10);
                                    }
                                    if (!isAvailable) {
                                        if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
                                ...
                                if (!skip && (allowed = this.mService.getAppStartModeLocked(info.activityInfo.applicationInfo.uid, info.activityInfo.packageName, info.activityInfo.applicationInfo.targetSdkVersion, -1, true, false, false)) != 0) {
                                    if (allowed == 3) {
                                        ...
                                        skip = this.mColorQueue != null ? this.mColorQueue.isSkipThisStaticBroadcastReceivers(r.intent, info) : true;
                                ...
                                if (!this.mService.mAmsExt.isComponentNeedsStart(info.activityInfo.packageName, "static_broadcast") ? true : skip) {
                                    if (ActivityManagerDebugConfig.DEBUG_BROADCAST) {
                                ...
                                try {
                                    ...
                                    Slog.w(TAG, "Failed trying to unstop package " + r.curComponent.getPackageName() + ": " + e12);
                                }
                                if (app == null || app.thread == null || app.killed) {
                                    ...
                                    try {
                                        ...
                                        try {
                                            ...
                                            RuntimeException e15 = e14;
                                            Slog.w(TAG, "Failed sending broadcast to " + r.curComponent + " with " + r.intent, e15);
                                            logBroadcastReceiverDiscardLocked(r);
                                            finishReceiverLocked(r, r.resultCode, r.resultData, r.resultExtras, r.resultAbort, false);
                                            scheduleBroadcastsLocked();
                                            r.state = 0;
