...
public final class ActivityManagerConstants extends ContentObserver {
    ...
    private static int DEFAULT_MAX_CACHED_PROCESSES = AmsExt.DEFAULT_MAX_CACHED_PROCESSES_EX;
    ...
    static final long USE_TRIM_ENABLE_MEMORY = 4294967296L;
    ...
    public long MEMORY_INFO_THROTTLE_TIME = BackupAgentTimeoutParameters.DEFAULT_FULL_BACKUP_AGENT_TIMEOUT_MILLIS;
    ...
    public long TOP_TO_FGS_GRACE_DURATION = 15000;
    long USAGE_STATS_INTERACTION_INTERVAL = 7200000;
    private boolean isMtkLowEndCPU = false;
    volatile boolean mFlagActivityStartsLoggingEnabled;
    volatile boolean mFlagBackgroundActivityStartsEnabled;
    private final DeviceConfig.OnPropertiesChangedListener mOnDeviceConfigChangedListener = new DeviceConfig.OnPropertiesChangedListener() {
    ...
    static {
        boolean z = true;
        if (Process.getTotalMemory() > USE_TRIM_ENABLE_MEMORY) {
            z = false;
        }
        USE_TRIM_SETTINGS = z;
    }
    ...
    ActivityManagerConstants(Context context, ActivityManagerService service, Handler handler) {
        ...
        if (Build.IS_DEBUGGABLE && context.getResources().getBoolean(17891396)) {
            z = true;
        }
        ...
        this.isMtkLowEndCPU = "mt6853".equals(Build.HARDWARE);
        if (this.isMtkLowEndCPU) {
            this.MAX_CACHED_PROCESSES = 16;
            DEFAULT_MAX_CACHED_PROCESSES = 16;
        } else if (Process.getTotalMemory() > 3221225472L && Process.getTotalMemory() <= USE_TRIM_ENABLE_MEMORY) {
            this.MAX_CACHED_PROCESSES = 16;
            DEFAULT_MAX_CACHED_PROCESSES = 16;
        } else if (Process.getTotalMemory() > 5368709120L && Process.getTotalMemory() <= 6442450944L) {
            this.MAX_CACHED_PROCESSES = 32;
            DEFAULT_MAX_CACHED_PROCESSES = 32;
        } else if (Process.getTotalMemory() > 7516192768L) {
            this.MAX_CACHED_PROCESSES = 128;
            DEFAULT_MAX_CACHED_PROCESSES = 128;
        }
    }
