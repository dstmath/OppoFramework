    ...
    private Handler mHandler = new Handler(UiThread.get().getLooper(), null, true) {
        ...
        public void handleMessage(Message msg) {
            ...
            if (i == 1) {
                ...
                notifManager.notifyAsUser(PreBootBroadcaster.TAG, 13, new Notification.Builder(PreBootBroadcaster.this.mService.mContext, SystemNotificationChannels.UPDATES).setSmallIcon(17303520).setWhen(0).setOngoing(true).setTicker(title).setColor(context.getColor(17170460)).setContentTitle(title).setContentIntent(contentIntent).setVisibility(1).setProgress(max, index, false).build(), UserHandle.of(PreBootBroadcaster.this.mUserId));
            } else if (i == 2) {
