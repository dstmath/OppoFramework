...
public class HdmiCecLocalDeviceAudioSystem extends HdmiCecLocalDeviceSource {
    ...
    public void onStandby(boolean initiatedByCec, int standbyAction) {
        ...
        synchronized (this.mLock) {
            this.mService.writeStringSystemProperty("persist.sys.hdmi.last_system_audio_control", isSystemAudioActivated() ? TemperatureProvider.SWITCH_ON : TemperatureProvider.SWITCH_OFF);
        }
    ...
    public boolean handleReportPowerStatus(HdmiCecMessage command) {
        updateDevicePowerStatus(command.getSource(), command.getParams()[0] & OppoNfcChipVersion.NONE);
        return true;
    }
