...
public final class DeviceDiscoveryAction extends HdmiCecFeatureAction {
            ...
            if (i != 3) {
                if (i != 4) {
                    ...
                    if (cmd.getOpcode() == 144) {
                        ...
                        return true;
                    } else if (cmd.getOpcode() != 0 || (cmd.getParams()[0] & OppoNfcChipVersion.NONE) != 144) {
                        return false;
                    } else {
                        handleReportPowerStatus(cmd);
                        return true;
                    ...
                    handleVendorId(cmd);
                    return true;
                } else if (cmd.getOpcode() != 0 || (cmd.getParams()[0] & OppoNfcChipVersion.NONE) != 140) {
                    return false;
                } else {
                    handleVendorId(cmd);
                    return true;
                ...
                return true;
            } else if (cmd.getOpcode() != 0 || (cmd.getParams()[0] & OppoNfcChipVersion.NONE) != 70) {
                return false;
            } else {
                handleSetOsdName(cmd);
                return true;
    ...
    private void handleReportPhysicalAddress(HdmiCecMessage cmd) {
        ...
        current.mDeviceType = params[2] & OppoNfcChipVersion.NONE;
    ...
    private void handleReportPowerStatus(HdmiCecMessage cmd) {
        ...
        if (cmd.getOpcode() != 0) {
            current.mPowerStatus = cmd.getParams()[0] & OppoNfcChipVersion.NONE;
        }
        increaseProcessedDeviceCount();
        checkAndProceedStage();
    }
