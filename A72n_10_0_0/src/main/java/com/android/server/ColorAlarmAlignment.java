...
public class ColorAlarmAlignment implements IColorAlarmAlignment {
    ...
    private ColorAcmeAlarmPolicy mAcmeAlarmPolicy = null;
            ...
            synchronized (ColorAlarmAlignment.this.mLock) {
                if (alignInterval2 <= 0) {
                    ...
                    if (ColorAlarmAlignment.this.mInAcmeState != restrict) {
                        ColorAlarmAlignment.this.mInAcmeState = restrict;
                        if (restrict) {
                    ...
                    if (screenOffSwitch) {
                        ColorAlarmAlignment.this.mNeedAlign = true;
                    }
    ...
    private boolean ignoreAlarm(AlarmManagerService.Alarm a) {
        ...
        boolean z = false;
        if (ColorStartingWindowContants.WECHAT_PACKAGE_NAME.equals(pkgName) && this.mSysState.isSuperPowersaveOn()) {
            return false;
        }
        ...
        ColorAcmeAlarmPolicy colorAcmeAlarmPolicy = this.mAcmeAlarmPolicy;
        Context context = this.mContext;
        if (alignWhiteList && isNotRestrictApp) {
            z = true;
        }
        boolean restricted = colorAcmeAlarmPolicy.isRestrictedByAcme(context, a, pkgName, z, alignEnforcedWhiteList);
        if (this.ADBG) {
    ...
    public class WorkerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (i == 1) {
                ...
                ColorAlarmAlignment.this.mNeedAlign = false;
                ColorAlarmAlignment.this.mAcmeAlarmPolicy.triggerScreenOn(ColorAlarmAlignment.this.mContext);
            } else if (i == 2) {
                ColorAlarmAlignment.this.mAlignInterval = OppoFeatureCache.get(IColorAlarmManagerHelper.DEFAULT).getAlignInterval() * 60 * 1000;
                ColorAlarmAlignment.this.mAlignFirstDelay = OppoFeatureCache.get(IColorAlarmManagerHelper.DEFAULT).getAlignFirstDelay() * 60 * 1000;
                if (ColorAlarmAlignment.this.ADBG) {
                ...
                ColorAlarmAlignment.this.mScreenOffElapsed = SystemClock.elapsedRealtime();
