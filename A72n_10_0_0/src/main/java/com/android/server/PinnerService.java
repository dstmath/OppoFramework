...
public final class PinnerService extends SystemService {
    ...
    public PinnerService(Context context) {
        ...
        boolean shouldPinCamera = context.getResources().getBoolean(17891493);
        boolean shouldPinHome = context.getResources().getBoolean(17891494);
        if (shouldPinCamera) {
            this.mPinKeys.add(0);
        }
        if (shouldPinHome) {
    ...
    private void handlePinOnStart() {
        ...
        if (SystemProperties.get("dalvik.vm.boot-image", "").endsWith("apex.art")) {
            filesToPin = this.mContext.getResources().getStringArray(17235985);
        } else {
