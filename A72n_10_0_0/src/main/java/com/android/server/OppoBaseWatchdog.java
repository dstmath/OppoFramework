...
public abstract class OppoBaseWatchdog extends Thread {
    ...
    protected int mColorosPersistMultimediaPid;
    ...
    private boolean isProcessWaitForZygoteSocket() {
        ...
        for (int i = 0; i < count2; i++) {
            for (StackTraceElement element : threads[i].getStackTrace()) {
    ...
    public void dumpStackAndAddDropbox(final String subject) {
        ...
        if (NoFocusWindow.HUNG_CONFIG_ENABLE.equals(SystemProperties.get("ro.debuggable"))) {
            onBinderStateRead();
        }
    ...
    public void maybeRecordPerfInfo() {
        boolean agingTestVersion = NoFocusWindow.HUNG_CONFIG_ENABLE.equals(SystemProperties.get("SPECIAL_OPPO_CONFIG"));
    ...
    public void onProcessStarted(String name, int pid) {
        ...
        if ("com.coloros.persist.multimedia".equals(name)) {
            this.mColorosPersistMultimediaPid = pid;
        }
    }
