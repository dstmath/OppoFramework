...
import android.os.OppoRecoverySystem;
...
public class MasterClearReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        if (!intent.getAction().equals("com.google.android.c2dm.intent.RECEIVE") || "google.com".equals(intent.getStringExtra("from"))) {
            ...
            final boolean bFormatBackup = intent.getBooleanExtra("formatdata_backup", false);
            ...
            Thread thr = new Thread("Reboot") {
                ...
                public void run() {
                    try {
                        String operatorValue = SystemProperties.get("ro.oppo.operator", "");
                        if (operatorValue.equals("FET") || operatorValue.equals("TWM") || operatorValue.equals("APT")) {
                            Log.d(MasterClearReceiver.TAG, "reset operator to TWOP on masterclear!");
                            OppoEngineerManager.setCarrierVersion("00011010");
                            OppoEngineerManager.setSimOperatorSwitch(NoFocusWindow.HUNG_CONFIG_ENABLE);
                        }
                        ColorFrameworkFactory.getInstance().getColorFontManager().handleFactoryReset();
                        if (bFormat) {
                            OppoRecoverySystem.rebootFormatUserData(context, shutdown, reason, forceWipe, MasterClearReceiver.this.mWipeEsims);
                        } else if (bFormatBackup) {
                            OppoRecoverySystem.rebootFormatUserDataBackup(context, shutdown, reason, forceWipe, MasterClearReceiver.this.mWipeEsims);
                        } else {
    ...
    private class WipeDataTask extends AsyncTask<Void, Void, Void> {
        ...
        public void onPreExecute() {
            ...
            this.mProgressDialog.setMessage(this.mContext.getText(17040889));
