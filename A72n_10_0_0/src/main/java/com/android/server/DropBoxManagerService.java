...
public final class DropBoxManagerService extends SystemService {
    ...
    private static IColorEapManager mColorEapManager = null;
    private static OppoDropBoxManagerService mOppoDropBoxManagerService = null;
    ...
    public DropBoxManagerService(Context context, File path, Looper looper) {
        ...
        mOppoDropBoxManagerService = new OppoDropBoxManagerService(getContext());
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:60:?, code lost:
        ...
        r13.append(r10);
        r13.append(" bytes)");
        android.util.Slog.w(com.android.server.DropBoxManagerService.TAG, r13.toString());
        r4.delete();
        r4 = null;
     */
    ...
    public void add(DropBoxManager.Entry entry) {
        ...
        try {
            ...
            if ((flags & 1) == 0) {
                ...
                if (mOppoDropBoxManagerService != null) {
                    try {
                        mOppoDropBoxManagerService.addSystemLogFile(entry, tag);
                    } catch (IOException e2) {
                        e = e2;
                    }
                }
                long max3 = trimToFit();
                long max4 = System.currentTimeMillis();
                byte[] buffer = new byte[this.mBlockSize];
                input2 = entry.getInputStream();
                int read = 0;
                while (read < buffer.length && (n = input2.read(buffer, read, buffer.length - read)) > 0) {
                    try {
                        read += n;
                ...
                try {
                    ...
                    sb.append(".tmp");
                    temp = new File(file, sb.toString());
                ...
                try {
                    ...
                    OutputStream output3 = new BufferedOutputStream(foutput, bufferSize);
                    try {
                        if (read == buffer.length && (flags & 4) == 0) {
                            try {
                                output3 = new GZIPOutputStream(output3);
                                flags |= 4;
                        ...
                        File temp2 = null;
                        OppoFeatureCache.get(IColorEapManager.DEFAULT).addEntryToEap(tag, entry);
                        ...
                        if (mOppoDropBoxManagerService != null) {
                            mOppoDropBoxManagerService.addDropBoxFile(time, tag, flags);
                        }
    ...
    public synchronized void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        if (DumpUtils.checkDumpAndUsageStatsPermission(getContext(), TAG, pw)) {
            try {
                ...
                pw.println("Can't initialize: " + e13);
                Slog.e(TAG, "Can't init", e13);
            }
        }
    }
    ...
    public static final class EntryFile implements Comparable<EntryFile> {
        ...
        public EntryFile(File temp, File dir, String tag2, long timestampMillis2, int flags2, int blockSize) throws IOException {
            if ((flags2 & 1) == 0) {
                ...
                if (temp.renameTo(file)) {
                    ...
                    OppoFeatureCache.get(IColorEapManager.DEFAULT).setFileName(file.getName());
                    return;
                }
        ...
        public void deleteFile(File dir) {
            if (hasFile()) {
                getFile(dir).delete();
                if (DropBoxManagerService.mOppoDropBoxManagerService != null) {
                    DropBoxManagerService.mOppoDropBoxManagerService.deleteFile(hasFile(), dir, getFilename());
                }
            }
        }
    }
    ...
    private synchronized long createEntry(File temp, String tag, int flags) throws IOException {
        ...
        SortedSet<EntryFile> tail2 = this.mAllFiles.contents.tailSet(new EntryFile(10000 + t2));
    ...
    private void getLowPriorityResourceConfigs() {
        ...
        String[] lowPrioritytags = Resources.getSystem().getStringArray(17236022);
