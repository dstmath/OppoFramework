...
public class WindowToken extends WindowContainer<WindowState> {
    ...
    WindowToken(WindowManagerService service, IBinder _token, int type, boolean persistOnEmpty, DisplayContent dc, boolean ownerCanManageAppTokens, boolean roundedCornerOverlay) {
        ...
        this.mWindowComparator = new Comparator() {
            /* class com.android.server.wm.$$Lambda$WindowToken$tFLHn4S6WuSXW1gp1kvT_sp7WC0 */
    ...
    public void addWindow(WindowState win) {
        if (WindowManagerDebugConfig.DEBUG_FOCUS) {
            Slog.d("WindowManager", "addWindow: win=" + win + " Callers=" + Debug.getCallers(5));
        }
        if (!win.isChildWindow() && !this.mChildren.contains(win)) {
            if (WindowManagerDebugConfig.DEBUG_ADD_REMOVE) {
                Slog.v("WindowManager", "Adding " + win + " to " + this);
            }
    ...
    public String toString() {
        if (this.stringName == null) {
            this.stringName = "WindowToken{" + Integer.toHexString(System.identityHashCode(this)) + StringUtils.SPACE + this.token + '}';
        }
