...
public class OppoActivityStarter extends ActivityStarter {
    ...
    public int checkSpecialApp(int userId, String callingPackage, boolean componentSpecified, Intent intent, RootActivityContainer container) {
        ...
        if ("com.eg.android.AlipayGphone".equals(callingPackage) && componentSpecified && intent.getComponent().getClassName() != null && ("com.taobao.login4android.activity.AlipaySSOResultActivity".equals(intent.getComponent().getClassName()) || "com.taobao.taobao.apshare.ShareEntryActivity".equals(intent.getComponent().getClassName()))) {
            userId = getChildPackageUserIdIfExist(container.getDefaultDisplay(), "com.taobao.taobao", userId);
        }
        if (!PackageManagerService.PLATFORM_PACKAGE_NAME.equals(callingPackage) || !componentSpecified || intent.getComponent().getClassName() == null || !"com.facebook.katana.ProxyAuth".equals(intent.getComponent().getClassName())) {
            return userId;
        }
        return getChildPackageUserIdIfExist(container.getDefaultDisplay(), "com.instagram.android", userId);
    }
