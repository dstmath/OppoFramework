...
public class BoundsAnimationController {
    ...
    public final class BoundsAnimator extends ValueAnimator implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
        ...
        public void onAnimationUpdate(ValueAnimator animation) {
            ...
            if (this.mAnimationType != 1) {
                ...
                if (BoundsAnimationController.DEBUG) {
                    Slog.d("WindowManager", "animateUpdate: mTarget=" + this.mTarget + " mBounds=" + this.mTmpRect + " from=" + this.mFrom + " mTo=" + this.mTo + " value=" + value + " remains=" + remains);
                }
        ...
        public void onAnimationEnd(Animator animation) {
            ...
            if (!BoundsAnimationController.this.mAppTransition.isRunning() || BoundsAnimationController.this.mFinishAnimationAfterTransition) {
                ...
                synchronized (BoundsAnimationController.this.mRunningAnimations) {
    ...
    public void cancel(BoundsAnimationTarget target) {
        ...
        if (existing != null) {
            if (DEBUG) {
                Slog.d("WindowManager", "cancel: mTarget= " + target);
            }
    ...
    public BoundsAnimator animateBoundsImpl(BoundsAnimationTarget target, Rect from, Rect to, int animationDuration, int schedulePipModeChangedState, boolean moveFromFullscreen, boolean moveToFullscreen, @AnimationType int animationType) {
        ...
        if (DEBUG) {
            Slog.d("WindowManager", "animateBounds: target=" + target + " from=" + from + " to=" + to + " schedulePipModeChangedState=" + schedulePipModeChangedState3 + " replacing=" + replacing);
        }
        ...
        if (!replacing) {
            ...
            if (DEBUG) {
                Slog.d("WindowManager", "animateBounds: same destination and moveTo/From flags as existing=" + existing + ", ignoring...");
            }
            return existing;
        }
        if (animationType2 == 1) {
        ...
        animator.setFloatValues(OppoBrightUtils.MIN_LUX_LIMITI, 1.0f);
        if (animationType2 == 1) {
            j = 500;
        } else {
    ...
    public void onAllWindowsDrawn() {
        ...
        this.mHandler.post(new Runnable() {
            ...
            public final void run() {
                BoundsAnimationController.m35lambda$MoVv_WhxoMrTVoxz1qu2FMcYrM(BoundsAnimationController.this);
            }
        });
    }
