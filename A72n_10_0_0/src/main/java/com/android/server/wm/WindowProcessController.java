...
public class WindowProcessController extends ConfigurationContainer<ConfigurationContainer> implements ConfigurationContainerListener {
    ...
    public boolean areBackgroundActivityStartsAllowed() {
        ...
        if ((now - this.mLastActivityLaunchTime < 10000 || now - this.mLastActivityFinishTime < 10000) && (this.mLastActivityLaunchTime > this.mAtm.getLastStopAppSwitchesTime() || this.mLastActivityFinishTime > this.mAtm.getLastStopAppSwitchesTime())) {
            if (this.mAtm != null) {
                OppoFeatureCache.get(IColorAppStartupManager.DEFAULT).monitorActivityStartInfo("Q_CallerActivityStartOrFinishRecently");
            }
            return true;
        } else if (this.mInstrumentingWithBackgroundActivityStartPrivileges) {
            return true;
        } else {
            if (hasActivityInVisibleTask()) {
                if (this.mAtm != null) {
                    OppoFeatureCache.get(IColorAppStartupManager.DEFAULT).monitorActivityStartInfo("Q_CallerHasActivityInVisibleTask");
                }
                return true;
                ...
                if (this.mAtm != null) {
                    OppoFeatureCache.get(IColorAppStartupManager.DEFAULT).monitorActivityStartInfo("Q_CallerIsBoundByForegroundUid");
                }
                return true;
            }
        }
    ...
    public boolean updateTopResumingActivityInProcessIfNeeded(ActivityRecord activity) {
        ...
        if (display == topDisplay && (this.mPreQTopResumedActivity.getActivityStack().mTaskStack.compareTo((WindowContainer) activity.getActivityStack().mTaskStack) <= 0 || this.mPreQTopResumedActivity.getActivityStack().getWindowingMode() == 2)) {
            canUpdate = true;
        }
        if (canUpdate) {
    ...
    public ArraySet<TaskRecord> getReleaseSomeActivitiesTasks() {
        ...
        if (ActivityTaskManagerDebugConfig.DEBUG_RELEASE) {
            String str = TAG_RELEASE;
            Slog.d(str, "Trying to release some activities in " + this);
        }
        for (int i = 0; i < this.mActivities.size(); i++) {
            ...
            if (!r.finishing && !r.isState(ActivityStack.ActivityState.DESTROYING, ActivityStack.ActivityState.DESTROYED)) {
                if (!r.visible && r.stopped && r.haveState && !r.isState(ActivityStack.ActivityState.RESUMED, ActivityStack.ActivityState.PAUSING, ActivityStack.ActivityState.PAUSED, ActivityStack.ActivityState.STOPPING)) {
                    ...
                    if (task != null) {
                        if (ActivityTaskManagerDebugConfig.DEBUG_RELEASE) {
                            String str2 = TAG_RELEASE;
                            Slog.d(str2, "Collecting release task " + task + " from " + r);
                        }
                        if (firstTask == null) {
                            firstTask = task;
                    ...
                    String str3 = TAG_RELEASE;
                    Slog.d(str3, "Not releasing in-use activity: " + r);
                }
            } else if (!ActivityTaskManagerDebugConfig.DEBUG_RELEASE) {
                return null;
            } else {
                String str4 = TAG_RELEASE;
                Slog.d(str4, "Abort release; already destroying: " + r);
                return null;
            }
        }
        return tasks;
    ...
    public int computeOomAdjFromActivities(int minTaskLayer, ComputeOomAdjCallback callback) {
        synchronized (this.mAtm.mGlobalLockWithoutBoost) {
            ...
            while (true) {
                ...
                if (r.app != this) {
                    Log.e(TAG, "Found activity " + r + " in proc activity list using " + r.app + " instead of expected " + this);
            ...
            try {
                ...
                if (this.mAtm.mController != null) {
                    try {
                        ...
                        if (this.mAtm.mOppoActivityControlerScheduler != null) {
                            this.mAtm.mOppoActivityControlerScheduler.exitRunningScheduler();
                            this.mAtm.mOppoActivityControlerScheduler = null;
                        }
                    }
                    WindowManagerService.resetPriorityAfterLockedSection();
                }
    ...
    public boolean appNotResponding(String info, Runnable killAppCallback, Runnable serviceTimeoutCallback) {
        ...
        synchronized (this.mAtm.mGlobalLock) {
            try {
                WindowManagerService.boostPriorityForLockedSection();
                if (NoFocusWindow.HUNG_CONFIG_ENABLE.equals(SystemProperties.get("SPECIAL_OPPO_CONFIG", "0"))) {
                    return false;
                }
                if (this.mAtm.mController == null) {
                    WindowManagerService.resetPriorityAfterLockedSection();
