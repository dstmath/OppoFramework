...
public class BarController {
    ...
    private static final String SYSTEM_BAR_STATE = "systembar_state";
    ...
    private OppoWindowManagerInternal mOppoWindowManagerInternal;
    ...
    public boolean setBarShowingLw(boolean show) {
        ...
        if (!show || this.mTransientBarState != 3) {
            ...
            notifyWindowStateChange(show, change, state, stateChanged);
    ...
    private void setTransientBarState(int state) {
        ...
        if (this.mWin != null && state != (i = this.mTransientBarState)) {
            ...
            if (OppoWindowManagerDebugConfig.DEBUG_OPPO_SYSTEMBAR) {
                ...
                Slog.d(str, "mTransientBarState: " + transientBarStateToString(state));
            }
        }
    }
    ...
    private void notifyWindowStateChange(boolean show, boolean change, int state, boolean stateChanged) {
        int i = 2;
        if (OppoWindowManagerDebugConfig.DEBUG_OPPO_SYSTEMBAR) {
            String str = this.mTag;
            Slog.d(str, "setBarShowingLw: " + show + " mStatusBarManagerId:" + this.mStatusBarManagerId + " change:" + change + " state:" + StatusBarManager.windowStateToString(state) + " stateChanged:" + stateChanged + " Callers=" + Debug.getCallers(2));
        }
        if (change && this.mStatusBarManagerId == 1) {
            Bundle data = new Bundle();
            data.putInt("systembar_id", this.mStatusBarManagerId);
            if (show) {
                i = 0;
            }
            data.putInt("systembar_state", i);
