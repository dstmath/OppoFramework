...
public class KeyguardController {
    ...
    public void setKeyguardShown(boolean keyguardShowing, boolean aodShowing) {
        ...
        if (keyguardChanged || aodChanged) {
            if (ActivityTaskManagerDebugConfig.DEBUG_VISIBILITY) {
                Slog.d(TAG, "setKeyguardShown: keyguardShowing:" + keyguardShowing + ", aodShowing:" + aodShowing);
            }
    ...
    public void keyguardGoingAway(int flags) {
        if (this.mKeyguardShowing) {
            ...
            try {
                ...
                this.mRootActivityContainer.getDefaultDisplay().mDisplayContent.prepareAppTransition(flags == 1000 ? flags : 20, false, flags == 1000 ? 0 : convertTransitFlags(flags), flags == 1000);
                updateKeyguardSleepToken();
    ...
    public void dismissKeyguard(IBinder token, IKeyguardDismissCallback callback, CharSequence message) {
        ...
        if (!this.mService.mColorAtmsEx.execInterceptWindow(this.mService.mContext, activityRecord.packageName, false)) {
    ...
    public static class KeyguardDisplayState {
        ...
        public void visibilitiesUpdated(KeyguardController controller, ActivityDisplay display) {
            ...
            if (lastOccluded != this.mOccluded) {
                controller.handleOccludedChanged(i);
            }
