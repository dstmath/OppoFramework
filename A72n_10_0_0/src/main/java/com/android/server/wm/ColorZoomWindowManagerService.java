...
public class ColorZoomWindowManagerService implements IColorZoomWindowManager {
            ...
            try {
                ...
                if (zoomStack != null) {
                    if (baseZoomStack != null) {
                        ...
                        if (nextStack != null) {
                            ActivityRecord r = nextStack.topRunningActivityLocked();
                            if (r != null && r.moveFocusableActivityToTop("hideZoomWindow")) {
                                this.mAtms.mRootActivityContainer.resumeFocusedStacksTopActivities();
                            }
                            zoomStack.moveToBack("push zoomStack to back", (TaskRecord) null);
                        }
                        Binder.restoreCallingIdentity(ident);
                    }
                }
    ...
    public void notifyWindowDied(ActivityStack stack) {
        ...
        if (baseStack != null && stack != null) {
            if ((stack.getWindowingMode() == ColorZoomWindowManager.WINDOWING_MODE_ZOOM || baseStack.mZoomBubble) && this.mAtms.mRootActivityContainer.getDefaultDisplay().getTopStackInWindowingMode(ColorZoomWindowManager.WINDOWING_MODE_ZOOM) == null) {
                updateZoomUIDsOnDisplay(stack, false);
                clearZoomWindow();
    ...
    public void amendWindowTapExcludeRegion(DisplayContent dc, Region region) {
        if (!(dc == null || region == null)) {
            ...
            int deltawidth = WindowManagerService.dipToPixel(12, dm);
    ...
    public void addTapExcluedWindow(WindowState win) {
        ...
        if ((win.mAttrs.flags & 16) == 0 || win.mAttrs.type == 2315) {
            if (DEBUG_INPUT) {
        ...
        Slog.w(TAG, "This window can not be added to TapExcludeWindow: " + win);
    }
