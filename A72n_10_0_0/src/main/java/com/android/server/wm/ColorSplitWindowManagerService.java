...
public class ColorSplitWindowManagerService implements IColorSplitWindowManager {
    ...
    private AppVisibilityObserver mAppVisibilityObserver = new AppVisibilityObserver() {
                ...
                public boolean handleMessage(Message message) {
                    try {
                        ...
                        if (i == 1) {
                            ...
                            ColorSplitWindowManagerService.this.startSplitScreen();
                        } else if (i == 3) {
    ...
    public int getSplitScreenState(Intent intent) {
        ...
        if (isSnapshotEditing()) {
            ...
            return 1003;
        } else if (this.mAtms.isInLockTaskMode()) {
            Slog.d(TAG, "getSplitScreenState: lock stack mode");
            return 1009;
        } else {
    ...
    private boolean isInForbidActivityListInner(TaskRecord task) {
        ...
        if (!isSystemApp(info) || topRunningActivity.isActivityTypeHome() || supportsSplitScreenWindowingMode(task)) {
            return false;
        }
        return true;
    }
    ...
    private boolean supportsSplitScreenWindowingMode(TaskRecord record) {
        ...
        if (topRunningActivity != null) {
            return taskSupportSplitScreen && topRunningActivity.supportsSplitScreenWindowingMode();
        }
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void setSplitLaunchState(SplitLaunchState state) {
        synchronized (this.mAppVisibilityObserver) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private boolean startSplitScreen() {
