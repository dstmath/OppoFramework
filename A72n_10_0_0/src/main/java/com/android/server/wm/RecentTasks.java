...
public class RecentTasks {
    ...
    RecentTasks(ActivityTaskManagerService service, ActivityStackSupervisor stackSupervisor) {
        ...
        this.mGlobalMaxNumTasks = ActivityTaskManager.getMaxRecentTasksStatic();
        this.mHasVisibleRecentTasks = res.getBoolean(17891465);
    ...
    public void loadRecentsComponent(Resources res) {
        ComponentName cn;
        String rawRecentsComponent = res.getString(17039767);
    ...
    private ArrayList<ActivityManager.RecentTaskInfo> getRecentTasksImpl(int maxNum, int flags, boolean getTasksAllowed, boolean getDetailedTasks, int userId, int callingUid) {
        ...
        OppoFeatureCache.get(IColorMultiAppManager.DEFAULT).addRecentTaskUsers(userId, includedUsers);
        if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
            String str = TAG_RECENTS;
            Slog.d(str, "includedUsers: " + includedUsers + " withExcluded " + withExcluded + " maxNum " + maxNum + " userId " + userId);
        }
        ArrayList<ActivityManager.RecentTaskInfo> res = new ArrayList<>();
        int size = this.mTasks.size();
        int numVisibleTasks = 0;
        if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
            String str2 = TAG_RECENTS;
            Slog.d(str2, "getTasksAllowed: " + getTasksAllowed + " getDetailedTasks " + getDetailedTasks + " size " + size);
        }
        for (int i = 0; i < size; i++) {
            TaskRecord tr = this.mTasks.get(i);
            if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                String str3 = TAG_RECENTS;
                Slog.d(str3, "getRecentTasks tr: " + tr);
            }
                ...
                if (isInVisibleRange(tr, i, numVisibleTasks, withExcluded)) {
                    if (res.size() < maxNum) {
                        if (!includedUsers.contains(Integer.valueOf(tr.userId))) {
                            if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                                String str4 = TAG_RECENTS;
                                Slog.d(str4, "Skipping, not user: " + tr);
                            }
                            ...
                            if (!getTasksAllowed) {
                                if (!tr.isActivityTypeHome()) {
                                    if (tr.effectiveUid != callingUid) {
                                        if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                                            String str5 = TAG_RECENTS;
                                            Slog.d(str5, "Skipping, not allowed: " + tr);
                                        }
                                    }
                                }
                            }
                            if (!tr.autoRemoveRecents || tr.getTopActivity() != null) {
                                if ((flags & 2) == 0 || tr.isAvailable) {
                                    if (!tr.mUserSetupComplete) {
                                        if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                                            String str6 = TAG_RECENTS;
                                            Slog.d(str6, "Skipping, user setup not complete: " + tr);
                                        }
                                    } else if (!OppoFeatureCache.get(IColorMultiAppManager.DEFAULT).shouldFilterTask(tr.mUserSetupComplete, tr.userId)) {
                                        if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                                            String str7 = TAG_RECENTS;
                                            Slog.d(str7, "after skip id: " + tr.taskId + " tr " + tr);
                                        }
                                        if (!OppoFeatureCache.get(IColorAppListInterceptManager.DEFAULT).shouldFilterTask(tr.getBaseIntent())) {
                                    ...
                                    String str8 = TAG_RECENTS;
                                    Slog.d(str8, "Skipping, unavail real act: " + tr);
                                }
                            } else if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                                String str9 = TAG_RECENTS;
                                Slog.d(str9, "Skipping, auto-remove without activity: " + tr);
                            }
                        } else if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                            String str10 = TAG_RECENTS;
                            Slog.d(str10, "Skipping, activity suspended: " + tr);
                        }
                    }
                }
            }
    ...
    private void trimInactiveRecentTasks() {
        if (!this.mFreezeTaskListReordering) {
            ...
            while (recentsCount > this.mGlobalMaxNumTasks) {
                ...
                recentsCount--;
                if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS_TRIM_TASKS) {
                    Slog.d(TAG, "Trimming over max-recents task=" + tr + " max=" + this.mGlobalMaxNumTasks);
                }
            }
    ...
    public boolean isVisibleRecentTask(TaskRecord task) {
        ...
        if (windowingMode == 3) {
            ...
            if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS_TRIM_TASKS) {
                ...
                sb.append("\ttop=");
                sb.append(stack != null ? task.getStack().topTask() : "null stack");
    ...
    private boolean isInVisibleRange(TaskRecord task, int taskIndex, int numVisibleTasks, boolean skipExcludedCheck) {
        ...
        return i2 >= 0 ? numVisibleTasks <= i2 : (this.mActiveTasksSessionDurationMs > 0 && task.getInactiveDuration() <= this.mActiveTasksSessionDurationMs) || OppoFeatureCache.get(IColorAthenaManager.DEFAULT).isRecentLockTask(task);
    }
    ...
    public boolean isTrimmable(TaskRecord task) {
        ...
        ActivityRecord top = task.topRunningActivityLocked();
        if (top != null && ActivityThread.inCptWhiteList((int) CompatibilityHelper.UNTRIMMABLE_COMPONENT_LIST, top.shortComponentName)) {
            return false;
        }
    ...
    private boolean moveAffiliatedTasksToFront(TaskRecord task, int taskIndex) {
        ...
        while (true) {
            ...
            if (ActivityTaskManagerDebugConfig.DEBUG_RECENTS) {
                Slog.d(TAG_RECENTS, "addRecent: looking at next chain @" + endIndex + StringUtils.SPACE + cur);
            }
            if (cur != top) {
    ...
    private boolean hasCompatibleActivityTypeAndWindowingMode(TaskRecord t1, TaskRecord t2) {
        ...
        return (activityType == otherActivityType || isUndefinedType || (otherActivityType == 0)) && (windowingMode == otherWindowingMode || isUndefinedMode || (otherWindowingMode == 0) || windowingMode == 5 || windowingMode == WindowConfiguration.WINDOWING_MODE_ZOOM || otherWindowingMode == WindowConfiguration.WINDOWING_MODE_ZOOM || windowingMode == 4 || otherWindowingMode == 4 || otherWindowingMode == 3);
    }
}
