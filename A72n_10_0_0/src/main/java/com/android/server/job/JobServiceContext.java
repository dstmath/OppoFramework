...
public final class JobServiceContext extends OppoBaseJobServiceContext implements ServiceConnection {
    ...
    JobServiceContext(Context context, Object lock, IBatteryStats batteryStats, JobPackageTracker tracker, JobCompletedListener completedListener, Looper looper) {
        super(context, lock, batteryStats, tracker);
    ...
    private void handleCancelLocked(String reason) {
        if (JobSchedulerService.DEBUG) {
            Slog.d(TAG, "Handling cancel for: " + this.mRunningJob.getJobId() + StringUtils.SPACE + VERB_STRINGS[this.mVerb]);
        }
        int i = this.mVerb;
        if (i == 0 || i == 1) {
    ...
    private void scheduleOpTimeOutLocked() {
        ...
        long ajustedTimeoutMillis = OppoFeatureCache.get(IColorJobScheduleManager.DEFAULT).getJobScheduleTimeoutIfNeed(this.mRunningJob, this.mVerb);
        if (ajustedTimeoutMillis != 0) {
            Slog.d(TAG, "scheduleOpTimeOut set job timeout ajustedTimeoutMillis = " + ajustedTimeoutMillis);
            timeoutMillis = ajustedTimeoutMillis;
        }
        if (DEBUG) {
    ...
    /* access modifiers changed from: package-private */
    @Override // com.android.server.job.OppoBaseJobServiceContext
