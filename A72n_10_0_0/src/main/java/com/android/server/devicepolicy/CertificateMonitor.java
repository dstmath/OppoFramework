    ...
    private Notification buildNotification(UserHandle userHandle, int pendingCertificateCount) {
        ...
        try {
            ...
            if (this.mService.getProfileOwner(userHandle.getIdentifier()) != null) {
                String contentText2 = resources.getString(17041060, this.mService.getProfileOwnerName(userHandle.getIdentifier()));
                parentUserId = this.mService.getProfileParentId(userHandle.getIdentifier());
                contentText = contentText2;
                smallIconId = 17303550;
                ...
                smallIconId = 17303550;
                contentText = resources.getString(17041060, this.mService.getDeviceOwnerName());
            } else {
                parentUserId = parentUserId2;
                contentText = resources.getString(17041059);
            ...
            return new Notification.Builder(userContext, SystemNotificationChannels.SECURITY).setSmallIcon(smallIconId).setContentTitle(resources.getQuantityText(18153497, pendingCertificateCount)).setContentText(contentText).setContentIntent(this.mInjector.pendingIntentGetActivityAsUser(userContext, 0, dialogIntent, 134217728, null, UserHandle.of(parentUserId))).setShowWhen(false).setColor(17170460).build();
