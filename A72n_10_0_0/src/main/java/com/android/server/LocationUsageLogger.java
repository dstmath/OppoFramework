...
public class LocationUsageLogger {
    ...
    private static int bucketizeSmallestDisplacementToStatsdEnum(float smallestDisplacement) {
        if (smallestDisplacement == OppoBrightUtils.MIN_LUX_LIMITI) {
            return 1;
        }
        if (smallestDisplacement <= OppoBrightUtils.MIN_LUX_LIMITI || smallestDisplacement > 100.0f) {
            return 3;
        }
        return 2;
    }
    ...
    private static int bucketizeRadiusToStatsdEnum(float radius) {
        if (radius < OppoBrightUtils.MIN_LUX_LIMITI) {
            return 7;
        }
        if (radius < 100.0f) {
            return 1;
