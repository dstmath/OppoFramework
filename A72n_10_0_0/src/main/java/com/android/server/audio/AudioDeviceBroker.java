...
public final class AudioDeviceBroker {
    ...
    public boolean setSpeakerphoneOn(boolean on, String eventSource) {
        ...
        synchronized (this.mDeviceStateLock) {
            ...
            String value = OppoAtlasManager.getInstance((Context) null).getParameters("check_listinfo_bypid=donot-set-ForcedUse=" + Binder.getCallingPid());
            if (value == null || !value.equals(TemperatureProvider.SWITCH_ON) || this.mForcedUseForCommExt != 3) {
                this.mForcedUseForCommExt = this.mForcedUseForComm;
            } else {
                Log.i(TAG, "In setSpeakerphoneOn(), do not set mForcedUseForCommExt to NONE");
            }
    ...
    public void postBluetoothA2dpDeviceConnectionStateSuppressNoisyIntent(BluetoothDevice device, int state, int profile, boolean suppressNoisyIntent, int a2dpVolume) {
        sendLMsgNoDelay(27, 2, new BtDeviceConnectionInfo(device, state, profile, suppressNoisyIntent, a2dpVolume));
    }
    ...
    public void postBluetoothHearingAidDeviceConnectionState(BluetoothDevice device, int state, boolean suppressNoisyIntent, int musicDevice, String eventSource) {
        sendLMsgNoDelay(28, 2, new HearingAidDeviceConnectionInfo(device, state, suppressNoisyIntent, musicDevice, eventSource));
    }
    ...
    public void setBluetoothScoOn(boolean on, String eventSource) {
        if (AudioService.DEBUG_DEVICES) {
            Log.i(TAG, "setBluetoothScoOn: " + on + StringUtils.SPACE + eventSource);
        }
        synchronized (this.mDeviceStateLock) {
            if (on) {
    ...
    public boolean isBluetoothScoAvailableOffCall() {
        ...
        synchronized (this.mDeviceStateLock) {
            isBluetoothScoAvailableOffCall = this.mBtHelper.isBluetoothScoAvailableOffCall();
        }
    ...
    public void handleSetA2dpSinkConnectionState(int state, BtHelper.BluetoothA2dpDeviceInfo btDeviceInfo) {
        ...
        if (AudioService.DEBUG_DEVICES) {
            Log.d(TAG, "handleSetA2dpSinkConnectionState btDevice= " + btDeviceInfo + " state= " + state + " is dock: " + btDeviceInfo.getBtDevice().isBluetoothDock());
        }
        sendILMsg(6, 2, state, btDeviceInfo, delay);
    }
    ...
    private void onSetForceUse(int useCase, int config, String eventSource) {
        ...
        if (AudioService.DEBUG_DEVICES) {
            Log.i(TAG, "setForceUse(" + AudioSystem.forceUseUsageToString(useCase) + ", " + AudioSystem.forceUseConfigToString(config) + ") due to " + eventSource);
        }
    ...
    public class BrokerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 27:
                    AudioDeviceBroker.this.mAudioService.waitReadAudioSetting();
    ...
    public static boolean isMessageHandledUnderWakelock(int msgId) {
        if (msgId == 2 || msgId == 6 || msgId == 7 || msgId == 8 || msgId == 10 || msgId == 11 || msgId == 17 || msgId == 18 || msgId == 27 || msgId == 28) {
            return true;
        }
        return false;
    }
    ...
    private void sendIILMsg(int msg, int existingMsgPolicy, int arg1, int arg2, Object obj, int delay) {
        ...
        if (isMessageHandledUnderWakelock(msg)) {
            ...
            try {
                this.mBrokerEventWakeLock.acquire(5000);
            } catch (Exception e) {
        ...
        synchronized (sLastDeviceConnectionMsgTimeLock) {
            ...
            if (msg == 2 || msg == 18 || msg == 27 || msg == 6 || msg == 7 || msg == 8 || msg == 10 || msg == 11) {
    ...
    public String getScoClientInfo() {
        return this.mBtHelper.getScoClientInfo();
    }
    ...
    public int getA2dpVolume(boolean cmpToSafeVolume, int a2dpVolume) {
        return this.mAudioService.getA2dpVolume(cmpToSafeVolume, a2dpVolume);
    }

    public boolean isScoDeviceActive() {
        return this.mBtHelper.isScoDeviceActive();
    }
}
