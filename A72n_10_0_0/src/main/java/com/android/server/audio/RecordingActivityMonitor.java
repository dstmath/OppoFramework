...
public final class RecordingActivityMonitor implements AudioSystem.AudioRecordingCallback {
    ...
    private List<AudioRecordingConfiguration> updateSnapshot(int event, int riid, AudioRecordingConfiguration config) {
        ...
        synchronized (this.mRecordStates) {
            ...
            if (state.mConfig != null && "com.tencent.mm".equals(state.mConfig.getClientPackageName())) {
                WindowManagerInternal wmi = (WindowManagerInternal) LocalServices.getService(WindowManagerInternal.class);
                if (state.mIsActive) {
                    wmi.addNonHighRefreshRatePackage("com.tencent.mm");
                } else {
                    wmi.removeNonHighRefreshRatePackage("com.tencent.mm");
                }
            }
            return configs;
        }
