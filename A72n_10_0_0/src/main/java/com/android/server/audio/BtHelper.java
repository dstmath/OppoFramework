...
public class BtHelper {
    ...
    public boolean isBluetoothScoAvailableOffCall() {
        ...
        if (bluetoothHeadset != null) {
            return bluetoothHeadset.isBluetoothScoAvailableOffCall();
        }
        return true;
    }
    ...
    public synchronized void receiveBtEvent(Intent intent) {
        ...
        if (action.equals("android.bluetooth.headset.profile.action.ACTIVE_DEVICE_CHANGED")) {
            ...
            if (AudioService.DEBUG_DEVICES) {
                Log.d(TAG, "receiveBtEvent() BTactiveDeviceChangedbtDevice=" + btDevice);
            }
            ...
            if (AudioService.DEBUG_DEVICES) {
                Log.d(TAG, "receiveBtEvent() AudioStateChangedbtState=" + btState);
            }
            boolean z = true;
    ...
    public synchronized void resetBluetoothSco() {
        ...
        BluetoothAdapter adapter = BluetoothAdapter.getDefaultAdapter();
        if (AudioService.DEBUG_DEVICES && adapter != null) {
            Log.d(TAG, "A2DP Service State =" + BluetoothAdapter.nameForState(adapter.getState()));
        }
        if (adapter == null || adapter.getState() != 12) {
            if (AudioService.DEBUG_DEVICES) {
                Log.d(TAG, "resetBluetoothSco A2dpSuspended=true");
            }
            AudioSystem.setParameters("A2dpSuspended=true");
        } else {
            if (AudioService.DEBUG_DEVICES) {
                Log.d(TAG, "resetBluetoothSco A2dpSuspended=false");
            }
    ...
    public class ScoClient implements IBinder.DeathRecipient {
        ...
        public void incCount(int scoAudioMode) {
            ...
            if (i == 1) {
                Log.i(BtHelper.TAG, "mStartcount is 1, calling mDeviceBroker.setBluetoothScoOn(true)in system context");
                BtHelper.this.mDeviceBroker.setBluetoothScoOn(true, "BtHelper.receiveBtEvent");
            } else if (i == 0) {
                try {
    ...
    public String getScoClientInfo() {
        String scoClientInfo = "";
        synchronized (this.mScoClients) {
            int size = this.mScoClients.size();
            Log.d(TAG, "getScoClientInfo size:" + size);
            for (int i = 0; i < size; i++) {
                ScoClient client = this.mScoClients.get(i);
                scoClientInfo = scoClientInfo + client.getPid() + "," + client.getCount();
                if (i < size - 1) {
                    scoClientInfo = scoClientInfo + ",";
                }
                Log.d(TAG, "scoClientInfo :" + scoClientInfo);
            }
        }
        return scoClientInfo;
    }

    public boolean isScoDeviceActive() {
        return this.mBluetoothHeadsetDevice != null;
    }
}
