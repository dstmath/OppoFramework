...
public class AudioService extends IAudioService.Stub implements AccessibilityManager.TouchExplorationStateChangeListener, AccessibilityManager.AccessibilityServicesStateChangeListener {
    ...
    private static final int MSG_MUTE_PHONE = 62;
    ...
    private static final String PHONE_MUTE_STATE_FOR_CUSTOM = "phone_mute_state_for_custom";
    ...
    private boolean mIsPhoneMutedAS = false;
    ...
    public AudioService(Context context) {
        ...
        String mutePhoneSettingStr = Settings.Global.getString(this.mContentResolver, PHONE_MUTE_STATE_FOR_CUSTOM);
        forceMuteOrUnmutePhone(!TextUtils.isEmpty(mutePhoneSettingStr) && mutePhoneSettingStr.equals(TemperatureProvider.SWITCH_ON));
        ...
        for (int i = 0; i < preScale.length; i++) {
            if (OppoBrightUtils.MIN_LUX_LIMITI <= preScale[i] && preScale[i] <= 1.0f) {
    ...
    public void onAudioServerDied() {
        ...
        setRingerModeInt(getRingerModeInternal(), false);
        Settings.Global.putString(this.mContentResolver, PHONE_MUTE_STATE_FOR_CUSTOM, this.mIsPhoneMutedAS ? TemperatureProvider.SWITCH_ON : TemperatureProvider.SWITCH_OFF);
        if (this.mMonitorRotation) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void updateInputDevice(ContentResolver cr) {
    ...
    public void oppoSetParameters(String keyValuePairs) {
        ...
        if (keyValuePairs.startsWith("restoreVolumeBeforeSafeMediaVolume")) {
            ...
            mDialogOn = false;
        } else if (keyValuePairs.startsWith("OPLUS_AUDIO_SET_MUTE_PHONE") && (s = keyValuePairs.split(":")) != null && s.length == 2) {
            forceMuteOrUnmutePhone(s[1].equals(TemperatureProvider.SWITCH_ON));
        }
    }
    ...
    public String oppoGetParameters(String keyValuePairs) {
        Log.d(TAG, "oppoGetParameters() keyValuePairs:" + keyValuePairs);
        if (!keyValuePairs.startsWith("OPLUS_AUDIO_GET_MUTE_PHONE")) {
            return null;
        }
        return this.mIsPhoneMutedAS ? TemperatureProvider.SWITCH_ON : TemperatureProvider.SWITCH_OFF;
    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:224:0x03df, code lost:
        r0 = th;
     */
    public void adjustStreamVolume(int streamType, int direction, int flags, String callingPackage, String caller, int uid) {
        ...
        if (!this.mUseFixedVolume) {
            ...
            if (SystemProperties.get("persist.sys.custom.enable", TemperatureProvider.SWITCH_OFF).equals(TemperatureProvider.SWITCH_ON) && SystemProperties.get("persist.sys.volumechange.forbid", "0").equals(NoFocusWindow.HUNG_CONFIG_ENABLE)) {
                Log.e(TAG, "Do not adjust volume by private setStreamVolume()");
                return;
            } else if (!this.mIsPhoneMutedAS || !isStreamAffectedByPhoneMute(streamType)) {
                ...
                if (!isMuteAdjust || (!(streamType == 0 || streamType == 6) || this.mContext.checkCallingOrSelfPermission("android.permission.MODIFY_PHONE_STATE") == 0)) {
                    ...
                    VolumeStreamState streamState3 = this.mStreamStates[streamTypeAlias];
                    ...
                    if ((device2 & 896) != 0 || (flags & 64) == 0) {
                        ...
                        if (this.mAppOps.noteOp(STREAM_VOLUME_OPS[streamTypeAlias], uid2, callingPackage) == 0) {
                            ...
                            int flags4 = flags & -33;
                            if (streamTypeAlias != 3 || (this.mFixedVolumeDevices & device2) == 0) {
                                step = rescaleIndex(10, streamType, streamTypeAlias);
                                aliasIndex = aliasIndex2;
                            } else {
                                flags4 |= 32;
                                if (this.mSafeMediaVolumeState != 3 || (this.mSafeMediaVolumeDevices & device2) == 0) {
                            ...
                            if ((flags4 & 2) != 0 || streamTypeAlias == getUiSoundsStreamType()) {
                                if (getRingerModeInternal() == 1) {
                                    flags4 &= -17;
                                }
                                device = device2;
                                streamState = streamState3;
                                int result = OppoCheckForRingerModeChange(aliasIndex, direction, step, streamState3.mIsMuted, callingPackage, flags4);
                            ...
                            if (!adjustVolume || direction == 0) {
                                ...
                                if (isMuteAdjust) {
                                    ...
                                    int tempflags = isNeedShowUiWarnings(flags2, callingPackage);
                                    if (!mDialogOn) {
                                    ...
                                    if (!streamState.mIsMuted) {
                                        ...
                                        oldIndex2 = oldIndex3;
                                        flags3 = flags2;
                                        sendMsg(this.mAudioHandler, 18, 2, streamTypeAlias, flags2, null, UNMUTE_STREAM_DELAY);
                                    } else {
                                        streamState2 = streamState;
                                        oldIndex2 = oldIndex3;
                                        flags3 = flags2;
                                ...
                                if (streamTypeAlias == i && (device & 896) != 0 && (flags3 & 64) == 0) {
                                    if (DEBUG_VOL) {
                            ...
                            sendVolumeUpdate(streamType, oldIndex, this.mStreamStates[streamType].getIndex(device), flags3, device);
                            return;
                        }
                        return;
                    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:135:0x028e, code lost:
        if (r20.mHdmiManager == null) goto L_0x02c4;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:138:0x0292, code lost:
        if (r20.mHdmiAudioSystemClient == null) goto L_0x02c4;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:140:0x0296, code lost:
        if (r20.mHdmiSystemAudioSupported == false) goto L_0x02c4;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:142:0x0299, code lost:
        if (r11 != 3) goto L_0x02c4;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:143:0x029b, code lost:
        if (r10 == r6) goto L_0x02c4;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:144:0x029d, code lost:
        r2 = android.os.Binder.clearCallingIdentity();
     */
    /* JADX WARNING: Code restructure failed: missing block: B:147:?, code lost:
        r20.mHdmiAudioSystemClient.sendReportAudioStatusCecCommand(false, getStreamVolume(3), getStreamMaxVolume(3), isStreamMute(3));
        android.os.Binder.restoreCallingIdentity(r2);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:148:0x02ba, code lost:
        r0 = th;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:150:0x02be, code lost:
        r0 = th;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:154:?, code lost:
        monitor-exit(r1);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:156:0x02c8, code lost:
        if (r21 != 3) goto L_0x02d6;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:158:0x02d0, code lost:
        if (r24.equals("com.tencent.mm") == false) goto L_0x02d6;
     */
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:160:0x02d6, code lost:
        sendVolumeUpdate(r21, r10, r6, r16, r0);
     */
    /* JADX WARNING: Code restructure failed: missing block: B:161:0x02e4, code lost:
        return;
     */
    /* JADX WARNING: Code restructure failed: missing block: B:162:0x02e5, code lost:
        r0 = th;
     */
                    ...
                    if (this.mAppOps.noteOpNoThrow(STREAM_VOLUME_OPS[streamTypeAlias], uid2, callingPackage) == 0) {
                        if (isAndroidNPlus(callingPackage) && wouldToggleZenMode(getNewRingerMode(streamTypeAlias, index2, flags)) && !this.mNm.isNotificationPolicyAccessGrantedForPackage(callingPackage)) {
                            ...
                            synchronized (this.mSafeMediaVolumeStateLock) {
                                try {
                                    this.mPendingVolumeCommand = null;
                                    int oldIndex2 = streamState.getIndex(device);
                                    ...
                                    if (streamTypeAlias == 3) {
                                        setSystemAudioVolume(oldIndex2, index5, getStreamMaxVolume(streamType), flags);
                                    }
                                    int flags3 = flags & -33;
                                    if (streamTypeAlias == 3) {
                                        try {
    ...
    private void setMicrophoneMuteNoCallerCheck(boolean on, int userId) {
        ...
        if (SystemProperties.get("persist.sys.custom.enable", TemperatureProvider.SWITCH_OFF).equals(TemperatureProvider.SWITCH_ON) && SystemProperties.get("persist.sys.mic.forbid", "0").equals(NoFocusWindow.HUNG_CONFIG_ENABLE)) {
            Log.e(TAG, "Do not change microphone state by function");
        } else if (getCurrentUserId() == userId) {
    ...
    private void setRingerMode(int ringerMode, String caller, boolean external) {
        ...
        if (!this.mIsSingleVolume) {
            ...
            } else if (this.mIsPhoneMutedAS) {
                Log.e(TAG, "the phone is muted by custom.img, can't setRingerMode");
            } else {
    ...
    private void muteRingerModeStreams() {
        ...
        while (streamType >= 0) {
            ...
            boolean muteAllowedBySco = (!shouldRingSco || streamType != 2) ? z : i2;
            if (shouldZenMuteStream(streamType) || (ringerModeMute && isStreamAffectedByRingerMode(streamType) && muteAllowedBySco != 0)) {
                int i3 = z ? 1 : 0;
                boolean z2 = z ? 1 : 0;
                boolean z3 = z ? 1 : 0;
                boolean z4 = z ? 1 : 0;
            ...
            if (isMuted == i) {
                ...
                int i5 = this.mRingerAndZenModeMutedStreams;
                int i6 = z ? 1 : 0;
                int i7 = z ? 1 : 0;
                int i8 = z ? 1 : 0;
                int i9 = z ? 1 : 0;
                this.mRingerAndZenModeMutedStreams = i5 | (i6 << streamType);
            }
            streamType--;
            i2 = numStreamTypes2;
    ...
    public void startBluetoothSco(IBinder cb, int targetSdkVersion) {
        ...
        String value = OppoAtlasManager.getInstance((Context) null).getParameters("get_listinfo_bypid=start-sco-delay=" + Binder.getCallingPid());
        Log.d(TAG, "startBluetoothSco() start-sco-delay value=" + value);
        if (value != null && !this.mDeviceBroker.isScoDeviceActive()) {
            try {
                int tryCount = Integer.parseInt(value);
                int i = 0;
                while (true) {
                    if (i >= tryCount) {
                        break;
                    } else if (this.mDeviceBroker.isScoDeviceActive()) {
                        Log.d(TAG, "startBluetoothSco() delay=" + i);
                        break;
                    } else {
                        Thread.sleep(20);
                        i++;
                ...
                Log.e(TAG, "startBluetoothSco() e=" + e);
            }
        }
    ...
    public class VolumeStreamState {
        ...
        /* JADX WARNING: Code restructure failed: missing block: B:32:0x0050, code lost:
            r8 = android.provider.Settings.System.getIntForUser(r11.this$0.mContentResolver, getSettingNameForDevice(r5), r7, -2);
         */
        ...
        private void setStreamVolumeIndex(int index, int device) {
            if (this.mStreamType == 6 && index == 0 && !this.mIsMuted && !this.mIsPhoneMuted) {
                index = 1;
            }
        ...
        public void muteForPhone(boolean state) {
            synchronized (VolumeStreamState.class) {
            ...
            if (i != 0) {
                ...
                if (i == 1) {
                    ...
                    if (i == 7) {
                        ...
                        Settings.Global.putString(AudioService.this.mContentResolver, AudioService.PHONE_MUTE_STATE_FOR_CUSTOM, AudioService.this.mIsPhoneMutedAS ? TemperatureProvider.SWITCH_ON : TemperatureProvider.SWITCH_OFF);
                    } else if (i != 100) {
                        switch (i) {
                            case 10:
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        private void updateEncodedSurroundOutput() {
            ...
            if (AudioService.this.mEncodedSurroundMode != newSurroundMode) {
                AudioService.this.sendEncodedSurroundMode((AudioService) newSurroundMode, (int) "SettingsObserver");
    ...
    public boolean hasMediaDynamicPolicy() {
        synchronized (this.mAudioPolicies) {
            ...
            for (AudioPolicyProxy app : this.mAudioPolicies.values()) {
                if (app.hasMixAffectingUsage(1) && !checkIsOppoRecord(app.mPid)) {
                    return true;
                }
            }
            return false;
            ...
            try {
                if (this.mAudioPolicies.containsKey(pcb.asBinder())) {
                    ...
                    try {
                        ...
                        Log.d(TAG, "registerAudioPolicy getCallingPid: " + callingPid);
                        hashMap = hashMap2;
                    ...
                    try {
                        AudioPolicyProxy app = new AudioPolicyProxy(policyConfig, pcb, hasFocusListener, isFocusPolicy, isTestFocusPolicy, isVolumeController, projection, callingPid);
    ...
    public class AudioPolicyProxy extends AudioPolicyConfig implements IBinder.DeathRecipient {
        ...
        AudioPolicyProxy(AudioPolicyConfig config, IAudioPolicyCallback token, boolean hasFocusListener, boolean isFocusPolicy, boolean isTestFocusPolicy, boolean isVolumeController, IMediaProjection projection, int callingPid) {
            super(config);
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void onOppoRestoreVolumeBeforeSafeMediaVolume() {
        int i;
        int device = getDeviceForStream(3);
    ...
    public String getParameters(String keyValuePairs) {
        return oppoGetParameters(keyValuePairs);
    }
    ...
    public boolean checkIsOppoRecord(int pid) {
        String value = OppoAtlasManager.getInstance((Context) null).getParameters("check_listinfo_bypid=check-audiopolicy-proxy=" + pid);
        Log.d(TAG, "checkIsOppoRecord, pid: " + pid + "return value: " + value);
    ...
    private void forceMuteOrUnmutePhone(boolean mute) {
        Log.d(TAG, "forceMuteOrUnmutePhone: " + mute + "mIsPhoneMutedAS : " + this.mIsPhoneMutedAS);
        if (mute != this.mIsPhoneMutedAS) {
            int index = 0;
            while (true) {
                int[] iArr = AFFECTED_BY_PHONE_MUTE_STREAMS;
                if (index < iArr.length) {
                    this.mStreamStates[iArr[index]].muteForPhone(mute);
                    index++;
                } else {
                    this.mIsPhoneMutedAS = mute;
                    sendMsg(this.mAudioHandler, 62, 2, 0, 0, null, 0);
                    return;
                }
            }
        }
    ...
    private boolean isStreamAffectedByPhoneMute(int streamType) {
        int index = 0;
        while (true) {
            int[] iArr = AFFECTED_BY_PHONE_MUTE_STREAMS;
            if (index >= iArr.length) {
