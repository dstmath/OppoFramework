...
public class CameraDelegate {
    private static final int ADJUSTMENT_FRONT_CAMERA = 6536;
    ...
    public int adjustCameraBright(int screenAutoBrightness) {
        ...
        if (mCameraBacklight) {
            int screenAutoBrightness2 = adjustCameraBrightness(screenAutoBrightness);
            Slog.d("ColorAutomaticBrightnessController", "updateBrightness from CAMERA brightness=" + screenAutoBrightness2);
    ...
    public boolean isSpecialAdj(float value) {
        ...
        if (value == (((float) mMaxBrightness) * 500.0f) / 255.0f || value == (((float) mMaxBrightness) * 300.0f) / 255.0f || value == (((float) mMaxBrightness) * 301.0f) / 255.0f || value == 16384.0f || value == 32768.0f || value == 16385.0f || value == 32769.0f || value == 16386.0f || Math.round(value) == 1930286 || Math.round(value) == 1930287 || value == 32770.0f || ((int) value) == ADJUSTMENT_FRONT_CAMERA) {
            specialAdj = true;
        }
    ...
    private int adjustCameraBrightness(int brightness) {
        if (1 != DevicePropertyHelper.sBrightnessBitsConfig) {
            int i = mNormalMaxBrightness;
            if (brightness > i) {
                return brightness;
            }
            float uniform_brightness = ((float) brightness) / ((float) i);
            float adjGamma = 0.6f;
            if (uniform_brightness > 0.196f) {
                adjGamma = 0.4f;
            }
            int adjust_brightness = Math.round(((float) mNormalMaxBrightness) * MathUtils.pow(uniform_brightness, adjGamma));
            if (ColorAutomaticBrightnessController.DEBUG) {
                Slog.d("ColorAutomaticBrightnessController", "adjustCameraBrightness=" + adjust_brightness);
            }
            return adjust_brightness;
        } else if (brightness >= 256 || brightness <= 0) {
            return ColorHansRestriction.HANS_RESTRICTION_BLOCK_BINDER;
        } else {
