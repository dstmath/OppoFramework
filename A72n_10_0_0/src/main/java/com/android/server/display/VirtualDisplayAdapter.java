...
public class VirtualDisplayAdapter extends DisplayAdapter {
    ...
    private final ArrayMap<IBinder, VirtualDisplayDevice> mOppoVirtualDisplayDevices;
    ...
    VirtualDisplayAdapter(DisplayManagerService.SyncRoot syncRoot, Context context, Handler handler, DisplayAdapter.Listener listener, SurfaceControlDisplayFactory surfaceControlDisplayFactory) {
        ...
        this.mOppoVirtualDisplayDevices = new ArrayMap<>();
    ...
    public DisplayDevice releaseVirtualDisplayLocked(IBinder appToken) {
        ...
        if (device != null) {
            ...
            device = this.mOppoVirtualDisplayDevices.remove(appToken);
            if (device != null) {
                Slog.i(TAG, "releaseVirtualDisplayLocked mOppoVirtualDisplayDevices = " + this.mOppoVirtualDisplayDevices.size());
                Slog.i(TAG, "device = " + device.mName + " appToken = " + appToken);
            }
            this.mOppoVirtualDisplayDevices.clear();
        }
        return device;
    }
    ...
    private void handleMediaProjectionStoppedLocked(IBinder appToken) {
        ...
        if (device != null) {
            ...
            device.stopLocked();
            Slog.i(TAG, "device = " + device.mName + " appToken = " + appToken);
            this.mOppoVirtualDisplayDevices.put(appToken, device);
        }
    }
