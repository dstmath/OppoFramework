...
public class OppoBrightUtils implements IOppoBrightness {
    ...
    private static final String LCD_BACKLIGHT_CONFIG_PROPERTY_HX_TM_TWELVEBITS = "ro.lcd.backlight.tm_hx_4096";
    private static final String LCD_BACKLIGHT_CONFIG_PROPERTY_ILI_TM_TWELVEBITS = "ro.lcd.backlight.tm_ili_4096";
    ...
    private static final String LCD_BACKLIGHT_CONFIG_PROPERTY_NT_JDI_TWELVEBITS = "ro.lcd.backlight.jdi_nt_4096";
    ...
    private static final String LCD_BACKLIGHT_CONFIG_PROPERTY_SAMSUNG_TWELVEBITS = "ro.lcd.backlight.samsung_4096";
    ...
    private static final String LCD_MANUFACTURE_HX_TM_TWELVEBITS = "hx_tm_4096";
    private static final String LCD_MANUFACTURE_ILI_TM_TWELVEBITS = "ili_tm_4096";
    private static final String LCD_MANUFACTURE_JDI = "jdi";
    private static final String LCD_MANUFACTURE_NT_JDI_ELEVENBITS = "nt2048";
    private static final String LCD_MANUFACTURE_NT_JDI_TWELVEBITS = "nt_jdi_4096";
    ...
    private static final String LCD_MANUFACTURE_SAMSUNG_TWELVEBITS = "samsung4096";
    ...
    private static final int SCREEN_AUTO_BRIGHTNESS_TWELVEBITS_LENGTH = 17;
    ...
    public static final float TWELVEBITS_QUICKLY_DARK_AMOUNT = 30.0f;
    public static final int TWELVE_BITS_BRIGHTNESS_CONFIG = 4;
    private static final int TWELVE_BITS_BRIGHT_MAX_RATE = 1200;
    public static final int TWELVE_BITS_MAXBRIGHTNESS = 4095;
    ...
    private static CabcManager mCabcManager = null;
    ...
    private static boolean mManualBrightnessFlag = false;
    ...
    private static final int mNightBrihtnessModeThresholdValue = 167;
    ...
    private static final float mSunnyBrightnessStandardTransValue = 0.87f;
    ...
    private float mManulAmbientLuxValue;
    ...
    public static class OppoBrightUtilsHolder {
    ...
    private OppoBrightUtils() {
        ...
        this.mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (i == 1) {
                    ...
                    if (!OppoBrightUtils.this.mLastAppProcessName.equals(AppBrightnessStat.DEFAULT_LAUNCHER_APP) || time >= 60000) {
                        if (OppoBrightUtils.this.mReportMap == null) {
                            OppoBrightUtils.this.mReportMap = new HashMap();
                        }
                        ...
                        OppoBrightUtils.this.mAppDurationTime = System.currentTimeMillis();
                        OppoBrightUtils oppoBrightUtils2 = OppoBrightUtils.this;
                        oppoBrightUtils2.mLastAppProcessName = oppoBrightUtils2.mCurrentAppProcessName;
                        OppoBrightUtils.this.mReportMap.clear();
                        return;
                    }
                    OppoBrightUtils oppoBrightUtils3 = OppoBrightUtils.this;
                    oppoBrightUtils3.mLastAppProcessName = oppoBrightUtils3.mCurrentAppProcessName;
                } else if (i == 3) {
                ...
                if (OppoBrightUtils.this.mLightSensorAlwaysOn) {
                    ...
                    if (OppoBrightUtils.this.mAdStatus != 0 || lux >= OppoBrightUtils.ENABLE_AD_LUX) {
                        ...
                        if (OppoBrightUtils.this.mAdStatus != 2 && OppoBrightUtils.this.mAdStatus != OppoBrightUtils.this.mAdStatus_before) {
                            ...
                            OppoBrightUtils oppoBrightUtils = OppoBrightUtils.this;
                            oppoBrightUtils.mAdStatus_before = oppoBrightUtils.mAdStatus;
                        }
                    }
                }
            }
        ...
        this.mReportLightSensorListener = new SensorEventListener() {
            ...
            public void onSensorChanged(SensorEvent event) {
                OppoBrightUtils.this.mManulAmbientLuxValue = event.values[0];
            }
    ...
    public void init(Context context) {
        ...
        if (backLightStat != null) {
            backLightStat.setMaxBtn(TEN_BITS_MAXBRIGHTNESS);
            this.mLightStat.setLowBtn(8);
        }
    }
    ...
    public void getScreenAutoBrightnessConfig() {
        ...
        if (lcd_manufacture.contains(LCD_MANUFACTURE_HX_TM_TWELVEBITS)) {
            config_string_from_property = SystemProperties.get(LCD_BACKLIGHT_CONFIG_PROPERTY_HX_TM_TWELVEBITS, "");
            if (mSunnyBrightnessSupport && mScreenGlobalHBMSupport) {
                HBM_EXTEND_MAXBRIGHTNESS = TWELVE_BITS_MAXBRIGHTNESS;
            }
        } else if (lcd_manufacture.contains(LCD_MANUFACTURE_NT_JDI_TWELVEBITS)) {
            config_string_from_property = SystemProperties.get(LCD_BACKLIGHT_CONFIG_PROPERTY_NT_JDI_TWELVEBITS, "");
            if (mSunnyBrightnessSupport && mScreenGlobalHBMSupport) {
                HBM_EXTEND_MAXBRIGHTNESS = TWELVE_BITS_MAXBRIGHTNESS;
            }
        } else if (lcd_manufacture.contains(LCD_MANUFACTURE_SAMSUNG_TWELVEBITS)) {
            config_string_from_property = SystemProperties.get(LCD_BACKLIGHT_CONFIG_PROPERTY_SAMSUNG_TWELVEBITS, "");
        } else if (lcd_manufacture.contains(LCD_MANUFACTURE_ILI_TM_TWELVEBITS)) {
            config_string_from_property = SystemProperties.get(LCD_BACKLIGHT_CONFIG_PROPERTY_ILI_TM_TWELVEBITS, "");
            if (mSunnyBrightnessSupport && mScreenGlobalHBMSupport) {
                HBM_EXTEND_MAXBRIGHTNESS = TWELVE_BITS_MAXBRIGHTNESS;
            }
        } else if (lcd_manufacture.contains(LCD_MANUFACTURE_BOE)) {
            ...
            config_string_from_property = lcd_manufacture.contains(LCD_MANUFACTURE_NT_JDI_ELEVENBITS) ? SystemProperties.get(LCD_BACKLIGHT_CONFIG_PROPERTY_NT_JDI_ELEVENBITS, "") : lcd_manufacture.contains(LCD_MANUFACTURE_NT_TIANMA_ELEVENBITS) ? SystemProperties.get(LCD_BACKLIGHT_CONFIG_PROPERTY_NT_TIANMA_ELEVENBITS, "") : lcd_manufacture.contains(LCD_MANUFACTURE_HX_JDI_ELEVENBITS) ? SystemProperties.get(LCD_BACKLIGHT_CONFIG_PROPERTY_HX_JDI_ELEVENBITS, "") : null;
        }
        ...
        if (lcd_manufacture.contains(LCD_MANUFACTURE_SAMSUNG)) {
            ...
            SystemProperties.set(BRIGHTNESS_MODE, NoFocusWindow.HUNG_CONFIG_ENABLE);
        }
        ...
        if (!(config_string_from_property == null || (brightness_string_config = config_string_from_property.trim().split(",")) == null || brightness_string_config.length <= 2)) {
            ...
            if (config_length + 1 != brightness_string_config.length) {
                ...
                for (int i = 0; i < config_length - 1; i++) {
                    try {
    ...
    public int getMinimumScreenBrightnessSetting() {
        ...
        if (i == 4) {
            mMinBrightness = this.mContext.getResources().getInteger(17694907);
        } else if (i == 3) {
    ...
    public int getMaximumScreenBrightnessSetting() {
        int i = mBrightnessBitsConfig;
        if (i == 4) {
            mMaxBrightness = this.mContext.getResources().getInteger(17694906);
        } else if (i == 3) {
        ...
        if (mSunnyBrightnessSupport) {
            mMaxBrightness = (int) (((float) mMaxBrightness) * mSunnyBrightnessStandardTransValue);
            mSunnyMaxStandardBrightnessValue = mMaxBrightness;
            Slog.d(TAG, "(SunnyBrightness)--mSunnyMaxStandardBrightnessValue = " + mSunnyMaxStandardBrightnessValue);
        }
    ...
    public void readFeatureProperty() {
        ...
        mSunnyBrightnessSupport = this.mContext.getPackageManager().hasSystemFeature("oppo.sunny.brightness.support");
        Slog.i(TAG, "mSunnyBrightSuport = " + mSunnyBrightnessSupport);
        if (mSunnyBrightnessSupport) {
            mCabcManager = CabcManager.getCabcManagerInstance();
            this.mFeatures.add("mSunnyBrightSuport");
        }
    ...
    private void getSensorAndSfPrepared() {
        ...
        this.mFlinger = ServiceManager.getService(OppoLightsService.SURFACE_FLINGER);
    }
    ...
    public void configAutoBrightness() {
        ...
        int i = mBrightnessBitsConfig;
        if (4 == i) {
            BRIGHTNESS_RAMP_RATE_FAST = 4800;
            BRIGHTNESS_RAMP_RATE_SCREENON = 1920;
            BRIGHTNESS_RAMP_RATE_SLOW = 640;
            ...
            int i5 = BRIGHTNESS_STEPS;
            mAutoBrightnessLux = new float[i5];
            mAutoBrightnessLuxMinLimit = new float[i5];
            mAutoBrightnessLuxMaxLimit = new float[i5];
        }
    }
    ...
    public int[] readAutoBrightnessLuxConfig() {
        ...
        int i = mBrightnessBitsConfig;
        if (4 == i) {
            lux = resources.getIntArray(17236087);
            minLimit = resources.getIntArray(17236089);
            maxLimit = resources.getIntArray(17236088);
        } else if (3 == i) {
            lux = resources.getIntArray(17236024);
            minLimit = resources.getIntArray(17236026);
            maxLimit = resources.getIntArray(17236025);
        } else if (2 == i) {
            lux = resources.getIntArray(17236076);
            minLimit = resources.getIntArray(17236078);
            maxLimit = resources.getIntArray(17236077);
        } else {
            lux = resources.getIntArray(17235991);
            minLimit = resources.getIntArray(17235993);
            maxLimit = resources.getIntArray(17235992);
        }
        ...
        if (this.mStkCmLightSensor && 1 == mBrightnessBitsConfig) {
            ...
            maxLimit[3] = 100;
            maxLimit[4] = 500;
            maxLimit[5] = TWELVE_BITS_BRIGHT_MAX_RATE;
        }
    ...
    public int caclurateRateForIndex(float lux, int originBrightness, int targetBrightness) {
        ...
        if (4 == i) {
            if (targetBrightness >= originBrightness) {
                ...
                int rate2 = rate + rate;
                if (rate2 > TWELVE_BITS_BRIGHT_MAX_RATE) {
                    return TWELVE_BITS_BRIGHT_MAX_RATE;
                }
                return rate2;
            ...
            if (targetBrightness >= originBrightness) {
                int rate4 = Math.round(((Math.abs(lux) * 150.0f) + 809850.0f) / 9000.0f);
                if (targetBrightness <= brighness_int_config[3]) {
                    rate4 = EIGHT_BITS_BRIGHT_MAX_RATE;
                }
                int[] iArr3 = brighness_int_config;
                if (originBrightness <= iArr3[3] && targetBrightness >= iArr3[4]) {
                    rate4 = Math.round(((Math.abs(lux) * 100.0f) + 2699900.0f) / 9000.0f);
                }
                int[] iArr4 = brighness_int_config;
                if (originBrightness >= iArr4[4] && targetBrightness >= iArr4[4]) {
                    rate4 = Math.round(((Math.abs(lux) * 120.0f) + 1079880.0f) / 9000.0f);
                }
                if (!this.mStkCmLightSensor) {
                    return rate4;
                }
                int rate5 = rate4 + rate4;
                if (rate5 > 500) {
                    return 500;
                }
                return rate5;
                ...
                int rate6 = Math.round(((Math.abs(lux) * 80.0f) + 359920.0f) / 9000.0f);
                if (this.mStkCmLightSensor) {
                    rate6 += rate6;
                }
                if (rate6 > TEN_BITS_BRIGHT_MAX_RATE) {
            ...
            int rate7 = Math.round(((Math.abs(lux) * 96.0f) + 215880.0f) / 8999.0f);
            if (this.mStkCmLightSensor) {
                rate7 += rate7;
            }
            if (rate7 > TEN_BITS_BRIGHT_MAX_RATE) {
                return TEN_BITS_BRIGHT_MAX_RATE;
            }
            return rate7;
        }
    ...
    public int caclurateRate(float nowLux, float lastLux) {
        ...
        if (4 == i) {
            if (nowLux > lastLux) {
                return Math.round((((nowLux - lastLux) * 720.0f) + 1.07964E7f) / 8999.0f);
            }
            int rate = Math.round((((lastLux - nowLux) * 300.0f) + 1349550.0f) / 8999.0f);
            ...
            int rate2 = Math.round((((lastLux - nowLux) * 100.0f) + 449850.0f) / 8999.0f);
            if (rate2 > TEN_BITS_BRIGHT_MAX_RATE) {
                return TEN_BITS_BRIGHT_MAX_RATE;
            }
            return rate2;
        } else if (2 == i) {
            ...
            int rate3 = Math.round((((nowLux - lastLux) * 20.0f) + 49979.0f) / 4999.0f);
            if (!this.mStkCmLightSensor) {
                return rate3;
            }
            int rate4 = rate3 + rate3;
            if (rate4 > EIGHT_BITS_BRIGHT_MAX_RATE) {
                return EIGHT_BITS_BRIGHT_MAX_RATE;
            }
            return rate4;
        } else {
            int rate5 = Math.round((((lastLux - nowLux) * 14.0f) + 29980.0f) / 4999.0f);
            if (this.mStkCmLightSensor) {
                rate5 += rate5;
            }
            if (rate5 > 30) {
    ...
    public boolean isAgingTestVersion() {
        return NoFocusWindow.HUNG_CONFIG_ENABLE.equals(SystemProperties.get("SPECIAL_OPPO_CONFIG"));
    }

    public int getBootupBrightness() {
        return TEN_BITS_MAXBRIGHTNESS;
    }
    ...
    public void setAIBrightnessLux(float lux) {
        ...
        if (aIBrightnessHelper != null) {
            ...
            this.mLightStat.setLux((int) lux);
        }
    }
    ...
    public boolean isSunnyBrightnessSupport() {
        return mSunnyBrightnessSupport;
    }

    public void setCurrentBrightnessRealValue(int currentbrightness) {
        if (currentbrightness > mSunnyMaxStandardBrightnessValue && !isLockedCabcMode && !sNightBrightnessMode) {
            if (DEBUG) {
                Slog.d(TAG, "(SunnyBrightness)--sunny brightness mode in, lock cabc");
            }
            setCabcForSunnyBrightness(true);
            sSunnyBrightnessMode = true;
        } else if (currentbrightness <= mSunnyMaxStandardBrightnessValue && isLockedCabcMode && !sNightBrightnessMode) {
            if (DEBUG) {
                Slog.d(TAG, "(SunnyBrightness)--sunny brightness mode out, unlock cabc");
            }
            setCabcForSunnyBrightness(false);
            sSunnyBrightnessMode = false;
        }
        if (currentbrightness <= mNightBrihtnessModeThresholdValue && !isLockedCabcMode && !sSunnyBrightnessMode) {
            if (DEBUG) {
                Slog.d(TAG, "(SunnyBrightness)--night brightness mode in, lock cabc");
            }
            setCabcForSunnyBrightness(true);
            sNightBrightnessMode = true;
        } else if (currentbrightness > mNightBrihtnessModeThresholdValue && isLockedCabcMode && !sSunnyBrightnessMode) {
            if (DEBUG) {
                Slog.d(TAG, "(SunnyBrightness)--night brightness mode out, unlock cabc");
            }
            setCabcForSunnyBrightness(false);
            sNightBrightnessMode = false;
        }
        mCurrentBrightnessRealValue = currentbrightness;
    }

    public void setCabcForSunnyBrightness(boolean enable) {
        if (DEBUG) {
            Slog.d(TAG, "(SunnyBrightness)--setCabcForSunnyBrightness = " + enable);
        }
        isLockedCabcMode = enable;
        mCabcManager.isSunnyBrightnessMode(enable);
    }

    public void setManualBrightnessFlag(boolean changeByUser) {
        if (changeByUser != mManualBrightnessFlag) {
            Slog.d(TAG, "setManualBrightnessFlag = " + changeByUser);
            mManualBrightnessFlag = changeByUser;
        }
    }

    public boolean getManualBrightnessFlag() {
        return mManualBrightnessFlag;
    }

    @Override // com.android.server.display.IOppoBrightness
    public int getFinalMaxBrightness() {
        return mScreenGlobalHBMSupport ? HBM_EXTEND_MAXBRIGHTNESS : mMaxBrightness;
    }
}
