...
public class TrainedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        TrainedBrightnessResult result;
        if (intent != null && BrightnessConstants.ACTION_SPLINES_TRAINED.equals(intent.getAction())) {
            ...
            try {
                TrainedBrightnessResult result2 = intent.getParcelableExtra(BrightnessConstants.EXTRA_CENTRAL_POINTS);
                if (result2 != null) {
                    Map<String, List<TrainedBrightnessPoint>> trainedBrightnessPoints = result2.getTrainedBrightnessPoints();
