...
public class ColorInputMethodManagerService extends InputMethodManagerService {
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        switch (code) {
            ...
            case 10003:
                data.enforceInterface(DESCRIPTOR);
                setCusInputMethod(data.readString());
                reply.writeNoException();
                return true;
            case 10004:
                data.enforceInterface(DESCRIPTOR);
                String mid = getCurrentInputMethod();
                reply.writeNoException();
                reply.writeString(mid);
                return true;
            case 10005:
                data.enforceInterface(DESCRIPTOR);
                resetDefultInputMethod();
                reply.writeNoException();
                return true;
            default:
    ...
    public boolean showCurrentInputLocked(int flags, ResultReceiver resultReceiver) {
        synchronized (this.mMethodMap) {
            ...
            if (this.useSecure) {
                ...
                return this.mSecureInputMethodService.showCurrentInputLocked(flags, resultReceiver);
            }
            if (this.mSecureInputMethodService.mInputShown) {
                this.mSecureInputMethodService.hideCurrentInputLocked(0, null);
            }
            return ColorInputMethodManagerService.super.showCurrentInputLocked(flags, resultReceiver);
        }
    }
    ...
    public boolean hideCurrentInputLocked(int flags, ResultReceiver resultReceiver) {
        synchronized (this.mMethodMap) {
            if (!this.useSecure || this.isSwitchFormSecureIME) {
                return ColorInputMethodManagerService.super.hideCurrentInputLocked(flags, resultReceiver);
            }
            return this.mSecureInputMethodService.hideCurrentInputLocked(flags, resultReceiver);
        }
    }
    ...
    private boolean checkHasCustomePermission() {
        return this.mContext.getPackageManager().hasSystemFeature("oppo.business.custom");
    }

    public void setCusInputMethod(String id) {
        if (checkHasCustomePermission()) {
            setInputMethodLocked(id, -1);
        }
    }

    public String getCurrentInputMethod() {
        InputMethodInfo inputMethodInfo;
        if (checkHasCustomePermission() && (inputMethodInfo = (InputMethodInfo) this.mMethodMap.get(this.mCurMethodId)) != null) {
            return inputMethodInfo.getId();
        }
        return "";
    }

    public void resetDefultInputMethod() {
        if (checkHasCustomePermission()) {
            List<InputMethodInfo> suitableImes = InputMethodUtils.getDefaultEnabledImes(this.mContext, this.mSettings.getEnabledInputMethodListLocked());
            if (suitableImes.isEmpty()) {
                Log.i(this.TAG, "Customize: No default found");
                return;
            }
            InputMethodInfo defIm = suitableImes.get(0);
            if (DEBUG) {
                String str = this.TAG;
                Log.i(str, "Customize: Default found, using " + defIm.getId());
            }
            setInputMethodLocked(defIm.getId(), -1);
        }
    }
}
