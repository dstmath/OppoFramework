...
public class OppoLocationBlacklistUtil implements IPswLocationBlacklistUtil {
    ...
    public static final String KEY_OPPO_GNSS_ALWAYS_ENABLE = "persist.sys.gps.always_enable";
    public static final String KEY_OPPO_GNSS_DISABLE = "persist.sys.gps_disable";
    ...
    private boolean mGnssAlwaysEnable;
    private boolean mGnssDisable;
    private Handler mHandler;
    private LocationManager mLocationManager;
    private final Object mLock;
    private OppoLocationBlacklist mOppoBlackList;
    private boolean mOppoGnssAlwaysEnable;
    private boolean mOppoGnssDisable;
    private HashMap<String, PackageStatusInLocationService> mPackageLocationStatusRecordMap;
    ...
    private OppoLocationBlacklistUtil() {
        ...
        this.mOppoGnssAlwaysEnable = false;
        this.mGnssAlwaysEnable = false;
        this.mOppoGnssDisable = false;
        this.mGnssDisable = false;
    }

    private static class GenerateSingletonInstance {
        private static final OppoLocationBlacklistUtil INSTANCE = new OppoLocationBlacklistUtil();
    ...
    public void init(Context context, Looper loop) {
        ...
        this.mGnssAlwaysEnable = SystemProperties.get(KEY_OPPO_GNSS_ALWAYS_ENABLE, "0").equals("1");
        onOppoGnssAlwaysEnableLocked();
        this.mGnssDisable = SystemProperties.get("persist.sys.gps_disable", "0").equals("1");
        onOppoGnssDisableLocked();
        ...
        this.mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor("persist.sys.gps_disable"), true, new ContentObserver(this.mHandler) {
            ...
            public void onChange(boolean selfChange) {
                synchronized (OppoLocationBlacklistUtil.this.mLock) {
                    OppoLocationBlacklistUtil.this.onOppoGnssDisableLocked();
                }
            }
        }, -1);
        this.mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor(KEY_OPPO_GNSS_ALWAYS_ENABLE), true, new ContentObserver(this.mHandler) {
            /* class com.android.server.location.OppoLocationBlacklistUtil.AnonymousClass2 */

            public void onChange(boolean selfChange) {
                synchronized (OppoLocationBlacklistUtil.this.mLock) {
                    OppoLocationBlacklistUtil.this.onOppoGnssAlwaysEnableLocked();
                }
            }
        }, -1);
        ...
        this.mContext.registerReceiverAsUser(new BroadcastReceiver() {
            /* class com.android.server.location.OppoLocationBlacklistUtil.AnonymousClass3 */
    ...
    public boolean getHighPowerMonitor(String packageName) {
        synchronized (this.mLock) {
            ...
            return this.mPackageLocationStatusRecordMap.get(packageName).highPowerMonitorFlag;
        }
    }
    ...
    private void startActiveRequest(String providerName) {
    ...
    /* access modifiers changed from: public */
    @GuardedBy({"mLock"})
    private void onOppoGnssDisableLocked() {
        boolean z = false;
        if (Settings.Global.getInt(this.mContext.getContentResolver(), "persist.sys.gps_disable", 0) == 1) {
            z = true;
        }
        this.mOppoGnssDisable = z;
        if (DEBUG) {
            Log.d(TAG, "change mOppoGnssDisable to:" + this.mOppoGnssDisable);
        }
    }
    ...
    /* access modifiers changed from: public */
    @GuardedBy({"mLock"})
    private void onOppoGnssAlwaysEnableLocked() {
        boolean z = false;
        if (Settings.Global.getInt(this.mContext.getContentResolver(), KEY_OPPO_GNSS_ALWAYS_ENABLE, 0) == 1) {
            z = true;
        }
        this.mOppoGnssAlwaysEnable = z;
        if (DEBUG) {
            Log.d(TAG, "change mOppoGnssAlwaysEnable to:" + this.mOppoGnssAlwaysEnable);
        }
    }

    public boolean getOppoLocationMode(int userId) {
        boolean mOppoLocationMode = Settings.Secure.getIntForUser(this.mContext.getContentResolver(), "location_mode", 0, userId) != 0;
        if (mOppoLocationMode) {
            if ((this.mGnssDisable && !this.mGnssAlwaysEnable) || (this.mOppoGnssDisable && !this.mGnssAlwaysEnable && !this.mOppoGnssAlwaysEnable)) {
                Settings.Secure.putIntForUser(this.mContext.getContentResolver(), "location_mode", 0, userId);
                return false;
            }
        } else if (this.mGnssAlwaysEnable || (this.mOppoGnssAlwaysEnable && !this.mGnssDisable)) {
            Settings.Secure.putIntForUser(this.mContext.getContentResolver(), "location_mode", 3, userId);
            return true;
        }
        return mOppoLocationMode;
    }
