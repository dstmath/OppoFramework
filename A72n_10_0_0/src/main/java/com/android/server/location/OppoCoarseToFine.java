...
public class OppoCoarseToFine implements IPswCoarseToFine {
    ...
    public boolean isAllowCoarseToFine(String packageName) {
        if (!this.mRomUpdateUtil.getBoolean(KEY_COARSE_TO_FINE_ENABLED) || this.mRomUpdateUtil.getStringArray(KEY_COARSE_TO_FINE_LIST) == null || !this.mRomUpdateUtil.getStringArray(KEY_COARSE_TO_FINE_LIST).contains(packageName)) {
            return false;
        }
        if (!DEBUG) {
            return true;
