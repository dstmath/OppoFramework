...
public class OppoGnssWhiteListProxy implements IPswOppoGnssWhiteListProxy {
    ...
    private static final String KEY_ACCURACY_IMPROVE_ENABLE = "config_accuracyImproveEnable";
    private static final String KEY_ACCURACY_IMPROVE_LIST = "config_accuracyImproveList";
    ...
    private boolean mAccImproveEnable;
    private ArrayList<String> mAccImproveList;
    ...
    private OppoGnssWhiteListProxy(Context context) {
        ...
        this.mAccImproveEnable = true;
        this.mAccImproveList = null;
    ...
    private void initValue(Context context) {
        ...
        this.mAccImproveEnable = this.mRomUpdateUtil.getBoolean(KEY_ACCURACY_IMPROVE_ENABLE);
        this.mAccImproveList = new ArrayList<>();
        this.mAccImproveList = this.mRomUpdateUtil.getStringArray(KEY_ACCURACY_IMPROVE_LIST);
    }
    ...
    private boolean collectExtraCommandData(String provider, String command, int uid, String[] packages) {
        ...
        for (String name : packages) {
            packageNames = packageNames == null ? name : packageNames + ", " + name;
        }
    ...
    public boolean improveAccForApps(Location location, String packageName) {
        if (!this.mAccImproveEnable || location == null || !"gps".equalsIgnoreCase(location.getProvider()) || location.getAccuracy() <= 0.0f || !inAccImproveList(packageName)) {
            return false;
        }
        if (mIsDebug) {
            Log.d(TAG, "will set setAccuracy: " + (((double) ((float) Math.sqrt((double) location.getAccuracy()))) * 0.5d) + " to location:" + location.toString());
        }
        location.setAccuracy((float) Math.sqrt(((double) location.getAccuracy()) * 0.5d));
        return true;
    }

    private boolean inAccImproveList(String packageName) {
        if (mIsDebug) {
            Log.d(TAG, packageName + " inAccImproveList: " + this.mAccImproveList.contains(packageName));
        }
        ArrayList<String> arrayList = this.mAccImproveList;
