...
public class TelephonyRegistry extends ITelephonyRegistry.Stub {
    ...
    static {
        if (SystemProperties.get("ro.vendor.mtk_telephony_add_on_policy", "0").equals("0")) {
            ...
            if (clz != null) {
                try {
                    sMethodIdMatchEx = clz.getDeclaredMethod("idMatchEx", Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE);
    ...
    public TelephonyRegistry(Context context) {
        ...
        this.mVoLteServiceState = new VoLteServiceState[numPhones];
    ...
    private void listen(String callingPackage, IPhoneStateListener callback, int events, boolean notifyNow, int subId) {
        ...
        this.mAppOps.checkPackage(Binder.getCallingUid(), callingPackage);
        this.mListenLog.log("listen: E pkg=" + callingPackage + " events=0x" + Integer.toHexString(events) + " notifyNow=" + notifyNow + " subId=" + subId + " myUserId=" + UserHandle.myUserId() + " callerUserId=" + callerUserId);
        if (events == 0) {
            remove(callback.asBinder());
                    ...
                    if (r != null) {
                        ...
                        try {
                            ...
                            if (notifyNow && validatePhoneId(phoneId)) {
                                if ((events & 1) != 0) {
                                    try {
                                        ServiceState rawSs = makeServiceState(this.mServiceState[phoneId]);
                                ...
                                if ((8388608 & events) != 0) {
                                    try {
                                        r.callback.onRadioPowerStateChanged(this.mRadioPowerState[phoneId]);
                ...
                if (validatePhoneId(phoneId)) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (r.matchPhoneStateListenerEvent(1) && idMatch(r.subId, subId, phoneId)) {
                            try {
                                if (checkFineLocationAccess(r, 29)) {
                                    new ServiceState(state);
                                } else if (checkCoarseLocationAccess(r, 29)) {
                                    state.sanitizeLocationInfo(false);
                                } else {
                                    state.sanitizeLocationInfo(true);
                                }
        ...
        if (checkNotifyPermission("notifyDataActivity()")) {
            ...
            synchronized (this.mRecords) {
                if (validatePhoneId(phoneId)) {
                    ...
                    while (it.hasNext()) {
                        Record r = it.next();
                        if (r.matchPhoneStateListenerEvent(128) && idMatchEx(r.subId, subId, r.phoneId, phoneId)) {
                            try {
        ...
        if (checkNotifyPermission("notifyDataConnection()")) {
            synchronized (this.mRecords) {
                if (validatePhoneId(phoneId)) {
                    ...
                    if (!BatteryService.HealthServiceWrapper.INSTANCE_VENDOR.equals(apnType)) {
                        if (!"mms".equals(apnType)) {
                            ...
                            it = this.mRecords.iterator();
                            while (it.hasNext()) {
                                Record r = it.next();
                                if (r.matchPhoneStateListenerEvent(4096) && idMatchEx(r.subId, subId, r.phoneId, phoneId)) {
                                    try {
                                        r.callback.onPreciseDataConnectionStateChanged(this.mPreciseDataConnectionState[phoneId]);
                                    } catch (RemoteException e) {
                                        this.mRemoveList.add(r.binder);
                    ...
                    if (state != 2) {
                        if (this.mConnectedApns[phoneId].remove(apnType) && this.mConnectedApns[phoneId].isEmpty()) {
                            this.mDataConnectionState[phoneId] = state;
                            modified2 = true;
                        }
                        if (!modified2 && -1 == this.mDataConnectionState[phoneId] && this.mDataConnectionState[phoneId] != state) {
                            this.mDataConnectionState[phoneId] = state;
                            modified2 = true;
                        }
                    } else if (!this.mConnectedApns[phoneId].contains(apnType)) {
                    ...
                    if (modified) {
                        String str = "[" + phoneId + "]onDataConnectionStateChanged(" + TelephonyManager.dataStateToString(state) + ", " + TelephonyManager.getNetworkTypeName(networkType) + ") subId=" + subId + ", phoneId=" + phoneId;
                        log(str);
                        this.mLocalLog.log(str);
                        Iterator<Record> it2 = this.mRecords.iterator();
                        while (it2.hasNext()) {
                            Record r2 = it2.next();
                            if (r2.matchPhoneStateListenerEvent(64) && idMatchEx(r2.subId, subId, r2.phoneId, phoneId)) {
                                try {
                                    r2.callback.onDataConnectionStateChanged(this.mDataConnectionState[phoneId], this.mDataConnectionNetworkType[phoneId]);
                                } catch (RemoteException e2) {
            ...
            broadcastPreciseDataConnectionStateChanged(state, networkType, apnType, apn, linkProperties, 0, phoneId);
        }
    }
    ...
    public void notifyDataConnectionFailedForSubscriber(int phoneId, int subId, String apnType) {
        if (checkNotifyPermission("notifyDataConnectionFailed()")) {
            ...
            broadcastPreciseDataConnectionStateChanged(-1, 0, apnType, null, null, 0, phoneId);
        }
    }
    ...
    public void notifyPreciseDataConnectionFailed(int phoneId, int subId, String apnType, String apn, int failCause) {
        if (checkNotifyPermission("notifyPreciseDataConnectionFailed()")) {
            ...
            broadcastPreciseDataConnectionStateChanged(-1, 0, apnType, apn, null, failCause, phoneId);
        }
    }
        ...
        if (checkNotifyPermission("notifyRadioPowerStateChanged()")) {
            synchronized (this.mRecords) {
                if (validatePhoneId(phoneId)) {
                    ...
                    while (it.hasNext()) {
                        Record r = it.next();
                        if (r.matchPhoneStateListenerEvent(DumpState.DUMP_VOLUMES) && idMatchEx(r.subId, subId, r.phoneId, phoneId)) {
                            try {
    ...
    private void broadcastPreciseDataConnectionStateChanged(int state, int networkType, String apnType, String apn, LinkProperties linkProperties, int failCause, int phoneId) {
    ...
    private void checkPossibleMissNotify(Record r, int phoneId) {
        ...
        if ((events & 1) != 0) {
            try {
                r.callback.onServiceStateChanged(makeServiceState(this.mServiceState[phoneId]));
            } catch (RemoteException e) {
                this.mRemoveList.add(r.binder);
            }
        }
        if ((events & 256) != 0) {
            try {
                r.callback.onSignalStrengthsChanged(makeSignalStrength(phoneId, this.mSignalStrength[phoneId]));
            } catch (RemoteException e2) {
                this.mRemoveList.add(r.binder);
            }
        }
    ...
    public SignalStrength makeSignalStrength(int phoneId) {
        ...
        try {
            Constructor clazzConstructfunc = Class.forName("mediatek.telephony.MtkSignalStrength").getConstructor(Integer.TYPE);
            clazzConstructfunc.setAccessible(true);
            return (SignalStrength) clazzConstructfunc.newInstance(new Integer(phoneId));
    ...
    public SignalStrength makeSignalStrength(int phoneId, SignalStrength signalStrength) {
        ...
        try {
            Constructor clazzConstructfunc = Class.forName("mediatek.telephony.MtkSignalStrength").getConstructor(Integer.TYPE, SignalStrength.class);
            clazzConstructfunc.setAccessible(true);
            return (SignalStrength) clazzConstructfunc.newInstance(new Integer(phoneId), signalStrength);
        } catch (InstantiationException e) {
            e.printStackTrace();
            Rlog.e(TAG, "MtkSignalStrength InstantiationException! Used AOSP instead!");
            return new SignalStrength(signalStrength);
    ...
    public ServiceState makeServiceState() {
        ...
        try {
            Constructor clazzConstructfunc = Class.forName("mediatek.telephony.MtkServiceState").getConstructor(new Class[0]);
            clazzConstructfunc.setAccessible(true);
            return (ServiceState) clazzConstructfunc.newInstance(new Object[0]);
        } catch (InstantiationException e) {
            e.printStackTrace();
    ...
    public ServiceState makeServiceState(ServiceState serviceState) {
        ...
        try {
            Constructor clazzConstructfunc = Class.forName("mediatek.telephony.MtkServiceState").getConstructor(ServiceState.class);
            clazzConstructfunc.setAccessible(true);
            return (ServiceState) clazzConstructfunc.newInstance(serviceState);
        } catch (InstantiationException e) {
            e.printStackTrace();
            Rlog.e(TAG, "MtkServiceState InstantiationException! Used AOSP instead!");
            return new ServiceState(serviceState);
    ...
    /* access modifiers changed from: package-private */
    public boolean idMatchEx(int rSubId, int subId, int rPhoneId, int phoneId) {
        Method method = sMethodIdMatchEx;
        if (method != null) {
            try {
                return ((Boolean) method.invoke(null, Integer.valueOf(rSubId), Integer.valueOf(subId), Integer.valueOf(this.mDefaultSubId), Integer.valueOf(rPhoneId), Integer.valueOf(phoneId))).booleanValue();
            } catch (Exception e) {
                e.printStackTrace();
                log(e.toString());
            }
