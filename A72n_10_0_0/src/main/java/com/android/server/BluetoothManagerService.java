...
public class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    BluetoothManagerService(Context context) {
        ...
        if (SystemProperties.getInt("persist.sys.bluetooth_policy", 2) == 3) {
            Log.d(TAG, "The  bluetooth_policy is MODE_BT_ENABLED  and save bluetooth'state ON to enable.");
            persistBluetoothSetting(1);
        } else if (SystemProperties.getInt("persist.sys.bluetooth_policy", 2) == 0) {
            Log.d(TAG, "The  bluetooth_policy is MODE_BT_DISABLE  and save bluetooth'state OFF to keep off");
        ...
        if (!isBluetoothDisallowed()) {
            try {
                ...
                if (this.mEnableExternal && isBluetoothPersistedStateOnBluetooth()) {
                    ...
                    if (SystemProperties.get("ro.vendor.bluetooth.noconsyschip").equals(NoFocusWindow.HUNG_CONFIG_ENABLE)) {
                        Slog.d(TAG, "Getting adapter name and address return");
                        return;
                    }
