...
public class OppoStorageManagerFeature implements IOppoStorageManagerFeature {
    ...
    private static final int mLowBatteryThresholdA = 5;
    private static final int mLowBatteryThresholdB = 20;
    ...
    private OppoStorageManagerFeature() {
        ...
        this.mBatteryReceiver = new BroadcastReceiver() {
            /* class com.android.server.storage.OppoStorageManagerFeature.AnonymousClass1 */

            /* JADX WARNING: Removed duplicated region for block: B:13:0x003d A[Catch:{ Exception -> 0x0079 }] */
            /* JADX WARNING: Removed duplicated region for block: B:15:0x0047 A[Catch:{ Exception -> 0x0079 }] */
                ...
                try {
                    if (intent.getAction().equals("android.intent.action.BATTERY_CHANGED")) {
                        ...
                        if (status != 2) {
                            if (status != 5) {
                                mChargeOnCurr = false;
                                boolean mBatteryPropCurr = SystemProperties.getBoolean(OppoStorageManagerFeature.mLowBatterySqlPropStr, false);
                                if ((((float) level) / ((float) scale)) * 100.0f >= 5.0f) {
                                    if (!mBatteryPropCurr) {
                                        SystemProperties.set(OppoStorageManagerFeature.mLowBatterySqlPropStr, Boolean.toString(true));
                                        return;
                                    }
                                    return;
                                } else if ((((float) level) / ((float) scale)) * 100.0f > 20.0f || (((float) level) / ((float) scale)) * 100.0f < 5.0f) {
                                    if (mBatteryPropCurr) {
                                        SystemProperties.set(OppoStorageManagerFeature.mLowBatterySqlPropStr, Boolean.toString(false));
                                        return;
                                    }
                                    return;
                                } else if (mChargeOnCurr) {
                                    if (mBatteryPropCurr) {
                                        SystemProperties.set(OppoStorageManagerFeature.mLowBatterySqlPropStr, Boolean.toString(false));
                                        return;
                                    }
                                    return;
                                } else if (!mBatteryPropCurr) {
                                    SystemProperties.set(OppoStorageManagerFeature.mLowBatterySqlPropStr, Boolean.toString(true));
                                    return;
                                } else {
                                    return;
                        ...
                        mChargeOnCurr = true;
                        boolean mBatteryPropCurr2 = SystemProperties.getBoolean(OppoStorageManagerFeature.mLowBatterySqlPropStr, false);
                        if ((((float) level) / ((float) scale)) * 100.0f >= 5.0f) {
                        }
                    }
                } catch (Exception e) {
                    Slog.wtf(OppoStorageManagerFeature.TAG, e);
                }
            }
        };
    ...
    private void init(Context context) {
        if (!this.mIsInited) {
            ...
            this.mContext.registerReceiver(this.mBatteryReceiver, batteryFilter, null, this.mAospStorageManagerHandler);
            SystemProperties.set(mLowBatterySqlPropStr, Boolean.toString(SystemProperties.getBoolean(mLowBatterySqlPropStr, false)));
            this.mIsInited = true;
        }
    }
    ...
    private boolean onMessageVolumeMount(Message msg) {
        ...
        Handler handler = this.mAospStorageManagerHandler;
        if (handler != null) {
            Message delaymsg = handler.obtainMessage(5);
            delaymsg.obj = vol;
