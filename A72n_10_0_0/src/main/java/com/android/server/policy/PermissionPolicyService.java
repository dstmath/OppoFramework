...
public final class PermissionPolicyService extends SystemService {
                ...
                if (!perm.isHardRestricted()) {
                    if (perm.backgroundPermission == null) {
                        if (perm.isSoftRestricted()) {
                            appOpsService.startWatchingModeWithFlags(getSwitchOp(perm.name), (String) null, Integer.MIN_VALUE, appOpsListener);
                            ...
                            if (policy.resolveAppOp() != -1) {
                                appOpsService.startWatchingModeWithFlags(policy.resolveAppOp(), (String) null, Integer.MIN_VALUE, appOpsListener);
                            }
                        }
                    }
                }
    ...
    private void grantOrUpgradeDefaultRuntimePermissionsIfNeeded(int userId) {
        ...
        if (packageManagerInternal.wereDefaultPermissionsGrantedSinceBoot(userId) && !OppoFeatureCache.get(IColorRuntimePermGrantPolicyManager.DEFAULT).isRuntimePermissionFingerprintNew(userId)) {
            if (userId != 999) {
                CountDownLatch latch = new CountDownLatch(1);
                new PermissionControllerManager(getUserContext(getContext(), UserHandle.of(userId)), FgThread.getHandler()).grantOrUpgradeDefaultRuntimePermissions(FgThread.getExecutor(), new Consumer(latch) {
                    /* class com.android.server.policy.$$Lambda$PermissionPolicyService$8D9Zbki65ND_Q20MTrexl6cHcQ */
                    ...
                    public final void accept(Object obj) {
                        PermissionPolicyService.lambda$grantOrUpgradeDefaultRuntimePermissionsIfNeeded$0(this.f$0, (Boolean) obj);
                    }
                });
                try {
                    latch.await();
    ...
    static /* synthetic */ void lambda$grantOrUpgradeDefaultRuntimePermissionsIfNeeded$0(CountDownLatch latch, Boolean success) {
    ...
    public static class PermissionToOpSynchroniser {
        ...
        private boolean isBgPermRestricted(String pkg, String perm, int uid) {
            try {
                ...
                if (!bgPermInfo.isHardRestricted() || (this.mPackageManager.getPermissionFlags(perm, pkg, UserHandle.getUserHandleForUid(uid)) & 16384) == 0) {
                    return false;
                }
                return true;
            } catch (PackageManager.NameNotFoundException e) {
