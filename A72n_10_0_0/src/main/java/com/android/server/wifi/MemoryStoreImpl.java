...
final class MemoryStoreImpl implements WifiScoreCard.MemoryStore {
    ...
    public void read(String key, WifiScoreCard.BlobListener blobListener) {
        if (!this.mBroken) {
            try {
                this.mIpMemoryStore.retrieveBlob(key, "com.android.server.wifi", WIFI_FRAMEWORK_IP_MEMORY_STORE_DATA_NAME, new CatchAFallingBlob(key, blobListener));
    ...
    public void write(String key, byte[] value) {
        if (!this.mBroken) {
            ...
            blob.data = value;
            try {
                this.mIpMemoryStore.storeBlob(key, "com.android.server.wifi", WIFI_FRAMEWORK_IP_MEMORY_STORE_DATA_NAME, blob, (OnStatusListener) null);
            } catch (RuntimeException e) {
                handleException(e);
            }
        }
