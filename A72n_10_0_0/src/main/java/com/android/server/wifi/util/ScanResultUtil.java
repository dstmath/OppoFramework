...
public class ScanResultUtil {
    ...
    public static boolean isScanResultForOpenNetwork(ScanResult scanResult) {
        return !isScanResultForWepNetwork(scanResult) && !isScanResultForPskNetwork(scanResult) && !MtkWapi.isScanResultForWapiNetwork(scanResult) && !isScanResultForEapNetwork(scanResult) && !isScanResultForSaeNetwork(scanResult) && !isScanResultForEapSuiteBNetwork(scanResult);
    }
    ...
    public static void setAllowedKeyManagementFromScanResult(ScanResult scanResult, WifiConfiguration config) {
        if (MtkWapi.isScanResultForWapiNetwork(scanResult)) {
            config.allowedKeyManagement.set(scanResult.capabilities.contains("PSK") ? 13 : 14);
    ...
    public static void dumpScanResults(PrintWriter pw, List<ScanResult> scanResults, long nowMs) {
        ...
        if (!(scanResults == null || scanResults.size() == 0)) {
            ...
            for (ScanResult r : scanResults) {
                ...
                pw.printf("  %17s  %9d  %18s   %7s    %-32s  %s\n", r.BSSID, Integer.valueOf(r.frequency), rssiInfo, age, String.format("%1.32s", ssid), r.capabilities);
            }
        }
    }
}
