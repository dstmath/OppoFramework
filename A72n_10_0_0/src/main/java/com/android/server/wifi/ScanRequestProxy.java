...
public class ScanRequestProxy {
    ...
    @VisibleForTesting
    public static int SCAN_REQUEST_THROTTLE_MAX_IN_TIME_WINDOW_FG_APPS = 0;
    @VisibleForTesting
    public static int SCAN_REQUEST_THROTTLE_TIME_WINDOW_FG_APPS_MS = 0;
    private static final int SCREEN_TIME = 20000;
    private static final String TAG = "WifiScanRequestProxy";
    private final int DEFAULT_SCAN_REQUEST_THROTTLE_MAX_IN_TIME_WINDOW_FG_APPS = 1;
    private final int DEFAULT_SCAN_REQUEST_THROTTLE_TIME_WINDOW_FG_APPS_MS = 30000;
    ...
    ScanRequestProxy(Context context, AppOpsManager appOpsManager, ActivityManager activityManager, WifiInjector wifiInjector, WifiConfigManager configManager, WifiPermissionsUtil wifiPermissionUtil, WifiMetrics wifiMetrics, Clock clock, FrameworkFacade frameworkFacade, Handler handler) {
        ...
        SCAN_REQUEST_THROTTLE_TIME_WINDOW_FG_APPS_MS = getRomUpdateIntegerValue("RUS_SCAN_REQUEST_THROTTLE_TIME_WINDOW_FG_APPS_MS", 30000).intValue();
        SCAN_REQUEST_THROTTLE_MAX_IN_TIME_WINDOW_FG_APPS = getRomUpdateIntegerValue("RUS_SCAN_REQUEST_THROTTLE_MAX_IN_TIME_WINDOW_FG_APPS", 1).intValue();
    }
    ...
    private void trimPastScanRequestTimesForForegroundApp(List<Long> scanRequestTimestamps, long currentTimeMillis) {
        ...
        while (timestampsIter.hasNext() && currentTimeMillis - timestampsIter.next().longValue() > ((long) SCAN_REQUEST_THROTTLE_TIME_WINDOW_FG_APPS_MS)) {
    ...
    private boolean shouldScanRequestBeThrottledForForegroundApp(int callingUid, String packageName) {
        ...
        OppoScanResultsProxy oppoScanResultsProxy2 = this.mOppoScanResultsProxy;
        if (oppoScanResultsProxy2 != null && oppoScanResultsProxy2.checkAppInterTimeLessThanThread(currentTimeMillis)) {
            return true;
        }
