...
public class OsuProviderInfo {
    ...
    public static OsuProviderInfo parse(ByteBuffer payload) throws ProtocolException {
        int length = ((int) ByteBufferReader.readInteger(payload, ByteOrder.LITTLE_ENDIAN, 2)) & 65535;
        if (length >= 9) {
            List<I18Name> friendlyNameList = parseI18Names(getSubBuffer(payload, ((int) ByteBufferReader.readInteger(payload, ByteOrder.LITTLE_ENDIAN, 2)) & 65535));
            ...
            ByteBuffer iconBuffer = getSubBuffer(payload, ((int) ByteBufferReader.readInteger(payload, ByteOrder.LITTLE_ENDIAN, 2)) & 65535);
            List<IconInfo> iconInfoList = new ArrayList<>();
            while (iconBuffer.hasRemaining()) {
                iconInfoList.add(IconInfo.parse(iconBuffer));
            }
            return new OsuProviderInfo(friendlyNameList, serverUri, methodList, iconInfoList, ByteBufferReader.readStringWithByteLength(payload, StandardCharsets.UTF_8), parseI18Names(getSubBuffer(payload, ((int) ByteBufferReader.readInteger(payload, ByteOrder.LITTLE_ENDIAN, 2)) & 65535)));
        }
