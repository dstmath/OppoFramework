...
public class WifiConnectivityManager {
    ...
    @VisibleForTesting
    public static int MAX_PERIODIC_SCAN_INTERVAL_MS = WifiMetrics.MIN_DATA_STALL_WAIT_MS;
    @VisibleForTesting
    ...
    private static boolean mOshareTrafficEnable = false;
    private static int mPeriodicSingleScanInterval;
    private int INITAL_SCAN_INTER_VAL_TIMES_THRESHOLD = 8;
    ...
    private int mInitalScanIntervalMsTimes = 0;
    ...
    WifiConnectivityManager(Context context, ScoringParams scoringParams, ClientModeImpl stateMachine, WifiInjector injector, WifiConfigManager configManager, WifiInfo wifiInfo, WifiNetworkSelector networkSelector, WifiConnectivityHelper connectivityHelper, WifiLastResortWatchdog wifiLastResortWatchdog, OpenNetworkNotifier openNetworkNotifier, CarrierNetworkNotifier carrierNetworkNotifier, CarrierNetworkConfig carrierNetworkConfig, WifiMetrics wifiMetrics, Looper looper, Clock clock, LocalLog localLog) {
        ...
        this.mBand5GHzBonus = context.getResources().getInteger(17694917);
        this.mCurrentConnectionBonus = context.getResources().getInteger(17694934);
        this.mSameNetworkBonus = context.getResources().getInteger(17694924);
        this.mSecureBonus = context.getResources().getInteger(17694925);
        this.mRssiScoreOffset = context.getResources().getInteger(17694922);
        this.mRssiScoreSlope = context.getResources().getInteger(17694923);
        this.mEnableAutoJoinWhenAssociated = false;
        this.mUseSingleRadioChainScanResults = context.getResources().getBoolean(17891586);
        this.mFullScanMaxTxRate = context.getResources().getInteger(17694937);
        this.mFullScanMaxRxRate = context.getResources().getInteger(17694936);
    ...
    private void startPeriodicSingleScan() {
        ...
        if (isScanNeeded) {
            ...
            int i = this.mInitalScanIntervalMsTimes;
            if (i < this.INITAL_SCAN_INTER_VAL_TIMES_THRESHOLD) {
                mPeriodicSingleScanInterval = PERIODIC_SCAN_INTERVAL_MS;
                this.mInitalScanIntervalMsTimes = i + 1;
            }
            if (this.mInitalScanIntervalMsTimes >= this.INITAL_SCAN_INTER_VAL_TIMES_THRESHOLD) {
                mPeriodicSingleScanInterval *= 2;
            }
            int i2 = mPeriodicSingleScanInterval;
            int i3 = MAX_PERIODIC_SCAN_INTERVAL_MS;
            if (i2 > i3) {
                mPeriodicSingleScanInterval = i3;
                return;
            }
            return;
        }
    ...
    private void resetScanInterval() {
        this.mInitalScanIntervalMsTimes = 0;
    }
}
