...
public class OppoScanResultsProxy implements Handler.Callback {
    ...
    private static final String DEFAULT_GET_SCAN_RESULTS_PACKAGE = "com.coloros.assistantscreen,com.oppo.ohome,com.coloros.musiclink,com.coloros.sceneservice,com.coloros.backuprestore";
    ...
    private static final String DEFAULT_WIFI_APP_BLACKLIST = "com.sankuai.meituan.dispatch.homebrew,com.sankuai.meituan.meituanwaimaibusiness";
    ...
    private static final int mScanThreadTime = 15000;
    ...
    public class OppoScanResult {
        ...
        private long getLastScannedTime() {
        ...
        /* access modifiers changed from: private */
        /* access modifiers changed from: public */
        private int getScanMissedCount() {
    ...
    public List<ScanResult> getScanResults(String packageName, int callingUid) {
        ...
        if (this.mWifiInjector.getActiveModeWarden().isWifiRealOff()) {
            logd("getScanResults() callingUid=" + callingUid + " packageName:" + packageName + " size:" + scanList.size() + " when wifi is off.");
            return scanList;
        }
        synchronized (mScanResultLock) {
            if (this.mLastScanResults != null) {
                if (this.mOppoScanResultHashMap != null) {
                    ...
                    if (!this.mReportScanCache || scanList.size() <= 0) {
    ...
    public boolean checkAppInterTimeLessThanThread(long currentTimeStamp) {
        List<ScanResult> list = this.mLastScanResults;
        if (list == null || list.size() <= 0 || currentTimeStamp - this.mLastScanTimeStamp >= 15000) {
            return false;
        }
        return true;
    }
