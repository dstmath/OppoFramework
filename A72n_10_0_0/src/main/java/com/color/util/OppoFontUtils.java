...
public class OppoFontUtils extends OppoBaseFontUtils {
    ...
    public static void handleFactoryReset() {
        if (sIsROM6d0FlipFont) {
            try {
                File fontFileFolder = new File(DATA_FONT_DIRECTORY);
                if (fontFileFolder.exists()) {
                    File[] subFiles = fontFileFolder.listFiles();
                    for (File subfile : subFiles) {
                        if (subfile.exists()) {
                            boolean cleanUserFontResult = FileUtils.deleteContentsAndDir(subfile);
                            if (DEBUG) {
                                Log.d("OppoFontUtils", "handleFactoryReset clear font dirs: " + cleanUserFontResult);
                            }
                        }
                    }
                }
