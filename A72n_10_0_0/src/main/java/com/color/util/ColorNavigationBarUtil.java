...
public class ColorNavigationBarUtil {
    ...
    private static final List<AdaptationEnvelopeInfo> mAdaptationEnvelopeInfoList = new ArrayList();
    private static final List<AdaptationAppInfo> mDefaultAdapationApps = new ArrayList();
    private static final List<AdaptationImeInfo> mDefaultAdapationIme = new ArrayList();
    ...
    private boolean mReadEnvelopeData = false;
    private boolean mReadImeData = false;
    ...
    private void addAdaptationIme(String pkg, int color) {
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        ColorNavigationBarUtil.mStatusDefaultAdapationApps.clear();
                        String str = ColorNavigationBarUtil.TAG;
                        Log.w(str, "updateStatusBgColorListFromDB query error:" + e);
                        if (0 == 0) {
                            return;
                        }
                    } catch (Throwable th) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void updateImeBgColorListFromDB() {
        if (!this.mHasInitialized) {
            Log.w(TAG, "color navigation bar util isn't init.");
        } else {
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (cursor != null) {
                            if (cursor.getCount() != 0) {
                                synchronized (ColorNavigationBarUtil.mObject) {
                                    ...
                                    ColorNavigationBarUtil.this.mReadImeData = true;
                                }
                                if (cursor == null) {
                                    return;
                                }
                        ...
                        ColorNavigationBarUtil.mDefaultAdapationIme.clear();
                        String str = ColorNavigationBarUtil.TAG;
                        Log.w(str, "mDefaultAdapationIme query error:" + e);
                        if (0 == 0) {
                            return;
                        }
                    } catch (Throwable th) {
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private void updateEnvelopeInfoFromDB() {
        if (!this.mHasInitialized) {
            Log.w(TAG, "color navigation bar util isn't init.");
        } else {
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (cursor != null) {
                            if (cursor.getCount() != 0) {
                                synchronized (ColorNavigationBarUtil.mObject) {
                                    ...
                                    ColorNavigationBarUtil.this.mReadEnvelopeData = true;
                                }
                                if (cursor == null) {
                                    return;
                                }
                        ...
                        String str = ColorNavigationBarUtil.TAG;
                        Log.w(str, "mAdaptationEnvelopeInfoList query error:" + e);
                        if (0 == 0) {
                            return;
                        }
                    } catch (Throwable th) {
                        ...
                        for (Map.Entry<String, String> entry : appInfo.mActivityColorList.entrySet()) {
                            if (entry.getValue() != null) {
                                if (!entry.getValue().equals("")) {
                                    if (entry.getKey().equals(activityName)) {
                                        return -16777216 | Integer.valueOf(entry.getValue(), 16).intValue();
                                    }
                                }
                            }
                            return 0;
        ...
        synchronized (mObject) {
            ...
            if (installedVersion != null && !installedVersion.isEmpty()) {
                ...
                if (index >= 0) {
                    ...
                    switch (c) {
                        case 0:
                            return mAdaptationEnvelopeInfoList.get(index).mEnvelopeFilterField;
                        case 1:
                            return mAdaptationEnvelopeInfoList.get(index).mEnvelopeFilterValue;
                        case 2:
                            return mAdaptationEnvelopeInfoList.get(index).mEnvelopeUserField;
                        case 3:
                            return mAdaptationEnvelopeInfoList.get(index).mEnvelopeGroupTag;
                        case 4:
                            return mAdaptationEnvelopeInfoList.get(index).mEnvelopeUserNameTagFirst;
                        case 5:
                            return mAdaptationEnvelopeInfoList.get(index).mEnvelopeUserNameTagLast;
                        case 6:
                            return mAdaptationEnvelopeInfoList.get(index).mEnvelopeContentTag;
                    }
                }
            }
            return "";
