...
public class ColorResolverAlertLinearLayout extends LinearLayout {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int maxHeight = (isPortrait() || isZoomMode()) ? this.mPortraitMaxHeight : this.mLandscapeMaxHeight;
        if (height > maxHeight) {
    ...
    private boolean isZoomMode() {
        return ((IColorResolverManager) ColorFrameworkFactory.getInstance().getFeature(IColorResolverManager.DEFAULT, new Object[0])).isZoomMode(getContext());
    }
    ...
    private void updateSpaceView() {
        if (!isPortrait() || isZoomMode() || isNavigationBarShow()) {
