...
public class NetworkMonitorManager {
    ...
    public boolean notifyPrepareCaptiveResponse(int returnCode) {
        long token = Binder.clearCallingIdentity();
        try {
            this.mNetworkMonitor.notifyPrepareCaptiveResponse(returnCode);
            return true;
        } catch (RemoteException e) {
            log("Error in notifyPrepareCaptiveResponse", e);
            return false;
        } finally {
            Binder.restoreCallingIdentity(token);
        }
