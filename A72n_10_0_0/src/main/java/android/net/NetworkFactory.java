...
public class NetworkFactory extends Handler {
    ...
    private static final boolean VDBG = Log.isLoggable("NetworkFactory", 2);
    private final String LOG_TAG;
    private AsyncChannel mAsyncChannel;
    private NetworkCapabilities mCapabilityFilter;
    private final Context mContext;
    private boolean mHasWifiAssistant = false;
    ...
    public NetworkFactory(Looper looper, Context context, String logTag, NetworkCapabilities filter) {
        ...
        this.mHasWifiAssistant = isWlanAssistantEnable();
    }
    ...
    public void handleMessage(Message msg) {
        ...
        if (i != 69633) {
            ...
            if (VDBG) {
                log("NetworkFactory fully connected");
            }
    ...
    private void evalRequest(NetworkRequestInfo n) {
        if (VDBG) {
            ...
            log(" mSerialNumber = " + this.mSerialNumber);
        }
    ...
    private boolean shouldNeedNetworkFor(NetworkRequestInfo n) {
        boolean myScoreIsSatisfied;
        if (!this.mHasWifiAssistant || this.mScore != 50) {
            myScoreIsSatisfied = n.score < this.mScore;
        } else {
            myScoreIsSatisfied = n.score <= this.mScore;
        }
        return !n.requested && (myScoreIsSatisfied || n.factorySerialNumber == this.mSerialNumber) && n.request.networkCapabilities.satisfiedByNetworkCapabilities(this.mCapabilityFilter) && acceptRequest(n.request, n.score);
    }
