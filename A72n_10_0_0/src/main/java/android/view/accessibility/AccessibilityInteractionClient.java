...
public final class AccessibilityInteractionClient extends IAccessibilityInteractionConnectionCallback.Stub {
    ...
    /* JADX WARNING: Removed duplicated region for block: B:53:0x00c5 A[Catch:{ RemoteException -> 0x00cc }] */
        ...
        if ((prefetchFlags & 2) == 0 || (prefetchFlags & 1) != 0) {
            try {
                ...
                try {
                    ...
                    try {
                        packageNames = connection.findAccessibilityNodeInfoByAccessibilityId(accessibilityWindowId, accessibilityNodeId, interactionId2, this, prefetchFlags, Thread.currentThread().getId(), arguments);
                        Binder.restoreCallingIdentity(identityToken);
                    } catch (NullPointerException e4) {
                        e = e4;
                        try {
                            Log.e(LOG_TAG, "Error findAccessibilityNodeInfoByAccessibilityId" + e);
                            Binder.restoreCallingIdentity(identityToken);
