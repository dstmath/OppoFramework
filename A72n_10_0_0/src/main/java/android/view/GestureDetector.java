...
public class GestureDetector {
    ...
    /* JADX INFO: Multiple debug info for r4v30 int: [D('distance' int), D('deltaX' int)] */
        ...
        if (i2 == 0) {
            if (this.mDoubleTapListener != null) {
                ...
                if (motionEvent4 == null || (motionEvent2 = this.mPreviousUpEvent) == null || !hadTapMessage || !isConsideredDoubleTap(motionEvent4, motionEvent2, ev)) {
                    ...
                    handled2 = false | this.mDoubleTapListener.onDoubleTap(this.mCurrentDownEvent) | this.mDoubleTapListener.onDoubleTapEvent(ev);
                    ...
                    if (this.mIsLongpressEnabled) {
                        ...
                        handler.sendMessageAtTime(handler.obtainMessage(2, 3, 0), this.mCurrentDownEvent.getDownTime() + ((long) ViewConfiguration.getLongPressTimeout()));
                    }
                    this.mHandler.sendEmptyMessageAtTime(1, this.mCurrentDownEvent.getDownTime() + ((long) TAP_TIMEOUT));
                    handled = handled2 | this.mListener.onDown(ev);
                }
            }
            handled2 = false;
            this.mLastFocusX = focusX;
            ...
            if (i2 != 2) {
                if (i2 == 3) {
                    ...
                    float x12 = this.mVelocityTracker.getXVelocity(id12);
                    ...
                    while (true) {
                        ...
                        if (i3 == upIndex2) {
                            ...
                            id1 = id12;
                            x1 = x12;
                            if ((this.mVelocityTracker.getXVelocity(id2) * x12) + (this.mVelocityTracker.getYVelocity(id2) * y12) < 0.0f) {
                ...
                if (this.mIsDoubleTapping) {
                    ...
                    handled = this.mDoubleTapListener.onDoubleTapEvent(ev) | false;
                } else if (this.mAlwaysInTapRegion) {
                    int deltaX = (int) (focusX - this.mDownFocusX);
                    int deltaY = (int) (focusY - this.mDownFocusY);
                    int deltaX2 = (deltaX * deltaX) + (deltaY * deltaY);
                    ...
                    if (hasPendingLongPress2 && (motionClassification2 == 1)) {
                        ...
                        if (deltaX2 > deltaY2) {
                            ...
                            hasPendingLongPress = hasPendingLongPress2;
                            handled3 = false;
                            handler2.sendMessageAtTime(handler2.obtainMessage(2, 3, 0), ev.getDownTime() + ((long) (((float) longPressTimeout) * multiplier)));
                        } else {
                            motionClassification = motionClassification2;
                            hasPendingLongPress = hasPendingLongPress2;
                            handled3 = false;
                    ...
                    if (deltaX2 > (isGeneratedGesture ? 0 : this.mDoubleTapTouchSlopSquare)) {
                    ...
                    if (Math.abs(scrollX) >= 1.0f || Math.abs(scrollY) >= 1.0f) {
                        recordGestureClassification(5);
                        boolean handled4 = this.mListener.onScroll(this.mCurrentDownEvent, ev, scrollX, scrollY);
                        this.mLastFocusX = focusX;
                        this.mLastFocusY = focusY;
                        handled = handled4;
                    } else {
