...
public class OppoSystemVibrator {
    ...
    public OppoSystemVibrator(Context context) {
        ...
        this.mDisableVibrateInCall = context.getPackageManager().hasSystemFeature("oppo.camera.disablevibrateincall.support");
    ...
    public boolean doVibrate(int uid, String opPkg, VibrationEffect effect) {
        ...
        if (this.mCameraAntiShake && ((!shouldVib || this.mDisableVibrateInCall) && cameraPkgName.equals(OppoAtlasManagerDefine.CAMERA_PACKAGE_NAME))) {
        ...
        if (this.mLogEnable || InputLog.DEBUG) {
