...
public class GradientDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (ensureValidRect()) {
            ...
            if (i != 0) {
                ...
                if (!this.mSmoothRound) {
                    canvas.drawRoundRect(this.mRect, rad2, rad2, this.mFillPaint);
                } else {
                    canvas.drawPath(ColorRoundRectUtil.getInstance().getPath(this.mRect, rad2), this.mFillPaint);
                }
                if (haveStroke) {
                    if (!this.mSmoothRound) {
                        canvas.drawRoundRect(this.mRect, rad2, rad2, this.mStrokePaint);
                    } else {
                        canvas.drawPath(ColorRoundRectUtil.getInstance().getPath(this.mRect, rad2), this.mStrokePaint);
                    }
                }
            } else {
    ...
    /* JADX INFO: Multiple debug info for r4v0 android.graphics.Paint: [D('newColor' int), D('strokePaint' android.graphics.Paint)] */
            ...
            if (type != 1) {
                ...
                if (type == 2 && depth <= innerDepth) {
                    ...
                    if (name.equals(DumpHeapActivity.KEY_SIZE)) {
                        ...
                        updateGradientDrawablePadding(a6);
                        a6.recycle();
                    } else if (name.trim().equals("smooth-rect")) {
    ...
    private void updateLocalState(Resources res) {
        ...
        this.mSmoothRound = state.mSmoothRound;
