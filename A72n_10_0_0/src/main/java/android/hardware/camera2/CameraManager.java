...
public final class CameraManager {
    ...
    private static final boolean bLimitsPreviewSize = true;
    private static HypnusManager sHM = null;
    private final boolean DEBUG = false;
    private final Context mContext;
    private ArrayList<String> mDeviceIdList;
    private boolean mHypnusCtrl = false;
                ...
                if (cameraService != null) {
                    try {
                        ...
                        if (isHiddenPhysicalCamera(cameraId) || supportsCamera2ApiLocked(cameraId)) {
                            ...
                            characteristics.setAppNameAndSensorId(this.mContext.getOpPackageName(), cameraId);
                        } else {
    ...
    private String getRunningActivityName() {
        ...
        if (activityManager.getAppTasks().isEmpty() || activityManager.getAppTasks().get(0).getTaskInfo() == null || activityManager.getAppTasks().get(0).getTaskInfo().topActivity == null) {
            return "";
        }
        return activityManager.getAppTasks().get(0).getTaskInfo().topActivity.getClassName();
    }

    /* JADX WARNING: Removed duplicated region for block: B:34:0x00d2 A[Catch:{ ServiceSpecificException -> 0x00c9, RemoteException -> 0x00b7, all -> 0x0117 }] */
    /* JADX WARNING: Removed duplicated region for block: B:56:0x010a A[Catch:{ ServiceSpecificException -> 0x00c9, RemoteException -> 0x00b7, all -> 0x0117 }] */
            ...
            try {
                ...
                try {
                    if (supportsCamera2ApiLocked(cameraId)) {
                        ICameraService cameraService = CameraManagerGlobal.get().getCameraService();
                        if (cameraService != null) {
                            String componentName = this.mContext.getOpPackageName() + "/" + getRunningActivityName();
                            Log.i(TAG, "openCameraDeviceUserAsync, componentName: " + componentName);
                            try {
                                cameraUser = cameraService.connectDevice(callbacks, cameraId, componentName, uid);
                            } catch (ServiceSpecificException e2) {
                                e = e2;
                                if (e.errorCode == 9) {
                                }
    ...
    public void openCamera(String cameraId, CameraDevice.StateCallback callback, Handler handler) throws CameraAccessException {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(154, Os.getuid());
    ...
    public void openCamera(String cameraId, Executor executor, CameraDevice.StateCallback callback) throws CameraAccessException {
        if (executor != null) {
            if ("off".equals(SystemProperties.get("oppo.camera.hypnus.contrl", Camera.Parameters.FLASH_MODE_ON)) || "android".equals(ActivityThread.currentOpPackageName())) {
            ...
            if (sHM == null) {
                sHM = HypnusManager.getHypnusManager();
            }
            HypnusManager hypnusManager = sHM;
            if (hypnusManager != null && this.mHypnusCtrl) {
                hypnusManager.hypnusSetSignatureAction(15, 1000, Hypnus.getLocalSignature());
            }
    ...
    public static final class CameraManagerGlobal extends ICameraServiceListener.Stub implements IBinder.DeathRecipient {
        ...
        public void setTorchMode(String cameraId, boolean enabled) throws CameraAccessException {
            synchronized (this.mLock) {
                if (cameraId != null) {
                    Log.i(TAG, "setTorchMode  cameraId=" + cameraId);
                    if (Integer.valueOf(cameraId).intValue() > 1) {
                        if (!isOppoApp()) {
                            throw new IllegalArgumentException("cameraId was wrong");
                        }
                    }
        ...
        private void postSingleUpdate(final AvailabilityCallback callback, Executor executor, final String id, int status) {
            Log.i(TAG, "postSingleUpdate  onCameraAvailable=" + id);
            if (Integer.valueOf(id).intValue() > 1 && !isOppoApp()) {
                return;
            }
        ...
        private boolean isOppoApp() {
            String packageName = ActivityThread.currentOpPackageName();
            if (OppoAtlasManagerDefine.CAMERA_PACKAGE_NAME.equals(packageName) || "com.oppo.engineermode.camera".equals(packageName) || "com.oppo.engineermode".equals(packageName) || "com.coloros.healthcheck".equals(packageName) || "com.mediatek.emcamera2".equals(packageName) || "com.mediatek.camera".equals(packageName) || "com.oppo.autotest.olt.cameratest".equals(packageName) || "com.mediatek.engineermode".equals(packageName)) {
                return true;
            }
            return false;
        }
