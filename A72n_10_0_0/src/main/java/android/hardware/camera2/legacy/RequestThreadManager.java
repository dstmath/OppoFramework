...
public class RequestThreadManager {
    ...
    private final Handler.Callback mRequestHandlerCb = new Handler.Callback() {
        ...
        /* JADX WARNING: Removed duplicated region for block: B:145:0x044e  */
        /* JADX WARNING: Removed duplicated region for block: B:149:0x047b  */
        /* JADX WARNING: Removed duplicated region for block: B:150:0x04ab A[RETURN, SYNTHETIC] */
        @Override // android.os.Handler.Callback
        public boolean handleMessage(Message msg) {
            RequestQueue.RequestQueueEntry nextBurst;
            Handler handler;
            char c;
            ...
            if (i == 1) {
                ...
                RequestQueue.RequestQueueEntry nextBurst2 = RequestThreadManager.this.mRequestQueue.getNext();
                if (nextBurst2 == null) {
                    try {
                        if (!RequestThreadManager.this.mCaptureCollector.waitForEmpty(4000, TimeUnit.MILLISECONDS)) {
                        ...
                        synchronized (RequestThreadManager.this.mIdleLock) {
                            nextBurst2 = RequestThreadManager.this.mRequestQueue.getNext();
                            if (nextBurst2 == null) {
                ...
                if (nextBurst2.isQueueEmpty()) {
                    RequestThreadManager.this.mDeviceState.setRequestQueueEmpty();
                }
                BurstHolder burstHolder = nextBurst2.getBurstHolder();
                List<RequestHolder> requests2 = burstHolder.produceRequestHolders(nextBurst2.getFrameNumber().longValue());
                Iterator<RequestHolder> it = requests2.iterator();
                while (it.hasNext()) {
                    RequestHolder holder = it.next();
                    ...
                    if (RequestThreadManager.this.mLastRequest == null || RequestThreadManager.this.mLastRequest.captureRequest != request) {
                        ...
                        if (!RequestThreadManager.this.mParams.same(legacyRequest.parameters)) {
                            try {
                                ...
                                handler = handler2;
                                nextBurst = nextBurst2;
                            } catch (RuntimeException e6) {
                                Log.e(RequestThreadManager.this.TAG, "Exception while setting camera parameters: ", e6);
                                holder.failRequest();
                                handler = handler2;
                                nextBurst = nextBurst2;
                    ...
                    try {
                        try {
                            if (!RequestThreadManager.this.mCaptureCollector.queueRequest(holder, RequestThreadManager.this.mLastRequest, 4000, TimeUnit.MILLISECONDS)) {
                                ...
                                try {
                                    ...
                                    RequestThreadManager.this.mDeviceState.setError(1);
                                    long lastFrameNumber22 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                                    if (RequestThreadManager.DEBUG) {
                                    }
                                    RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber22, burstHolder.getRequestId());
                                    if (RequestThreadManager.DEBUG) {
                                    }
                                }
                            } else {
                                ...
                                try {
                                    ...
                                    Log.e(RequestThreadManager.this.TAG, "Interrupted waiting for request completion: ", e11);
                                    RequestThreadManager.this.mDeviceState.setError(1);
                                }
                            }
                            ...
                            Log.e(RequestThreadManager.this.TAG, "Received device exception during capture call: ", e);
                            RequestThreadManager.this.mDeviceState.setError(1);
                            long lastFrameNumber222 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                            if (RequestThreadManager.DEBUG) {
                            }
                            RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber222, burstHolder.getRequestId());
                            if (RequestThreadManager.DEBUG) {
                            }
                            ...
                            Log.e(RequestThreadManager.this.TAG, "Interrupted during capture: ", e2);
                            RequestThreadManager.this.mDeviceState.setError(1);
                            long lastFrameNumber2222 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                            if (RequestThreadManager.DEBUG) {
                            }
                            RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber2222, burstHolder.getRequestId());
                            if (RequestThreadManager.DEBUG) {
                            }
                            ...
                            Log.e(RequestThreadManager.this.TAG, "Received device exception during capture call: ", e3);
                            RequestThreadManager.this.mDeviceState.setError(1);
                            long lastFrameNumber22222 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                            if (RequestThreadManager.DEBUG) {
                            }
                            RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber22222, burstHolder.getRequestId());
                            if (RequestThreadManager.DEBUG) {
                            }
                        }
                        ...
                        Log.e(RequestThreadManager.this.TAG, "Received device exception during capture call: ", e);
                        RequestThreadManager.this.mDeviceState.setError(1);
                        long lastFrameNumber222222 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                        if (RequestThreadManager.DEBUG) {
                        }
                        RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber222222, burstHolder.getRequestId());
                        if (RequestThreadManager.DEBUG) {
                        }
                        ...
                        Log.e(RequestThreadManager.this.TAG, "Interrupted during capture: ", e2);
                        RequestThreadManager.this.mDeviceState.setError(1);
                        long lastFrameNumber2222222 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                        if (RequestThreadManager.DEBUG) {
                        }
                        RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber2222222, burstHolder.getRequestId());
                        if (RequestThreadManager.DEBUG) {
                        }
                        ...
                        Log.e(RequestThreadManager.this.TAG, "Received device exception during capture call: ", e3);
                        RequestThreadManager.this.mDeviceState.setError(1);
                        long lastFrameNumber22222222 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                        if (RequestThreadManager.DEBUG) {
                        }
                        RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber22222222, burstHolder.getRequestId());
                        if (RequestThreadManager.DEBUG) {
                        }
                    }
                }
                if (anyRequestOutputAbandoned && burstHolder.isRepeating()) {
                    long lastFrameNumber222222222 = RequestThreadManager.this.cancelRepeating(burstHolder.getRequestId());
                    if (RequestThreadManager.DEBUG) {
                        Log.d(RequestThreadManager.this.TAG, "Stopped repeating request. Last frame number is " + lastFrameNumber222222222);
                    }
                    RequestThreadManager.this.mDeviceState.setRepeatingRequestError(lastFrameNumber222222222, burstHolder.getRequestId());
                }
                if (RequestThreadManager.DEBUG) {
                    return true;
                }
                ...
                try {
                    ...
                    Log.e(RequestThreadManager.this.TAG, "Interrupted while waiting for requests to complete: ", e18);
                    RequestThreadManager.this.mDeviceState.setError(1);
                }
                if (RequestThreadManager.this.mGLThreadManager != null) {
    ...
    private void stopPreview() {
        if (VERBOSE) {
            String str = this.TAG;
            Log.v(str, "stopPreview - preview running? " + this.mPreviewRunning);
        }
    ...
    private void startPreview() {
        if (VERBOSE) {
            String str = this.TAG;
            Log.v(str, "startPreview - preview running? " + this.mPreviewRunning);
        }
        if (!this.mPreviewRunning) {
    ...
    private void doJpegCapturePrepare(RequestHolder request) throws IOException {
        if (DEBUG) {
            String str = this.TAG;
            Log.d(str, "doJpegCapturePrepare - preview running? " + this.mPreviewRunning);
        }
        if (!this.mPreviewRunning) {
            if (DEBUG) {
    ...
    private void doPreviewCapture(RequestHolder request) throws IOException {
        if (VERBOSE) {
            String str = this.TAG;
            Log.v(str, "doPreviewCapture - preview running? " + this.mPreviewRunning);
        }
        if (!this.mPreviewRunning) {
    ...
    private void configureOutputs(Collection<Pair<Surface, Size>> outputs) {
        ...
        try {
            ...
            try {
                ...
                if (DEBUG) {
                    Log.d(this.TAG, "doPreviewCapture - Selected range [" + bestRange[0] + SmsManager.REGEX_PREFIX_DELIMITER + bestRange[1] + "]");
                }
