...
public class TelephonyManager {
    ...
    private static final String CTA_FUNCTION_NAME = "getLine1Number";
    private static final String CTA_KEYWORD = "phonenumber";
    private static final String CTA_OPERATION = "读取用户本机号码";
    ...
    private static boolean isJS_ESurfing = SystemProperties.get("ro.customize.type", "").equals("JS_ESurfing");
    ...
    public String getDeviceId() {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(53, Binder.getCallingUid());
        try {
            ITelephony telephony = getITelephony();
            if (telephony == null) {
                return null;
    ...
    public String getImei(int slotIndex) {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(52, Binder.getCallingUid());
        ITelephony telephony = getITelephony();
        if (telephony == null) {
            return null;
        }
    ...
    public String getNetworkCountryIso() {
        String countryIso = getNetworkCountryIsoForPhone(getPhoneId());
        if (countryIso == null || countryIso.length() == 0) {
            return getNetworkCountryIsoForPhone(1 - getPhoneId());
        }
        return countryIso;
    }
    ...
    public String getOemSimOperator(int subId) {
    ...
    public String getSubscriberId() {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(41, Binder.getCallingUid());
    ...
    public String getSubscriberId(int subId) {
        int deltaSubId;
        if (isJS_ESurfing && (deltaSubId = subId - 100000) > 0) {
            try {
                ITelephony telephony = getITelephony();
                if (telephony == null) {
                    return null;
                }
                String[] imsi = telephony.getLTECDMAImei(SubscriptionManager.getSlotIndex(deltaSubId));
    ...
    public boolean isOemEnhanced4gLteModeSettingEnabledByUser(int subId) {
        try {
            Rlog.d(TAG, "VOLTE IS " + getITelephony().isAdvancedCallingSettingEnabled(subId));
            return getITelephony().isAdvancedCallingSettingEnabled(subId);
        } catch (RemoteException | NullPointerException e) {
            return false;
        }
    }
    ...
    public String getGroupIdLevel1() {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(42, Binder.getCallingUid());
        try {
            IPhoneSubInfo info = getSubscriberInfo();
            if (info == null) {
                return null;
    ...
    private void printCTALog(String chineseName, String processName) {
        String systemTime = getTime();
        Log.d(CTA_TAG, systemTime + " <" + chineseName + ">[" + CTA_KEYWORD + "][" + processName + "]:[" + CTA_FUNCTION_NAME + "]" + CTA_OPERATION + "..");
    }

    public String getLine1Number() {
        try {
            PackageManager packageManager = this.mContext.getPackageManager();
            isCTAVersion = packageManager.hasSystemFeature(CTA_FEATURE, 0);
            if (isCTAVersion) {
                ApplicationInfo info = packageManager.getApplicationInfo(packageManager.getNameForUid(Binder.getCallingUid()), 0);
                printCTALog(info.loadLabel(packageManager).toString(), info.processName);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String getLine1Number(int subId) {
        ...
        try {
            ...
            ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(51, Binder.getCallingUid());
        } catch (RemoteException | NullPointerException e) {
        }
        if (number != null) {
            return number;
        ...
        try {
            ...
            ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(45, Binder.getCallingUid());
    ...
    public String getVoiceMailNumber(int subId) {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(43, Binder.getCallingUid());
        try {
            IPhoneSubInfo info = getSubscriberInfo();
            if (info == null) {
                return null;
    ...
    public String getVoiceMailAlphaTag() {
        ((IColorAntiVirusBehaviorManager) OppoFeatureCache.getOrCreate(IColorAntiVirusBehaviorManager.DEFAULT, new Object[0])).setAction(44, Binder.getCallingUid());
    ...
    public void listen(PhoneStateListener listener, int events) {
        ...
        if (this.mContext != null && listener != null) {
            try {
                ...
                Log.e(TAG, "listen: NullPointerException");
            }
        }
    }
                            ...
                            Binder.withCleanCallingIdentity(new FunctionalUtils.ThrowingRunnable(executor, callback, errorCode, detail) {
                                ...
                                public final void runOrThrow() {
                                    this.f$0.execute(new Runnable(this.f$2, this.f$3) {
                                        ...
                                        public final void run() {
                                            TelephonyManager.CellInfoCallback.this.onError(this.f$1, this.f$2.getCause());
                                        }
                                    });
                                }
                            });
    ...
    public void setBasebandVersionForPhone(int phoneId, String version) {
        if (getSimCount() > 1) {
            ...
            SystemProperties.set(TelephonyProperties.PROPERTY_BASEBAND_VERSION, "M_V3_P10");
        }
    }
    ...
    public void setSlotState(int slotId, boolean on) {
        ...
        String state = WifiEnterpriseConfig.ENGINE_DISABLE;
        if (getTelephonyProperty(slotId, "persist.sys.oem_forbid_slots", state).equals(state) == on) {
            Rlog.d(TAG, "setSlotState, the same status, return!");
            return;
        }
        if (!on) {
            state = WifiEnterpriseConfig.ENGINE_ENABLE;
        }
    ...
    public void setNetworkRoamingTypeForPhone(int phoneId, int roamingType) {
        if (SubscriptionManager.isValidPhoneId(phoneId)) {
            setTelephonyProperty(phoneId, TelephonyProperties.PROPERTY_OPERATOR_ROAMING_TYPE, String.valueOf(roamingType));
        }
    }

    public int getNetworkRoamingTypeForPhone(int subId) {
        String type = getTelephonyProperty(SubscriptionManager.getPhoneId(subId), TelephonyProperties.PROPERTY_OPERATOR_ROAMING_TYPE, null);
        if (type == null || type.isEmpty()) {
            return 0;
        }
        return Integer.parseInt(type);
