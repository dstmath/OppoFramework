...
public final class MediaMuxer {
    ...
    private void setUpMediaMuxer(FileDescriptor fd, int format) throws IOException {
        if (format < 0 || format > 4) {
            String result_muxer_format = OppoAtlasManager.getInstance(null).getParameters("get_listinfo_bypid=muxer-format=" + Binder.getCallingPid());
            if (result_muxer_format == null) {
                throw new IllegalArgumentException("format: " + format + " is invalid");
            } else if (result_muxer_format.equals("mpeg4")) {
                format = 0;
            } else if (result_muxer_format.equals("webm")) {
                format = 1;
            } else if (result_muxer_format.equals("3gpp")) {
                format = 2;
            } else if (result_muxer_format.equals("heif")) {
                format = 3;
            } else {
                throw new IllegalArgumentException("format: " + format + " is invalid");
            }
