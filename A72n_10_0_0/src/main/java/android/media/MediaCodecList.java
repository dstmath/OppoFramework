...
import com.oppo.media.OppoMediaCodecList;
...
public final class MediaCodecList {
    ...
    private static final void initCodecList() {
        synchronized (sInitLock) {
            if (sRegularCodecInfos == null) {
                ...
                OppoMediaCodecList.checkUseOppoh264();
                for (int index = 0; index < count; index++) {
                    try {
                        MediaCodecInfo info = getNewCodecInfoAt(index);
                        if (!OppoMediaCodecList.checkSaveOppoh264Info(info, index)) {
                            all.add(info);
                ...
                OppoMediaCodecList.checkHandleOppoh264Info(regulars, all);
    ...
    public static MediaCodecInfo getNewCodecInfoAtOppo(int index) {
        return getNewCodecInfoAt(index);
    }
    ...
    private String findCodecForFormat(boolean encoder, MediaFormat format) {
        ...
        MediaCodecInfo[] mediaCodecInfoArr = this.mCodecInfos;
        for (MediaCodecInfo info : mediaCodecInfoArr) {
