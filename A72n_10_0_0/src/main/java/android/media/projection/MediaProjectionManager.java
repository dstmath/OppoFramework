...
public final class MediaProjectionManager {
    ...
    public MediaProjection getMediaProjection(int resultCode, Intent resultData) {
        ...
        String strDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(System.currentTimeMillis()));
        int labelRes = this.mContext.getApplicationInfo().labelRes;
        String cpnLabel = labelRes > 0 ? this.mContext.getResources().getString(labelRes) : "Unkown-labelRes";
        Log.w(TAG, strDate + "<" + cpnLabel + ">[picture][" + this.mContext.getApplicationInfo().processName + "]:[new MediaProjection(..)]");
