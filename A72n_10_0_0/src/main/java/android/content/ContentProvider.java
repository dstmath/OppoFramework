...
public abstract class ContentProvider implements ContentInterface, ComponentCallbacks2 {
    ...
    public boolean checkUser(int pid, int uid, Context context) {
        if (!ColorMultiAppManager.getInstance().isMultiAppUserId(UserHandle.getUserId(uid)) && UserHandle.getUserId(uid) != context.getUserId() && !this.mSingleUser && context.checkPermission(Manifest.permission.INTERACT_ACROSS_USERS, pid, uid) != 0) {
            return false;
        }
        return true;
    }
