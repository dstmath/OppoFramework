...
public class Time {
    ...
    private boolean parse3339Internal(String s) {
        ...
        if (len >= 10) {
            ...
            if (len >= 19) {
                checkChar(s, 10, 'T');
                this.allDay = false;
                int hour2 = getChar(s, 11, 10) + getChar(s, 12, 1);
                checkChar(s, 13, ':');
                int minute2 = getChar(s, 14, 10) + getChar(s, 15, 1);
                checkChar(s, 16, ':');
                ...
                if (len > tzIndex) {
                    ...
                    if (offset != 0) {
                        if (len >= tzIndex + 6) {
                            hour2 += (getChar(s, tzIndex + 1, 10) + getChar(s, tzIndex + 2, 1)) * offset;
                            minute2 += (getChar(s, tzIndex + 4, 10) + getChar(s, tzIndex + 5, 1)) * offset;
                        } else {
    ...
    public void set(long millis) {
        ...
        timeCalculator.setTimeInMillis(millis);
        this.calculator.copyFieldsToTime(this);
    }
    ...
    public String format3339(boolean allDay2) {
        ...
        return String.format(Locale.US, "%s%s%02d:%02d", base, sign, Integer.valueOf(offset / 3600), Integer.valueOf((offset % 3600) / 60));
    }
