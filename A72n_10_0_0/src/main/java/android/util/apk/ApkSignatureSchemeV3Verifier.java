...
public class ApkSignatureSchemeV3Verifier {
    ...
    /* JADX INFO: Multiple debug info for r11v3 boolean: [D('sigVerified' boolean), D('sig' java.security.Signature)] */
    /* JADX INFO: Multiple debug info for r7v7 int: [D('certificateCount' int), D('bestSigAlgorithm' int)] */
    /* JADX INFO: Multiple debug info for r8v12 int: [D('digestCount' int), D('signatureCount' int)] */
                ...
                if (sigVerified) {
                    ...
                    if (signaturesSigAlgorithms.equals(digestsSigAlgorithms)) {
                        ...
                        for (ByteBuffer certificates = ApkSigningBlockUtils.getLengthPrefixedSlice(signedData); certificates.hasRemaining(); certificates = certificates) {
                            int bestSigAlgorithm2 = certificateCount + 1;
                            ...
                            try {
                                ...
                                certificateCount = bestSigAlgorithm2;
                                bestSigAlgorithm = bestSigAlgorithm;
                                digestAlgorithm = digestAlgorithm;
                            } catch (CertificateException e9) {
                                throw new SecurityException("Failed to decode certificate #" + bestSigAlgorithm2, e9);
                            }
                        }
                        if (certs.isEmpty()) {
        ...
        try {
            ...
            while (porBuf.hasRemaining()) {
                ...
                try {
                    ...
                    e2 = e5;
                    throw new SecurityException("Failed to verify signature over signed data for certificate #" + levelCount + " when verifying Proof-of-rotation record", e2);
                } catch (CertificateException e6) {
                    e3 = e6;
                    throw new SecurityException("Failed to decode certificate #" + levelCount + " when verifying Proof-of-rotation record", e3);
                }
            }
