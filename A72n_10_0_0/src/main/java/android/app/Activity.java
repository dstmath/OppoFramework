...
public class Activity extends ContextThemeWrapper implements LayoutInflater.Factory2, Window.Callback, KeyEvent.Callback, View.OnCreateContextMenuListener, ComponentCallbacks2, Window.OnWindowDismissedCallback, Window.WindowControllerCallback, AutofillManager.AutofillClient, ContentCaptureManager.ContentCaptureClient {
    ...
    public static boolean DEBUG_STOPPED = false;
    ...
    final String[] mActivitiesToImprove = {"com.tencent.mm/com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyDetailUI"};
    ...
    public void onCreate(Bundle savedInstanceState) {
        boolean z = false;
        ((IColorCommonInjector) OppoFeatureCache.getOrCreate(IColorCommonInjector.DEFAULT, new Object[0])).onCreateForActivity(this, savedInstanceState);
    ...
    public void onResume() {
        View focus;
        ((IColorDarkModeManager) OppoFeatureCache.getOrCreate(IColorDarkModeManager.DEFAULT, new Object[0])).refreshForceDark(this.mDecor);
    ...
    public void onBackPressed() {
        ...
        } else if (DEBUG_PANIC || InputLog.DEBUG) {
    ...
    @Override // android.view.Window.WindowControllerCallback
    public int getWindowMode() throws RemoteException {
        return ActivityManager.getService().getWindowMode(this.mToken);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (hypnusManager != null) {
            hypnusManager.hypnusManagerSendFling(ev, 20);
        }
        if (ev.getAction() == 0) {
            onUserInteraction();
        }
    ...
    public void startActivityForResult(Intent intent, int requestCode, Bundle options) {
        if (!(intent == null || intent.getComponent() == null || !this.mActivitiesToImproveList.contains(intent.getComponent().flattenToString()))) {
            ...
            HypnusManager hypnusManager = mHM;
            if (hypnusManager != null) {
                hypnusManager.hypnusSetAction(12, 1200);
            }
        }
        if (intent != null) {
    ...
    private void finish(int finishTask) {
        ...
        if (this.mIsFinishBoost) {
            ...
            if (this.mActivityInfo.name == null || !this.mActivityInfo.name.equals("com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyBeforeDetailUI")) {
                mHM.hypnusSetAction(12, 10);
            } else {
                mHM.hypnusSetAction(12, 1200);
            }
        }
        Activity activity = this.mParent;
        if (activity == null) {
    ...
    public boolean shouldUpRecreateTask(Intent targetIntent) {
        try {
            ...
            ComponentName cn2 = targetIntent.getComponent();
            if (cn2 == null) {
                cn2 = targetIntent.resolveActivity(pm);
            }
            ActivityInfo info = pm.getActivityInfo(cn2, 0);
    ...
    public final void attach(Context context, ActivityThread aThread, Instrumentation instr, IBinder token, int ident, Application application, Intent intent, ActivityInfo info, CharSequence title, Activity parent, String id, NonConfigurationInstances lastNonConfigurationInstances, Configuration config, String referrer, IVoiceInteractor voiceInteractor, Window window, ViewRootImpl.ActivityConfigCallback activityConfigCallback, IBinder assistToken) {
        ...
        if (this.mActivityInfo.name != null && this.mActivityInfo.name.contains(LMManager.MM_PACKAGENAME)) {
            this.mIsFinishBoost = true;
        }
        if (!(this.mActivityInfo.name == null || LMManager.getOpenHbActivity() == null || !LMManager.getOpenHbActivity().contains(this.mActivityInfo.name))) {
            LMManager.getLMManager().noteOpenLuckMoney();
        }
    ...
    public final void performStart(String reason) {
        ...
        if (this.mCalled) {
            ...
            if (((isAppDebuggable && "true".equals(SystemProperties.get("persist.sys.assert.panic"))) || isDlwarningEnabled) && (dlwarning = getDlWarning()) != null) {
    ...
    public final void performRestart(boolean start, String reason) {
        ...
        this.mFragments.noteStateNotSaved();
        if (DEBUG_STOPPED) {
            Log.w(TAG, "performRestart mToken=" + this.mToken + ",mParent=" + this.mParent + ",this=" + this);
        }
        ...
        if (this.mCalled) {
            if (!this.mVisibleFromClient && !this.mFinished) {
                ...
                if (getApplicationInfo().targetSdkVersion > 22) {
                    if (!"true".equals(SystemProperties.get("persist.sys.assert.panic"))) {
